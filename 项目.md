...
- 将clean与`LEO_weak`混合、拼接、配对、一致性约束、teacher-student蒸馏、replay或source-anchor后，仍把该row标为正式Phase2候选；Phase2可使用的source anchor也必须是协议允许且不向算法暴露clean样本的预注册部署artifact，不能在Phase2阶段回读clean数据。
...
- K1允许使用source validation和注册support标签进行角色对称的轻量域对齐，包括全局对角CORAL、低参数FiLM、support增强view一致性、稳健原型和仅基于support原型Gram矩阵的去混淆；所有已注册类必须使用同一规则。禁止依据old/new身份选择不同adapter、不同shrinkage强度、不同阈值或不同决策分支，禁止把source旧类prototype bank只作用于一部分注册类后声称无角色Oracle。K1的增强view仍按同一物理sample ID计为1-shot，必须单列一次性enrollment前向与状态开销。
...
- 自2026-07-15起，MRIOR-SDA是极轻型快速适应路线的强制性能/资源比较基线。比较必须复用同一ADV3B02 checkpoint、target receiver、seed、K、物理support/query ID、`Y_new^5⊂Y_new^10⊂Y_new^20`、`leo_*_weak`场景和推理View策略；历史仅含旧类的Stage2-B MRIOR结果不得替代加入新类后的Stage2-C配对基线。对每个`K∈{1,5,10,20}`，Stage2-B的`old_acc`以及Stage2-C的`old_acc`、`seen_new_acc`和`H_old_new`在独立确认矩阵上的配对均值必须至少高于matched MRIOR-SDA 2个百分点且paired 95%置信区间下界大于0；逐target receiver聚合不得低于MRIOR-SDA。资源侧必须同时报告可训练参数、optimizer step、端到端适配时延、峰值显存、持久状态和平均/P95 backbone forward；极轻型候选的可训练参数与optimizer step必须分别不超过matched MRIOR-SDA的5%和10%，平均适配时延不超过其25%，峰值显存不超过其50%，持久状态不超过256KB。任一性能或资源门槛未满足时只能标记为诊断/Pareto候选，不能声明“显著优于MRIOR且更加轻量高效”。
...
- 自2026-07-15起，极轻型正式候选的target-old/target-new support与query必须逐行来自对应的`leo_clear_weak`、`leo_low_elev_weak`或`leo_rain_weak`目标缓存，禁止把clean waveform、clean feature或clean prototype混入support、query、门限选择或候选promotion。新训练的`effective_feature` ground adapter及其source receiver holdout也采用`leo_weak-only`reference/prototype，不再借clean source特征选择adapter/head/TTA；历史ADV3B02基础checkpoint既有训练来源不据此改写，但必须与新增适配阶段的输入权限分开报告。任何cache中`clean`channel view、场景字段缺失或场景与row不一致均应fail closed。
