...
2026-07-14起，Phase2 Stage2-B/Stage2-C采用“极轻型快速适应目标模式”。该模式不替代第9节的数据权限与声明边界：Stage2-B只评估合法target-old support/query上的目标域旧类校准，Stage2-C同时评估合法target-old/target-new support/query上的旧类适应与新类学习；两者都必须使用deployment-primary LEO视图。
