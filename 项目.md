...
### 7.4 Phase2一次性准入与实验优先执行规则（2026-07-16）

> **Phase2的clean/source不可达性是二元准入门禁，不是持续优化目标。协议检查只负责回答“当前密封输入能否进入实验”；一旦当前输入组合取得`EXPERIMENT_READY`，后续工作必须转入target-support-only候选设计、矩阵验证、Runner执行和指标优化，不得反复追溯Phase1训练数据、source特征、clean control、历史cache或旧artifact来重新证明同一件事。协议PASS、补充说明、报告润色或重复审计本身不能替代实验推进。**

Phase2准入必须绑定不可变输入组合，而不是绑定模糊的run名称。推荐生成并持久化`phase2_input_admission.json`，至少记录：

```text
phase2_admission_key=sha256(
  sealed_phase1_checkpoint_hash,
  sealed_leo_target_package_hash,
  strict_loader_hash,
  predictor_input_schema_hash,
  phase2_policy_version
)
phase2_admission_status=EXPERIMENT_READY
phase2_admission_scope=<row/candidate scope>
phase2_admission_evidence_hash=<pre-open audit and runtime-policy evidence hash>
```

执行规则：

1. **一次性pre-open准入**：对新的输入组合只执行一次有界的pre-open审计，核对checkpoint、密封LEO target package、成员/路径allowlist、loader、predictor schema和本节政策版本。审计不得打开clean/source payload，也不得通过读取source/clean样本或其feature、统计、cache来“补证据”。
2. **PASS后复用**：准入记录为`EXPERIMENT_READY`且上述哈希未变化时，matrix generator、validator、runner和报告器只能校验admission key及签名/哈希完整性，必须复用该准入结论。不得重新审计Phase1数据构造、source receiver划分、clean性能、source feature分布或历史训练过程，也不得要求为每个candidate重复生成同一套协议说明。
3. **FAIL只阻断依赖项**：准入失败时标记`LOCAL_PROTOCOL_REPAIR_REQUIRED`，修复范围只允许覆盖密封package、allowlist、strict loader、predictor schema、manifest或访问审计缺口。不得把修复扩展为重新训练Phase1、重新分析source数据、重做clean control或生成新的clean/source诊断。一个row或package失败不能阻塞其他已经具有有效`EXPERIMENT_READY`记录的row、GPU lane或候选族。
4. **输入变化才重审**：只有checkpoint hash、LEO target package hash、strict loader hash、predictor schema hash或Phase2政策版本发生变化时，旧准入记录才失效并触发新的有界pre-open审计。candidate的target-support-only超参数、prototype规则、adapter配置或资源档变化，如果不扩大输入可达性且未改变上述边界组件，不得触发对Phase1/source/clean历史的重新审计。
5. **报告引用而非复述**：实验报告只需记录admission ID/key、状态、适用范围和证据路径。除非发生准入失败或边界组件变化，不应重复大段协议论证；主要篇幅必须用于候选机制、对照、loss/求解过程、同row指标、资源开销、失败原因和下一轮优化。

Phase2优化目标只在`EXPERIMENT_READY`可行域内定义：

- 主要优化量为同row的`old_acc`、`min_old_class_acc`、`seen_new_acc`、`H_old_new`、分K遗忘/适应增益及资源Pareto。
- clean/source不可达、query无Oracle和sealed prediction/scorer隔离属于必须满足的feasibility constraints，不得被写成可反复加分的优化指标、路线创新点或替代性能进展的“主要成果”。
- 以Stage2优化和执行为目标的任务，在存在`EXPERIMENT_READY`候选后，必须继续完成候选选择或修改、矩阵/命令生成、最窄本地验证并进入Runner gates。若任务已授权实际实验且N607安全/容量门禁允许，不能停在协议PASS、dry-run、报告更新或state更新；必须取得真实runner结果。若容量阻塞，则保存精确deferred命令和依赖条件，但不得回到重复协议讨论。
- 每轮优化必须产出可验证的实验推进物，例如新候选及其可执行矩阵、完成的本地路径验证、已landed run、完成结果表或基于完整日志的机制级修订。仅重复“不能访问clean/source”不构成一次优化轮次。
...
- K1允许使用Phase1/source validation预先锁定且不携带source统计进入Phase2的固定超参数，并使用注册support标签进行角色对称的轻量域对齐，包括target-support-only对角CORAL、低参数FiLM、support增强view一致性、稳健原型和仅基于support原型Gram矩阵的去混淆；所有已注册类必须使用同一规则。任何source旧类prototype bank、source covariance、source feature或source memory均不得进入Stage2。K1的增强view仍按同一物理sample ID计为1-shot，必须单列一次性enrollment前向与状态开销。
...
- 开发阶段仍不得使用K1/K5/K20 query重新选择candidate、adapter、epoch、域适应超参数或门限。跨K抗遗忘机制只能通过source validation、注册support的leave-one-view/sample-out稳定性和K10开发row确定；锁定后K1/K5/K20只用于压力评估与独立确认。若锁定candidate在任一K产生较identity-only更大的遗忘，该结果用于下一轮机制级修订，不得在同一query上继续逐K调参。
...
- 主确认矩阵固定覆盖5个target receiver、至少5个独立确认seed和`leo_clear_weak`、`leo_low_elev_weak`、`leo_rain_weak`三个场景，并同时保留matched K10/K5结果。开发seed只允许在K10选择一套统一超参数；选定后必须锁定，并在独立确认seed上同时评估5/10/20类和K5稳健性。不得跨K、跨seed或跨新类规模拼接最佳指标。
- 正式推理必须逐样本可部署：禁止使用query真实old/new角色、全批类别数量、每类quota、query标签或query集合图结构。query不得参与adapter拟合、阈值拟合、模型选择或早停。
- 默认冻结ADV3B02 backbone，不执行backbone梯度更新。极轻型首选资源档为1-view或逐样本1→3→5-view门控、adapter可训练参数不超过50,000、适配不超过20epoch、无dense query图、持久化适配状态不超过256KB；任何超限row必须显式标为资源消融或非极轻型对照。
- 经用户明确授权，为优先逼近性能门槛，可启用`performance-relaxed`资源档：相对首选档的adapter参数、适配轮数/步数、持久状态或平均View计算最多提高50%–100%，绝对上限分别为100,000参数、40epoch、512KB和5次backbone前向。放宽不得改变无角色Oracle、无类别配额、无query拟合、无dense query图和逐样本决策硬约束；每个row必须记录实际放宽比例，并与首选档及identity-only单qKNN做同row Pareto比较。未超过某一首选上限的资源项仍按实测值报告，不能把授权上限冒充实际消耗。
...
- 允许把已注册的K个物理support样本在同一`leo_weak`场景族内生成至多3个预注册增强view，用于一次性support-only enrollment；K仍按互不重复的物理sample ID计数，不得把增强view重复计入K，不得生成或使用任何query增强用于拟合。正式query推理仍以1-view为首选，且support增强策略、每个物理support的backbone/FFT前向次数、增强场景清单和一次性enrollment计算量必须单列报告。该机制只能声明为`support-only leo_weak augmentation`，不能据此声称跨场景或真实在轨泛化。
- 自2026-07-16起，极轻型正式候选的target-old/target-new support与query必须逐行来自对应的`leo_clear_weak`、`leo_low_elev_weak`或`leo_rain_weak`目标缓存。Stage2不得导入新训练的`effective_feature` ground adapter、source receiver holdout、source reference/prototype或其他额外source-derived artifact；只允许封存的Phase1 ADV3B02 checkpoint。历史ADV3B02基础checkpoint既有训练来源不据此改写，但必须与Stage2输入权限分开报告。任何cache中出现source角色、clean channel、场景字段缺失或场景与row不一致均应fail closed。
...
- 当前checkpoint/package/loader/schema/policy组合是否已有第7.4节定义的有效`EXPERIMENT_READY`准入记录。若哈希未变化，自动化必须复用准入结论，禁止重复追溯Phase1/source/clean历史。
...
- 某个row或package准入失败时，只阻断其依赖项并生成有界修复任务；其他具有有效`EXPERIMENT_READY`记录的row必须继续进入Runner gates。协议修复、manifest补写、报告补写和state更新不得被当成已有launchable row的实验完成。
...
例外：第7.1节禁止的Phase2 clean接触候选和第7.2节禁止的target query role/quota Oracle不得按上述通用规则新建为`NON_LAUNCH_DIAGNOSTIC`；只允许封存约束生效前已经存在的历史artifact，并分别统一标记为`PROTOCOL_INVALID_FOR_PHASE2`或`PROTOCOL_INVALID_FOR_DEPLOYMENT`。
