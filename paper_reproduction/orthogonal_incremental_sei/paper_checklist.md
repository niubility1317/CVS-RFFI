...
|增量阶段冻结特征提取器、类均值初始化新权重|`train.py`|已覆盖正式runner流程；基类旧权重按论文算法1使用已分配伪目标`W0`并冻结，新类权重由5-shot支持集特征均值初始化，增量阶段显式冻结encoder并只优化新类权重；`class_mean_features`仅保留为诊断选项|
...
|算法1基础阶段训练|`run_formal_wisig`|已支持真实WiFi/ManyTx闭集FSCIL训练、早停和论文伪目标基类权重`W0`；严格论文官方WiFi数据源仍需另行提供|
