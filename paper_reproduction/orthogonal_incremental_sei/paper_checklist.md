...
|`A_bar/H_bar/F_bar`指标|`metrics.py`|已实现；`F_bar`默认按论文总session分母，增量session分母需显式指定|
...
|公式(4)正交空间目标|`pseudo_target_orthogonal_loss`、`optimize_pseudo_targets`|已覆盖闭式构造和可选迭代优化|
|公式(18)-(29)基础阶段损失|`orthogonal_base_loss`|已覆盖；`Ls`已按论文负样本集合和逐正样本平均修正|
|公式(30)-(36)增量校准|`incremental_calibration_loss`|已覆盖；已补shape/device检查|
|公式(37)-(41)评价指标|`metrics.py`|已覆盖；已补空old/new集合、长度检查和`F_bar`分母口径|
|表1仿真参数|配置文件与清单登记|已使用论文符号`tau_s`、`tau_c`、`q`登记；数值仍属implementation choice，需作者代码或网格确认|
