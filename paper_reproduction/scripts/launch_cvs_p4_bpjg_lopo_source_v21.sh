...
EXPERIMENT_ID=qknnv42_p4_bpjg_lopo_source_k10_20260715_v21
...
SCREEN_SHA256=3c4ad69ee148831f0d401a9f5fb73287400bc3a8cc994c6c85dd166c058b194f
ARM_INDEXES_RAW="${ARM_INDEXES:-0 1 2 3}"

for required in "$CONDA_SH" "$CHECKPOINT" "$SOURCE_VALIDATION" "$GROUND_STATE" "$TRAINER" "$SCREEN"; do
  if [[ ! -f "$required" ]]; then
    echo "missing source-screen artifact: $required" >&2
    exit 2
  fi
done
...
labels=(JG_R8_LR005 JG_R8_LR020 IJ_R8_LR010 FJ_R8_LR010)
scopes=(joint_gate joint_gate identity_joint fusion_joint)
lrs=(0.005 0.02 0.01 0.01)
gpus=(0 1 2 3)
selected=()
for index in $ARM_INDEXES_RAW; do
  if [[ ! "$index" =~ ^[0-3]$ ]]; then
    echo "ARM_INDEXES contains an invalid arm: $index" >&2
    exit 4
  fi
  selected+=("$index")
done
if [[ "${#selected[@]}" -eq 0 ]]; then
  echo "ARM_INDEXES selects no arms" >&2
  exit 4
fi
...
if [[ ! -e "$manifest" ]]; then
  ...
  echo "launch manifest is not a regular file" >&2
  exit 4
fi
for index in "${selected[@]}"; do
  ...
  if [[ -e "$out_dir" || -e "$log" || -e "$status_file" ]]; then
    echo "refusing to overwrite existing v21 arm: $label" >&2
    exit 4
  fi
done
...
for index in "${selected[@]}"; do
  ...
  pid=$!
  printf '%s\t%s\t%s\t8\t5\t10\t%s\t%s\t%s\t%s\t%s\ttrue\n' \
