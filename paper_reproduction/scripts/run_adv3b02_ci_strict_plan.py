#!/usr/bin/env python3
"""Build sealed packages and execute a sharded ADV3B02 CI strict plan."""
...
def _load_plan(path: Path) -> dict[str, Any]:
    ...
    if plan.get("schema") != "cvs.phase2.adv3b02_ci_strict_plan.v1":
        raise ValueError("strict CI plan schema drift")
    if plan.get("counts") != {"packages": 75, "cells": 900, "scenario_rows": 2700}:
        raise ValueError("strict CI plan count drift")
    return plan
...
def _build_package(plan: dict[str, Any], package: dict[str, Any], *, project_root: Path) -> dict[str, Any]:
    ...
    result = _run_json(
        [
            ...
            "--scorer-out-root", package["scorer_root"],
            "--detached-seal-path", package["detached_seal"],
            "--stage", "stage2c",
            ...
            "--query-per-tx", "20",
            "--candidate-lock", artifacts["candidate_lock"]["path"],
            "--checkpoint", artifacts["base_runtime"]["path"],
            "--adapter", artifacts["adapter"]["path"],
            "--head-artifact", artifacts["head_artifact"]["path"],
            "--tta-policy-json", artifacts["tta_policy"]["path"],
        ],
        cwd=project_root,
    )
    receipt = {
        "schema": "cvs.phase2.adv3b02_ci_package_build_receipt.v1",
        "status": "PASS",
...
def _run_cell(
    plan: dict[str, Any], cell: dict[str, Any], package: dict[str, Any],
    *, project_root: Path, device: str,
) -> dict[str, Any]:
    ...
    if cell_receipt.is_file():
        ...
        if value.get("status") != "PROTOCOL_VALID":
            raise ValueError("existing cell receipt is not PROTOCOL_VALID")
        return value
    ...
    predictor = _run_json(
        [
            sys.executable,
            str(project_root / "paper_reproduction/scripts/run_adv3b02_ci_truth_free_predictor.py"),
            "--package-root", package["predictor_package_root"],
            "--detached-seal", package["detached_seal"],
            "--expected-seal-sha256", build_receipt["predictor_package_seal_sha256"],
            "--method", cell["method"],
            "--old-class-count", "6",
            "--k-shot", str(cell["k_shot"]),
            "--seed", str(cell["seed"]),
            "--head-steps", str(plan["head_steps_per_phase"]),
            "--row-id", cell["cell_id"],
            "--output-dir", str(predictor_root),
            "--device", device,
        ],
        cwd=project_root,
    )
    ...
    scoring = _run_json(
        [
            ...
            "--prediction-artifact", predictor["prediction_artifact"],
            "--expected-prediction-artifact-sha256", predictor["prediction_artifact_sha256"],
            "--expected-prediction-seal-sha256", predictor["prediction_seal_sha256"],
            "--scoring-manifest", str(scoring_manifest),
            "--expected-scoring-manifest-sha256", build_receipt["scoring_manifest_sha256"],
            "--formal-rows", str(scoring_root / "formal_rows.json"),
            "--formal-predictions", str(scoring_root / "formal_predictions.json"),
            "--scoring-receipt", str(scoring_root / "scoring_receipt.json"),
        ],
        cwd=project_root,
    )
    ...
    receipt = {
        "schema": "cvs.phase2.adv3b02_ci_cell_receipt.v1",
        "status": "PROTOCOL_VALID",
        **{key: cell[key] for key in (
            "cell_id", "package_id", "receiver", "seed", "new_class_count", "method", "k_shot"
        )},
    ...
    if args.stage == "build_shard":
        for index, package in enumerate(plan["packages"]):
            if _selected(index, args.shard_index, args.shard_count):
                completed.append(_build_package(plan, package, project_root=project_root)["package_id"])
    elif args.stage == "smoke":
        smoke = next(item for item in plan["cells"] if item["cell_id"] == plan["smoke_cell_id"])
        if int(args.shard_index) != 0:
            raise ValueError("smoke runs only on shard 0")
        receipt = _run_cell(plan, smoke, packages[smoke["package_id"]], project_root=project_root, device=args.device)
        smoke_receipt = {
            "schema": "cvs.phase2.adv3b02_ci_smoke_receipt.v1",
            "status": "PASS",
            "package_id": smoke["package_id"],
            "method": smoke["method"],
            "k_shot": smoke["k_shot"],
            "cell_receipt_sha256": _sha256(Path(smoke["output_root"]) / "cell_receipt.json"),
        }
        _write_new(Path(plan["run_root"]) / "smoke_receipt.json", smoke_receipt)
        completed.append(smoke["cell_id"])
    else:
        if plan.get("launch_authority") is not True or plan.get("authority_state") != "N607_CI_SMOKE_PASS":
            raise ValueError("formal matrix lacks smoke-derived launch authority")
        cells_by_package: dict[str, list[dict[str, Any]]] = {}
        ...
        for index, package in enumerate(plan["packages"]):
            ...
            for cell in cells_by_package[package["package_id"]]:
                completed.append(
                    _run_cell(
                        plan, cell, package, project_root=project_root, device=args.device
                    )["cell_id"]
                )
    return {"status": "PASS", "stage": args.stage, "shard_index": args.shard_index, "completed": completed}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stage", choices=("build_shard", "smoke", "matrix_shard"), required=True)
