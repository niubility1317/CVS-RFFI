...
OUT_ROOT="${OUT_ROOT:-${ROOT}/runs/cvs_publication_adv3b02_feature_cache_20260713}"
LOG_ROOT="${LOG_ROOT:-${ROOT}/paper_reproduction/logs/cvs_publication_adv3b02_feature_cache_20260713}"
...
run_one() {
  ...
  printf '[FEATURE-EXPORT] scenario=%s gpu=%s out=%s\n' "$scenario" "$gpu" "$out_npz"
  if [[ "$DRY_RUN" == "1" ]]; then
...
if [[ "$failed" != "0" ]]; then
  echo "one or more feature exports failed" >&2
  exit 3
fi
echo "[FEATURE-EXPORT-DONE] out_root=${OUT_ROOT}"
