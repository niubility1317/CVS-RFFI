...
OUT_ROOT="${OUT_ROOT:-${ROOT}/runs/cvs_publication_adv3b02_fft96_singleview_20260714}"
LOG_ROOT="${LOG_ROOT:-${ROOT}/paper_reproduction/logs/cvs_publication_adv3b02_fft96_singleview_20260714}"
...
run_one() {
  ...
  printf '[FFT96-SINGLEVIEW-EXPORT] scenario=%s gpu=%s out=%s\n' "$scenario" "$gpu" "$out_npz"
  if [[ "$DRY_RUN" == "1" ]]; then
...
if [[ "$failed" != "0" ]]; then
  echo "one or more FFT96 single-view feature exports failed" >&2
  exit 3
fi
echo "[FFT96-SINGLEVIEW-EXPORT-DONE] out_root=${OUT_ROOT}"
