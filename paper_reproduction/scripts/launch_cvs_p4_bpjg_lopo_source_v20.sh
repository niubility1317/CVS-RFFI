...
EXPERIMENT_ID=qknnv42_p4_bpjg_lopo_source_k10_20260715_v20
...
for required in "$CONDA_SH" "$CHECKPOINT" "$SOURCE_VALIDATION" "$GROUND_STATE" "$TRAINER"; do
  if [[ ! -f "$required" ]]; then
...
if [[ -e "$OUT_ROOT" || -e "$LOG_ROOT" ]]; then
  echo "refusing to overwrite source-screen output" >&2
  exit 4
fi
