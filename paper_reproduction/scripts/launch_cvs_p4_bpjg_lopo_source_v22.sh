...
EXPERIMENT_ID=qknnv42_p4_bpjg_lopo_source_kgrid_20260715_v22
...
SCREEN_SHA256=ff061f84ea279bdee50299c1f2a7da83e7dd6abb9f75410771e7c420b07a25bc
ARM_INDEXES_RAW="${ARM_INDEXES:-0 1 2}"
...
labels=(JG020_K1 JG020_K5 JG020_K20)
ks=(1 5 20)
gpus=(0 1 2)
selected=()
...
for index in "${selected[@]}"; do
  ...
  if [[ -e "$out_dir" || -e "$log" || -e "$status_file" ]]; then
    echo "refusing to overwrite existing v22 arm: $label" >&2
    exit 4
  fi
done
...
for index in "${selected[@]}"; do
  ...
  pid=$!
  printf '%s\tjoint_gate\t0.02\t8\t5\t%s\t%s\t%s\t%s\t%s\t%s\ttrue\n' \
    "$label" "$k" "$gpu" "$pid" "$log" "$status_file" "$out_dir" >> "$manifest"
done
