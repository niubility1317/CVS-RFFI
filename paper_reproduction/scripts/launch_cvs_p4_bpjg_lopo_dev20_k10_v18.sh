...
EXPERIMENT_ID=qknnv42_p4_bpjg_lopo_dev20_k10_20260715_v18
CONFIG="$ROOT/paper_reproduction/configs/cvs_qknnv42_p4_bpjg_lopo_dev20_k10_20260715_n607.json"
...
if [[ -e "$OUT_ROOT" || -e "$LOG_ROOT" ]]; then
  echo "refusing to overwrite existing BP-JG-LOPO development output" >&2
  exit 5
fi
...
arms=(JG_R8_LOPO_LR005 JG_R8_LOPO_LR020 IJ_R8_LOPO_LR010 FJ_R8_LOPO_LR010)
scopes=(joint_gate joint_gate identity_joint fusion_joint)
learning_rates=(0.005 0.02 0.01 0.01)
learning_rate_tags=(0p005 0p02 0p01 0p01)
gpus=(0 1 2 3)
manifest="$OUT_ROOT/launch_manifest.tsv"
printf 'arm\tobjective\tscope\trank\tlearning_rate\tgpu\tpid\tlog\n' > "$manifest"

for index in "${!arms[@]}"; do
  arm="${arms[$index]}"
  scope="${scopes[$index]}"
  learning_rate="${learning_rates[$index]}"
  learning_rate_tag="${learning_rate_tags[$index]}"
  gpu="${gpus[$index]}"
  log="$LOG_ROOT/$arm.log"
  run_name="support_p4_${GROUND_SHA:0:12}_bp_jg_lopo_${scope}_r8_lr${learning_rate_tag}_rx_8-8_new_20_seed_713101_k_10"
  ...
  (
    ...
    "$PYTHON" -u -m paper_reproduction.scripts.train_export_cvs_support_lora_adapter \
      ...
      --adapt_objective bp_jg_lopo --adapter_type lora \
      --scope "$scope" --rank 8 --alpha 8 \
      --epochs 5 --optimizer sgd --max_optimizer_steps 50 \
      --learning_rate "$learning_rate" --weight_decay 0.0001 \
      --temperature 18 --grad_clip 1 --view_sampling_mode shot_index \
...
PY
  ...
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$arm" bp_jg_lopo "$scope" 8 "$learning_rate" "$gpu" "$pid" "$log" >> "$manifest"
done

printf '[P4-BPJG-LOPO-V18-LAUNCHED] manifest=%s\n' "$manifest"
