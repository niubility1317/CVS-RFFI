...
| Task definition | Section III: labeled source receiver plus unlabeled target receiver; `Y_s=Y_t` | `paper_reproduction/DADDA/data.py`; `train.py` dry-run payload | `tests/test_dadda_cross_receiver.py::test_dadda_dry_run_declares_closed_set_uda_not_cvs` | Implemented as closed-set UDA |
...
| Input shape | Section V-A: `2x256` IQ samples | `DADDAFeatureExtractor2D`; `build_manysig_task_datasets` | Model and data task tests | Implemented |
| WiSig subset | Section V-A: six transmitters, twelve receivers, four days; sample-count prose says 4000 samples. Empirical positive-control evidence indicates the local ManySig-compatible interpretation should be 4000 samples per transmitter within each receiver domain, i.e. 24000 total samples per receiver domain. | `build_manysig_task_datasets` validates counts and keeps `paper_domain_sample_count` as a diagnostic cap, but the paper-faithful config does not cap the local ManySig receiver domain to 4000 total samples. | synthetic ManySig fixture test and DADDA positive-control diagnostics | Implemented validation; total-4000 cap demoted to diagnostic-only |
| Backbone `G_f` | Section IV-B and Fig. 3: modified ResNet18 | `DADDAFeatureExtractor2D`; legacy `DADDAFeatureExtractor` retained for ablation | model shape test | Default config now uses the 2-D paper-shaped variant; still not guaranteed to be a full author-code replica |
| Multiscale `G_m` | Fig. 4: four branches `2x1`, `2x1+1x3`, `2x1+1x5`, `AvgPool+2x1` | `DADDAMultiscaleExtractor2D`; legacy 1-D variant retained for ablation | model shape test | Implemented with 2-D Fig. 4 kernel shapes |
...
| MMD Eq. (2) | global alignment on `G_f(X_s),G_f(X_t)` using one RKHS kernel | `mmd_loss` | objective test with shared batch bandwidth | Implemented with shared source/target bandwidth |
| LMMD Eq. (3)-(4) | source one-hot labels; target predicted probabilities | `lmmd_loss` | objective test | Implemented with shared source/target bandwidth and optional detached target probabilities |
