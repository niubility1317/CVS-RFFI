...
维护日期：2026-09-16。此文件是导航，不是第二套权限或gate。
...
| 实验命名/数据seed/存储模板 | ../.agents/skills/experiment-management/SKILL.md；../docs/EXPERIMENT_MANAGEMENT.md | 每次启动及历史整理 |
| 实验快速定位 | ../experiment_registry/README.md；tools/experiment_registry.py search/show | 新对话查找实验，先索引后命中证据 |
| 每次实验登记与状态 | 当前run/experiment.json、report.md、events.jsonl | 预登记、启动读回、状态变化、评分与交接 |
...
代码、报告、冒烟测试、临时文件及归档的查询与整理，先读[统一导航](../PROJECT_NAVIGATION.md)和[资料管理技能](../.agents/skills/project-file-management/SKILL.md)。当前清单、历史治理库桥接及恢复映射见[资料管理](../docs/project_governance/README.md)。先查现有索引，处理前核实当前路径/引用；保留代码镜像、独立变体和有追溯价值的冒烟证据，不凭同名、日期或tmp命名批量删除。实验管理及科学协议继续按原规则执行。
