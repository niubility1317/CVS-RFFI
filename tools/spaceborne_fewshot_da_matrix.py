...
def render_launcher(run_id: str, candidates: Sequence[Candidate]) -> str:
    ...
    blocks = [
        ...
        "STATUS=0",
        f"for idx in $(seq 0 {max(len(candidates) - 1, 0)}); do DONE[${{idx}}]=0; done",
        "",
        "reap_finished() {",
        "  local idx pid rc",
        '  for idx in "${!PIDS[@]}"; do',
    ...
    for idx, candidate in enumerate(_launcher_order(candidates)):
        cmd = _candidate_command(candidate)
        blocks.extend(
            [
                f'  if [[ "${{DONE[{idx}]}}" == "0" ]]; then',
                f'    GPU="{candidate.gpu}"',
                '    if [[ "${DRY_RUN}" == "1" ]] || gpu_slot_available "${GPU}"; then',
    ...
    blocks.extend(
        [
            '  if (( PROGRESS == 0 )); then',
            "    reap_finished",
            '    echo "[SPACEBORNE-FSDA-WAIT-ANY] remaining=${REMAINING} max=${STAGE2_MAX_ACTIVE_PER_GPU}"',
