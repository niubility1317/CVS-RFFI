...
5. **最小预登记**：所有CVS及外部对比按[experiment-management](../.agents/skills/experiment-management/SKILL.md)建立`automation_reports/CV-SincNet/<run-id>/experiment.json`、`report.md`和`events.jsonl`，记录名称/说明、候选/逐行矩阵、六类seed、数据契约和模型来源、commit、命令、环境/CWD、输入输出/log路径、GPU、技术停止规则和预期artifact。已有run保留原路径并补登记；新run不得覆盖。方法简述与科学晋级条件直接写在该报告，索引自动生成，不另设设计审批。
...
启动后、row/run状态变化及完成后，用`tools/experiment_registry.py record`追加带证据路径的事件并增量更新索引；手工补充配置后执行`build --managed-only`。索引是检索入口，不是运行状态或科学有效性的独立证明；历史数据、对比结果、失败/停止/替代/partial都保留。
