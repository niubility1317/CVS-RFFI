"""One Phase2 lane/GPU, waiting for registered source rows; truth last."""
import argparse
...
def main():
    ...
    def lane(gpu):
        ...
        for row in rows:
            update(row['row_id'], status='WAITING_SOURCE')
            while not (Path(row['source_root']) / 'completion.json').is_file():
                upstream = json.loads(Path(row['source_state']).read_text(encoding='utf-8'))
                if upstream[row['row_id']]['status'] == 'TECHNICAL_FAILURE':
                    update(row['row_id'], status='DEPENDENCY_FAILED')
                    return
                time.sleep(30)
            ...
            with Path(row['log_path']).open('x', encoding='utf-8') as log:
                child = subprocess.Popen([sys.executable, 'tools/run_practical_phase2_baseline.py', '--config', row['config_ref']],
                ...
                code = child.wait()
            completion = Path(row['output_root']) / 'predictions_complete.json'
            ok = code == 0 and completion.is_file()
            update(row['row_id'], status='PREDICTIONS_COMPLETE' if ok else 'TECHNICAL_FAILURE',
    ...
    if all(row['status'] == 'PREDICTIONS_COMPLETE' for row in state.values()):
        truth = Path(spec['data']['dataset']).parent / 'score_only' / 'truth.json'
        with (logs/'scorer.log').open('x',encoding='utf-8') as log:
            result = subprocess.run([sys.executable,'tools/score_practical_phase2_baselines.py',
                '--run-root',str(runroot),'--truth',str(truth)],cwd=ROOT,stdout=log,stderr=subprocess.STDOUT)
        (runroot/'scorer_exit.json').write_text(json.dumps({'exit_code':result.returncode,
            'artifact_exists':(runroot/'scored_results.json').is_file()}),encoding='utf-8')
