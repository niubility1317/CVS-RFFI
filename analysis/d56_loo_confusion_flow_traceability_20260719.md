...
|需求|实现|验证|状态|
|---|---|---|---|
|D46底座|继承类级LOO full/block融合|D46回归|PREREGISTERED|
|support混淆图|inner-held独立argmax生成`y→p`|分区、rank覆盖、图重算|PENDING_IMPLEMENTATION|
|零和修正|`Delta b=(out-in)/(K*C)`|边流守恒与数值和为0|PENDING_IMPLEMENTATION|
|类对称|无class ID/role/scene/receiver分支|类标签置换测试|PENDING_IMPLEMENTATION|
