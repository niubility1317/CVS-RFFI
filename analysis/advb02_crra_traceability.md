...
| CRRA-11 | 信道元数据来自同一次`mixed_orbit`生成，不重新生成第二视图 | `code/baseline_origin_sat_view.py`、`code/cvsrffi/eval.py` | planned | `test_crra_mixed_orbit_metadata.py` |
| CRRA-12 | Phase1禁止目标接收机访问和目标adapter-only校准 | `code/SSDG/train_ssdg.py`、`code/tests/test_crra_protocol_negatives.py` | planned | protocol negative tests |
| CRRA-13 | 旧checkpoint在CRRA关闭时兼容加载 | `code/post_stage_common.py`、`code/cvsrffi/checkpoint_loading.py` | planned | `test_exact_ssdg_checkpoint_loading.py` |
