...
本表状态针对**实现正确性**：42项`verified`、6项`deferred`，0项`pending`/`blocked`/`rejected`。T1–T9代码及57项聚焦测试/旧接口回归通过，真实H0/source技术链路通过；详见[验收报告](core90_anchored_geometry_acceptance_20260911.md)。`verified`不代表完整source预算、收益或独立确认已完成；这些仍需实际科学实验结果。

`deferred`均有报告规定的分阶段原因；不因缺少后续实验数据阻止当前source主线实现。source章节编号来自原报告，所列文件均相对当前隔离Git承载面。

|ID|来源章节|具体要求|目标文件/任务|状态|拟采用验证|备注|
|---|---|---|---|---|---|---|
|R01|2.1、8|冻结原H0完整推理含义|`anchored_cache.py`，T1|verified|真实H0完整/fast/identity三方前向对照|不只保留weight|
|R02|2.1|baseline不含标签margin|`anchored_cache.py`，T1|verified|修改监督label不改变s0；labels=None与CosFace原推理对照|margin logits只能用于监督支路|
|R03|3.1|L_s、U_s、V角色不串用|`anchored_cache.py`，T1/T7|verified|训练拒绝V/U_s/target角色，标签只在合法层存在|当前G不使用U_s|
|R04|3.2|一次提取绑定来源的缓存|`anchored_cache.py`，T1|verified|checkpoint/contract/preprocess/view不匹配拒绝命中|哈希仅缓存身份，不新增验证链|
|R05|3.2|eval/no_grad与普通训练tensor|`anchored_cache.py`，T1|verified|cache重载参与weight反传；BN/参数不更新|不把eval当作关闭autograd|
|R06|3.2–3.3|physical ID与view唯一性和权重|`anchored_cache.py`、`anchored_fit.py`，T1/T3|verified|同样本多view总权重不变；所有view同fold|view不是shot|
|R07|2.2|共享低秩正定M，r=4/cond≤2|`anchored_geometry.py`，T2|verified|谱/正交/稠密参考/数值边界|允许正负log特征值修正|
|R08|2.2–2.3|a=0退化H0且可学习|`anchored_geometry.py`，T2|verified|H0等价；非退化数据b梯度非零|不使用全零AAᵀ死初始化|
|R09|2.2|保持角度范数不变性|`anchored_geometry.py`，T2|verified|正缩放、多幅度、eps边界测试|eps区不声称严格尺度不变|
|R10|2.2–2.3|W/tau冻结、无类bias/自由协方差|`anchored_geometry.py`，T2|verified|trainable参数白名单与step后不变|首轮仅B/b共644参数|
|R11|9.1|低秩公式与类别投影预计算|`anchored_geometry.py`，T2/T9|verified|dense参考/梯度；推理分配和计时|不每样本生成M|
|R12|3.3|匹配clean/LEO配对拟合|`anchored_fit.py`，T3|verified|相同物理/增强seed、多view权重、四场景生成记录|clean/LEO各0.5是建议起点|
|R13|3.3|180physical/360feature/35batch|`anchored_fit.py`，T3|verified|全量无重复无遗漏；4RX/3RX折同比例|公平报告step而非仅epoch|
|R14|4.1–4.2|分类、正确样本keep、metric正则|`anchored_fit.py`，T3|verified|错误H0样本无keep梯度、截断/无正确样本边界|不对全部样本做教师KL|
|R15|4.3、10.2|损失贡献看加权梯度|`anchored_fit.py`，T3/T9|verified|分别求梯度范数/夹角不增加step|loss负或大不等于梯度主导|
...
|R35|6.2、9.2|复用正确边缘化与低秩NLL|`partial_gaussian_head.py`，T8|verified|低秩/稠密对照；仅一次solve；全缺失|共享logdet不是类判别新信息|
...
|R48|10.3|独立确认数据及完整骨干多seed|计划第1.1、9.2、12节|deferred|更独立目标的实际契约与全部继承来源；truth-last|本次完成实现验收，独立确认数据未指定；旧target不洗白|
