...
可用Woodbury把偏移均值求解降为一个`6×6`系统。首个功能版本也允许在row enrollment中使用数值等价的`160×160`稠密解与瞬时协方差，以先验证真实功能；它们不得进入持久状态，且必须记录实际开销。该实现优化不改变任何预测公式，也不构成G0前的阻塞项。冻结资源目标为：
...
`v_b`的trace可使用相同低秩解、固定解析trace或瞬时稠密协方差；不得持久化全精度`160×160`协方差。
