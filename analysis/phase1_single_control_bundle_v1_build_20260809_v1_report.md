...
## 9.N607执行终态（2026-08-09）

状态：`STOPPED_EARLY_SYSTEMIC_TECHNICAL_FAILURE / NO_PERFORMANCE_RESULT`；唯一启动一次，`retry_authorized=false`。本节覆盖并取代§1中的`NOT_LAUNCHED`运行状态；不含任何性能结论。

|项目|证据|
|---|---|
|direct preflight|PASS；N607直连身份、项目根、Python、GPU可见；8卡均0%/1MiB。每次SSH/SCP后本地无残留`ssh.exe`/`scp.exe`及TCP/22。|
|archive|no-prefix tar本地与远端SHA256=`2be3348223355bcf95b63681d610e512f525a3355a0aaf98f465d86f013e8a41`；大小262778880B；commit=`bb5b7f05dbb3c94e3925fd8754a60321250fb94b`。|
|release/CWD|`/home/szu2070436088/2510044040/CV-SincNet/releases/phase1_single_control_bundle_v1_build_20260809_v1_bb5b7f05`；解包后无`code/code`；CWD为release根。|
|远端归档成员SHA|`phase1_single_control_bundle_v1.py=e72fad2fa64c04154276df5771d71bd90f70763ed4e48e50f4feed74033b4fbf`；`build_phase1_single_control_bundle_v1.py=c843a49ec152105f7841e941c4415d80f9a4e7e47c88a3cfec2e39a844ad151b`；`test_phase1_single_control_bundle_v1.py=b10127ace625a7dce158cf33fcf3879ef8ca7d408800050545ea3f9a56523814`；design=`426a7063b57eda7590a2b27f3641a461d0d129d5ad9e7ab934757955c0369d3f`；trace=`9a465f4f0c6ee73e547b727fb586ff6366c896839ffc2d887be0ed3700611607`。工作树SHA仍见§4；差异仅归档LF与Windows工作树换行口径。|
|远端静态核验|固定Python`py_compile`三代码文件PASS；CLI`--help`PASS。|
|唯一启动|PID=`397060`；GPU=`7`；命令/CWD按§5冻结命令，stdout+stderr=`/home/szu2070436088/2510044040/CV-SincNet/logs/phase1_single_control_bundle_v1_build_20260809_v1/build.out`。|
|失败指纹|PID已退出；未创建run output或staging。`build.out`（1834B）为未捕获`SingleControlBundleError: model config resolved dom_feature_key must be a string without implicit conversion`，栈：`build_phase1_single_control_bundle_v1.py:173`→`phase1_single_control_bundle_v1.py:3232,1540,1477,207`。该确定性loader/config异常触发冻结技术停止规则，不重试。|
|sanitized checkpoint args|固定Python只读审计确认`dom_feature_key`为`present=false,type=null`（不是None）；同次输出仅28项键presence/type/repr，未读取性能。|
|PID/exit receipt|远端`.../logs/.../build_pid_exit.json`：547B，SHA=`62455e807ee8512f1bf3914ceb1075ca8fe3f6a3747fb5418ab022afb520a913`；记录PID已消失、Python未捕获异常按约定exit=1、retry=false。|
|小artifact回收|本地[`artifacts`](E:/type10-7/automation_reports/CV-SincNet/phase1_single_control_bundle_v1_build_20260809_v1/artifacts)：`build.out`1834B，SHA=`683b1277404c9290f38fe9f02cbcebfab0809cf312bb10fe82e04e885d6507ef`；`build_pid_exit.json`547B，SHA=`62455e807ee8512f1bf3914ceb1075ca8fe3f6a3747fb5418ab022afb520a913`。未回收checkpoint/ManySig/runtime/NPZ。|
|终态清理|GPU7回到0%/1MiB；run/staging无残留、release staging无残留；无run-owned进程；SSH/SCP/TCP22均已断开。10-member bundle、content_root、strict verify、resource/parity/state/N1门未执行且不宣称通过，因为真实构建在配置解析前失败。|
