...
|---|---|---|---|---|
|D47-01|复用D42–D46同一fixed received-IQ development cell、old-only B20和15个outer held折，不打开query|probe/receipt|verified|105/105行；development seed713101；query0|
...
|D47-06|该规则仅称“positive-part anchored reliability shrinkage”；不得称为校准后的经验贝叶斯posterior|claim/report|verified|metadata锁定`eb_inspired_deterministic_shrinkage_not_calibrated_posterior`|
...
|D47-11|真实105行输出的协议、lifecycle、source、ground、state、resource与artifact闭包全部通过|probe|verified|elapsed73.7794s；30条D47 fit row与105行总闭包通过|
|D47-12|相对D42全部通用聚合/场景/floor/forgetting/混淆门通过；new与min-new不低于D46，rain旧类/forgetting至少恢复到D42|decision|rejected|forgetting10.00pp>8.89pp；rain after76.67%<78.33%、forgetting13.33pp>10.00pp；new84.00%<84.67%、min-new70.00%<73.33%|
|D47-13|D47相对D46至少改变1个final held预测；若`tau²=0`使其退回D45或全部预测不变，则机制拒绝且不加第二arm|decision|verified|2/15个outer SHA、2/330个final argmax相对D46改变；但0/15和0/330相对D45改变|
|D47-14|before/final int8-FP32 argmax变化与margin翻转均为0|decision|verified|真实15折为`0/0/0`，max score error0.0016141|
|D47-15|探针不生成125、不访问N607；仅在另行正式化候选且开发门全过后生成125 handles|scope|verified|当前仅本地support-held开发流程|

当前计数：`pending=0`、`implemented=0`、`verified=14`、`deferred=0`、`rejected=1`、`blocked=0`。
