...
状态：`RETROSPECTIVE_COMPLETE / JOINT_CANDIDATE_RESEARCH_IN_PROGRESS / NO_RUN_AUTHORITY`

## 1.目标逐项追溯

|ID|来源|要求|目标文件或证据|状态|验证|说明|
|---|---|---|---|---|---|---|
|R1|持续目标、`项目.md`|遵守`p2_min_v1`、单LEO观测、K个独立物理support、query零fit/update/selection、全注册类逐样本竞争|新method lock、runtime负测、真实checkpoint smoke|pending|尚未形成NEXT-R6实现|不得借用外部source/clean状态|
|R2|持续目标|同时研发轻型DA与精简D92头，不能把Q-only当最终联合方案|NEXT-R6方法锁、六臂矩阵|pending|两位Terra/max作者并行设计|FA-RDCE3只作为已证实DA核心|
|R3|持续目标|候选不超过3条、完整负收益立即关闭、不盲调|本文件、实验报告|verified|D130两候选、CER与LOO头均按完整或独立反证关闭|NEXT-R6本轮只允许1条原理不同候选|
|R4|持续目标|同一真实checkpoint/received-IQ上执行2种表示×3种头共享缓存六臂|NEXT-R6 plan/runtime/scorer|pending|尚未冻结|臂为DA0/DA1×Q/Full机制对照/Lite新头|
|R5|持续目标|分别证明DA、Lite头和联合替换提升同row H与总正确数且不牺牲old、seen-new和floor|四状态score与主效应表|pending|尚无NEXT-R6性能|REG0的seen-new/H必须为`N/A`|
|R6|持续目标|实测降低D92状态、拟合计算或时延|resource receipt|pending|D130仅证明Lite160效率，不证明NEXT-R6性能|必须与formal D92资源口径分开|
...
最高风险不是实现，而是头部可辨识性：K5 support足以估计共享对角统计，但support代理上的margin或floor证书不能自动外推到query；Q与新头若没有共同评分尺度，任何连续混合系数都缺乏可解释性。独立监督必须在冻结前给出`MERGE/REVISE/REJECT`。
