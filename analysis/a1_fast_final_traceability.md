...
最新授权：不使用历史checkpoint；使用CORE90须在相同实验配置下重训。执行新CORE90从零E200→本轮checkpoint→三组A1各E200，保留RC4锚点语义，不采用临时拟议的无锚点方案。
...
|R04|4|批量教师|train_ssdg.py|verified|CPU输出及梯度；拒绝train/无running-stat BN|GPU/AMP在发布smoke核验；容差不代替性能|
...
|R10b|8.4|EMA缓存回归和修复分离|原CosFace/_update_ema_model|deferred|确认.data更新后自动与强制刷新输出不同|已知旧缺陷；三组保持，修复归Stable|
