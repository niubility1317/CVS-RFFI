...
|P1-TARGET-METRICS|目标2.3(3)(4)、2.4、2.5|独立truth-side scorer封存known DG、unknown rejection、open-set、scene／RX／class／day结果|`phase1_clic_target_metrics_20260812_v1`|pending|N607 clean release已落地，当前STATIC／formal launch阶段|此阶段不使用ADV；固定`ADV_COMPARISON_PENDING`|
|P1-U70|目标2.5、2.6(5)(6)|真实unknown显式拒识率global及clear／low-elev／rain分别≥0.70；defer不计分子；known拒绝按错|12份target metrics receipt|pending|需读取完成后的不可变receipt；失败仍是有效结果且禁止重跑反馈|AUROC、AUPR-out、FPR95、FAR、safe handling、coverage、defer均须同一行|
...
1.完成`phase1_clic_target_metrics_20260812_v1`并封存12臂同一行target-known DG与真实unknown指标；不按结果调参或重跑。
2.在读取／使用target结果进行任何方法决策前，冻结source-V clean／单观测LEO weak评测口径与6fold ADV3B02配置等价训练矩阵。
