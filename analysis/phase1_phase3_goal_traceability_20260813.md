...
|P1-ADV-CONFIG|目标2.4|ADV3B02与候选训练配置、known测试配置和指标定义逐字段等价；不要求同一capsule字节|`phase1_adv3b02_clic6_20260816_v2`六fold技术工件；待生成sealed train／known config|pending|六fold checkpoint／terminal／completion已技术封存；Task2 sealer本地实现并独立ALLOW，尚未生成六份真实sealed config|技术工件完成不等于配置等价闭合；不得把旧checkpoint改名|
|P1-ADV-PREDICT|目标2.4|从精确baseline terminal tuple与checkpoint-bound WiSig物理轴封train-config；3120个opaque target row逐行一次forward，零fit／update／retry／selection|`code/evaluate_phase1_adv3b02_target_leo.py`；`code/tests/test_phase1_adv3b02_target_reference.py`；待生成六fold blind prediction|implemented|本地完整测试fresh `34/34`；独立`P0=0/P1=0/P2=2/ALLOW`；真实SSDG重建得到有限4-logit|仅`LOCAL_VERIFIED`；未访问N607、未生成blind artifact、无性能结果|
|P1-ADV-REF|目标2.4|每foldADV reference含三scene overall／macro／min、class、RX、day及class×RX／class×day交叉单元|待建ADV truth-side评测／reference ingest入口|pending|现有combined scorer保持缺reference fail-closed|Task3必须等待Task2不可变blind prediction工件；同foldC／G可共享一个配置等价reference，但比较各自local4|
