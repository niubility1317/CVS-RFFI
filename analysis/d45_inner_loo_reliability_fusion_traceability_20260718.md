...
|---|---|---|---|---|
|D45-01|复用D42–D44同一fixed received-IQ development cell、old-only B20和15折，不打开query|probe/receipt|pending|真实105行待执行|
...
|D45-08|聚合、逐类、逐场景、forgetting、joint与三类混淆相对D42全部不退化且final floor严格改善|decision|pending|`1e-12`固定门|
|D45-09|before/final int8-FP32 argmax变化与margin翻转均为0|decision|pending|真实15折待执行|
...
|D45-12|D45完成后执行D43–D45强制技术复盘|retrospective|pending|复盘后才能D46|

当前计数：`pending=4`、`implemented=0`、`verified=8`、`deferred=0`、`rejected=0`、`blocked=0`。
