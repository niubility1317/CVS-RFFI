...
|T03|`项目.md`7.1|sample-level LEO overlay provenance、scenario、satellite seed和post-channel IQ SHA在打开IQ payload前进入独立trust root|`somph_predictor_bundle.py`;专用sealer|implemented|package sealer要求外部`overlay_provenance_sha256`；错误checkpoint/provenance均在IQ前阻断；跨场景token与support assignment检查PASS|真实overlay exporter、源包clean-unreachability与实际OS ledger尚缺，因此不升为正式`verified`|
...
|T05|`项目.md`7.2、9.3|注册前/后采用物理独立统一class registry包；predictor内部无old/new边界，scorer侧才配对角色与physical ID|`somph_predictor_bundle.py`;`somph_predictor_runtime.py`;`somph_metric_scorer.py`|implemented|head绑定stage/state/receiver/seed/K/registry/enrollment package/support token与feature摘要；artifact v2只携带当前registry count与opaque snapshot|before/after物理包schema已实现；独立pair/K-family manifest尚未接入真实runner，不能用after切片冒充before|
...
|T10|`项目.md`10.3、`项目实验.md`14–16|开发K10锁定后完成5receiver×至少5确认seed×3场景×new5/10/20及K1/5/10/20、逐类/逐receiver/日志/Pareto|matrix、runner、自动化报告|blocked|本地缺713106与formal maxK20真实sealed包|数据覆盖补齐前不启动正式矩阵|
...
|ID|要求|状态|证据/下一步|
|---|---|---|---|
|R01|Phase2`LEO_weak-only`且clean/clean-derived物理不可达|structural_only_real_recompute_required|专用bundle要求外部overlay provenance SHA、精确成员与sample-level自洽；runner绑定外部pre-run/runtime SHA和固定system-root allowlist；但真实overlay exporter、target-only包和Linux OS ledger尚未接入|
...
|R04|Stage2-B注册前与Stage2-C注册后matched pair|structural_pair_only|registration-pair结构与scorer指标已实现；当前physical-ID摘要仍未从真实sealed包重算，正式路线改为两个物理独立统一registry包并由scorer配对|
