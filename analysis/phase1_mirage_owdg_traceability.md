...
| T3-PROXY-01 | Task 3简报；设计规格§3.3、§5.4、§10 | 只从获批的labeled split生成确定性、周期平衡且标签置换等价的source proxy episode；proxy类在registered logits/prototypes前移除；收据只保留置换不变统计；全部异常输入与未获批角色fail closed。 | `code/cvsrffi/phase1_mirage/proxy.py` | `tests/phase1_mirage/test_proxy.py` | verified | RED为缺少`proxy`模块的17项失败；GREEN为proxy测试17项通过，限定Task1-3回归39项通过。实现复用`Phase1DataPolicy`的`proxy_train/P_cal/P_select`唯一origin语义，不复制权限表。 |
