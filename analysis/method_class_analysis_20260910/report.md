...
### 第一条训练路线：复盘已有跨接收机配对，保护身份margin

本报告冻结的八行中，ECRS跨RX成功步数全为0；B1完整stdout明确记录`balanced_sampler=0`。但本对话另有完整X0—X7矩阵，X1—X7已真实启用跨RX并完成E200，不能说“当前实验都没尝试”。X1的clean跨RX相对X0仅提高target LEO均值0.3921pp，X2追加LEO相对X1下降0.3093pp。更关键的是，X5均衡采样确实开启，但相对X2下降3.6921pp；X7组合下降4.1087pp。详见[全对话机制审计](../mechanism_activation_audit_20260910/report.md)。

因此下一步应复盘已有X1/X2/X5的source类×RX覆盖、样本重复、margin和梯度方向，而不是原样重跑均衡采样。既有`labeled_cross_rx_objective`使用同TX不同RX正例，以及不同TX但同RX/day/view负例；X1平均127.916/128个有效anchor，覆盖已充分。损失覆盖不是主要缺口的充分解释。若source诊断支持修改采样或配对，应把分布变化与损失变化分开，不从U_s读取TX真值。该实现复用z_id，无新增主干前向；O(B²)计算的实际耗时和显存仍需匹配测量。
...
|B|复盘已有X1/X2/X5的source采样与margin|解释充分配对下X5为何仍退化|已有负向结果，不原样重跑|
|C|仅按诊断修改cross-RX或采样的一项|同类跨RX偏移减小且类间margin保留|保留X1匹配参考，不把小幅单seed收益当定论|
...
不建议一次把B–E全部打开，X7已有组合退化证据。保留各自匹配对照，source诊断支持后才检查连续学习率与cross-RX机制的单独作用及交互；B1有source侧改善证据，可以作为候选日程，但不能因为其当前target最高就把它选为正式晋级基座。另一个延长预算的X2_E600还出现source机会水平和连续零身份梯度探针，应优先定位其训练退化。类似地，不能直接采用“B1专治类1、F3专治类3”的真值路由。
...
最值得推进的是把source LEO从一个总体均值扩展为类别与接收条件的可解释风险，结合已有X系列正向有限、均衡采样负向及延长训练退化的证据，定位身份几何和优化问题。跨RX是已有小幅信号的候选，不是尚未尝试的必然增强方向。
