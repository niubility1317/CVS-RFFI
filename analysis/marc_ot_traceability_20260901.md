...
设计来源：`docs/superpowers/specs/2026-09-01-marc-ot-design.md`、`docs/superpowers/plans/2026-09-01-marc-ot.md`、`项目.md`及Task1～8实现报告。冻结代码提交为`cba32b2bfb59e3a0e26979f4b1237452ab0839e3`。
...
|BANK-01|设计4|稳定block schema、delta提取、确定性低秩重构、base checkpoint和bundle geometry绑定、禁止成员检查|`code/cvsrffi/meta_weight_bank.py`、`meta_weight_bank_checkpoint.py`、`marc_ot_phase1.py`|verified|bank/bundle测试覆盖canonical block、bitwise冻结、SVD、round-trip、geometry、禁止成员及训练后严格bundle回读；最终聚焦回归通过|测试bundle来自合成fixture；未生成真实Phase1训练bundle，不声明bank数值质量|
...
|OT-01|设计2、6.3|OT只发生在support与冻结bank task features之间；FP32 log-Sinkhorn、bank detach、边际和有限性检查|`code/cvsrffi/stage2_marc_ot.py`|verified|OT测试覆盖均匀边际、确定性、bank detach、非法输入和未收敛失败；最终聚焦回归通过|公开API无query/truth/role/quota参数|
...
|SCORE-01|设计2、8、10|prediction纯预检完成后只打开truth一次；输出REG0绝对指标、同row增量、per-class、help/harm和资源状态|`code/cvsrffi/stage2_marc_ot_scoring.py`、`code/scripts/run_stage2_marc_ot_pilot.py`|verified|scoring与CLI测试覆盖prediction闭合、truth-last、同row重算、资源schema和old-only REG0|未连接真实truth sidecar，无性能结果|
...
|ID|要求|状态|证据或原因|
|---|---|---|---|
|VERIFY-LOCAL-01|Task1～8完整聚焦pytest、CLI help、14模块compile、JSON和最终diff检查|verified|fresh聚焦回归`332 passed`、0 failures、exit0；CLI help、14模块`py_compile`、JSON和最终文档`git diff --check`均exit0|
|REVIEW-01|一次最终独立P0/P1审查|pending|Task7三项P1与Task8 P0/P1已关闭；本次最终独立P0/P1审查仍由主Agent执行，本Task不替代|
...
当前反向审计统计为`verified=14`、`implemented=0`、`pending=3`、`blocked=1`、`deferred=1`、`rejected=0`。软件能力层已闭合首轮设计范围；最高本地状态为`LOCAL_VERIFIED / FINAL_P0P1_REVIEW_PENDING / N607_NOT_RUN`。这不代表真实Phase1训练覆盖、真实bundle、checkpoint smoke、K10 pilot、truth-last评分或任何性能结论；`pilot_executed=false`保持不变。
