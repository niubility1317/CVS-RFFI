...
|TPCE-07|单FULL fit和低注册计算|E0 FULL实际fit=1；TPCE新增fit=0；两次support打分加一次码更新|fit inventory、support MAC和wall/peak receipt|DESIGN_FROZEN|
...
|TPCE-11|数值失败精确回退|非有限、无可移动坐标、聚合饱和、零码差或任一守卫失败均返回原E0 state|byte-exact fallback及reason测试|DESIGN_FROZEN|
