...
F仅作历史机制对照，不属于NEXT-R6部署候选。K1的F严格alias Q。artifact主状态统一为`DA0_REG0、DA1_REG0、DA0_REG1、DA1_REG1`；REG0的held-proxy准确率和H_proxy为`N/A`。
...
完整prediction封存后一次性打开truth并评分。K5的`DA_EFFECT_Q、LITE_BASE、JOINT_REPLACE`必须各自使H_proxy和总正确数严格增加，且`A_retained、A_held_proxy、F_retained`不下降；`DIRECT_UTILITY`必须使H_proxy与总正确数严格增加且三个guardrail不下降。K1要求RPPF相对同row F/Q提高H_proxy与总正确数，并且retained、held-proxy和floor不下降。
...
3.K1四个alias关系逐logit、state SHA、prediction和资源计数闭合。
