...
|---|---|---|---|---|---|---|
|RAU-01|Abstract, Sec.III-A|两阶段UDA：先DANN全局域对抗，再LMMD相关子域适配。|`paper_reproduction/receiver_agnostic_twostage_uda/model.py`; `losses.py`; `train.py`|partial|模型/loss原语与Eq.16组合目标有单测；正式训练入口仍gate|Stage1/Stage2正式训练循环未落地，不能声明完整复现。|
...
|RAU-04|Eq.14-Eq.16, Fig.3|LMMD按source one-hot label与target预测概率计算类别权重，支持多层activation；Eq.16组合source CE与LMMD。|`losses.py`; tests|partial|LMMD backward、target_probs校验、Eq.16组合目标测试|正式Stage2训练循环未落地；缺真实ManySig batch覆盖验证。|
|RAU-05|Sec.III-C, Fig.4|fine-tuning使用target不确定性采样得到少量标注样本，并混入少量source样本保持source性能。|`sampling.py`; `train.py`; tests|helper-only|uncertainty/random排序、1/50预算和source replay helper单测|缺fine-tune训练循环、source replay loader/loss和性能曲线。|
|RAU-06|Sec.IV-A|WiSig ManySig，6 TX、12 RX、4 days；equalized time-domain preamble，power normalization，保留CFO，取前256 IQ并组织为`[batch,1,256,2]`。|`model.py`; config/report|partial|config/protocol校验4 days；模型入口强制256 IQ|正式数据split/loader/preprocess长跑未执行，预处理执行证据仍blocked。|
...
|RAU-08|Sec.IV-C Fig.7|总体实验按source:target receiver比例`R`评估：source-only下界、target-labeled retrain上界、DANN、DANN+LMMD。|`configs/receiver_agnostic_twostage_uda_manysig_paper_faithful.json`; `train.py`; `protocol.py`|partial|CLI dry-run只生成论文矩阵骨架|Fig.7固定比例、TableI论文参考值、Fig.8迭代点和split/seed未公开边界已登记；正式训练结果未生成。|
...
|RAU-10|Sec.IV-C Fig.8|`R<4:8`时比较fine-tuning不确定性/随机采样策略和迭代次数；标注样本数为unlabeled dataset的1/50。|`sampling.py`; `protocol.py`; report|helper-only|sampling helper、random seed和100iteration报告点测试|fine-tune训练循环和100iteration曲线待正式实现/验证。|
