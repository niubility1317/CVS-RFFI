...
|HCF-009|第六节|receiver→day→channel/联合swap课程|`phase1_hcfdg/config.py`、`trainer.py`|pending|日程测试|same-TX receiver先行|
...
|HCF-013|第十节|六类以内核心总损失及冻结权重|`phase1_hcfdg/config.py`、`losses.py`|pending|配置/总loss测试|V1按候选启用子集|
...
|HCF-018|第十二节|4000-update V1和6300-update V2|`phase1_hcfdg/config.py`、`trainer.py`|pending|精确update测试|不以U loader定义epoch|
...
|HCF-022|第十五节|A0–A5快速筛选矩阵|launcher、matrix config|pending|36行dry-run|2 folds×3 seeds|
|HCF-023|第十五节|A6–A9深层外推矩阵|launcher、matrix config|pending|依赖门槛测试|A4/A5通过后|
|HCF-024|第十五节|A10–A12残差矩阵|launcher、matrix config|pending|依赖门槛测试|A8/A9通过后|
|HCF-025|第十六节|5-fold×3-seed确认及最终8-seed|launcher、scorer|pending|矩阵计数测试|仅source选模|
...
|HCF-033|第二十节|按报告顺序推进，不堆叠未过机制|matrix policy|pending|依赖图测试|低性能不算技术失败|
