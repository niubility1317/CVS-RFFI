...
|R4-P0-QBIND-01|3、5|predictor/runtime/prediction只保留与类别无关的扁平query physical/observation ID及root/count；class→query映射只留truth sidecar|matrix、runtime、artifact、score、CLI|implementing|Terra/max负责core；随后Luna/max机械适配CLI；最终独立复审|不改变24行矩阵或方法，只关闭query truth/role泄漏|
