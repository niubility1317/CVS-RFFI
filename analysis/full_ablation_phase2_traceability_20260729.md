...
|P2-TR-32|9.1、11|正式predictor package生成的v2 truth sidecar必须在不改变rows语义的前提下发布成当前formal scorer要求的v3 Stage2-A与Stage2-B sidecar；未知schema、顶层漂移或非法rows必须在写出前拒绝|`stage2_ablation_scoring_sidecars.py`；Stage2-A/B发布CLI；sidecar兼容测试；v10/v11/v12报告|verified|v10 feature达到A/B/C各100且300/300 loader通过后，首个Stage2-A sidecar因source schema=v2而fail-fast；兼容实现仅接受精确顶层的v2/v3 Stage2-B truth，先校验rows，再发布v3：Stage2-A仅改stage，Stage2-B保持stage2b，两者rows不变且由正式loader验收；fresh v11 Stage2-A已25/25验证通过|v10 sidecar失败根永久保留为`NO_PERFORMANCE_RESULT`；registry/seal前还需fresh v12 Stage2-B v3 sidecar 25/25逐项正式loader验收|
|P2-TR-33|9.1、11、13|CUDA row executor必须在重置峰值显存统计前显式完成device选择与CUDA context初始化，并且CUDA初始化失败不得创建row输出根|`stage2_ablation_row_executor.py`；row executor测试；v13报告|implemented|v13正式states首波在预测前以`reset_peak_memory_stats`首次触发CUDA而14行同指纹失败，15行落地、0 prediction/score、308未启动并正确系统停派；修复顺序为`set_device`→`torch.empty(0,device)`→`reset_peak_memory_stats`，并把初始化整体移到output root创建前；相关4文件23项通过|待独立复审、Git封存、按新code commit重生成states/Stage2-C计划并用fresh v14 run ID发布；v13永久为`NO_PERFORMANCE_RESULT`|
