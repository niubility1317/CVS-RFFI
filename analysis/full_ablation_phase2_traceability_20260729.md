...
|P2-TR-32|9.1、11|正式predictor package生成的v2 truth sidecar必须在不改变rows语义的前提下发布成当前formal scorer要求的v3 Stage2-A sidecar；未知schema、顶层漂移或非法rows必须在写出前拒绝|`stage2_ablation_scoring_sidecars.py`；sidecar兼容测试；v10/v11报告|verified|v10 feature达到A/B/C各100且300/300 loader通过后，首个Stage2-A sidecar因source schema=v2而fail-fast，其余24项未启动；兼容实现仅接受精确顶层的v2/v3 Stage2-B truth，先校验rows，再只改顶层schema→v3和stage→stage2a，成功产物由正式loader验收；相关23项与独立组合36项通过，代码复审P0=0、P1=0|v10 sidecar失败根永久保留为`NO_PERFORMANCE_RESULT`；fresh v11必须25/25逐项正式loader验收后才允许registry/seal|
