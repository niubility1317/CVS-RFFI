...
|MRI-08|Sec.IV-D, Eq.(10)-(11)|总目标为`min_{theta_E,theta_C} max_{theta_T} weighted CE + lambda*zeta`，源/目标分类损失由`mu`权衡。|`losses.py`: `gada_minimax_objective`; `algorithm.py`: `gada_batch_step`; `train.py`: `run_gada_training_loop`|implemented_helper_level|单测确认`gada_minimax_objective`返回`loss/loss_weighted_ce/loss_source/loss_target/loss_kl`，batch step执行`T`上升、`E/C`下降，helper级训练循环可跨epoch汇总history并保存checkpoint|正式WiSig split/loader/CLI runner未接入，未声明表格结果。|
|MRI-09|Algorithm 1|GAD训练：每batch先更新`T`共`m`次，再生成伪标签、计算class weight和总损失，更新`E/C`。|`algorithm.py`: `PseudoLabelState`, `gada_batch_step`; `train.py`: `run_gada_training_loop`; `protocol.py`; config|implemented_helper_level|单测确认`estimate_steps=m`时`optimizer_t.step()`执行`m`次、`optimizer_ec.step()`执行1次并更新`sigma/sigma'/n_t`状态；默认路径覆盖论文`m=7`；训练helper要求无标签target batch且保存checkpoint|正式ManySig数据加载、初始`h0`文件接入和Table II长跑仍待N607执行。Eq.(9)首批无历史预测时使用全1权重、空伪标签batch目标loss为0、零计数阈值下限均列入paper-unspecified。|
|MRI-10|Sec.V-A|实验子集为WiSig 6个Tx、12个Rx、4天；cross-receiver任务包括`14-7->3-19`、`1-1->1-19`、`1-1->8-8`、`7-7->8-8`，另有`d01->d23`跨天控制。|config; `protocol.py`; dry-run payload|implemented_plan_only|config和validator记录`capture_days=4`，dry-run写入任务行|尚无真实ManySig split/loader执行证据；target标签只能最终评估使用。|
...
|MRI-12|Sec.V-B|对比方法：Source only、DANN、MCD、SHOT、Proposed。|`protocol.py`: `build_paper_task_plan`|implemented_plan_only|单测确认展示名为`Source only/DANN/MCD/SHOT/Proposed`，内部method id不再出现DANN+LMMD、receiver-ratio或target-retrain上界字段|DANN/MCD/SHOT baseline训练入口未在本轮落地。|
...
- 2026-07-08：`conda run -n ssr-gpu python -m pytest tests/test_mitigating_receiver_impact_da.py -q`，exit 0，`12 passed`。
...
1.`gada_batch_step`和helper级`run_gada_training_loop`已存在；剩余是接入正式ManySig split/loader、初始`h0`来源、CLI runner和日志。
