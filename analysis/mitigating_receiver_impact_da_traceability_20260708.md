...
|MRI-09|Algorithm 1|GAD训练：每batch先更新`T`共`m`次，再生成伪标签、计算class weight和总损失，更新`E/C`。|`algorithm.py`: `PseudoLabelState`, `gada_batch_step`; `train.py`: `run_gada_training_loop`; `protocol.py`; config|implemented_proposed_runner_with_stability_repairs|单测确认`estimate_steps=m`时`optimizer_t.step()`执行`m`次、`optimizer_ec.step()`执行1次并更新`sigma/sigma'/n_t`状态；默认路径覆盖论文`m=7`；新增测试确认T步特征抽取不改写E的BatchNorm状态；训练history记录`loss_source/loss_target/loss_kl/target_conf/class_weight_min/max`|2026-07-08审查发现旧bounded run的Proposed从随机初始化直接GAD且T步会隐式更新E的BN buffer，已修复；Eq.(9)首批无历史预测时使用全1权重、空伪标签batch目标loss为0仍列入paper-unspecified。|
...
|MRI-12|Sec.V-B|对比方法：Source only、DANN、MCD、SHOT、Proposed。|`protocol.py`: `build_paper_task_plan`; `train.py`: `run_table2_reproduction`|partial_source_only_proposed_stabilized|单测确认展示名为`Source only/DANN/MCD/SHOT/Proposed`，Table II runner已支持`source_only/proposed`，内部method id不再出现DANN+LMMD、receiver-ratio或target-retrain上界字段；Proposed行默认先source pretrain再GAD并记录`source_pretrain_history`|DANN/MCD/SHOT baseline训练入口未在本轮落地，相关远端结果不能声明完整Table II复现。|
