...
SCRIPT = ROOT / "code" / "scripts" / "probe_d51_resultant_median_centroid_residual.py"
SPEC = importlib.util.spec_from_file_location("d51_probe_test_module", SCRIPT)
...
def test_active_geometry_is_finite_and_resultant_scaled() -> None:
    rows, labels = _support()
    geometry = d51._geometry(rows, labels, 3, 5)
    assert geometry["status"] == "resultant_scaled_coordinate_median_residual_active"
    assert geometry["correction"].shape == (3, 8)
    assert np.isfinite(geometry["correction"]).all()
    assert np.all(geometry["resultant"] > 0.0)
    assert np.all(geometry["resultant"] <= 1.0 + d51.UNIT_TOLERANCE)
...
def test_k1_k2_are_exact_zero_residual_fallbacks(k: int) -> None:
    ...
    geometry = d51._geometry(rows, labels, 3, k)
...
def test_rank_permutation_is_invariant() -> None:
    ...
    baseline = d51._geometry(rows, labels, 3, 8)
    order = np.concatenate(
        [np.asarray([5, 1, 7, 0, 3, 6, 2, 4]) + 8 * index for index in range(3)]
    )
    permuted = d51._geometry(rows[order], labels[order], 3, 8)
...
def test_class_permutation_is_equivariant() -> None:
    ...
    baseline = d51._geometry(rows, labels, 4, 5)
    ...
    permuted = d51._geometry(permuted_rows, permuted_labels, 4, 5)
...
def test_coordinate_median_rejects_single_outlier() -> None:
    ...
    geometry = d51._geometry(rows, labels, 2, 5)
    assert not np.allclose(
        geometry["mean_prototype"][0], geometry["median_prototype"][0]
    )
    assert np.linalg.norm(geometry["residual_direction"][0]) > 0.0
...
def test_nonunit_and_unequal_k_fail_closed() -> None:
    ...
    bad = rows.copy()
    bad[0] *= 2.0
    with pytest.raises(d51.D51ProbeError, match="unit-sphere"):
        d51._geometry(bad, labels, 3, 4)
    with pytest.raises(d51.D51ProbeError, match="cardinality"):
        d51._geometry(rows[:-1], labels[:-1], 3, 4)


def test_resource_upper_bound_is_deterministic_and_under_one_million() -> None:
    numeric, comparisons = d51._extra_resource(8, 6, 11, 288)
...
def test_formula_has_no_tunable_residual_coefficient() -> None:
    assert "(1-rho_c)" in d51.FORMULA
    assert "alpha" not in d51.FORMULA.lower()
    assert "threshold" not in d51.FORMULA.lower()
