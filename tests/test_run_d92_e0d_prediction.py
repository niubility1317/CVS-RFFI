...
from scripts import run_d92_e0d_prediction as entry
...
def test_prediction_entry_never_accepts_or_forwards_truth_role_quota_or_score(
    monkeypatch, tmp_path: Path
):
    args = _args(tmp_path)
    forbidden = ("truth", "role", "quota", "score")
    assert all(
        all(token not in action.dest for token in forbidden)
        for action in entry.parser()._actions
    )

    def fake_evaluator(**kwargs):
        assert all(
            all(token not in key for token in forbidden) for key in kwargs
        )
        ...
        return {
            "candidate": "d92_e0d_e0_fixed50",
            "schema": "cvs.phase2.d92_e0d.e0_fixed50.full_query_evaluation.v1",
            "arm_id": "E0_FIXED50",
        }

    monkeypatch.setattr(entry, "run_d92_e0d_query_evaluation", fake_evaluator)
    result = entry.run(args)
    assert result["status"] == "D92_E0D_TRUTH_FREE_PREDICTIONS_COMPLETE"
...
def test_prediction_entry_refuses_existing_output_before_evaluation(
    ...
    with pytest.raises(entry.D92E0DPredictionEntryError, match="output"):
        entry.run(args)
    assert called == []
