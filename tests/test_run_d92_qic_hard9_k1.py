...
from scripts import run_d92_qic_hard9_k1 as runner  # noqa: E402
...
def _row(
    ...
    *,
    ...
    postprocess_mode: object = "d42_quantization_intercept_closure",
) -> dict[str, object]:
    active = k_shot > 2
    prefix = "d92_e0d_qic_"
    row: dict[str, object] = {
        ...
        "after_state_postprocess_mode": postprocess_mode if active else None,
        ...
        prefix + "coefficient_decode_count": 1 if active else 0,
        prefix + "e0_state_sha256": "a" * 64,
        prefix + "final_state_sha256": ("b" if active else "a") * 64,
        prefix + "intercept_fp16_bit_change_count": 1 if active else 0,
        prefix + "candidate_intercept_fp16_bit_change_count": 1 if active else 0,
        prefix + "e0_residual_l1": 2.0 if active else None,
        prefix + "candidate_residual_l1": 1.0 if active else None,
        prefix + "residual_reduction_l1": 1.0 if active else None,
        prefix + "support_only": True if active else None,
        prefix + "clean_sample_access": False if active else None,
        prefix + "source_sample_access": False if active else None,
        prefix + "persistent_state_bytes_delta": 0 if active else None,
        prefix + "query_macs_delta": 0 if active else None,
    }
    for field in (
        "coef1_byte_exact",
        "coef2_byte_exact",
        ...
        row[prefix + field] = True if active else None
    for field in (
        ...
        row[prefix + field] = 0 if active else None
...
def test_fit_audit_accepts_qic_k_gt_2_and_k1_exact_alias(tmp_path: Path) -> None:
    for k_shot in (10, 1):
...
def test_query_access_zero_requires_base_and_approved_qic_mirror_only() -> None:
    prefix = "d92_e0d_qic_"
    row = {
        key: False
        for field in runner.QUERY_ZERO_FIELDS
...
def test_fit_audit_rejects_each_single_scene_resource_or_integrity_drift(
    ...
    with pytest.raises(runner.D92QICHard9K1RunnerError, match=field):
...
def test_fit_audit_rejects_any_query_access(tmp_path: Path) -> None:
    ...
    with pytest.raises(runner.D92QICHard9K1RunnerError, match="query access"):
        runner._validate_fit_audit(
            path,
            k_shot=10,
            reference_resources=_reference_resources(),
...
def test_systemic_failure_needs_two_distinct_pre_prediction_outers(
    ...
    assert stop["schema"] == "cvs.phase2.d92_qic_hard9_k1.systemic_failure.v1"
...
def test_score_evidence_requires_actual_readonly_sidecar_before_and_after_scoring(
    ...
    with pytest.raises(runner.D92QICHard9K1RunnerError, match="truth sidecar"):
        runner._verify_truth_sidecar_snapshot(
            truth_path,
            expected_sha256=expected_sha256,
        )
...
def test_score_artifact_binds_actual_inputs_to_the_frozen_job_identity(
    ...
    with pytest.raises(runner.D92QICHard9K1RunnerError, match="score artifact"):
...
def test_outer_score_binding_rejects_fit_and_rewritten_commit_drift(
    ...
    with pytest.raises(
        runner.D92QICHard9K1RunnerError, match="score binding closure"
    ):
        runner._validate_score_artifact(
            score_path,
            job=job,
...
def test_runtime_source_lock_closes_scientific_entry_and_rejects_file_drift(
    ...
    lock = json.loads(
        (ROOT / "configs" / "stage2_d92_qic_hard9_k1_v1.json").read_text(
            encoding="utf-8"
        )
    )
    protected = {
    ...
    ] == "2392a8b79444036d66edb85bab27b2cc827ebc5b"
    ...
    with pytest.raises(runner.D92QICHard9K1RunnerError, match="runtime source"):
...
def _run_extracted_archive_prepare_probe(
    ...
    *,
    ...
    archive = (
        ...
        / "automation_reports"
        / "CV-SincNet"
        / "d92_e0_full_ccoc_hard9k1_20260817_v8"
        / "runtime"
        / "d92_ccoc_hard9_k1_source_4e267130_20260817_v8.tar.gz"
    )
    ...
    extracted_config = (
        extracted / "configs" / "stage2_d92_qic_hard9_k1_v1.json"
    )
    extracted_config.write_bytes(
        (ROOT / "configs" / "stage2_d92_qic_hard9_k1_v1.json").read_bytes()
    )
    ...
    extracted_runner = extracted / "code" / "scripts" / "run_d92_qic_hard9_k1.py"
    extracted_runner.write_bytes(
        (ROOT / "code" / "scripts" / "run_d92_qic_hard9_k1.py").read_bytes()
    )
    extracted_matrix = (
        extracted / "code" / "cvsrffi" / "stage2_d92_qic_hard9_k1.py"
    )
    extracted_matrix.write_bytes(
        (ROOT / "code" / "cvsrffi" / "stage2_d92_qic_hard9_k1.py").read_bytes()
    )
...
def test_runtime_source_gate_requires_frozen_commit_and_both_git_blob_views(
    ...
    with pytest.raises(runner.D92QICHard9K1RunnerError, match="frozen commit"):
        runner._verify_runtime_source_files(
            source_lock,
            code_root=tmp_path / "code",
            git_runner=lambda _root, _arguments: (_ for _ in ()).throw(
                runner.D92QICHard9K1RunnerError("frozen commit missing")
            ),
        )
    ...
    with pytest.raises(runner.D92QICHard9K1RunnerError, match="frozen blob"):
        runner._verify_runtime_source_files(
            source_lock,
            code_root=tmp_path / "code",
    ...
    with pytest.raises(runner.D92QICHard9K1RunnerError, match="HEAD blob"):
        runner._verify_runtime_source_files(
            source_lock,
            code_root=tmp_path / "code",
...
def test_embedded_e0_resource_projection_rejects_identity_and_value_tamper() -> None:
    ...
    for label, tampered, error in cases:
        with pytest.raises(
            runner.D92QICHard9K1RunnerError,
            match=error,
        ):
