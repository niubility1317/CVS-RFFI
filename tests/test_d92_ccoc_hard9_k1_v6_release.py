...
def test_v6_surface_archive_manifest_and_external_mirror_are_exact() -> None:
    required = (
        RELEASE / "report.md",
        RELEASE / "launch.sh",
    ...
    assert not missing, f"missing v6 release artifacts: {missing}"
    for relative in (
        "report.md",
        "launch.sh",
    ...
    v5_config = json.loads(V5_CONFIG.read_text(encoding="utf-8"))
    assert config["runtime"]["output_root"] == OUTPUT_ROOT
    v5_config["runtime"]["output_root"] = OUTPUT_ROOT
    assert config == v5_config
...
def test_exact_v6_archive_imports_helps_and_sha_only_prepare_pass() -> None:
    launch_text = (RELEASE / "launch.sh").read_text(encoding="utf-8")
    names = _launch_import_names(launch_text)
    with tempfile.TemporaryDirectory(prefix="d92_ccoc_hard9_v6_") as raw:
        extracted = Path(raw)
        with tarfile.open(ARCHIVE, "r:gz") as bundle:
...
def test_v6_launch_is_hash_closed_prepare_smoke_then_eight_shards() -> None:
    text = (RELEASE / "launch.sh").read_text(encoding="utf-8")
    assert RUN_ID in text
    assert ARCHIVE_NAME in text
    assert "stage2_d92_ccoc_hard9_k1_v6.json" in text
...
def test_v6_report_manifest_sync_and_trace_are_preregistered_only() -> None:
    expected = (
