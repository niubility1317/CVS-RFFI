...
SCRIPT = ROOT / "code" / "scripts" / "probe_d53_spectral_contracted_median_transport.py"
SPEC = importlib.util.spec_from_file_location("d53_probe_test_module", SCRIPT)
...
def test_active_geometry_is_finite_and_spectrally_contracted() -> None:
    ...
    geometry = d53._geometry(rows, labels, 3, 5, _base())
    ...
    assert np.all(geometry["resultant"] <= 1.0 + d53.UNIT_TOLERANCE)
...
def test_k1_k2_are_exact_zero_residual_fallbacks(k: int) -> None:
    ...
    geometry = d53._geometry(rows, labels, 3, k, _base())
...
def test_k1_fallback_precedes_centered_mean_spectral_degeneracy() -> None:
    ...
    geometry = d53._geometry(rows, labels, 3, 1, _base())
    assert geometry["exact_fallback"] is True
...
def test_rank_permutation_is_invariant() -> None:
    ...
    base = _base()
    baseline = d53._geometry(rows, labels, 3, 8, base)
    order = np.concatenate(
        [np.asarray([5, 1, 7, 0, 3, 6, 2, 4]) + 8 * index for index in range(3)]
    )
    permuted = d53._geometry(rows[order], labels[order], 3, 8, base)
...
def test_class_permutation_is_equivariant() -> None:
    ...
    base = _base(c=4)
    baseline = d53._geometry(rows, labels, 4, 5, base)
    ...
    permuted = d53._geometry(permuted_rows, permuted_labels, 4, 5, base[order])
...
def test_coordinate_median_rejects_single_outlier() -> None:
    ...
    geometry = d53._geometry(rows, labels, 2, 5, _base(c=2, d=6))
...
def test_nonunit_and_unequal_k_fail_closed() -> None:
    ...
    bad = rows.copy()
    bad[0] *= 2.0
    with pytest.raises(d53.D53ProbeError, match="unit-sphere"):
        d53._geometry(bad, labels, 3, 4, _base())
    with pytest.raises(d53.D53ProbeError, match="cardinality"):
        d53._geometry(rows[:-1], labels[:-1], 3, 4, _base())
    with pytest.raises(d53.D53ProbeError, match="base coefficient"):
        d53._geometry(rows, labels, 3, 4, np.zeros((3, 7)))


def test_resource_upper_bound_is_deterministic_and_under_one_million() -> None:
    numeric, comparisons = d53._extra_resource(8, 6, 11, 288)
...
def test_feature_dimension_comes_from_actual_state_not_resource_dict() -> None:
    ...
    assert d53._state_dimension(state) == 288
    with pytest.raises(d53.D53ProbeError, match="feature dimension"):
        d53._state_dimension(SimpleNamespace(log_diag_fp32=np.zeros((2, 2))))


def test_formula_has_no_tunable_residual_coefficient() -> None:
    assert "diag(1-rho)" in d53.FORMULA
    assert "U*M0.T/||M0||2^2" in d53.FORMULA
    assert "pinv" not in d53.FORMULA.lower()
    assert "ridge" not in d53.FORMULA.lower()
    assert "alpha" not in d53.FORMULA.lower()
    assert "threshold" not in d53.FORMULA.lower()
