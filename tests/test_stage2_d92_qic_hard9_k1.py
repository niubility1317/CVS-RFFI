...
def test_hard9_k1_selection_is_qic_only_disjoint_from_g0() -> None:
    assert matrix.ARM_ID == "E0_FULL_D42_QUANTIZATION_INTERCEPT_CLOSURE"
    assert matrix.CANDIDATE_ID == "d92_e0_full_d42_quantization_intercept_closure"
    assert matrix.REGISTERED_MODE == "full_only"
...
def test_v3_method_lock_uses_a_fresh_non_overwriting_output_root() -> None:
...
def test_manifest_binds_all_sealed_inputs_and_exact_hard9_k1_jobs() -> None:
    ...
    assert manifest["schema"] == "cvs.phase2.d92_qic_hard9_k1.matrix.v1"
...
def test_method_lock_freezes_qic_fit_query_and_absolute_peak_contract() -> None:
    lock = json.loads(CONFIG.read_text(encoding="utf-8"))

    assert lock["schema"] == "cvs.phase2.d92_qic_hard9_k1.method_lock.v1"
