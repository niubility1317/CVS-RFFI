...
from scripts import run_d92_e0d_hard12v2 as runner
...
def _manifest(tmp_path: Path) -> tuple[Path, str, dict]:
    ...
    job = {
        ...
        "job_id": "rx_7_7__seed_713104__k_1__new_20__arm_d92_full",
        "outer_key": "rx_7_7__seed_713104__k_1__new_20",
        "outer_role": "liveness",
        "receiver": "7-7",
        "seed": 713104,
        "k_shot": 1,
        "new_class_count": 20,
        "arm_id": "D92_FULL",
        "candidate": "d92_e0d_full",
        "packages": packages,
        "truth_sidecar": str(truth),
        "output_root": str(output / "jobs" / "outer" / "D92_FULL"),
    }
    payload = {
...
def test_prediction_and_score_commands_keep_truth_out_of_predictor(tmp_path: Path):
    ...
    assert all(
        "enrollment" not in token and "apply-package" not in token for token in score
    )
    assert prediction.index("--arm") < prediction.index("--output-root")


def test_run_shard_commits_prediction_before_starting_scorer(
    ...
    def fake_subprocess(command, **_kwargs):
        ...
        if command[1].endswith("run_d92_e0d_prediction.py"):
    ...
    assert len(calls) == 2
    assert calls[0][1].endswith("run_d92_e0d_prediction.py")
    assert calls[1][1].endswith("score_d92_be_prediction.py")
    receipt = json.loads(
        (Path(job["output_root"]) / "job_receipt.json").read_text(encoding="utf-8")
    )
    assert receipt["schema"] == "cvs.phase2.d92_e0d_hard12v2.job_receipt.v1"
