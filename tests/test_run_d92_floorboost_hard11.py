...
from scripts import run_d92_floorboost_hard11 as runner
...
METHOD_LOCK = Path("configs/stage2_d92_full_maxmin_floorboost_hard11_v1.json").resolve()
...
def test_cli_parser_exposes_all_runner_commands() -> None:
...
def test_truth_free_smoke_and_shards_cover_11_jobs(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert smoke["arm_id"] == "E0_FULL_MAXMIN_FLOORBOOST"
...
def test_runner_refuses_tampered_smoke_before_prediction_dispatch(
    ...
    with pytest.raises(runner.D92FloorboostHard11RunnerError, match="smoke"):
        runner.run_shard(
            SimpleNamespace(
                matrix_manifest=str(manifest_path),
                matrix_manifest_sha256=digest,
