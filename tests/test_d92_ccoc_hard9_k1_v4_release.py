...
ARCHIVE_NAME = "d92_ccoc_hard9_k1_source_fbe4e03a_20260817_v4.tar.gz"
ARCHIVE = RELEASE / "runtime" / ARCHIVE_NAME
CONFIG = ROOT / "configs" / "stage2_d92_ccoc_hard9_k1_v4.json"
V3_CONFIG = ROOT / "configs" / "stage2_d92_ccoc_hard9_k1_v3.json"
OUTPUT_ROOT = (
...
def test_v4_surface_archive_manifest_and_external_mirror_are_exact() -> None:
    required = (
        RELEASE / "report.md",
        RELEASE / "launch.sh",
    ...
    assert not missing, f"missing v4 release artifacts: {missing}"
    for relative in (
        "report.md",
        "launch.sh",
    ...
    v3_config["runtime"]["output_root"] = OUTPUT_ROOT
    assert config == v3_config
...
def test_exact_v4_archive_imports_helps_and_sha_only_prepare_pass() -> None:
    launch_text = (RELEASE / "launch.sh").read_text(encoding="utf-8")
    names = _launch_import_names(launch_text)
    with tempfile.TemporaryDirectory(prefix="d92_ccoc_hard9_v4_") as raw:
        extracted = Path(raw)
        with tarfile.open(ARCHIVE, "r:gz") as bundle:
...
def test_v4_launch_is_hash_closed_prepare_smoke_then_eight_shards() -> None:
    text = (RELEASE / "launch.sh").read_text(encoding="utf-8")
    assert RUN_ID in text
    assert ARCHIVE_NAME in text
    assert "stage2_d92_ccoc_hard9_k1_v4.json" in text
    assert "runtime_source_verification_mode" in text
    assert '"sha256_only"' in text
    assert "e0_resource_fit_audit_observed_sha256" in text
...
def test_v4_report_manifest_sync_and_trace_are_preregistered_only() -> None:
    expected = (
