...
|C2|Phase2每条物理IQ只有一个固定`leo_*_weak`观测；K-shot是K个独立物理样本|`项目.md`|协议定义|“one fixed received observation per physical sample”|“multi-view observations increase K”|在数据表补物理ID计数|
...
|C10|K1上RTB-IDR区别性模块不激活，paired增益为0|D92 K1 fallback和结果|结构边界与实测|“no identifiable one-shot gain”|“one-shot adaptation solved”|设计K1可识别弱先验|
...
|C14|WiSig/ManySig和LEO弱信道均为代理|`项目.md`、数据来源|场景边界|“terrestrial proxy and physics-inspired stress”|“real on-orbit validation”|真实卫星或硬件在环|
...
- terrestrial proxy
- physics-inspired weak-LEO stress
- support-only
