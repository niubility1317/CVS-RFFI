...
class RFFIAugmentor:
    def __init__(
        ...
        # Legacy switch kept for backward compatibility. New code should use
        # per-view controls below to avoid mixing hardware-only views with channel noise.
        defect_apply_channel: bool = True,

        # Per-view post-processing controls for defect-only views.
        # "pure" hardware views should usually keep both disabled.
        dac_only_apply_anti_shortcut: Optional[bool] = None,
        dac_only_apply_channel: Optional[bool] = None,
        pa_only_apply_anti_shortcut: Optional[bool] = None,
        pa_only_apply_channel: Optional[bool] = None,
        dac_pa_apply_anti_shortcut: Optional[bool] = None,
        dac_pa_apply_channel: Optional[bool] = None,
    ):
        ...
        # Fine-grained post-processing switches for defect-only views.
        # When None, fall back to the legacy defect_apply_channel behavior.
        legacy = bool(defect_apply_channel)
        self.dac_only_apply_anti_shortcut = legacy if dac_only_apply_anti_shortcut is None else bool(dac_only_apply_anti_shortcut)
        self.dac_only_apply_channel = legacy if dac_only_apply_channel is None else bool(dac_only_apply_channel)
        self.pa_only_apply_anti_shortcut = legacy if pa_only_apply_anti_shortcut is None else bool(pa_only_apply_anti_shortcut)
        self.pa_only_apply_channel = legacy if pa_only_apply_channel is None else bool(pa_only_apply_channel)
        self.dac_pa_apply_anti_shortcut = legacy if dac_pa_apply_anti_shortcut is None else bool(dac_pa_apply_anti_shortcut)
        self.dac_pa_apply_channel = legacy if dac_pa_apply_channel is None else bool(dac_pa_apply_channel)
    ...
    def _pa_memory_polynomial(self, z: torch.Tensor, s: torch.Tensor) -> torch.Tensor:
        ...
        # base linear gain slightly random
        ...
        # mild output normalization (avoid explosion)
    ...
    def _postprocess_defect_view(
        self,
        x: torch.Tensor,
        *,
        apply_anti_shortcut: bool,
        apply_channel: bool,
    ) -> torch.Tensor:
        out = x
        if apply_anti_shortcut:
            out = self._apply_antis_shortcut(out)
        if apply_channel:
            out = self._apply_general_channel(out)
    ...
    def __call__(
        ...
        # flags
        no_dac: bool = False,
        no_pa: bool = True,         # default: PA off for normal view (keep backward behavior)
        dac_only: bool = False,
        pa_only: bool = False,
        dac_pa: bool = False,       # DAC+PA combined view

        # Optional externally supplied strengths (e.g. fixed tiers for branch supervision)
        dac_strength: Optional[torch.Tensor] = None,
        pa_strength: Optional[torch.Tensor] = None,
    ):
        """
        ...
        Strength returns:
          - return_dac_strength -> (x, s_dac)
          - return_pa_strength  -> (x, s_pa)
          - return_defect_strengths -> (x, s_dac, s_pa)
        """
        ...
        # ---- defect-only views ----
        if dac_only:
            x_aug, s_d = self.simulate_dac(x0, labels=labels, dac_strength=dac_strength)
            x_aug = self._postprocess_defect_view(
                x_aug,
                apply_anti_shortcut=self.dac_only_apply_anti_shortcut,
                apply_channel=self.dac_only_apply_channel,
            )
        ...
        if pa_only:
            x_aug, s_p = self.simulate_pa(x0, labels=labels, pa_strength=pa_strength)
            x_aug = self._postprocess_defect_view(
                x_aug,
                apply_anti_shortcut=self.pa_only_apply_anti_shortcut,
                apply_channel=self.pa_only_apply_channel,
            )
            x_aug = x_aug.to(dtype_out)
            if return_defect_strengths:
        ...
        if dac_pa:
            x_aug, s_d, s_p = self.simulate_dac_pa(x0, labels=labels, dac_strength=dac_strength, pa_strength=pa_strength)
            x_aug = self._postprocess_defect_view(
                x_aug,
                apply_anti_shortcut=self.dac_pa_apply_anti_shortcut,
                apply_channel=self.dac_pa_apply_channel,
            )
            x_aug = x_aug.to(dtype_out)
            if return_defect_strengths:
