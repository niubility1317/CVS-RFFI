"""Nine full-budget reference/extension rows; preserve original jobs, at most two workers per GPU."""
import argparse
import json
import os
...
def occupancy(active):
    ...
    for proc in Path('/proc').iterdir():
        ...
        try:
            ...
            if not any(Path(v).name.startswith(('train','predict','check_full_dr_execution')) and v.endswith('.py') for v in argv):continue
...
def available_gpu(active):
    candidates=[(len(v['pids']),-v['free_mb'],gpu) for gpu,v in occupancy(active).items() if len(v['pids'])<2 and v['free_mb']>=12000]
...
def train_command(row,project,run):
    ...
    if row['pipeline']=='core90':
        return [sys.executable,'-u',str(SCRIPTS/'train_xuc.py'),'--matrix',str(CONFIG/'matrix_dr_full.json'),
...
def main():
    ...
    matrix=json.loads((CONFIG/'matrix_dr_full.json').read_text(encoding='utf-8'))
    if len(matrix['runs'])!=9 or {r['id'] for r in matrix['runs']}!={'F-'+k for k in ('M00','A1','M14','M11','M05','M08','M07','M12','M13')}:raise ValueError('not frozen FULL9')
    ...
    if a.recovery_from:
        ...
        if set(prior['rows'])!={r['id'] for r in matrix['runs']}:raise ValueError('recovery matrix row set mismatch')
    ...
    try:
        # Reuse the already validated physical-role contract, never model state.
        source_contract=project/'runs/phase1_adv3b02_xuc15_s392005_20260913_r1/source_contract.json'
        contract=json.loads(source_contract.read_text())
        if contract['counts']!={'L_s':6300,'U_s':56700,'V':27000}:raise ValueError('source role contract differs')
        write(run/'source_contract.json',contract)
        ...
        write(run/'predictions_complete.json',dict(rows=list(state['rows']),record_count=6048000,created=time.time()))
