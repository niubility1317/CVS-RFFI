...
from cvsrffi.original_leo import FusedCleanSatelliteForward, ORIGINAL, original_scenario
...
def main():
    ...
    for scene in ORIGINAL[1:]:
        cfg=make_sat_config(scene,args)
        assert cfg.channel_model=='legacy_full' and not cfg.use_residual_doppler
        ...
        results.append({'scene':scene,'channel_model':cfg.channel_model,'loss':float(loss.detach()),'forward_shape':calls[-1]})
    ...
    assert called and set(called)=={'clear_leo'}
    (a.output/'acceptance.json').write_text(json.dumps({'status':'PASS','scratch_checkpoint':str(path),'query_inputs':0,
