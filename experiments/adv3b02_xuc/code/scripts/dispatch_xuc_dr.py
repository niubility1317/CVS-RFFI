...
def train_command(row,project,run):
    ...
    if row['pipeline']=='core90':
        return [sys.executable,'-u',str(SCRIPTS/'train_xuc.py'),'--matrix',str(CONFIG/'matrix_dr.json'),
...
def main():
    ...
    matrix=json.loads((CONFIG/'matrix_dr.json').read_text(encoding='utf-8'))
    if len(matrix['runs'])!=7 or {r['id'] for r in matrix['runs']}!={'DR-'+k for k in ('M14','M11','M05','M08','M07','M12','M13')}:raise ValueError('not frozen DR7')
    ...
    try:
        ...
        write(run/'predictions_complete.json',dict(rows=list(state['rows']),record_count=4704000,created=time.time()))
