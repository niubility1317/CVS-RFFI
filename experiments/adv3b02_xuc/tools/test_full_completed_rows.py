"""User-requested early evaluation of the six frozen FULL9 E200 rows; no training writes."""
import argparse
...
def early_admission_safe(project, run):
    """Original owner cannot launch prediction while the three unfinished rows remain below E186."""
    current = json.loads((run / 'pipeline_state.json').read_text())
    if current['status'] != 'TRAINING':
        return False
    for rid in ('F-M00', 'F-M07', 'F-M13'):
        ...
        if not epochs or int(epochs[-1]) > 185:
            return False
    return True
...
def validate_source(run):
    ...
    for rid in ROWS:
        ...
        if state['rows'][rid]['status'] != 'TRAINING_COMPLETE' or c['epochs'] != 200 or c['steps'] != 44400:
...
def main():
    ...
    spec = importlib.util.spec_from_file_location('frozen_dispatch', release / 'code/scripts/dispatch_xuc_full.py')
