...
from scripts.dispatch_xuc_dr import available_gpu
gpu=available_gpu({})
...
with (project/'logs'/(c['run_id']+'.smoke.log')).open('x') as log:
    subprocess.run([python,str(release/'code/scripts/check_dr_execution.py'),'--output',str(check),
...
command=[python,'-u',str(release/'code/scripts/dispatch_xuc_dr.py'),'--project',str(project),'--run-id',c['run_id'],'--commit',c['commit']]
...
def main():
    ...
    release='adv3b02_xuc_dr_'+commit[:10];archive=a.output/(release+'.tar')
