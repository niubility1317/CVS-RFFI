...
from scripts.dispatch_xuc_full import available_gpu
gpu=available_gpu({})
...
with (project/'logs'/(c['run_id']+'.smoke.log')).open('x') as log:
    subprocess.run([python,str(release/'code/scripts/check_full_dr_execution.py'),'--output',str(check),
        '--device','cuda:0','--controlled-routes'],cwd=release,env=env,stdout=log,stderr=subprocess.STDOUT,check=True)
if json.loads((check/'acceptance.json').read_text())['status']!='PASS':raise ValueError('checkpoint smoke not PASS')
command=[python,'-u',str(release/'code/scripts/dispatch_xuc_full.py'),'--project',str(project),'--run-id',c['run_id'],'--commit',c['commit']]
...
def main():
    ...
    release='adv3b02_xuc_full_'+commit[:10];archive=a.output/(release+'.tar')
