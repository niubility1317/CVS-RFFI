...
def main():
    p=argparse.ArgumentParser();p.add_argument('--output',type=Path,required=True);p.add_argument('--inventory',type=Path,required=True);a=p.parse_args()
    ...
    state=dict(status='COPYING',run_id=run,rows=rows,created=time.time(),target_feedback=False,
        scope='previously_exposed_benchmark_descriptive_evaluation',device='local RTX5070Ti',batch_size=256,seed=392005,results={})
    def save():
    ...
    for name in ('iq.npy','manifest.json'):copy(f'{PROJECT}/runs/{TARGET}/target_inputs/{name}',inputs/name)
    ...
    for rid in rows:
        folder=a.output/rid;folder.mkdir()
