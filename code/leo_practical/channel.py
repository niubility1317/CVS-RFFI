...
VERSION = "leo_practical_v1_20260918"
C = 299792458.0
RE = 6371000.0
GM = 3.986004418e14
...
class Config:
    ...
    def __post_init__(self):
        ...
        nonnegative = (self.receiver_cfo_std_hz, self.residual_cfo_std_hz,
                       ...
                       self.phase_tracking_residual_std_deg)
...
class ChannelStream:
    ...
    AGC is blockwise, so its result depends on the caller's block boundaries.
    ...
    def __init__(self, cfg, seed, receiver, geometry=None, receiver_processor=None):
        ...
        if receiver_processor is not None and cfg.mode != "post_sync":
            raise ValueError("external receiver processing is only allowed in post_sync mode")
        self.rngs = {k: np.random.default_rng(stable_seed(seed, k)) for k in
                     ("geometry", "states", "shadow", "scatter", "phase", "noise", "sync", "tracking")}
        ...
        self.history_size = int(np.floor(self.delays.max())) + 3
        self.history = None

    def process(self, x, *, preceding_context=None, return_traces=False):
        cfg = self.cfg
        x = np.asarray(x, dtype=np.complex128)
        if x.ndim != 1 or len(x) < 4 or not np.isfinite(x).all():
        ...
        # Causal cubic Lagrange fractional delay (no wrap, no hidden latency).
        for s in range(3):
            mask = state == s
            if not mask.any():
                continue
            for tap, delay in enumerate(self.delays[s]):
                ...
                for j in range(4):
                    coefficient = math.prod((fraction-k)/(j-k) for k in range(4) if k != j)
        ...
        for k in range(n):
            tracking_residual[k] = math.radians(cfg.phase_tracking_residual_std_deg)*self.tracking_z
        ...
        if cfg.mode == "post_sync":
            if cfg.iq_compensation_enabled:
                ...
                noise = compensate_iq(noise, self.receiver.estimated_amplitude_imbalance_db,
                                      self.receiver.estimated_quadrature_error_deg)
            correction = np.exp(-2j*np.pi*self.f_hat*t)
            if cfg.phase_tracking_enabled:
        ...
        external_processing = {"timing_sync": "not_run_cropped_input",
                               "channel_equalization": "not_run_no_pilots_or_estimate"}
        if self.receiver_processor is not None:
            # No truth, channel taps, clean reference, seed or TX label is passed
            # to actual external receiver processing. This is the integration
            # point for an authorized pilot/preamble-based synchronizer/EQ.
            y, external_processing = self.receiver_processor(y.copy(), {
                "fs_hz": cfg.fs_hz, "fc_hz": cfg.fc_hz,
                "receiver_location": cfg.receiver_location, "start_sample": self.index})
            y = np.asarray(y, dtype=np.complex128)
            if y.shape != x.shape or not np.isfinite(y).all() or not isinstance(external_processing, dict):
                raise ValueError("receiver_processor must return finite same-length IQ and a metadata dict")
            json.dumps(external_processing, allow_nan=False)
        ...
        meta = dict(version=VERSION, config_hash=cfg.config_hash, seed=self.seed,
                    ...
                    output_frequency_hz=self.residual_hz if cfg.mode == "post_sync" else self.f_total,
                    ...
                    phase_tracking_applied=cfg.mode == "post_sync" and cfg.phase_tracking_enabled,
                    ...
                    snr_measurement_stage="after_parametric_compensation_before_external_processor_and_AGC",
