#!/usr/bin/env python
"""Export and independently bind the frozen source-only LEO slice for P1-CAGM.

The generic forward is the signed ICMT-v2 implementation, but this entry owns
the CAGM candidate/root/receipt identity and emits an independent CAGM
sidecar.  The sidecar replays the fixed ManySig selection, current NPZ hash,
physical-key receipt and every scenario's TX/RX/day coverage.
"""
...
EXPECTED_BINDING_SCHEMA = "cvs.phase1.cagm_leo_binding.v1"
...
class CAGMLEOBindingError(RuntimeError):
    """Raised when a CAGM LEO export cannot prove its frozen source binding."""


def _translate(error: BaseException) -> CAGMLEOBindingError:
    return CAGMLEOBindingError(str(error))
...
def _validate_frozen_args(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if fold not in range(1, 7) or arm not in {"C", "G"}:
        raise CAGMLEOBindingError("fold/arm must be F1..F6 and C/G")
    candidate = f"F{fold}{arm}_CAGM12"
    if str(args.candidate_id) != candidate:
        raise CAGMLEOBindingError(f"candidate_id must be {candidate}")
    ...
    if training_root == postfreeze_root or not postfreeze_root.is_dir():
        raise CAGMLEOBindingError("postfreeze root must exist and differ from training root")
    ...
    if checkpoint != expected_checkpoint or not checkpoint.is_file():
        raise CAGMLEOBindingError("checkpoint path does not bind frozen CAGM candidate")
    ...
    if out_npz != (candidate_dir / "source_leo_final_only.npz").resolve():
        raise CAGMLEOBindingError("LEO NPZ path does not match frozen candidate layout")
    if binding_json != (candidate_dir / "source_leo_binding.json").resolve():
        raise CAGMLEOBindingError("LEO binding path does not match frozen candidate layout")
    source_tx_ids = _parse_csv(args.source_tx_ids, field="source_tx_ids")
    if len(source_tx_ids) != 4:
        raise CAGMLEOBindingError("P1-CAGM LEO binding requires local4 source TX")
    for field, expected in (
        ...
        try:
            ...
            raise _translate(exc) from exc
        if observed != expected:  # defensive type narrowing; helper already enforces it.
            raise CAGMLEOBindingError(f"{field} drifted")
    for field, expected in (
        ...
        if int(getattr(args, field)) != expected:
            raise CAGMLEOBindingError(f"{field} must equal frozen value {expected}")
    for field, expected in (
        ("feature_name", "z_id"),
    ...
    if args.source_only_export is not True:
        raise CAGMLEOBindingError("P1-CAGM LEO export must be source-only")
    if int(args.wisig_out_len) != EXPECTED_OUT_LEN or int(args.max_samples_per_combo) != EXPECTED_MAX_PER_COMBO:
        raise CAGMLEOBindingError("frozen LEO shape or combo selection drifted")
    dataset = Path(args.wisig_pkl).resolve()
    if not dataset.is_file():
        raise CAGMLEOBindingError(f"missing frozen ManySig dataset: {dataset}")
    if str(args.expected_wisig_sha256).lower() != FROZEN_WISIG_SHA256:
        raise CAGMLEOBindingError("expected WiSig SHA256 does not equal frozen value")
    dataset_sha256 = _icmt_leo._sha256_file(dataset)
    if dataset_sha256 != FROZEN_WISIG_SHA256:
        raise CAGMLEOBindingError("ManySig input bytes do not match frozen SHA256")
    checkpoint_payload = torch.load(checkpoint, map_location="cpu")
    if not isinstance(checkpoint_payload, Mapping):
        raise CAGMLEOBindingError("checkpoint payload must be a mapping")
    # The LEO sidecar repeats the CAGM receipt check at the point where its
    # current checkpoint byte hash is sealed.
    _, receipt, observed_arm = _cagm_export.validate_cagm_training_checkpoint(
        checkpoint_payload,
        checkpoint_path=checkpoint,
        source_tx_ids=source_tx_ids,
        known_validation_tx_ids=(str(getattr(args, "known_validation_tx_id", "")),)
        if str(getattr(args, "known_validation_tx_id", ""))
        else tuple(str(item) for item in checkpoint_payload.get("cagm_receipt", {}).get("source_known_validation_tx", [])),
        proxy_unknown_tx_ids=(str(getattr(args, "proxy_unknown_tx_id", "")),)
        if str(getattr(args, "proxy_unknown_tx_id", ""))
        else tuple(str(item) for item in checkpoint_payload.get("cagm_receipt", {}).get("source_proxy_unknown_tx", [])),
    )
    if observed_arm != arm:
        raise CAGMLEOBindingError("CAGM checkpoint receipt arm does not bind LEO arm")
    return {
        "fold": fold,
        "arm": arm,
        "candidate": candidate,
        ...
        "cagm_receipt_sha256": _cagm_export._canonical_json_sha256(
            dict(checkpoint_payload["cagm_receipt"])
        ),
    }
...
def build_binding_from_existing(args: argparse.Namespace) -> dict[str, Any]:
    """Bind an already-exported LEO NPZ and attach CAGM terminal evidence."""

    try:
        with _patched_signed_binding():
            binding = _icmt_leo.build_binding_from_existing(args)
    except (_icmt_leo.ICMTLEOBindingError, _cagm_export.CAGMSplitExportError) as exc:
        raise _translate(exc) from exc
    ...
    result.update(
        {
            ...
            "method": "P1_CAGM",
            "cagm_receipt_schema": _cagm_export.EXPECTED_RECEIPT_SCHEMA,
            "cagm_receipt_sha256": frozen["cagm_receipt_sha256"],
            "cagm_terminal_contract": str(frozen["cagm_receipt"]["cagm_terminal_contract"]),
            "cagm_terminal_contract_passed": True,
            "cagm_loss_divisor": _cagm_export.FROZEN_CAGM_DIVISOR,
            "cagm_clean_statistics_detached": True,
            "cagm_joint_zero_mask_aux_only": frozen["cagm_receipt"][
...
def _atomic_write_json(path: Path, payload: Mapping[str, Any]) -> None:
    if path.exists():
        raise CAGMLEOBindingError(f"refusing to overwrite LEO binding: {path}")
    temporary = path.with_name(path.name + ".tmp")
    if temporary.exists():
        raise CAGMLEOBindingError(f"refusing to overwrite temporary LEO binding: {temporary}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if frozen["out_npz"].exists() or frozen["binding_json"].exists():
        raise CAGMLEOBindingError("refusing to overwrite CAGM LEO artifact")
