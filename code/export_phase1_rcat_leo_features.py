#!/usr/bin/env python
"""Export and independently bind the frozen source-only LEO slice for RCAT.

The generic forward is the signed ICMT-v2 implementation.  This file owns the
RCAT candidate/root/receipt identity, replays the fixed ManySig selection,
and seals an RCAT sidecar with current NPZ, physical-key and complete
...
EXPECTED_BINDING_SCHEMA = "cvs.phase1.rcat_leo_binding.v1"
...
class RCATLEOBindingError(RuntimeError):
    """Raised when an RCAT LEO export cannot prove its frozen source binding."""


def _translate(error: BaseException) -> RCATLEOBindingError:
    return RCATLEOBindingError(str(error))
...
def _validate_frozen_args(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if fold not in range(1, 7) or arm not in {"C", "G"}:
        raise RCATLEOBindingError("fold/arm must be F1..F6 and C/G")
    candidate = f"F{fold}{arm}_RCAT12"
    if str(args.candidate_id) != candidate:
        raise RCATLEOBindingError(f"candidate_id must be {candidate}")
    ...
    if training_root == postfreeze_root or not postfreeze_root.is_dir():
        raise RCATLEOBindingError("postfreeze root must exist and differ from training root")
    ...
    if checkpoint != expected_checkpoint or not checkpoint.is_file():
        raise RCATLEOBindingError("checkpoint path does not bind frozen RCAT candidate")
    ...
    if out_npz != (candidate_dir / "source_leo_final_only.npz").resolve():
        raise RCATLEOBindingError("LEO NPZ path does not match frozen candidate layout")
    if binding_json != (candidate_dir / "source_leo_binding.json").resolve():
        raise RCATLEOBindingError("LEO binding path does not match frozen candidate layout")
    source_tx_ids = _parse_csv(args.source_tx_ids, field="source_tx_ids")
    if len(source_tx_ids) != 4:
        raise RCATLEOBindingError("P1-RCAT LEO binding requires local4 source TX")
    for field, expected in (
        ...
        if observed != expected:
            raise RCATLEOBindingError(f"{field} drifted")
    for field, expected in (
        ...
        if int(getattr(args, field)) != expected:
            raise RCATLEOBindingError(f"{field} must equal frozen value {expected}")
    for field, expected in (
        ("feature_name", "z_id"),
    ...
    if args.source_only_export is not True:
        raise RCATLEOBindingError("P1-RCAT LEO export must be source-only")
    if int(args.wisig_out_len) != EXPECTED_OUT_LEN or int(args.max_samples_per_combo) != EXPECTED_MAX_PER_COMBO:
        raise RCATLEOBindingError("frozen LEO shape or combo selection drifted")
    dataset = Path(args.wisig_pkl).resolve()
    if not dataset.is_file():
        raise RCATLEOBindingError(f"missing frozen ManySig dataset: {dataset}")
    if str(args.expected_wisig_sha256).lower() != FROZEN_WISIG_SHA256:
        raise RCATLEOBindingError("expected WiSig SHA256 does not equal frozen value")
    dataset_sha256 = _icmt_leo._sha256_file(dataset)
    if dataset_sha256 != FROZEN_WISIG_SHA256:
        raise RCATLEOBindingError("ManySig input bytes do not match frozen SHA256")
    checkpoint_payload = torch.load(checkpoint, map_location="cpu")
    if not isinstance(checkpoint_payload, Mapping):
        raise RCATLEOBindingError("checkpoint payload must be a mapping")
    _, receipt, observed_arm = _rcat_export.validate_rcat_training_checkpoint(
        ...
        if str(getattr(args, "known_validation_tx_id", ""))
            ...
            str(item)
            for item in checkpoint_payload.get("rcat_receipt", {}).get(
    ...
    if observed_arm != arm:
        raise RCATLEOBindingError("RCAT checkpoint receipt arm does not bind LEO arm")
    return {
        "fold": fold,
        "arm": arm,
        "candidate": candidate,
        ...
        "rcat_receipt_sha256": _rcat_export._canonical_json_sha256(
            dict(checkpoint_payload["rcat_receipt"])
        ),
    }
...
def _patched_signed_binding() -> Iterator[None]:
    """Temporarily point signed LEO mechanics at RCAT identity."""
...
def build_binding_from_existing(args: argparse.Namespace) -> dict[str, Any]:
    """Bind an existing LEO NPZ and attach raw RCAT terminal evidence."""
    ...
    try:
        ...
        _rcat_export.RCATSplitExportError,
    ) as exc:
        raise _translate(exc) from exc
    frozen = _validate_frozen_args(args)
    receipt = frozen["rcat_receipt"]
    result = dict(binding)
    result.update(
        {
            "schema": EXPECTED_BINDING_SCHEMA,
            "method": "P1_RCAT",
            "rcat_receipt_schema": _rcat_export.EXPECTED_RECEIPT_SCHEMA,
            "rcat_receipt_sha256": frozen["rcat_receipt_sha256"],
            "rcat_terminal_contract": str(receipt["rcat_terminal_contract"]),
            "rcat_terminal_contract_passed": True,
            "rcat_lambda": _rcat_export.FROZEN_RCAT_LAMBDA if frozen["arm"] == "G" else 0.0,
...
def _atomic_write_json(path: Path, payload: Mapping[str, Any]) -> None:
    if path.exists():
        raise RCATLEOBindingError(f"refusing to overwrite LEO binding: {path}")
    temporary = path.with_name(path.name + ".tmp")
    if temporary.exists():
        raise RCATLEOBindingError(f"refusing to overwrite temporary LEO binding: {temporary}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if frozen["out_npz"].exists() or frozen["binding_json"].exists():
        raise RCATLEOBindingError("refusing to overwrite RCAT LEO artifact")
