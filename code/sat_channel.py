...
def wiener_phase_noise(B: int, T: int, sigma_rad: torch.Tensor, device, dtype, gen: Optional[torch.Generator]=None) -> torch.Tensor:
    ...
    phi = deterministic_cumsum(inc, dim=1)
...
class SatSimConfig:
    ...
    use_residual_doppler: bool = False
    apply_path_loss_to_iq: bool = True
    enable_atmospheric_fading: bool = True
    enable_iq_imbalance: bool = True
    fading_mode: str = "legacy_state"
...
def _sample_rician_gain(
    ...
    *,
    ...
    K = 10.0 ** (K_db / 10.0)
    gr = torch.randn(K.shape, device=device, dtype=torch.float32, generator=gen)
    gi = torch.randn(K.shape, device=device, dtype=torch.float32, generator=gen)
    scatter = ((gr + 1j * gi).to(torch.complex64) / math.sqrt(2.0)) * torch.sqrt(1.0 / (K + 1.0)).to(torch.complex64)
    phi0 = torch.rand(K.shape, device=device, dtype=torch.float32, generator=gen) * 2 * math.pi - math.pi
    los_amp = torch.sqrt(K / (K + 1.0))
    if shadowed:
        shadow_db = torch.randn(K.shape, device=device, dtype=torch.float32, generator=gen) * 1.5
        los_amp = los_amp * (10.0 ** (shadow_db / 20.0))
    los = los_amp.to(torch.complex64) * torch.exp(1j * phi0.to(torch.complex64))
    return los + scatter


def _apply_weak_multipath(
    xc: torch.Tensor,
    h0: torch.Tensor,
    cfg: SatSimConfig,
    *,
    ...
    L = int(torch.randint(low=int(Lmin), high=int(Lmax) + 1, size=(1,), device=device, generator=gen).item())
    L = max(1, L)
    delays = torch.zeros((L,), device=device, dtype=torch.long)
    if L > 1 and int(cfg.max_delay_samp) > 0:
        delays[1:] = torch.randint(
            low=1,
            high=int(cfg.max_delay_samp) + 1,
            size=(L - 1,),
            device=device,
            generator=gen,
        )

    rel_power = torch.ones((L,), device=device, dtype=torch.float32)
    if L > 1:
        rel_power[1:] = float(cfg.pwr_decay) ** torch.arange(1, L, device=device, dtype=torch.float32)
    rel_power = rel_power / rel_power.sum().clamp_min(1e-12)
    ...
    taps = taps * torch.sqrt(rel_power).to(torch.complex64).unsqueeze(0)
    ...
    return yc, torch.full((B,), L, device=device, dtype=torch.long)


@torch.no_grad()
def _apply_leo_residual_channel_batch(
    x_iq: torch.Tensor,
    cfg: SatSimConfig,
    *,
    gen: Optional[torch.Generator] = None,
    ...
    orbits = torch.zeros((B,), device=device, dtype=torch.long)

    u = torch.rand((B,), device=device, dtype=torch.float32, generator=gen)
    hlo, hhi = cfg.leo_h_km
    h_km = float(hlo) + (float(hhi) - float(hlo)) * u
    h_m = h_km * 1e3

    tlo, thi = cfg.theta_deg
    theta = float(tlo) + (float(thi) - float(tlo)) * torch.rand((B,), device=device, dtype=torch.float32, generator=gen)
    d_m = slant_range_from_elevation(theta, h_m)
    pl_db = fspl_db(d_m, fc)

    Klo, Khi = cfg.K_db_range
    theta_n = ((theta - float(tlo)) / max(1e-6, float(thi) - float(tlo))).clamp(0.0, 1.0)
    K_db = float(Klo) + (float(Khi) - float(Klo)) * theta_n
    fading_mode = str(cfg.fading_mode or "rician").lower()
    h0 = _sample_rician_gain(K_db, device=device, gen=gen, shadowed=(fading_mode == "shadowed_rician"))
    state = torch.ones((B,), device=device, dtype=torch.long) if fading_mode == "shadowed_rician" else torch.zeros((B,), device=device, dtype=torch.long)

    if bool(cfg.enable_multipath):
        yc, num_taps = _apply_weak_multipath(xc, h0, cfg, gen=gen)
    else:
        yc = h0.unsqueeze(1) * xc
        num_taps = torch.ones((B,), device=device, dtype=torch.long)

    path_loss_iq_applied = bool(cfg.apply_path_loss_to_iq)
    if path_loss_iq_applied:
        d_ref = slant_range_from_elevation(
            torch.tensor([60.0], device=device),
            torch.tensor([1_000_000.0], device=device),
        )[0]
        pl_ref = fspl_db(d_ref, fc)
        g_pl = 10.0 ** (-(pl_db - pl_ref) / 20.0)
        yc = g_pl.to(torch.complex64).unsqueeze(1) * yc

    atmospheric_fading_applied = bool(cfg.enable_atmospheric_fading)
    if atmospheric_fading_applied:
        ...
        yc = (ra.to(torch.complex64) * torch.exp(1j * phia.to(torch.complex64))).unsqueeze(1) * yc

    orbital_doppler_applied = not bool(cfg.use_residual_doppler)
    fD = torch.zeros((B,), device=device, dtype=torch.float32)
    cfo = torch.randn((B,), device=device, dtype=torch.float32, generator=gen) * float(cfg.cfo_std_hz)
    f_off = cfo
    if orbital_doppler_applied:
    ...
    yc = yc * torch.exp(1j * ph.to(torch.complex64))

    pn_lo, pn_hi = cfg.phase_noise_inc_std
    pn_std = float(pn_lo) + (float(pn_hi) - float(pn_lo)) * torch.rand((B,), device=device, dtype=torch.float32, generator=gen)
    yc = yc * wiener_phase_noise(B, T, pn_std, device, torch.float32, gen=gen).to(torch.complex64)

    ag_lo, ag_hi = cfg.agc_resid_db
    resid = float(ag_lo) + (float(ag_hi) - float(ag_lo)) * torch.rand((B,), device=device, dtype=torch.float32, generator=gen)
    yc = apply_mild_agc(yc, target_rms=1.0, resid_db=resid)

    sn_lo, sn_hi = cfg.snr_db
    snr = float(sn_lo) + (float(sn_hi) - float(sn_lo)) * torch.rand((B,), device=device, dtype=torch.float32, generator=gen)
    yc = complex_awgn_like(yc, snr, gen=gen)

    iq_imbalance_applied = bool(cfg.enable_iq_imbalance)
    if iq_imbalance_applied:
        a_lo, a_hi = cfg.iq_amp_db
        p_lo, p_hi = cfg.iq_phase_deg
        amp_db = float(a_lo) + (float(a_hi) - float(a_lo)) * torch.rand((B,), device=device, dtype=torch.float32, generator=gen)
        ph_deg = float(p_lo) + (float(p_hi) - float(p_lo)) * torch.rand((B,), device=device, dtype=torch.float32, generator=gen)
    ...
    if return_meta:
        meta = {
            ...
            "multipath_profile": str(cfg.multipath_profile),
            "orbital_doppler_applied": bool(orbital_doppler_applied),
            "path_loss_iq_applied": bool(path_loss_iq_applied),
            "atmospheric_fading_applied": bool(atmospheric_fading_applied),
            "iq_imbalance_applied": bool(iq_imbalance_applied),
        }
...
def apply_sat_gnd_channel_batch(
    ...
    *,
    ...
    if str(getattr(cfg, "channel_model", "legacy_full")).lower() == "leo_residual":
        return _apply_leo_residual_channel_batch(x_iq, cfg, gen=gen, return_meta=return_meta)
