#!/usr/bin/env python
"""Export and bind the frozen source-only LEO slice for P1-HNCCD.

The generic LEO forwarder is used only for stable source reconstruction.  This
facade reopens the current HNCCD terminal receipt, binds already received
source IQ to exactly the frozen clear/low/rain scenes, and writes HNCCD-only
physical TX/RX/day/class/order evidence.  It neither fits nor tunes a proxy.
"""
...
EXPECTED_BINDING_SCHEMA = "cvs.phase1.hnccd_leo_binding.v1"
...
class HNCCDLEOBindingError(RuntimeError):
    """Raised when an HNCCD LEO export cannot prove its frozen source binding."""


def _translate(error: BaseException) -> HNCCDLEOBindingError:
    return HNCCDLEOBindingError(str(error))
...
def _sanitize_binding(value: Any) -> Any:
    if isinstance(value, Mapping):
        result: dict[str, Any] = {}
        for raw_key, raw_value in value.items():
            key = str(raw_key).lower()
            if key in RAW_RECEIVER_TOKEN_FIELDS:
                raise HNCCDLEOBindingError(f"raw source receiver token leaked into HNCCD LEO binding: {raw_key}")
            if key.startswith(LEGACY_IDENTITY_PREFIXES):
                continue
            result[str(raw_key)] = _sanitize_binding(raw_value)
        return result
    if isinstance(value, list):
        return [_sanitize_binding(item) for item in value]
...
def _assert_hnccd_only_identity(value: Any, *, label: str) -> None:
    if isinstance(value, Mapping):
        for raw_key, raw_value in value.items():
            key = str(raw_key).lower()
            if key in RAW_RECEIVER_TOKEN_FIELDS:
                raise HNCCDLEOBindingError(f"raw source receiver token leaked into {label}: {raw_key}")
            if key.startswith(LEGACY_IDENTITY_PREFIXES):
                raise HNCCDLEOBindingError(f"legacy identity leaked into {label}: {raw_key}")
...
def _validate_frozen_args(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if fold not in range(1, 7) or arm not in {"C", "G"}:
        raise HNCCDLEOBindingError("fold/arm must be F1..F6 and C/G")
    candidate = f"F{fold}{arm}_HNCCD12"
    if str(args.candidate_id) != candidate:
        raise HNCCDLEOBindingError(f"candidate_id must be {candidate}")
    ...
    if training_root.name != EXPECTED_TRAINING_RUN_LEAF or not training_root.is_dir():
        raise HNCCDLEOBindingError(f"training root must be existing {EXPECTED_TRAINING_RUN_LEAF}")
    postfreeze_root = Path(args.postfreeze_output_root).resolve()
    if training_root == postfreeze_root or not postfreeze_root.is_dir():
        raise HNCCDLEOBindingError("postfreeze root must exist and differ from training root")
    ...
    if checkpoint != expected_checkpoint or not checkpoint.is_file():
        raise HNCCDLEOBindingError("checkpoint path does not bind frozen HNCCD candidate")
    ...
    if out_npz != (candidate_dir / "source_leo_final_only.npz").resolve() or binding_json != (candidate_dir / "source_leo_binding.json").resolve():
        raise HNCCDLEOBindingError("LEO artifact paths do not match frozen candidate layout")
    source_tx_ids = _parse_csv(args.source_tx_ids, field="source_tx_ids")
    if len(source_tx_ids) != 4:
        raise HNCCDLEOBindingError("HNCCD LEO binding requires local4 source TX")
    for field, expected in (
        ...
        if observed != expected:
            raise HNCCDLEOBindingError(f"{field} drifted")
    for field, expected in (
        ...
        if int(getattr(args, field)) != expected:
            raise HNCCDLEOBindingError(f"{field} must equal frozen value {expected}")
    for field, expected in (
        ("feature_name", "z_id"),
    ...
    if args.source_only_export is not True:
        raise HNCCDLEOBindingError("HNCCD LEO export must be source-only")
    if int(args.wisig_out_len) != EXPECTED_OUT_LEN or int(args.max_samples_per_combo) != EXPECTED_MAX_PER_COMBO:
        raise HNCCDLEOBindingError("frozen LEO shape or combo selection drifted")
    dataset = Path(args.wisig_pkl).resolve()
    if not dataset.is_file():
        raise HNCCDLEOBindingError(f"missing frozen ManySig dataset: {dataset}")
    if str(args.expected_wisig_sha256).lower() != FROZEN_WISIG_SHA256:
        raise HNCCDLEOBindingError("expected WiSig SHA256 does not equal frozen value")
    dataset_sha256 = _icmt_leo._sha256_file(dataset)
    if dataset_sha256 != FROZEN_WISIG_SHA256:
        raise HNCCDLEOBindingError("ManySig input bytes do not match frozen SHA256")
    ...
    if not isinstance(checkpoint_payload, Mapping) or not isinstance(checkpoint_payload.get("args"), Mapping):
        raise HNCCDLEOBindingError("checkpoint payload must contain args mapping")
    ...
    if len(known_tx_ids) != 1 or len(proxy_tx_ids) != 1:
        raise HNCCDLEOBindingError("checkpoint held/proxy role cardinality drifted")
    try:
        ...
        raise _translate(exc) from exc
    if observed_arm != arm:
        raise HNCCDLEOBindingError("HNCCD checkpoint receipt arm does not bind LEO arm")
    return {
        "fold": fold,
        "arm": arm,
        "candidate": candidate,
        ...
        "hnccd_receipt": receipt,
        "hnccd_receipt_sha256": _hnccd_export._canonical_json_sha256(dict(checkpoint_payload["hnccd_receipt"])),
    }


@contextmanager
def _patched_signed_binding() -> Iterator[None]:
    """Temporarily point generic LEO mechanics at HNCCD root and schema."""
...
def build_binding_from_existing(args: argparse.Namespace) -> dict[str, Any]:
    """Bind a sealed single-view LEO NPZ and attach HNCCD terminal evidence."""

    try:
        with _patched_signed_binding():
            binding = _icmt_leo.build_binding_from_existing(args)
    except (_icmt_leo.ICMTLEOBindingError, _hnccd_export.HNCCDSplitExportError) as exc:
        raise _translate(exc) from exc
    frozen = _validate_frozen_args(args)
    receipt = frozen["hnccd_receipt"]
    result = dict(_sanitize_binding(binding))
    result.update(
        {
            "schema": EXPECTED_BINDING_SCHEMA,
            "method": "P1_HNCCD",
            "hnccd_receipt_schema": _hnccd_export.EXPECTED_RECEIPT_SCHEMA,
            "hnccd_receipt_sha256": frozen["hnccd_receipt_sha256"],
            "hnccd_terminal_contract": str(receipt["hnccd_terminal_contract"]),
            "hnccd_terminal_contract_passed": True,
            "hnccd_enabled": frozen["arm"] == "G",
            "hnccd_lambda": _hnccd_export.FROZEN_HNCCD_LAMBDA if frozen["arm"] == "G" else 0.0,
            "hnccd_frozen_batch_size": _hnccd_export.FROZEN_HNCCD_BATCH_SIZE,
            "hnccd_feature_dim": _hnccd_export.FROZEN_HNCCD_FEATURE_DIM,
            "hnccd_local_class_count": _hnccd_export.FROZEN_HNCCD_LOCAL_CLASS_COUNT,
            "hnccd_loss_global_denominator": _hnccd_export.FROZEN_HNCCD_GLOBAL_DENOMINATOR,
            "hnccd_fixed_batch_size": _hnccd_export.FROZEN_HNCCD_BATCH_SIZE,
            "hnccd_fixed_feature_dim": _hnccd_export.FROZEN_HNCCD_FEATURE_DIM,
            "hnccd_fixed_local_class_count": _hnccd_export.FROZEN_HNCCD_LOCAL_CLASS_COUNT,
            "hnccd_fixed_cells_per_scene": _hnccd_export.FROZEN_HNCCD_GLOBAL_DENOMINATOR,
            "hnccd_source_receiver_count": _hnccd_export.FROZEN_HNCCD_SOURCE_RECEIVER_COUNT,
            ...
            "hnccd_g_scenes_sha256": _hnccd_export._canonical_json_sha256(receipt.get("hnccd_scenes", {})) if frozen["arm"] == "G" else "",
            "hnccd_clean_head_bias_aux_vjp": "N_A_NONE_OR_ZERO_EXPECTED" if frozen["arm"] == "G" else "N_A",
            "hnccd_leo_encoder_head_weight_aux_vjp": "FINITE_NONZERO_REQUIRED" if frozen["arm"] == "G" else "N_A",
    ...
    _assert_hnccd_only_identity(result, label="HNCCD LEO binding")
    return result


def _atomic_write_json(path: Path, payload: Mapping[str, Any]) -> None:
    if path.exists():
        raise HNCCDLEOBindingError(f"refusing to overwrite LEO binding: {path}")
    temporary = path.with_name(path.name + ".tmp")
    if temporary.exists():
        raise HNCCDLEOBindingError(f"refusing to overwrite temporary LEO binding: {temporary}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if frozen["out_npz"].exists() or frozen["binding_json"].exists():
        raise HNCCDLEOBindingError("refusing to overwrite HNCCD LEO artifact")
