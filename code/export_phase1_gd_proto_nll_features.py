#!/usr/bin/env python
"""Export the exact frozen Phase1 L/V split for GD-ProtoNLL geometry.
...
class GDProtoNLLSplitExportError(RuntimeError):
...
def _require_frozen_dataset_sha256(
    *, actual: str, expected: str, checkpoint_declared: str
    ...
    if expected_value != FROZEN_WISIG_SHA256:
        raise GDProtoNLLSplitExportError(
            f"expected WiSig SHA256 must equal frozen value {FROZEN_WISIG_SHA256}"
        )
    if actual_value != expected_value:
        raise GDProtoNLLSplitExportError("WiSig input bytes do not match expected SHA256")
...
def _require_close(name: str, observed: Any, expected: float) -> None:
    try:
        ...
        raise GDProtoNLLSplitExportError(f"checkpoint arg {name} is not numeric") from exc
...
def _parse_csv(value: Any, *, field: str) -> tuple[str, ...]:
    ...
    if not items or len(items) != len(set(items)):
        raise GDProtoNLLSplitExportError(f"{field} must be non-empty and duplicate-free")
    return items
...
def _physical_keys_for_indices(base: WiSigCompactDataset, values: Sequence[Any]) -> tuple[str, ...]:
    ...
    if eq_list is None:
        raise GDProtoNLLSplitExportError("dataset lacks equalized-axis labels")
    keys: list[str] = []
    for value in values:
...
def _validate_checkpoint_contract(
    ...
    *,
    ...
    if not isinstance(checkpoint.get("model"), Mapping) or not isinstance(checkpoint.get("args"), Mapping):
        raise GDProtoNLLSplitExportError("checkpoint must contain model and args mappings")
    ...
    if str(args.get("checkpoint_selection", "")) != EXPECTED_CHECKPOINT_SELECTION:
        raise GDProtoNLLSplitExportError("checkpoint args do not preserve final_only selection")
    if str(args.get("candidate_id", "")) != checkpoint_path.parent.name:
        raise GDProtoNLLSplitExportError("checkpoint candidate_id does not match its parent arm directory")
    if str(checkpoint.get("candidate_id", "")) != checkpoint_path.parent.name:
        raise GDProtoNLLSplitExportError("top-level checkpoint candidate_id does not match its parent arm directory")
    receipt = checkpoint.get("gd_proto_nll_receipt")
    if not isinstance(receipt, Mapping):
        raise GDProtoNLLSplitExportError("checkpoint lacks GD-ProtoNLL terminal receipt")
    if receipt.get("gd_proto_nll_terminal_contract_passed") is not True:
        raise GDProtoNLLSplitExportError("GD-ProtoNLL terminal receipt did not pass")
    if str(receipt.get("checkpoint_role", "")) != EXPECTED_CHECKPOINT_ROLE:
        raise GDProtoNLLSplitExportError("GD-ProtoNLL receipt is not bound to training_final_only")
    if tuple(str(item) for item in receipt.get("source_train_tx", [])) != tuple(source_tx_ids):
        raise GDProtoNLLSplitExportError("GD-ProtoNLL receipt source_train_tx order drifted")
    if tuple(str(item) for item in receipt.get("source_known_validation_tx", [])) != tuple(known_validation_tx_ids):
        raise GDProtoNLLSplitExportError("GD-ProtoNLL receipt known-validation TX drifted")
    if tuple(str(item) for item in receipt.get("source_proxy_unknown_tx", [])) != tuple(proxy_unknown_tx_ids):
        raise GDProtoNLLSplitExportError("GD-ProtoNLL receipt proxy-unknown TX drifted")
    return args
...
def reconstruct_frozen_source_split(
    ...
    *,
    ...
    if any(all_sets[i] & all_sets[j] for i, j in ((0, 1), (0, 2), (1, 2))):
        raise GDProtoNLLSplitExportError("reconstructed L/U/V indices overlap")
    if set().union(*all_sets) != set(range(len(source_base))):
        raise GDProtoNLLSplitExportError("reconstructed L/U/V indices do not cover source_base exactly")
...
def _require_split_receipts_match(checkpoint: Mapping[str, Any], reconstructed: Mapping[str, Any]) -> None:
    ...
    if not isinstance(split_info, Mapping):
        raise GDProtoNLLSplitExportError("checkpoint lacks split_info")
    expected_split = split_info.get("source_split_receipt")
    if not isinstance(expected_split, Mapping):
        raise GDProtoNLLSplitExportError("checkpoint lacks source_split_receipt")
    if dict(expected_split) != dict(reconstructed["source_split_receipt"]):
        raise GDProtoNLLSplitExportError("reconstructed source split receipt does not equal checkpoint receipt")
    expected_partition = split_info.get("tx_partition_receipt")
    if not isinstance(expected_partition, Mapping):
        raise GDProtoNLLSplitExportError("checkpoint lacks tx_partition_receipt")
    if dict(expected_partition) != dict(reconstructed["tx_partition_receipt"]):
        raise GDProtoNLLSplitExportError("reconstructed TX partition receipt does not equal checkpoint receipt")
    expected_sizes = {
...
def _validate_local4_coverage(
    ...
    *,
    ...
    for value in indices:
        ...
        if tx not in counts:
            raise GDProtoNLLSplitExportError(f"{role} contains non-local4 TX {tx}")
        counts[tx] += 1
...
def _atomic_save_npz(path: Path, payload: Mapping[str, Any]) -> None:
    if path.exists():
        raise GDProtoNLLSplitExportError(f"refusing to overwrite L/V export: {path}")
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(path.name + ".tmp")
    if temporary.exists():
        raise GDProtoNLLSplitExportError(f"refusing to overwrite temporary L/V export: {temporary}")
...
def export(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not checkpoint_path.is_file():
        raise GDProtoNLLSplitExportError(f"missing final checkpoint: {checkpoint_path}")
    if not dataset_path.is_file():
        raise GDProtoNLLSplitExportError(f"missing WiSig dataset: {dataset_path}")
    ...
    if len(source_tx_ids) != 4 or len(known_validation_tx_ids) != 1 or len(proxy_unknown_tx_ids) != 1:
        raise GDProtoNLLSplitExportError("GD-ProtoNLL L/V export requires local4 plus one held and one proxy TX")
    ...
    if any(role_sets[i] & role_sets[j] for i, j in ((0, 1), (0, 2), (1, 2))):
        raise GDProtoNLLSplitExportError("source/known-validation/proxy-unknown TX roles overlap")
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    if not isinstance(checkpoint, Mapping):
        raise GDProtoNLLSplitExportError("checkpoint payload must be a mapping")
    ...
    reconstructed = reconstruct_frozen_source_split(
        ...
        known_validation_tx_ids=known_validation_tx_ids,
        proxy_unknown_tx_ids=proxy_unknown_tx_ids,
        # The v3 launcher did not declare a dataset digest.  Preserve the
    ...
    if set(labeled_physical) & set(validation_physical):
        raise GDProtoNLLSplitExportError("L/V physical keys overlap")
    ...
    proxy_ds, proxy_info = _build_wisig_dataset(
        ...
        out_len=int(checkpoint_args.get("wisig_out_len", 256)),
        domain=str(checkpoint_args.get("wisig_domain", "rx_day")),
        days=str(args.proxy_days),
        rxs=str(args.proxy_rxs),
        max_samples_per_combo=0,
        max_samples_per_tx=int(args.max_proxy_samples_per_tx),
        seed=EXPECTED_SEED + 43,
    ...
    if set(labeled_physical) & set(proxy_physical) or set(validation_physical) & set(proxy_physical):
        raise GDProtoNLLSplitExportError("L/V/proxy physical keys overlap")
    ...
    labeled_ds = WiSigSubsetDataset(source_base, labeled_indices, split_source="gd_proto_nll_labeled_fit")
    validation_ds = WiSigSubsetDataset(
        source_base,
        validation_indices,
        split_source="gd_proto_nll_source_validation_known",
    )
    ...
    manifest = {
        ...
        "proxy_days": str(args.proxy_days),
        "proxy_rxs": str(args.proxy_rxs),
        "proxy_seed": EXPECTED_SEED + 43,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--proxy_days", default="2021_03_01,2021_03_08")
    parser.add_argument("--proxy_rxs", default="1-1,1-19,14-7,18-2,19-2,2-1")
    parser.add_argument("--max_proxy_samples_per_tx", type=int, default=400)
