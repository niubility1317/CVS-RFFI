...
def build_baseline_model(model_args, device: torch.device) -> nn.Module:
    return build_dual_model(
        ...
        use_ntrs=bool(getattr(model_args, "use_ntrs", False)),
        ntrs_rank=int(getattr(model_args, "ntrs_rank", 8)),
        ntrs_alpha_max=float(getattr(model_args, "ntrs_alpha_max", 0.20)),
        ntrs_q_dim=int(getattr(model_args, "ntrs_q_dim", 32)),
        ntrs_fast_dim=int(getattr(model_args, "ntrs_fast_dim", 24)),
        ntrs_slow_dim=int(getattr(model_args, "ntrs_slow_dim", 24)),
        ntrs_metadata_dim=int(getattr(model_args, "ntrs_metadata_dim", 9)),
        ntrs_slow_ema_decay=float(getattr(model_args, "ntrs_slow_ema_decay", 0.95)),
        ...
        ntrs_q_trainable=bool(getattr(model_args, "ntrs_q_trainable", True)),
        ntrs_use_support_gate=bool(getattr(model_args, "ntrs_use_support_gate", False)),
        ...
        sid_architecture=str(getattr(model_args, "sid_architecture", "residual")),
        sid_fusion_mode=str(getattr(model_args, "sid_fusion_mode", "fused")),
        sid_spectral_dim=int(getattr(model_args, "sid_spectral_dim", 48)),
        sid_fusion_alpha_max=float(getattr(model_args, "sid_fusion_alpha_max", 0.20)),
