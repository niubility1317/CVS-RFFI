...
V2_WISIG_SHA256 = "2b0a7a7488dd3650bcae7b1d80efbcffd1598aaa671ae6b0a0df2a24dc0f694f"
...
def _write_v2_launcher_inputs(project_root: Path) -> None:
...
def _smoke_launcher_for_project(tmp_path: Path, project_root: Path) -> Path:
    ...
    fixture_launcher = fixture_code_root / "scripts" / V2_SMOKE_LAUNCHER.name
    fixture_launcher.parent.mkdir(parents=True, exist_ok=True)
    launcher_text = V2_SMOKE_LAUNCHER.read_text(encoding="utf-8")
    ...
    environment.update(
        {
            ...
            "RACE_WISIG_SHA256": V2_WISIG_SHA256,
        }
    )
    return environment
...
def test_source_metrics_v2_smoke_rejects_noncanonical_project_root_before_any_output(tmp_path: Path) -> None:
...
def test_source_metrics_v2_formal_rejects_post_guard_pid_root_race(tmp_path: Path) -> None:
    """Break caught: a rival root created after the guard cannot replace formal PID evidence."""

    project_root = tmp_path / "project"
    _write_v2_launcher_inputs(project_root)
    run_root = project_root / "runs" / V2_RUN_ID
    log_root = project_root / "logs" / V2_RUN_ID
...
def test_source_metrics_v2_smoke_rejects_post_guard_log_root_race(tmp_path: Path) -> None:
    """Break caught: a rival smoke log marker cannot be truncated after an exists check."""

    project_root = tmp_path / "project"
    _write_v2_launcher_inputs(project_root)
    run_root = project_root / "runs" / V2_SMOKE_ROOT_NAME
    log_root = project_root / "logs" / V2_SMOKE_ROOT_NAME
    ...
    assert result.returncode == 3
    assert marker_path.read_text(encoding="utf-8") == f"{marker_text}\n"
    assert not (run_root / V2_RUN_ID / "F1_SHARED" / "source_validation_known_leo_weak.npz").exists()
