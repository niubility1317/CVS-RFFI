...
V4_LAUNCHER = CODE_ROOT / "scripts" / "launch_phase1_clic_source_metrics12_v4_20260816.sh"
V4_SMOKE_LAUNCHER = CODE_ROOT / "scripts" / "smoke_phase1_clic_source_metrics_f1_v4_20260816.sh"
V4_RUN_ID = "phase1_clic_source_metrics_20260816_v4"
V4_SMOKE_ROOT_NAME = ".smoke_phase1_clic_source_metrics_20260816_v4_F1"
V4_WISIG_SHA256 = "2b0a7a7488dd3650bcae7b1d80efbcffd1598aaa671ae6b0a0df2a24dc0f694f"
...
def _write_v4_launcher_inputs(project_root: Path) -> None:
...
def _smoke_launcher_for_project(tmp_path: Path, project_root: Path) -> Path:
    ...
    fixture_launcher = fixture_code_root / "scripts" / V4_SMOKE_LAUNCHER.name
    fixture_launcher.parent.mkdir(parents=True, exist_ok=True)
    launcher_text = V4_SMOKE_LAUNCHER.read_text(encoding="utf-8")
    ...
    environment.update(
        {
            ...
            "RACE_WISIG_SHA256": V4_WISIG_SHA256,
        }
    )
    return environment


def test_source_metrics_v4_formal_dry_run_keeps_the_complete_frozen_matrix() -> None:
    """Break caught: v4 changes matrix stages, v1 identity, or source-only controls."""

    lines = _dry_run_script(V4_LAUNCHER)
    assert len(lines) == 25
    joined = "\n".join(lines)
    assert V4_RUN_ID in joined
...
def test_source_metrics_v4_formal_rejects_immutable_root_collision(tmp_path: Path) -> None:
    """Break caught: a new v4 launcher resumes or overwrites its formal run root."""

    project_root = tmp_path / "project"
    (project_root / "runs" / V4_RUN_ID).mkdir(parents=True)
    result = subprocess.run(
        [str(GIT_BASH), _git_bash_path(V4_LAUNCHER)],
        cwd=CODE_ROOT.parent,
...
def test_source_metrics_v4_f1_smoke_dry_run_is_path_strict_and_score_free() -> None:
    """Break caught: smoke mirrors formal inputs, scores performance, or launches more than F1 C/G."""

    lines = _dry_run_script(V4_SMOKE_LAUNCHER)
    assert len(lines) == 3
    joined = "\n".join(lines)
    assert V4_RUN_ID in joined
    assert V4_SMOKE_ROOT_NAME in joined
...
def test_source_metrics_v4_smoke_rejects_preexisting_root_before_input_checks(tmp_path: Path) -> None:
    """Break caught: a smoke can overwrite its cache/feature/log root on a second invocation."""

    project_root = tmp_path / "project"
    (project_root / "runs" / V4_SMOKE_ROOT_NAME).mkdir(parents=True)
    ...
    assert result.returncode == 3
    assert "refusing to overwrite source metrics v4 smoke run/log root" in result.stderr


def test_source_metrics_v4_smoke_rejects_noncanonical_project_root_before_any_output(tmp_path: Path) -> None:
    """Break caught: caller-controlled PROJECT_ROOT cannot redirect a smoke cache before export."""

    project_root = tmp_path / "mirror_only"
    result = subprocess.run(
        [str(GIT_BASH), _git_bash_path(V4_SMOKE_LAUNCHER)],
        cwd=CODE_ROOT.parent,
        env=_project_root_environment(project_root),
        capture_output=True,
        text=True,
...
def test_source_metrics_v4_formal_rejects_post_guard_pid_root_race(tmp_path: Path) -> None:
    """Break caught: a rival root created after the guard cannot replace formal PID evidence."""

    project_root = tmp_path / "project"
    _write_v4_launcher_inputs(project_root)
    run_root = project_root / "runs" / V4_RUN_ID
    log_root = project_root / "logs" / V4_RUN_ID
...
def test_source_metrics_v4_smoke_rejects_post_guard_log_root_race(tmp_path: Path) -> None:
    """Break caught: a rival smoke log marker cannot be truncated after an exists check."""

    project_root = tmp_path / "project"
    _write_v4_launcher_inputs(project_root)
    run_root = project_root / "runs" / V4_SMOKE_ROOT_NAME
    log_root = project_root / "logs" / V4_SMOKE_ROOT_NAME
    ...
    assert result.returncode == 3
    assert marker_path.read_text(encoding="utf-8") == f"{marker_text}\n"
    assert not (run_root / V4_RUN_ID / "F1_SHARED" / "source_validation_known_leo_weak.npz").exists()
