...
class Phase2SupportMetricQknnV51PolicyTest(unittest.TestCase):
    def test_v51_adds_tiny_unlabeled_query_pair_candidates_for_reliable_many_new_support(self):
        ...
        self.assertEqual(reliable["adaptive_qknn_policy"], "stable_dualview_v51")
        ...
        self.assertGreater(enabled["top_pairs"], 4)
        self.assertGreater(enabled["query_pair_weight"], 0.0)
        self.assertLessEqual(enabled["query_pair_weight"], 0.012)
        self.assertEqual(disabled["weight"], 0.0)
        self.assertEqual(disabled["query_pair_weight"], 0.0)
