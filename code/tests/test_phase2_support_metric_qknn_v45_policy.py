...
class Phase2SupportMetricQknnV45PolicyTest(unittest.TestCase):
    def test_v45_inherits_v44_adaptive_neighbor_switch(self):
        ...
        self.assertEqual(reliable["adaptive_qknn_policy"], "stable_dualview_v45")
        self.assertEqual(reliable["topm"], 1)
        self.assertTrue(reliable["scenario_class_fallback"])
        self.assertEqual(low_k["topm"], 4)
        self.assertFalse(low_k["scenario_class_fallback"])
        self.assertGreater(low_k["labelprop_weight"], 0.0)
        self.assertGreater(reliable["source_target_transport_weight"], 0.0)
        self.assertGreater(low_k["source_target_transport_weight"], 0.0)
