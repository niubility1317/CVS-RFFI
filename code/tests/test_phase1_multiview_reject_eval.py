...
from eval_phase1_prototype_reject import evaluate as evaluate_prototype  # noqa: E402
...
def test_prototype_reject_accepts_single_satellite_view(tmp_path):
    features = []
    logits = []
    roles = []
    tx_ids = []
    sig_ids = []
    groups = [
        ("source", "old-a", "s0", [1.0, 0.0], [5.0, 0.0]),
        ("source", "old-b", "s1", [0.0, 1.0], [0.0, 5.0]),
        ("proxy_unknown", "proxy-a", "s2", [-1.0, 0.0], [0.5, 0.4]),
        ("proxy_unknown", "proxy-b", "s3", [0.0, -1.0], [0.4, 0.5]),
        ("target_old", "old-a", "s4", [1.0, 0.0], [5.0, 0.0]),
        ("target_old", "old-b", "s5", [0.0, 1.0], [0.0, 5.0]),
        ("target_unknown", "new-a", "s6", [-1.0, 0.0], [0.5, 0.4]),
        ("target_unknown", "new-b", "s7", [0.0, -1.0], [0.4, 0.5]),
    ]
    for role, tx, sig, feat, logit in groups:
        features.append(feat)
        logits.append(logit)
        roles.append(role)
        tx_ids.append(tx)
        sig_ids.append(sig)
    npz = tmp_path / "single_satellite_features.npz"
    np.savez(
        npz,
        features=np.asarray(features, dtype=np.float32),
        tx_logits=np.asarray(logits, dtype=np.float32),
        ...
        eq_ids=np.asarray(["1"] * len(tx_ids)),
        sig_ids=np.asarray(sig_ids),
        channel_views=np.asarray(["satellite_unknown_leo"] * len(tx_ids)),
        sat_scenarios=np.asarray(["leo_clear_weak"] * len(tx_ids)),
        manifest_json=np.asarray(json.dumps({"contains_clean_view": False, "per_sample_observation_count": 1})),
    )
    metrics = evaluate_prototype(
        Namespace(
            feature_npz=str(npz),
            source_tx_ids="old-a,old-b",
            unknown_tx_ids="new-a,new-b",
    ...
    assert metrics["manifest"]["contains_clean_view"] is False
    assert metrics["known_closed_accuracy_no_reject"] == 1.0
    assert metrics["known_full_accuracy_after_reject"] == 1.0
    assert metrics["unknown_FAR"] == 0.0
    assert metrics["passes_dual_target"] is True
