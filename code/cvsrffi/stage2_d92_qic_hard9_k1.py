"""Frozen QIC Hard9+K1 matrix and method-lock validation.

This module owns only the immutable experiment identity and its sealed-package
joins. It never opens query truth, scores predictions, or changes the QIC
...
MATRIX_SCHEMA = "cvs.phase2.d92_qic_hard9_k1.matrix.v1"
METHOD_LOCK_SCHEMA = "cvs.phase2.d92_qic_hard9_k1.method_lock.v1"
JOB_RECEIPT_SCHEMA = "cvs.phase2.d92_qic_hard9_k1.job_receipt.v1"
SHARD_SUMMARY_SCHEMA = "cvs.phase2.d92_qic_hard9_k1.shard_summary.v1"
SYSTEMIC_FAILURE_SCHEMA = "cvs.phase2.d92_qic_hard9_k1.systemic_failure.v1"
E0_RESOURCE_SOURCE_MODE = "embedded_preregistered_projection"

ARM_ID = "E0_FULL_D42_QUANTIZATION_INTERCEPT_CLOSURE"
CANDIDATE_ID = "d92_e0_full_d42_quantization_intercept_closure"
ARM_ORDER = (ARM_ID,)
...
SCIENTIFIC_ENTRY_COMMIT = "2392a8b79444036d66edb85bab27b2cc827ebc5b"
RUNTIME_SOURCE_FILES = {
    ...
    "cvsrffi/stage2_d92_e0d_query_evaluation.py": {
        "git_blob": "6222a0523dd8af731cc1b39a3b78aae8843918e3",
        "sha256": "20e20e86a13bc0ec3e3051446b9c1f523ab61b89269ae7731ef01ba71706f8d7",
    },
    "cvsrffi/stage2_d92_e0d_slim.py": {
        "git_blob": "b80bb2baea38cbc807a60982143c3019b0b9aa89",
        "sha256": "84eb8d7daac74bd5dffae20102aa8bd4a400c21111b161fa4d01355efbe27d8a",
    },
    "cvsrffi/stage2_d92_cross_class_offblock_consensus.py": {
        "git_blob": "abb45a15514c8c5758e0cececb930a76d27a29b8",
        "sha256": "6f87d4eb041ba8874182a46eb3f2a76dc3f2f075a6692ee217f19bcd2f8ff331",
    },
...
class D92QICHard9K1Error(ValueError):
    """Raised when the frozen QIC Hard9+K1 identity drifts."""


D92QICHard9K1MatrixError = D92QICHard9K1Error
...
def _pure_path(value: Any) -> PurePath:
    if not isinstance(value, str) or not value or "\x00" in value:
        raise D92QICHard9K1Error("path identity drift")
...
def _parse_outer(key: str) -> tuple[str, int, int, int]:
    ...
    if match is None:
        raise D92QICHard9K1Error(f"invalid outer key: {key}")
    return (
...
def _selection_payload() -> dict[str, Any]:
    return {
        "schema": "cvs.phase2.d92_qic_hard9_k1.selection.v1",
        "selection_id": "D92-E0-FULL-QIC-Hard9-K1-v1",
...
def _expected_lock() -> dict[str, Any]:
    return {
        ...
        "experiment_id": "D92-E0-FULL-QIC-Hard9-K1-v1",
        "protocol_schema": "p2_min_v1",
        "claim_scope": CLAIM_SCOPE,
        ...
        "g0_prerequisite": {
            ...
            "status": "D92_QIC_G0_ACTIVE_QUANTIZATION_INTERCEPT_CLOSURE_RESOURCE_PASS",
...
def validate_method_lock(lock: Mapping[str, Any]) -> dict[str, Any]:
    """Fail closed unless the QIC method lock is exactly frozen."""

    if not isinstance(lock, Mapping) or _canonical_bytes(lock) != _canonical_bytes(
        _expected_lock()
    ):
        raise D92QICHard9K1Error("QIC Hard9+K1 method lock identity drift")
    return dict(lock)
...
def _read_method_lock(config_path: str | Path) -> tuple[Path, dict[str, Any], str]:
    ...
    if source.is_symlink() or not source.is_file():
        raise D92QICHard9K1Error("method lock must be a regular file")
    try:
        payload = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise D92QICHard9K1Error("method lock JSON drift") from error
    if not isinstance(payload, dict):
        raise D92QICHard9K1Error("method lock must be an object")
    ...
    *,
    ...
    for name, (package_parts, seal_parts) in PACKAGE_LAYOUT.items():
        ...
        if require_files and (
            ...
            raise D92QICHard9K1Error(f"sealed source package missing: {name}")
        entries[name] = {
...
def _preregistered_truth_sidecar_sha256(outer_key: str) -> str:
    ...
    if not _is_sha256(digest):
        raise D92QICHard9K1Error("pre-registered truth sidecar hash missing")
...
def validate_hard9_k1_manifest(
    ...
    *,
    ...
    """Validate the final QIC matrix schema without opening query truth."""

    if not isinstance(manifest, Mapping) or set(manifest) != _MANIFEST_KEYS:
        raise D92QICHard9K1Error("manifest allowed-key drift")
    expected = {
    ...
    if any(manifest.get(key) != value for key, value in expected.items()):
        raise D92QICHard9K1Error("manifest identity/count drift")
    ...
    if not _is_sha256(method_lock_sha) or (
        ...
        raise D92QICHard9K1Error("method-lock SHA drift")
    for field in (
        "method_lock",
        "source_d92_output_root",
        "ground_component_dir",
    ...
    if manifest.get("selected_rows") != rows or manifest.get("coverage") != _coverage(
        ...
        raise D92QICHard9K1Error("selected-row/coverage drift")
    jobs = manifest.get("jobs")
    if not isinstance(jobs, list) or len(jobs) != len(rows):
        raise D92QICHard9K1Error("job-count drift")
    ...
    for index, row in enumerate(rows):
        ...
        if (
            ...
            raise D92QICHard9K1Error("canonical job identity drift")
        if row["outer_key"] in EXCLUDED_OUTER_KEYS:
            raise D92QICHard9K1Error("G0 outer exclusion drift")
        if not _path_matches(
            ...
            raise D92QICHard9K1Error("job path drift")
        ...
        if not isinstance(e0_resource, Mapping) or set(e0_resource) != {
            ...
            raise D92QICHard9K1Error("E0 resource manifest record drift")
        fit_audit = e0_resource.get("fit_audit")
        if (
            not isinstance(fit_audit, Mapping)
            or set(fit_audit) != {"path", "sha256"}
        ):
            raise D92QICHard9K1Error("E0 resource fit-audit record drift")
        ...
        if (
            ...
            raise D92QICHard9K1Error("truth sidecar hash drift")
        packages = job.get("packages")
        if not isinstance(packages, Mapping) or set(packages) != set(PACKAGE_LAYOUT):
            raise D92QICHard9K1Error("package identity drift")
        for name, package in packages.items():
            ...
            if (
                ...
                raise D92QICHard9K1Error("package path/hash drift")
        job_id = str(job["job_id"])
        if job_id in seen_job_ids:
            raise D92QICHard9K1Error("duplicate job identity")
...
def build_hard9_k1_manifest(
    ...
    *,
    ...
    """Build the exact ten-job manifest from the frozen QIC method lock."""

    lock_path, lock, method_lock_sha = _read_method_lock(config_path)
    runtime = lock["runtime"]
    if not isinstance(runtime, Mapping):
        raise D92QICHard9K1Error("method lock runtime identity drift")
...
build_qic_hard9_k1_manifest = build_hard9_k1_manifest
