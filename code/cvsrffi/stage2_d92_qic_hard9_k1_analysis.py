"""Truth-last analyzer and sole frozen verdict for D92 QIC Hard9+K1.
...
try:
    from cvsrffi import stage2_d92_qic_hard9_k1 as matrix
except ImportError:  # pragma: no cover - only used before the parallel matrix lands
...
# as analysis constants only; QIC does not tune or redefine them.
...
class D92QICHard9K1AnalysisError(ValueError):
    """Raised when a frozen QIC evidence join is incomplete or detached."""


def _fail(message: str) -> D92QICHard9K1AnalysisError:
    return D92QICHard9K1AnalysisError(message)
...
def _fit_audit_prefix(row: Mapping[str, Any]) -> str:
    """Return the candidate's frozen fit-audit namespace.

    QIC receipts use ``d92_e0d_qic_``.  The CCOC fallback is retained only so
    this adapter remains runnable while a pre-existing CCOC-shaped fixture is
    used by a focused local test; it is never selected for a real QIC row.
    """

    configured = getattr(matrix, "FIT_AUDIT_PREFIX", None)
    if isinstance(configured, str) and configured:
        return configured
    for prefix in ("d92_e0d_qic_", "d92_e0d_ccoc_"):
        if prefix + "active" in row:
            return prefix
    return "d92_e0d_qic_"
...
def evaluate_resource_gate(candidate_rows: Sequence[Mapping[str, Any]], *, query_state_exact: bool = True) -> dict[str, Any]:
    """Evaluate keyed per-scene QIC resource receipts.
...
def _read_prediction_artifact(path: str | Path, *, label: str) -> dict[str, list[str]]:
    ...
    except D92QICHard9K1AnalysisError:
        raise
...
def _read_truth_surface(path: str | Path) -> dict[str, dict[str, str]]:
    """Parse the same manifest-bound truth sidecar used by E0 and QIC."""
...
def _four_state_metrics(aggregate: Mapping[str, Any]) -> dict[str, dict[str, Any]]:
    """Expose the four DA/registration states without inventing absent data."""
    ...
    return {
        ...
        "DA1_REG0": {
            "old_acc": aggregate.get("candidate_mean_da1_reg0_old_acc"),
            "old_floor": aggregate.get("candidate_mean_da1_reg0_old_floor"),
            "h_old_new": None,
            "seen_new_acc": None,
            "evidence": "raw E0 before-registration surface",
        },
        "DA0_REG1": {
            "old_acc": None,
            "old_floor": None,
        ...
        "DA1_REG1": {
            "old_acc": aggregate.get("candidate_mean_c_old_acc"),
            "old_floor": aggregate.get("candidate_mean_old_floor"),
            "h_old_new": aggregate.get("candidate_mean_h_old_new"),
            "seen_new_acc": aggregate.get("candidate_mean_seen_new_acc"),
            "evidence": "candidate after-registration surface",
        },
    }
...
def _validate_score_binding(
    ...
    *,
    ...
    expected_binding = {
        "schema": "cvs.phase2.d92_qic_hard9_k1.score_binding.v1",
        "job_id": job["job_id"],
        "outer_key": job["outer_key"],
        "outer_role": job["outer_role"],
        "arm_id": ARM_ID,
...
def _analyze_d92_qic_hard9_k1(
    ...
    *,
    ...
    return {
        ...
        "aggregate": aggregate,
        "four_state_metrics": _four_state_metrics(aggregate),
...
def _prepare_output_root(output_root: str | Path) -> Path:
    ...
    if root.exists() and (not root.is_dir() or root.is_symlink() or any(root.iterdir())):
        raise D92QICHard9K1AnalysisError(
...
def _controlled_reject_result(error: Exception) -> dict[str, Any]:
    ...
    return {
        ...
        "aggregate": {},
        "four_state_metrics": _four_state_metrics({}),
        "paired_rows": [],
        "per_old_class_rows": [],
...
def analyze_d92_qic_hard9_k1(
    ...
    *,
    ...
    try:
        result = _analyze_d92_qic_hard9_k1(
    ...
    except (D92QICHard9K1AnalysisError, OSError) as error:
...
analyze_qic_hard9_k1 = analyze_d92_qic_hard9_k1
...
def write_analysis_outputs(result: Mapping[str, Any], output_root: str | Path) -> dict[str, str]:
    ...
    if root.exists() and any(root.iterdir()):
        raise D92QICHard9K1AnalysisError(f"output overwrite refused: existing output root {root}")
...
__all__ = [
    ...
    "CANDIDATE_ID",
    "DIRECTION_GATE",
    "D92QICHard9K1AnalysisError",
    "EIGHT_PARETO_METRICS",
    "PARETO_METRICS",
    "SCENES",
    "STRICT_PARETO_THRESHOLDS",
    "VERDICTS",
    "analyze_qic_hard9_k1",
    "analyze_d92_qic_hard9_k1",
