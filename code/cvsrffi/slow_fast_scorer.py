...
def _score_shadow_row(
    ...
    *,
    ...
    for state in states:
        ...
        query_ids, predicted, _ = _load_and_validate_prediction(path, registered)
        ...
        predictions[state] = (query_ids, predicted)
    if reference_ids is None:
    ...
    scored_states = {
        ...
        for state, (_, predicted) in predictions.items()
    }
    baseline_pred = predictions["DA0_REG0"][1]
    return {
        "schema": "cvs.stage2.slow_fast.shadow_row_score.v2",
        "status": "ANALYZED",
