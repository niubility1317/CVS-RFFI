"""Independent post-prediction analyzer for D92 ParetoDistill Hard11."""
...
from scripts.run_d92_pareto_distill_hard11 import QUERY_ZERO_FIELDS, _prediction_closure_status
...
def _read_json(path: Path) -> dict[str, Any]:
    if not path.is_file() or path.is_symlink():
        raise D92ParetoDistillHard11AnalysisError(f"missing JSON artifact: {path}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise D92ParetoDistillHard11AnalysisError(f"invalid JSON artifact: {path}") from error
    if not isinstance(payload, dict):
        raise D92ParetoDistillHard11AnalysisError(f"JSON artifact is not an object: {path}")
    return payload
...
def _read_csv(path: Path, expected_sha256: str, *, expected_rows: int | None = None) -> list[dict[str, str]]:
    if not path.is_file() or path.is_symlink() or _sha256(path) != expected_sha256:
        raise D92ParetoDistillHard11AnalysisError(f"frozen CSV identity drift: {path}")
    with path.open(encoding="utf-8-sig", newline="") as handle:
        rows = [dict(row) for row in csv.DictReader(handle)]
    if expected_rows is not None and len(rows) != expected_rows:
        raise D92ParetoDistillHard11AnalysisError(f"frozen CSV row closure drift: {len(rows)}")
    return rows
...
def _finite(value: Any, label: str, *, lower: float | None = None, upper: float | None = None) -> float:
    try:
        ...
        raise D92ParetoDistillHard11AnalysisError(f"non-numeric {label}") from error
    if not math.isfinite(result) or (lower is not None and result < lower) or (upper is not None and result > upper):
        raise D92ParetoDistillHard11AnalysisError(f"out-of-range {label}")
    return result
...
def _mean(values: Iterable[float]) -> float:
    values = list(values)
    if not values:
        raise D92ParetoDistillHard11AnalysisError("empty mean")
...
def _rate(after: Mapping[str, Any], name: str) -> float:
    ...
    if isinstance(by_scene, Mapping):
        ...
        if values:
            return _mean(values)
    raise D92ParetoDistillHard11AnalysisError(f"missing {name}")
...
def compute_confusion_rates(score: Mapping[str, Any]) -> dict[str, float]:
    ...
    if not isinstance(before, Mapping) or not isinstance(after, Mapping):
        raise D92ParetoDistillHard11AnalysisError("score state surface missing")
    ...
    if set(before_scene) != set(after_scene) or not before_scene:
        raise D92ParetoDistillHard11AnalysisError("bidirectional scenario closure drift")
    old_total = 0.0
    new_total = 0.0
    old_to_new = 0.0
    new_to_old = 0.0
    for scene, after_row in after_scene.items():
        ...
        if not isinstance(before_row, Mapping) or not isinstance(after_row, Mapping):
            raise D92ParetoDistillHard11AnalysisError(f"scenario row missing: {scene}")
        ...
        if new_count < -_TOLERANCE:
            raise D92ParetoDistillHard11AnalysisError(f"scenario query count regressed: {scene}")
    ...
    if old_total <= 0.0 or new_total <= 0.0:
        raise D92ParetoDistillHard11AnalysisError("old/new query count closure is empty")
    ...
    for name, value in result.items():
        ...
        if aggregate is not None and abs(_finite(aggregate, name, lower=0.0, upper=1.0) - value) > 1.0e-9:
            raise D92ParetoDistillHard11AnalysisError(f"aggregate {name} disagrees with scenario weighting")
    return result
...
def validate_per_old_class_join(rows: Sequence[Mapping[str, Any]], raw_score: Mapping[str, Any], *, outer_key: str) -> dict[str, dict[str, float]]:
    ...
    if not isinstance(rows, Sequence) or not rows:
        raise D92ParetoDistillHard11AnalysisError(f"per-old rows missing for {outer_key}")
    by_tx = raw_score.get("after", {}).get("by_tx") if isinstance(raw_score.get("after"), Mapping) else None
    if not isinstance(by_tx, Mapping):
        raise D92ParetoDistillHard11AnalysisError(f"per-old raw by_tx missing for {outer_key}")
    raw_old = {
    ...
    if not raw_old:
        raise D92ParetoDistillHard11AnalysisError(f"per-old raw old-class closure missing for {outer_key}")
    joined: dict[str, dict[str, float]] = {}
    for row in rows:
        if str(row.get("outer_key")) != str(outer_key):
            raise D92ParetoDistillHard11AnalysisError(f"per-old outer join drift for {outer_key}")
        tx = str(row.get("tx"))
        if tx in joined or tx not in raw_old:
            raise D92ParetoDistillHard11AnalysisError(f"per-old TX join drift for {outer_key}")
        ...
        if abs(historical_candidate - raw_old[tx]) > 1.0e-9:
            raise D92ParetoDistillHard11AnalysisError(f"per-old E0 accuracy mismatch for {outer_key}/{tx}")
        joined[tx] = {
    ...
    if set(joined) != set(raw_old):
        raise D92ParetoDistillHard11AnalysisError(f"per-old TX set closure drift for {outer_key}")
    return joined
...
def evaluate_resource_gate(candidate_rows: Sequence[Mapping[str, Any]], baseline_rows: Sequence[Mapping[str, Any]], *, query_state_exact: bool) -> dict[str, Any]:
    ...
    for candidate, baseline in zip(candidate_rows, baseline_rows):
        ...
        if base_wall <= 0.0:
            raise D92ParetoDistillHard11AnalysisError("E0 registration wall is zero")
...
def validate_truth_binding(score: Mapping[str, Any], receipt: Mapping[str, Any], job: Mapping[str, Any], truth_path: str | Path) -> str:
    ...
    if not (same_absolute_path or same_logical_path) or not actual_path.is_file() or actual_path.is_symlink() or not re.fullmatch(r"[0-9a-f]{64}", expected):
        raise D92ParetoDistillHard11AnalysisError("truth sidecar path/hash closure drift")
    ...
    if actual != expected or str(receipt.get("truth_sidecar_sha256", "")).lower() != actual or str(score.get("truth_sidecar_sha256", "")).lower() != actual:
        raise D92ParetoDistillHard11AnalysisError("truth sidecar hash binding drift")
    return actual
...
def compute_old_balanced_accuracy(by_tx: Mapping[str, Any]) -> float:
    ...
    if len(values) != 6:
        raise D92ParetoDistillHard11AnalysisError(f"expected six old classes, got {len(values)}")
    return _mean(values)
...
def compute_score_metrics(score: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(by_tx, Mapping):
        raise D92ParetoDistillHard11AnalysisError("score by_tx surface missing")
    ...
    if len(old_acc) != 6:
        raise D92ParetoDistillHard11AnalysisError("old class closure drift")
...
def _fit_resource(job_root: Path, k_shot: int, *, baseline: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    if not path.is_file() or path.is_symlink():
        raise D92ParetoDistillHard11AnalysisError("after fit audit missing")
    try:
        rows = json.loads(path.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise D92ParetoDistillHard11AnalysisError("after fit audit invalid") from error
    ...
    for row in rows:
        if not isinstance(row, Mapping):
            raise D92ParetoDistillHard11AnalysisError("after fit audit row invalid")
        ...
        if int(k_shot) > 2:
            ...
            for field, expected_count in required_counts.items():
                if row.get(f"d92_e0d_pareto_distill_{field}") != expected_count:
                    raise D92ParetoDistillHard11AnalysisError(f"{field} receipt drift")
            if row.get("d92_e0d_pareto_distill_deployed_support_constraints_pass") is not True or row.get("d92_e0d_pareto_distill_deployed_full_head_byte_exact") is not False:
                raise D92ParetoDistillHard11AnalysisError("deployed support/head receipt drift")
            delta = row.get("d92_e0d_pareto_distill_persistent_state_bytes_delta")
            if isinstance(delta, bool) or not isinstance(delta, (int, float)) or not math.isfinite(float(delta)) or float(delta) != 0:
                raise D92ParetoDistillHard11AnalysisError("persistent state delta receipt drift")
            state_deltas.add(float(delta))
            for aliases, target in ((("support_optimization_macs_upper_bound", "support_macs", "support_macs_upper_bound"), support_macs), (("support_transient_bytes_upper_bound", "support_transient_bytes", "support_optimization_transient_bytes_upper_bound"), support_transient)):
                values = [row.get(f"d92_e0d_pareto_distill_{field}") for field in aliases if f"d92_e0d_pareto_distill_{field}" in row]
                value = values[0] if values else None
                if isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)) or float(value) < 0:
                    raise D92ParetoDistillHard11AnalysisError(f"{aliases[0]} receipt drift")
...
def _scenario_closure(score: Mapping[str, Any], label: str) -> dict[str, Mapping[str, Any]]:
    ...
    if not isinstance(before_scene, Mapping) or not isinstance(after_scene, Mapping) or set(before_scene) != set(SCENES) or set(after_scene) != set(SCENES):
        raise D92ParetoDistillHard11AnalysisError(f"{label} 3-scene closure drift")
    for scene in SCENES:
        if not isinstance(before_scene[scene], Mapping) or not isinstance(after_scene[scene], Mapping):
            raise D92ParetoDistillHard11AnalysisError(f"{label} scene row invalid: {scene}")
...
def analyze_d92_pareto_distill_hard11(matrix_manifest_path: str | Path, *, run_root: str | Path | None, method_lock_path: str | Path, baseline_paired_rows_path: str | Path = HISTORICAL_BASELINE_PATH, per_old_class_rows_path: str | Path = HISTORICAL_PER_OLD_CLASS_PATH, truth_sidecar_root: str | Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise D92ParetoDistillHard11AnalysisError("matrix/method lock drift") from error
    ...
    if (root / "SYSTEMIC_TECHNICAL_FAILURE_STOP.json").exists():
        raise D92ParetoDistillHard11AnalysisError("systemic stop marker exists")
    ...
    if str(baseline_path).replace("\\", "/").lower() != HISTORICAL_BASELINE_PATH.lower() or str(per_old_path).replace("\\", "/").lower() != HISTORICAL_PER_OLD_CLASS_PATH.lower():
        raise D92ParetoDistillHard11AnalysisError("historical baseline path drift")
    ...
    if len(baseline_by_key) != len(baseline_rows):
        raise D92ParetoDistillHard11AnalysisError("historical baseline key collision")
    ...
    for job in manifest["jobs"]:
        ...
        if key not in baseline_by_key or str(job["outer_key"]) not in raw_specs:
            raise D92ParetoDistillHard11AnalysisError("job/baseline identity mismatch")
        ...
        if _sha256(raw_path) != str(raw_spec["sha256"]).lower():
            raise D92ParetoDistillHard11AnalysisError("E0 raw score SHA drift")
        ...
        if score.get("candidate") != CANDIDATE_ID or score.get("query_truth_fed_back_to_predictor") is not False or score.get("query_truth_joined_only_after_immutable_predictions") is not True or score.get("before_prediction_sha256") != receipt.get("before_prediction_sha256") or score.get("after_prediction_sha256") != receipt.get("after_prediction_sha256"):
            raise D92ParetoDistillHard11AnalysisError("score binding drift")
        ...
        if set(candidate["old_class_accuracy"]) != set(historical_join) or set(candidate["old_class_accuracy"]) != set(baseline["old_class_accuracy"]):
            raise D92ParetoDistillHard11AnalysisError("candidate/E0 six-old TX closure drift")
    ...
    if len(paired_rows) != 11 or len(per_old_rows) != 66 or len(scenario_rows) != 33:
        raise D92ParetoDistillHard11AnalysisError("Hard11 result row closure drift")
