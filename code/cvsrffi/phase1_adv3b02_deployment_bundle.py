...
def _validate_component_from_opened(
    *, manifest: Mapping[str, Any], manifest_raw: bytes, sha_raw: bytes,
    npz_sha256: str, arrays: Mapping[str, np.ndarray], bindings: Mapping[str, str]
) -> CenterLowRankPrototypeComponent:
...
def load_formal_adv3b02_deployment_bundle(
    ...
    *,
    ...
    actual_npz_members = _zip_members_from_handle(npz_handle, context="v2 component")
    if set(actual_npz_members) != ALLOWED_NPZ_MEMBERS:
        raise ADV3B02DeploymentBundleError("v2 NPZ ZIP member allowlist drift")
