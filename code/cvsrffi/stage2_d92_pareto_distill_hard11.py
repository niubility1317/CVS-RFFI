"""Frozen D92 ParetoDistill Hard11 mechanical matrix.

This module owns only the immutable single-arm matrix identity and package
joins. The ParetoDistill scientific implementation is supplied by the Task1 arm;
...
ARM_ID = "E0_FULL_BLOCK_PARETO_DISTILL"
CANDIDATE_ID = "d92_e0_full_block_pareto_distill"
ARM_ORDER = (ARM_ID,)
...
class D92ParetoDistillHard11Error(ValueError):
    """Raised when frozen ParetoDistill Hard11 identity or closure drifts."""


D92ParetoDistillHard11MatrixError = D92ParetoDistillHard11Error
...
def _pure_path(value: Any) -> PurePath:
    if not isinstance(value, str) or not value or "\x00" in value:
        raise D92ParetoDistillHard11Error("path identity drift")
...
def _path_matches(actual: Any, root: Any, *parts: str) -> bool:
    ...
    except D92ParetoDistillHard11Error:
        return False
...
def _parse_outer(key: str) -> tuple[str, int, int, int]:
    ...
    if match is None:
        raise D92ParetoDistillHard11Error(f"invalid outer key: {key}")
...
def _package_layout(source_job_root: PurePath, *, require_files: bool) -> dict[str, Any]:
    ...
    for name, (pkg, seal) in _PACKAGE_LAYOUT.items():
        ...
        if require_files and (not Path(str(pkg_path)).is_dir() or not Path(str(seal_path)).is_file()):
            raise D92ParetoDistillHard11Error(f"sealed source package missing: {name}")
...
def _truth_sidecar_sha256(source_job_root: PurePath, *, require_files: bool) -> str:
    ...
    if require_files:
        if not truth_path.is_file() or truth_path.is_symlink():
            raise D92ParetoDistillHard11Error("truth sidecar missing")
...
def canonical_selection_sha256() -> str:
    payload = {"schema": "cvs.phase2.d92_pareto_distill_hard11.selection.v1", "selection_id": "D92-E0-FULL-BLOCK-PARETO_DISTILL-Hard11-v1", "protocol_schema": "p2_min_v1", "claim_scope": CLAIM_SCOPE, "order": "explicit_pre_registered_performance_then_k1_liveness", "outer_rows": [dict(row) for row in HARD11_ROWS], "coverage": {"outer_count": 11, "performance_outer_count": 10, "liveness_outer_count": 1, "scene_count": 3, "scene_row_count": 33, "shard_count": 8}}
...
def validate_method_lock(lock: Mapping[str, Any]) -> dict[str, Any]:
    if not isinstance(lock, Mapping) or _canonical_bytes(lock) != _canonical_bytes(_expected_lock()):
        raise D92ParetoDistillHard11Error("ParetoDistill Hard11 method lock identity drift")
    return dict(lock)
...
def validate_hard11_manifest(manifest: Mapping[str, Any], *, expected_method_lock_sha256: str | None = None, require_package_hashes: bool = False) -> dict[str, Any]:
    ...
    if not isinstance(manifest, Mapping) or set(manifest) != required:
        raise D92ParetoDistillHard11Error("manifest allowed-key drift")
    expected = {"schema": "cvs.phase2.d92_pareto_distill_hard11.matrix.v1", "status": "FROZEN_DEVELOPMENT_MATRIX", "claim_scope": CLAIM_SCOPE, "protocol_schema": "p2_min_v1", "selection_sha256": CANONICAL_SELECTION_SHA256, "context_sha256": CONTEXT_SHA256, "shard_count": 8, "outer_count": 11, "performance_outer_count": 10, "liveness_outer_count": 1, "job_count": 11, "scene_count": 3, "scene_arm_count": 33, "primary_arm": ARM_ID, "smoke_outer_key": SMOKE_OUTER_KEY, "liveness_outer_key": LIVENESS_OUTER_KEY}
    if any(manifest.get(key) != value for key, value in expected.items()) or manifest.get("arms") != [ARM_ID] or manifest.get("candidate_ids") != ARM_CANDIDATE_IDS or manifest.get("arm_roles") != ARM_ROLES:
        raise D92ParetoDistillHard11Error("manifest identity/count drift")
    ...
    if not isinstance(method_sha, str) or not re.fullmatch(r"[0-9a-f]{64}", method_sha) or (expected_method_lock_sha256 and method_sha.lower() != expected_method_lock_sha256.lower()):
        raise D92ParetoDistillHard11Error("method-lock SHA drift")
    ...
    if not _path_matches(manifest["source_d92_output_root"], SOURCE_D92_OUTPUT_ROOT) or not _path_matches(manifest["ground_component_dir"], GROUND_COMPONENT_DIR) or not _path_matches(manifest["ground_manifest_path"], GROUND_MANIFEST_PATH) or manifest.get("ground_manifest_sha256") != GROUND_MANIFEST_SHA256:
        raise D92ParetoDistillHard11Error("source identity drift")
    rows = _expected_rows()
    if manifest.get("selected_rows") != rows or manifest.get("coverage") != _coverage(rows):
        raise D92ParetoDistillHard11Error("selected-row/coverage drift")
    jobs = manifest.get("jobs")
    if not isinstance(jobs, list) or len(jobs) != 11:
        raise D92ParetoDistillHard11Error("job-count drift")
    seen = set()
    ...
    for index, row in enumerate(rows):
        ...
        if not isinstance(job, Mapping) or set(job) != job_keys or any(job.get(k) != v for k, v in expected_job.items()):
            raise D92ParetoDistillHard11Error("canonical job identity drift")
        if not _path_matches(job.get("output_root"), manifest["output_root"], "jobs", row["outer_key"], ARM_ID) or not _path_matches(job.get("source_job_root"), manifest["source_d92_output_root"], "jobs", row["outer_key"]):
            raise D92ParetoDistillHard11Error("job path drift")
        ...
        if _pure_path(job.get("truth_sidecar")) != expected_truth:
            raise D92ParetoDistillHard11Error("truth sidecar path drift")
        if not re.fullmatch(r"[0-9a-f]{64}", str(job.get("truth_sidecar_sha256"))):
            raise D92ParetoDistillHard11Error("truth sidecar SHA drift")
        if not isinstance(job.get("packages"), Mapping) or set(job["packages"]) != {"before_enrollment", "before_apply", "after_enrollment", "after_apply"}:
            raise D92ParetoDistillHard11Error("package identity drift")
        ...
        for name, pkg in job["packages"].items():
            if set(pkg) != {"package_root", "detached_seal_path", "expected_seal_sha256"}:
                raise D92ParetoDistillHard11Error("package key drift")
            ...
            if _pure_path(pkg["package_root"]) != source_job_root.joinpath(*expected_pkg) or _pure_path(pkg["detached_seal_path"]) != source_job_root.joinpath(*expected_seal):
                raise D92ParetoDistillHard11Error("package path drift")
            if require_package_hashes and not isinstance(pkg.get("expected_seal_sha256"), str):
                raise D92ParetoDistillHard11Error("package SHA missing")
            if require_package_hashes and not re.fullmatch(r"[0-9a-f]{64}", str(pkg.get("expected_seal_sha256"))):
                raise D92ParetoDistillHard11Error("package SHA format drift")
        if job["job_id"] in seen:
            raise D92ParetoDistillHard11Error("duplicate job identity")
        seen.add(job["job_id"])
...
def build_hard11_manifest(*, context_path: str | Path, method_lock_path: str | Path, output_root: str | Path, require_package_files: bool = True) -> dict[str, Any]:
    ...
    if _sha256_file(context_file) != CONTEXT_SHA256:
        raise D92ParetoDistillHard11Error("target125 context SHA drift")
    ...
    if context.get("protocol_schema") != "p2_min_v1" or not isinstance(context.get("rows"), list) or len(context["rows"]) != 125:
        raise D92ParetoDistillHard11Error("target125 context identity drift")
...
build_pareto_distill_hard11_manifest = build_hard11_manifest
build_hard11_matrix_manifest = build_hard11_manifest
validate_manifest = validate_hard11_manifest

__all__ = ["ARM_ID", "ARM_ORDER", "ARM_CANDIDATE_IDS", "ARM_ROLES", "CANDIDATE_ID", "CANONICAL_SELECTION_SHA256", "CLAIM_SCOPE", "CONTEXT_SHA256", "DEPLOYMENT_POLICY", "D92ParetoDistillHard11Error", "D92ParetoDistillHard11MatrixError", "FIT_GATE", "HARD11_ROWS", "HARD11_V1_ROWS", "HISTORICAL_BASELINE_PATH", "HISTORICAL_BASELINE_SHA256", "HISTORICAL_PER_OLD_CLASS_PATH", "HISTORICAL_PER_OLD_CLASS_SHA256", "LIVENESS_OUTER_KEY", "PRIMARY_ARM", "RAW_SCORE_ROOT", "SCENES", "SHARD_COUNT", "SMOKE_OUTER_KEY", "SOURCE_D92_OUTPUT_ROOT", "STRICT_PARETO_THRESHOLDS", "RESOURCE_GATE", "build_hard11_manifest", "build_pareto_distill_hard11_manifest", "build_hard11_matrix_manifest", "canonical_selection_sha256", "validate_hard11_manifest", "validate_manifest", "validate_method_lock"]
