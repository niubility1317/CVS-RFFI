...
RUN_ID="${RUN_ID:-phase2_r7_qknn8_collab_20260706}"
RUNS_ROOT="${RUNS_ROOT:-${ROOT}/runs/${RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${ROOT}/logs/${RUN_ID}}"
PHASE1_RUN_ROOT="${PHASE1_RUN_ROOT:-${ROOT}/runs/phase1_epoc_r7_floor_protected_shell_20260706}"
...
case_config() {
  case "$1" in
    FLOOR)
      echo "EPOC_R7_FLOOR_LOCKED_SHELL|0|706701|${PHASE1_RUN_ROOT}/EPOC_R7_FLOOR_LOCKED_SHELL/best_joint_safe_ssdg.pth"
      ;;
    BALANCED)
      echo "EPOC_R7_BALANCED_LOW_DENSITY|1|706711|${PHASE1_RUN_ROOT}/EPOC_R7_BALANCED_LOW_DENSITY/best_joint_safe_ssdg.pth"
      ;;
...
cases=()
case "${ONLY}" in
  all) cases=(FLOOR BALANCED) ;;
  FLOOR|BALANCED) cases=("${ONLY}") ;;
  *) echo "[ERROR] --only must be all, FLOOR, or BALANCED" >&2; exit 2 ;;
esac

echo "[R7-QKNN8-COLLAB] run_id=${RUN_ID} dry_run=${DRY_RUN} only=${ONLY}"
echo "[R7-QKNN8-COLLAB] target_receivers=${TARGET_RECEIVERS}"
echo "[R7-QKNN8-COLLAB] target_new_tx_ids=${TARGET_NEW_TX_IDS}"
echo "[R7-QKNN8-COLLAB] unknown_tx_ids=${UNKNOWN_TX_IDS}"
echo "[R7-QKNN8-COLLAB] proxy_unknown_tx_ids=${PROXY_UNKNOWN_TX_IDS}"
echo "[R7-QKNN8-COLLAB] collab_counts=all collab_group_policy=${COLLAB_GROUP_POLICY} partial_collab_min_receivers=${PARTIAL_COLLAB_MIN_RECEIVERS}"
echo "[R7-QKNN8-COLLAB] qknn_k=${QKNN_K} k_shot=${K_SHOT} query_per_class=${QUERY_PER_CLASS}"
echo "[R7-QKNN8-COLLAB] protocol=Stage2-C unknown_query_eval_only=true ground_training_unknown_seen=false"
echo "[R7-QKNN8-COLLAB] target_channel_view=leo_clear_weak,leo_low_elev_weak,leo_rain_weak"
...
launch_case() {
  ...
  echo "[R7-QKNN8-COLLAB-CASE] case=${case_id} gpu=${gpu} ckpt=${ckpt}"
  printf "[R7-QKNN8-COLLAB-EXPORT-CMD] "
  printf "%q " "${export_cmd[@]}"
  printf "\n"
  printf "[R7-QKNN8-COLLAB-EVAL-CMD] "
  printf "%q " "${eval_cmd[@]}"
  printf "\n"
  ...
  (
    ...
    "${export_cmd[@]}"
    "${eval_cmd[@]}"
    echo "[R7-QKNN8-COLLAB-DONE] case=${case_id} output_json=${output_json}"
  ) > "${log_path}" 2>&1 &
  echo "[R7-QKNN8-COLLAB-LAUNCHED] case=${case_id} pid=$! gpu=${gpu} log=${log_path} out_dir=${out_dir}"
}
