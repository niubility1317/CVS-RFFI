...
RUN_ID="${RUN_ID:-phase1_epoc_r9_source_anchor_20260706}"
...
validate_source_wisig_pkl() {
  ...
  case "${lower}" in
    *manytx.pkl*|*manyrx.pkl*|*singleday.pkl*|*new_wisig*|*target*|*unknown*)
      echo "[ERROR] refusing non-source Phase1 WISIG_PKL for R9 source-anchor: ${pkl_path}" >&2
      exit 4
      ;;
  esac
  if [[ "${lower}" != *manysig.pkl ]]; then
    echo "[ERROR] refusing non-source Phase1 WISIG_PKL for R9 source-anchor: expected ManySig.pkl, got ${pkl_path}" >&2
    exit 4
  fi
}
...
wait_for_gpu_slot() {
  ...
  while true; do
    ...
    echo "[EPOC-R9-WAIT] gpu=${gpu} active=${active} max=${MAX_ACTIVE_PER_GPU}"
    sleep 60
  done
}
...
launch_candidate() {
  ...
  local proxy_start="${12}"
  local proxy_virtual_count="${13}"
  local proxy_energy_margin="${14}"
  local source_radius_cap="${15}"
  local phase2_radius_cap="${16}"
  local lr="${17}"

  local out_dir="${RUNS_ROOT}/${cid}"
  local log_path="${LOG_ROOT}/${cid}.out"
  echo "[EPOC-R9-CANDIDATE] id=${cid} algorithm=ADV3B02_SOURCE_ANCHOR route=source_only_teacher_anchor_stable_feature_repair base=ADV3B02_CORE90_SOFT_E200 teacher_ckpt=ADV3B02_CORE90_SOFT_E200 phase1_dataset=ManySig_only target_visibility=source_only_ground_training_no_target_receiver real_unknown_classes_in_training=0 target_receiver_samples_in_training=0 target_unknown_training_count=0 manytx_in_training=0 proxy_unknown_real_tx_calibration=0 virtual_unknown_only=1 threshold_selection_label_scope=support_or_source_old_only stage2_unknown_query_eval_only=1 qknn8_same_row_eval_required=1 stage2_success_claim=0 deployment_success_claim=0"
  CMD=(env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" "${PYTHON}" -u "${ROOT}/code/SSDG/train_ssdg.py"
    ...
    --teacher_distill_warmup_epochs 28
    ...
    --proxy_unknown_warmup_epochs 42
    ...
    --proxy_unknown_placeholder_weight 0.08
    --proxy_unknown_virtual_detach true
    --proxy_unknown_vacuum_weight 0.12
    --proxy_unknown_vacuum_width_deg 9
    --proxy_unknown_vacuum_hard_k 4
    --proxy_unknown_vacuum_radius_deg 28
    --proxy_unknown_component_radius_mode core_quantile
    --proxy_unknown_component_radius_quantile 0.62
    --proxy_unknown_vaccept_weight 0.04
    --proxy_unknown_core_accept_weight 0.04
    --proxy_unknown_component_gate_weight 0.04
    --proxy_unknown_tail_quarantine_weight 0.10
    --proxy_unknown_source_safe_weight 0.18
    --proxy_unknown_bridge_accept_weight 0.04
    --proxy_unknown_shell_outward_accept_weight 0.06
    --proxy_unknown_low_density_accept_weight 0.06
    --proxy_unknown_energy_margin_quantile_weight 0.10
    --proxy_unknown_radius_budget_weight 0.08
    --proxy_unknown_radius_inter_ratio_weight 0.10
    --proxy_unknown_vaccept_cvar_alpha 0.12
    ...
    --proxy_unknown_energy_margin_target 0.24
    ...
    --soft_unknown_mixup_warmup_epochs 42
    ...
    --soft_unknown_mixup_energy_weight 0.20
    ...
    --source_episode_start_epoch 6
    --source_episode_warmup_epochs 38
    ...
    --source_episode_min_sigma_deg 2.1
    --source_episode_mixup_weight 0.62
  ...
  "${CMD[@]}" > "${log_path}" 2>&1 &
  echo "[EPOC-R9-LAUNCHED] id=${cid} pid=$! gpu=${gpu} log=${log_path}"
...
echo "[EPOC-R9] run_id=${RUN_ID} dry_run=${DRY_RUN} candidates=2 teacher=ADV3B02_CORE90_SOFT_E200 teacher_ckpt=ADV3B02_CORE90_SOFT_E200 phase1_dataset=ManySig_only algorithm=ADV3B02_SOURCE_ANCHOR route=source_only_teacher_anchor_stable_feature_repair real_unknown_classes_in_training=0 target_receiver_samples_in_training=0 target_unknown_training_count=0 manytx_in_training=0 proxy_unknown_real_tx_calibration=0 virtual_unknown_only=1 threshold_selection_label_scope=support_or_source_old_only qknn8_same_row_eval_required=1 stage2_success_claim=0 deployment_success_claim=0 only=${ONLY_CANDIDATES:-ALL}"

maybe_launch_candidate "EPOC_R9_ANCHOR_NOPROXY" 2 706901 2.20 0.90 0.420 0.0000 0.00000 0.0050 0.0100 0.058 120 16 1.50 20 13 0.000050
maybe_launch_candidate "EPOC_R9_GENTLE_VIRTUAL_LATE" 3 706911 2.05 0.82 0.390 0.0020 0.00003 0.0045 0.0090 0.052 90 24 1.55 21 14 0.000048

echo "[EPOC-R9-SUBMIT-COMPLETE] run_id=${RUN_ID}"
