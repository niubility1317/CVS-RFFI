...
RUN_ID="${RUN_ID:-phase1_adv3b02_proxy_unknown_keepold_20260702}"
...
PROXY_UNKNOWN_RXS="${PROXY_UNKNOWN_RXS:-1-1,1-19,14-7,18-2,19-2,2-1,2-19}"
...
FEATURE_DIR="${RUNS_ROOT}/ADV3B02_CORE90_SOFT_E200_PHASE1_PROXY_UNKNOWN"
FEATURE_NPZ="${FEATURE_DIR}/features.npz"

echo "[PHASE1-PROXY-OSR] run_id=${RUN_ID} dry_run=${DRY_RUN} gpu=${GPU}"
echo "[PHASE1-PROXY-OSR] objective=unknown_FAR<=0.05 and old_drop_pp_vs_closed<=3.0"
echo "[PHASE1-PROXY-OSR] proxy_unknown_tx_ids=${PROXY_UNKNOWN_TX_IDS}"
if [[ "${DRY_RUN}" != "1" ]]; then
...
printf "[PHASE1-PROXY-EXPORT-CMD] "; printf "%q " "${EXPORT_CMD[@]}"; printf "\n"
if [[ "${DRY_RUN}" != "1" ]]; then
...
for row in "${POLICIES[@]}"; do
  read -r NAME POLICY SRC_Q PROXY_Q <<<"${row}"
  ...
  printf "[PHASE1-PROXY-EVAL-CMD] policy=%s " "${NAME}"; printf "%q " "${EVAL_CMD[@]}"; printf "\n"
  if [[ "${DRY_RUN}" != "1" ]]; then
    "${EVAL_CMD[@]}" > "${LOG_ROOT}/${NAME}.out" 2>&1
  fi
done

SUMMARY_CSV="${RUNS_ROOT}/summary_phase1_proxy_unknown_keepold.csv"
if [[ "${DRY_RUN}" != "1" ]]; then
...
echo "[PHASE1-PROXY-OSR-DONE] run_id=${RUN_ID} summary=${SUMMARY_CSV}"
