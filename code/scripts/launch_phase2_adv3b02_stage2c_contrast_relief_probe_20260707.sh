...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_contrast_relief_probe_20260707}"
...
declare -a PROFILES=(
  "RELIEF_U090_W025_D002_R050 0.90 0.25 0.00 0.02 0.50 0.50 0.50"
  "RELIEF_U090_W025_D005_R050 0.90 0.25 0.00 0.05 0.50 0.50 0.50"
  "RELIEF_U095_W025_D005_R050 0.95 0.25 0.00 0.05 0.50 0.50 0.50"
...
echo "[STAGE2C-CONTRAST-RELIEF] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-CONTRAST-RELIEF] variants=${VARIANTS[*]} k=${K_SHOTS[*]} profiles=${#PROFILES[@]}"
echo "[STAGE2C-CONTRAST-RELIEF] diagnostic_only=true unknown_query_eval_only=true"
...
run_one() {
  ...
  mkdir -p "${out_dir}"
  echo "[STAGE2C-CONTRAST-RELIEF-RUN] variant=${variant} profile=${profile} k=${k} gate=${gate} weight=${contrast_weight} margin=${contrast_margin} delta=${delta_min} scales=${label_scale}/${event_scale}/${component_scale}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
...
for variant in "${VARIANTS[@]}"; do
  for row in "${PROFILES[@]}"; do
    read -r profile gate contrast_weight contrast_margin delta_min label_scale event_scale component_scale <<<"${row}"
    for k in "${K_SHOTS[@]}"; do
      run_one "${variant}" "${profile}" "${gate}" "${contrast_weight}" "${contrast_margin}" "${delta_min}" "${label_scale}" "${event_scale}" "${component_scale}" "${k}"
    done
  done
done

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-CONTRAST-RELIEF-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_contrast_relief_probe_summary.json" <<'PY'
import csv
import json
import sys
...
for path in sorted(case_dir.glob("*/*/stage2c_qknn_*_k*.json")):
    ...
    for row in payload.get("summary_rows", []):
        ...
        item["seen_new_contrast_risk_relief_enabled"] = payload.get("seen_new_contrast_risk_relief_enabled", False)
...
echo "[STAGE2C-CONTRAST-RELIEF-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
