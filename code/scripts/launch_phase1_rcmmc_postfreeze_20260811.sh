...
# Invoke explicitly as: bash scripts/launch_phase1_rcmmc_postfreeze_20260811.sh [--dry-run]
# Frozen P1-RCMMC postfreeze closure: 12 clean exports, 12 source-LEO/export
# bindings, 12 fixed-400 proxy bindings, then six same-fold C/G pair closures.
# This launcher is mechanical; RCMMC receipt and pair semantics are validated by
# the RCMMC exporter/evaluator, not selected here.
POSTFREEZE_RUN_ID="${POSTFREEZE_RUN_ID:-phase1_rcmmc_postfreeze_20260811_v1}"
...
TRAIN_RUN_ROOT="${TRAIN_RUN_ROOT:-${PROJECT_ROOT}/runs/phase1_rcmmc12_20260811_v1}"
POSTFREEZE_ROOT="${POSTFREEZE_ROOT:-${PROJECT_ROOT}/runs/${POSTFREEZE_RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/${POSTFREEZE_RUN_ID}}"
RCMMC_CLEAN_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_rcmmc_features.py"
RCMMC_LEO_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_rcmmc_leo_features.py"
LOGITS_REJECT_SCRIPT="${CODE_ROOT}/scripts/eval_phase1_logits_open_set_reject.py"
PAIR_EVAL_SCRIPT="${CODE_ROOT}/evaluate_phase1_rcmmc_postfreeze_pair.py"
DRY_RUN="${DRY_RUN:-0}"
...
[[ "${POSTFREEZE_RUN_ID}" == "phase1_rcmmc_postfreeze_20260811_v1" ]] || {
  echo "POSTFREEZE_RUN_ID is frozen to phase1_rcmmc_postfreeze_20260811_v1" >&2
  exit 3
}
[[ "$(basename "${TRAIN_RUN_ROOT}")" == "phase1_rcmmc12_20260811_v1" ]] || {
  echo "TRAIN_RUN_ROOT leaf must be phase1_rcmmc12_20260811_v1" >&2
  exit 3
}
[[ "$(basename "${POSTFREEZE_ROOT}")" == "phase1_rcmmc_postfreeze_20260811_v1" ]] || {
  echo "POSTFREEZE_ROOT leaf must bind the frozen RCMMC matrix" >&2
  exit 3
}
...
# Keep dry-run usable before Terra lands the three RCMMC postfreeze modules.
if [[ "${DRY_RUN}" == "0" ]]; then
  for required in "${RCMMC_CLEAN_EXPORT_SCRIPT}" "${RCMMC_LEO_EXPORT_SCRIPT}" "${LOGITS_REJECT_SCRIPT}" "${PAIR_EVAL_SCRIPT}"; do
    [[ -f "${required}" ]] || {
      echo "missing RCMMC postfreeze script: ${required}" >&2
      exit 2
    }
  done
fi
...
run_candidate() {
  ...
  local candidate="F${fold}${arm}_RCMMC12"
  ...
  # The generic source exporter keeps the established artifact leaf; the
  # manifest itself must carry only the RCMMC identity and is checked below.
  ...
  local -a clean_command=(
    "${PYTHON}" -u "${RCMMC_CLEAN_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}" --wisig_pkl "${WISIG_PKL}" --out_npz "${clean_npz}"
    --source_tx_ids "${train_tx}" --known_validation_tx_ids "${known_tx}"
    --proxy_unknown_tx_ids "${proxy_tx}" --expected-wisig-sha256 "${WISIG_SHA256}"
    --batch_size 256 --device cuda:0
  )
  local -a leo_command=(
    "${PYTHON}" -u "${RCMMC_LEO_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}" --wisig-pkl "${WISIG_PKL}" --out-npz "${leo_npz}"
    --binding-json "${leo_binding}" --training-run-root "${TRAIN_RUN_ROOT}"
    --postfreeze-output-root "${POSTFREEZE_ROOT}" --candidate-id "${candidate}"
    --fold-index "${fold}" --arm "${arm}" --feature-name z_id --source_only_export
    --source-tx-ids "${train_tx}" --source-days "${SOURCE_DAYS}" --source-rxs "${SOURCE_RXS}"
    --source-channel-view satellite --source-sat-scenarios "${SAT_SCENARIOS}"
  ...
  local -a proxy_command=(
    "${PYTHON}" -u "${LOGITS_REJECT_SCRIPT}" --feature_npz "${clean_npz}"
    --source_tx_ids "${train_tx}" --known_query_roles source_validation_known
    --unknown_query_roles proxy_unknown --calibration_roles source_validation_known
    --unknown_far_target 0.05 --output_json "${proxy_metrics}" --score_table_csv "${proxy_scores}"
  )
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command RCMMC_CLEAN_EXPORT "${gpu}" "${clean_command[@]}"
    print_command RCMMC_LEO_EXPORT_AND_BIND "${gpu}" "${leo_command[@]}"
    print_command FROZEN_LOGITS_PROXY_BINDING "${gpu}" "${proxy_command[@]}"
    return 0
  fi
  [[ -f "${checkpoint}" ]] || { echo "missing RCMMC final-only checkpoint: ${checkpoint}" >&2; return 2; }
...
run_pair() {
  local fold="$1"; local fold_index=$((fold - 1))
  local train_tx="${FOLD_TRAIN_TX[fold_index]}"
  local c_candidate="F${fold}C_RCMMC12"; local g_candidate="F${fold}G_RCMMC12"
  ...
  local -a pair_command=(
    ...
    --candidate-pair "F${fold}_C_vs_G" --fold-index "${fold}"
    --postfreeze-matrix-id "${POSTFREEZE_RUN_ID}" --postfreeze-output-root "${POSTFREEZE_ROOT}"
    --training-run-root "${TRAIN_RUN_ROOT}" --source-tx-ids "${train_tx}"
    --expected-scenarios "${SAT_SCENARIOS}" --expected-source-days "${SOURCE_DAYS}"
    --expected-source-rxs "${SOURCE_RXS}" --source-sat-seed "${SOURCE_SAT_SEED}"
    --expected-source-count 1600 --expected-proxy-count 400 --output-metrics-json "${output_json}"
  )
  if [[ "${fold}" == "6" ]]; then
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command RCMMC_PAIR_SCORE CPU "${pair_command[@]}"
    return 0
  fi
  [[ -f "${c_checkpoint}" && -f "${g_checkpoint}" ]] || {
    echo "missing final-only C/G checkpoint for fold ${fold}" >&2
    return 2
  }
