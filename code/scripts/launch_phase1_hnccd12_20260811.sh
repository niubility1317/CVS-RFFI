...
# Invoke explicitly as: bash scripts/launch_phase1_hnccd12_20260811.sh [--dry-run]
# Frozen six-fold P1-HNCCD C/G continuation matrix. C is the exact GeoSat-C
# continuation control; G adds only the frozen source-L HNCCD auxiliary.
# This launcher is mechanical and passes only parser-backed CLI arguments.
RUN_ID="${RUN_ID:-phase1_hnccd12_20260811_v1}"
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in
    C) arm_args=(--phase1_hnccd_enabled false --lambda_hnccd 0) ;;
    G) arm_args=(--phase1_hnccd_enabled true --lambda_hnccd 0.02) ;;
