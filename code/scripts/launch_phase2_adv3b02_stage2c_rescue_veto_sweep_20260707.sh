...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_rescue_veto_sweep_20260707}"
...
declare -a PROFILES=(
  "CONF_VETO_E90L90S90_M2"
  "CONF_VETO_E92L92S92_M2"
  "CONF_VETO_E95_ANY_M1"
  "CONF_VETO_EVENT_E92_DEFER"
)

mkdir -p "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}"
echo "[STAGE2C-RESCUE-VETO] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-RESCUE-VETO] profiles=${PROFILES[*]} k=${K_SHOTS[*]}"
echo "[STAGE2C-RESCUE-VETO] diagnostic_only=true unknown_query_eval_only=true"
  ...
  case "${profile}" in
    CONF_VETO_E90L90S90_M2)
      PROFILE_ARGS+=(
        ...
        --rescue_unknown_veto_component_agreement 1.0
    ...
    CONF_VETO_E92L92S92_M2)
      PROFILE_ARGS+=(
        --rescue_unknown_veto_event_risk 0.92
        --rescue_unknown_veto_label_risk 0.92
        --rescue_unknown_veto_shell_risk 0.92
        --rescue_unknown_veto_component_agreement 1.0
        --rescue_unknown_veto_min_sources 2
        --rescue_unknown_veto_action unknown_reject
      )
    ...
    CONF_VETO_E95_ANY_M1)
      PROFILE_ARGS+=(
        --rescue_unknown_veto_event_risk 0.95
        --rescue_unknown_veto_label_risk 0.95
        --rescue_unknown_veto_shell_risk 0.95
        --rescue_unknown_veto_component_agreement 1.0
        --rescue_unknown_veto_min_sources 1
        --rescue_unknown_veto_action unknown_reject
      )
      ;;
    CONF_VETO_EVENT_E92_DEFER)
      PROFILE_ARGS+=(
        --rescue_unknown_veto_event_risk 0.92
        --rescue_unknown_veto_label_risk 1.0
        --rescue_unknown_veto_shell_risk 1.0
        --rescue_unknown_veto_component_agreement 1.0
        --rescue_unknown_veto_min_sources 1
        --rescue_unknown_veto_action defer
      )
      ;;
    *)
...
run_one() {
  ...
  echo "[STAGE2C-RESCUE-VETO-RUN] variant=${variant} profile=${profile} k=${k} feature=${feature_npz}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
  EXTRA_ARGS=()
...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-RESCUE-VETO-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_rescue_veto_sweep_summary.json" <<'PY'
import csv
import json
import sys
...
out = {"rows": rows, "best_balanced": rows[:10], "best_feasible": feasible[:10]}
...
print(json.dumps({"best_balanced": rows[:5], "best_feasible": feasible[:5], "out": str(out_path), "csv": str(csv_path)}, ensure_ascii=False, indent=2))
PY

echo "[STAGE2C-RESCUE-VETO-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
