...
FACTOR_C: Mapping[str, Any] = {
    ...
    "proxy_weight": 0.60,
    "bridge_weight": 0.60,
    "low_density_weight": 0.30,
    "tail_weight": 0.25,
    "overflow_weight": 0.30,
    "ratio_weight": 0.50,
    "source_local_inter": 0.30,
    "source_local_overlap": 0.30,
...
def validate_matrix(rows: Sequence[Mapping[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        if bool(cfg["effective_negative_grad_guard"]) != bool(factor_a or factor_c):
            raise ValueError(f"effective-gradient mismatch: {row['candidate_id']}")
...
def build_command(
    ...
    *,
    ...
    replacements = {
        ...
        "--sat_view_schedule": (
            "1@0.55:clear_leo;"
            "21@0.75:clear_leo,low_elev_leo,rain_leo;"
            "61@0.90:clear_leo,low_elev_leo,rain_leo"
        ),
        ...
        "--u_route_use_teacher_weak": "true",
        "--u_route_use_reference_bank": str(bool(cfg["reference_bank"])).lower(),
        "--u_quarantine_include_sat_view": "false",
        "--u_tri_quota_routing": str(bool(row["factors"]["A"])).lower(),
        "--u_tri_core_quota": 0.25,
        "--u_tri_ambiguous_quota": 0.25,
        "--u_tri_quota_require_pseudo_mask": "true",
        "--u_direct_include_ambiguous": str(bool(row["factors"]["A"])).lower(),
        "--lambda_u_quarantine_accept": 0.0,
        "--lambda_u_direct_metric_accept": cfg["u_dm_lambda"],
        "--u_direct_metric_min_selected": 4,
        ...
        "--direct_metric_known_accept_target": 0.60,
        "--direct_metric_known_tpr_target": 0.85,
        "--direct_metric_positive_first": str(bool(cfg["positive_first"])).lower(),
        "--direct_metric_negative_start_tpr": 0.80,
        "--direct_metric_negative_full_tpr": 0.87,
        ...
        "--lambda_direct_metric_accept": cfg["dm_lambda"],
        "--lambda_proxy_unknown": 0.0,
        "--direct_metric_source_overflow_weight": cfg["source_overflow_weight"],
        "--direct_metric_proxy_vaccept_weight": cfg["proxy_weight"],
        "--direct_metric_bridge_accept_weight": cfg["bridge_weight"],
        "--direct_metric_low_density_accept_weight": cfg["low_density_weight"],
        "--direct_metric_tail_accept_weight": cfg["tail_weight"],
        "--direct_metric_overflow_accept_weight": cfg["overflow_weight"],
        "--direct_metric_radius_inter_ratio_weight": cfg["ratio_weight"],
        ...
        "--source_episode_local_inter_margin_deg": 18.0,
        "--source_episode_local_overlap_weight": cfg["source_local_overlap"],
        "--source_episode_local_overlap_margin_deg": 2.0,
        ...
        "--direct_metric_component_inter_margin_deg": 18.0,
        "--direct_metric_component_overlap_weight": cfg["query_overlap_weight"],
        "--direct_metric_component_overlap_margin_deg": 2.0,
    ...
    payload.update(
        {
            ...
            "satellite_train_protocol": "legacy_satellite_physics_holdout_v1",
            "satellite_train_eval_disjoint": True,
