#!/usr/bin/env python3
"""Analyze one immutable D92 ParetoDistill Hard11 artifact set."""
...
from cvsrffi.stage2_d92_pareto_distill_hard11_analysis import analyze_d92_pareto_distill_hard11  # noqa: E402
...
def _markdown(result: Mapping[str, Any]) -> str:
    ...
    lines = ["# D92 E0_FULL_BLOCK_PARETO_DISTILL Hard11分析", "", f"- 状态：`{result['status']}`", f"- 判定：`{result['verdict']}`", "- 范围：`DEVELOPMENT_ONLY_HARD_SCREEN`；K1仅作liveness，不进入性能均值。", "", "## 八项严格Pareto", "", "|指标|候选均值|相对E0_FULL_ONLY|", "|---|---:|---:|"]
...
def main() -> int:
    ...
    result = analyze_d92_pareto_distill_hard11(manifest, run_root=args.run_root, method_lock_path=args.method_lock, baseline_paired_rows_path=args.baseline_paired_rows, per_old_class_rows_path=args.per_old_class_rows, truth_sidecar_root=args.truth_sidecar_root)
