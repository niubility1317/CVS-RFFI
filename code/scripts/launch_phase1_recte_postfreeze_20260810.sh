...
# Invoke explicitly as: bash scripts/launch_phase1_recte_postfreeze_20260810.sh [--dry-run]
# The RECTE postfreeze contract has exactly 42 operations: 12 clean L/V/proxy
# exports, 12 source-only three-LEO exports plus RECTE bindings, 12 frozen
# logits proxy JSON/CSV bindings, then six CPU-serial C/G pair closures.
POSTFREEZE_RUN_ID="${POSTFREEZE_RUN_ID:-phase1_recte_postfreeze_20260810_v1}"
...
TRAIN_RUN_ROOT="${TRAIN_RUN_ROOT:-${PROJECT_ROOT}/runs/phase1_recte12_20260810_v1}"
POSTFREEZE_ROOT="${POSTFREEZE_ROOT:-${PROJECT_ROOT}/runs/${POSTFREEZE_RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/${POSTFREEZE_RUN_ID}}"
RECTE_CLEAN_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_recte_features.py"
RECTE_LEO_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_recte_leo_features.py"
LOGITS_REJECT_SCRIPT="${CODE_ROOT}/scripts/eval_phase1_logits_open_set_reject.py"
PAIR_EVAL_SCRIPT="${CODE_ROOT}/evaluate_phase1_recte_postfreeze_pair.py"
DRY_RUN="${DRY_RUN:-0}"
...
# The fixed source proxy selection is sealed by the RECTE exporter.
FIXED_PROXY_SELECTION_SEED="7281148"
FIXED_PROXY_MAX_PER_TX="400"
FIXED_PROXY_TOTAL_COUNT="400"
EXPORT_SEED="7281105"
...
[[ "${POSTFREEZE_RUN_ID}" == "phase1_recte_postfreeze_20260810_v1" ]] || {
  echo "POSTFREEZE_RUN_ID is frozen to phase1_recte_postfreeze_20260810_v1" >&2
  exit 3
}
[[ "$(basename "${TRAIN_RUN_ROOT}")" == "phase1_recte12_20260810_v1" ]] || {
  echo "TRAIN_RUN_ROOT leaf must be phase1_recte12_20260810_v1" >&2
  exit 3
}
[[ "$(basename "${POSTFREEZE_ROOT}")" == "phase1_recte_postfreeze_20260810_v1" ]] || {
  echo "POSTFREEZE_ROOT leaf must bind the frozen RECTE matrix" >&2
  exit 3
}
...
for required in "${RECTE_CLEAN_EXPORT_SCRIPT}" "${RECTE_LEO_EXPORT_SCRIPT}" "${LOGITS_REJECT_SCRIPT}" "${PAIR_EVAL_SCRIPT}"; do
  [[ -f "${required}" ]] || { echo "missing RECTE postfreeze script: ${required}" >&2; exit 2; }
done
...
run_candidate() {
  ...
  local candidate="F${fold}${arm}_RECTE12"
  ...
  # Keep the signed ICMT-v2 artifact leaf stable; schema and manifest are
  # RECTE-only and the pair evaluator rejects any ICMT identity.
  ...
  local -a clean_command=(
    "${PYTHON}" -u "${RECTE_CLEAN_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}"
    --wisig_pkl "${WISIG_PKL}"
    --out_npz "${clean_npz}"
  ...
  local -a leo_command=(
    "${PYTHON}" -u "${RECTE_LEO_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}"
    --wisig_pkl "${WISIG_PKL}"
    --out_npz "${leo_npz}"
    --binding-json "${leo_binding}"
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command RECTE_CLEAN_EXPORT "${gpu}" "${clean_command[@]}"
    print_command RECTE_LEO_EXPORT_AND_BIND "${gpu}" "${leo_command[@]}"
    print_command FROZEN_LOGITS_PROXY_BINDING "${gpu}" "${proxy_command[@]}"
    return 0
  fi
  [[ -f "${checkpoint}" ]] || { echo "missing RECTE final-only checkpoint: ${checkpoint}" >&2; return 2; }
...
# Preserve the established frozen 12-arm GPU mapping. Candidate-internal work
# is serial; independent RECTE candidates are concurrent only at this level.
...
run_pair() {
  ...
  local fold_index=$((fold - 1))
  local train_tx="${FOLD_TRAIN_TX[fold_index]}"
  local c_candidate="F${fold}C_RECTE12"
  local g_candidate="F${fold}G_RECTE12"
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command RECTE_PAIR_SCORE CPU "${pair_command[@]}"
    return 0
  fi
  ...
  CUDA_VISIBLE_DEVICES="" PYTHONPATH="${CODE_ROOT}:${PROJECT_ROOT}" "${pair_command[@]}" \
    >"${LOG_ROOT}/F${fold}_C_vs_G_pair.out" 2>&1
}
...
# F6 can aggregate only after F1--F5 immutable pair JSONs exist.
run_pair 1
run_pair 2
run_pair 3
run_pair 4
