...
# Final-only P1-PAMR postfreeze diagnostics.  This launcher never resumes a
# checkpoint or feeds proxy/LEO/held information back into any training choice.
POSTFREEZE_RUN_ID="${POSTFREEZE_RUN_ID:-phase1_pamr_postfreeze_20260809_v1}"
...
EXPORT_SEED="7281105"
MAX_PER_TX="400" # train4=1600; each target-old/proxy singleton TX=400
EXPORT_BATCH="32"
...
[[ "${POSTFREEZE_ROOT}" != "${TRAIN_RUN_ROOT}" ]] || {
  echo "postfreeze root must differ from immutable PAMR training root" >&2
  exit 3
}

# Frozen 4/1/1 source-known roles and C/G mapping.  Every candidate first
# exports clean development, then source-only LEO, then proxy diagnostics.
FOLD_TRAIN_TX=(
...
print_command() {
  ...
  local gpu="$2"
  shift 2
  printf '[DRY-RUN][%s] CUDA_VISIBLE_DEVICES=%q PYTHONPATH=%q' "${stage}" "${gpu}" "${CODE_ROOT}:${PROJECT_ROOT}"
  printf ' %q' "$@"
  printf '\n'
}
...
run_candidate() {
  ...
  local candidate="F${fold}${arm}_PAMR12"
  ...
  # The frozen source-calibrated proxy score is a separate diagnostic artifact;
  # it never feeds the pair evaluator, a checkpoint choice, or a PAMR setting.
...
# At most two candidate pipelines run on any card.  Mapping is exactly the
# already-frozen 12-task PAMR train schedule.
...
run_pair() {
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command PAIR_SCORE "${gpu}" "${pair_command[@]}"
    return 0
  fi
  PYTHONPATH="${CODE_ROOT}:${PROJECT_ROOT}" "${pair_command[@]}" >"${LOG_ROOT}/F${fold}_C_vs_G_pair.out" 2>&1
}
