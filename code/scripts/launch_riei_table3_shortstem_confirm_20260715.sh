...
# RIEI Table III confirmation for the pre-registered short-stem ResNet1D-18
# candidate. All non-architecture settings remain fixed to the best verified
# paper protocol: stable partition, SGD momentum 0, mean losses, no RMS or FN.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="${ROOT:-$(cd "${SCRIPT_DIR}/../.." && pwd)}"
PYTHON_BIN="${PYTHON_BIN:-/home/szu2070436088/.conda/envs/CVS-RFFI/bin/python}"
WISIG_PKL="${WISIG_PKL:-${ROOT}/Dataset_WigSig/ManySig.pkl}"
RUN_ID="${RUN_ID:-paper_repro_riei_table3_shortstem_confirm_seed1337_20260715_043000}"
...
echo "[RIEI-TABLE3-SHORTSTEM-CONFIRM] run_id=${RUN_ID} seed=${SEED} dry_run=${DRY_RUN} rows=${#ROWS[@]}"
echo "[RIEI-TABLE3-SHORTSTEM-CONFIRM] fed_variant=${FED_VARIANT} split=stable_group_seed_shared_train_test_holdout metric=paper_last10 reduction=mean"
...
for i in "${!ROWS[@]}"; do
  ...
  gpu="${GPU_IDS[$((i % 8))]}"
  job_id="riei_shortstem_table3_row${row}_${combo}_seed${SEED}"
  ...
  echo "[JOB] row=${row} variant=${FED_VARIANT} id=${job_id} gpu=${gpu} train=${train_rxs} test=${test_rxs} paper=${paper_mean}+/-${paper_sd}"
  echo "[CMD] $(format_cmd "${cmd[@]}")"
  [[ "${DRY_RUN}" != 1 ]] || continue
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "${row}" "${FED_VARIANT}" "${job_id}" "${gpu}" "${train_rxs}" "${test_rxs}" "${paper_mean}" "${paper_sd}" "${run_dir}" "$(format_cmd "${cmd[@]}")" >> "${RUN_ROOT}/scheduler_manifest.tsv"
...
echo "[RIEI-TABLE3-SHORTSTEM-CONFIRM] submitted dry_run=${DRY_RUN}"
