...
# Fresh-run repair of v3's axis-index/physical-label binding defect.  The
# frozen checkpoints, dataset, matrix, roles and GPU mapping are unchanged.
RUN_ID="phase1_clic_postfreeze_20260812_v4"
...
for arg in "$@"; do case "${arg}" in --dry-run) DRY_RUN=1 ;; *) echo "invalid argument: ${arg}" >&2; exit 2 ;; esac; done
...
[[ ! -e "${RUN_ROOT}" && ! -e "${LOG_ROOT}" ]] || { echo "refusing to overwrite clean v4 run/log root" >&2; exit 3; }
for fold in 1 2 3 4 5 6; do for arm in C G; do candidate="F${fold}${arm}_CLIC12"; [[ -f "${TRAINING_ROOT}/${candidate}/final_ssdg.pth" && -f "${TRAINING_ROOT}/${candidate}/phase1_clic_terminal_receipt.json" ]] || { echo "missing frozen candidate ${candidate}" >&2; exit 2; }; done; done
mkdir -p "${RUN_ROOT}" "${LOG_ROOT}"
declare -a pids candidates gpus logs
row=0
for fold in 1 2 3 4 5 6; do for arm in C G; do
  candidate="F${fold}${arm}_CLIC12"; gpu="${GPU_MAP[row]}"; row=$((row + 1)); mkdir -p "${RUN_ROOT}/${candidate}"
