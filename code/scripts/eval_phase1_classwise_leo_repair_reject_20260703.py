#!/usr/bin/env python
"""Class-conditional source-only LEO repair plus sat-only rejection.

V15 keeps the Phase1 backbone frozen. It fits one source-only clean<-LEO repair
direction per old transmitter from source clean/LEO paired features, applies the
repair by predicted old class, and evaluates source/proxy calibrated rejection on
sat-only target old/unknown rows.
"""
...
def row_key(payload: dict, i: int) -> tuple[str, ...]:
...
def fit_classwise(clean_npz: Path, train_sat_npzs: list[Path], source_tx_ids: list[str], source_roles: set[str], eps: float) -> dict:
    ...
    for tx in source_tx_ids:
        ...
        if c.size == 0 or s.size == 0:
            raise ValueError(f"missing classwise source pairs for tx={tx}")
        proto = c.mean(axis=0)
...
def repair_and_score(x: np.ndarray, model: dict, alpha: float, beta: float, mode: str) -> dict:
    protos = model["prototypes"]
    deltas = model["deltas"]
    var = model["var"]
    x0 = x / max(float(np.linalg.norm(x)), 1.0e-6)
    raw_sims = protos @ x0
    pred = int(raw_sims.argmax())
    z = x + float(alpha) * deltas[pred]
    if float(beta) > 0:
        z = (1.0 - float(beta)) * z + float(beta) * protos[pred]
    z = z / max(float(np.linalg.norm(z)), 1.0e-6)
    sims = protos @ z
    delta = z.reshape(1, -1) - protos
    mah = np.mean((delta * delta) / var, axis=1)
    if mode == "oracle_class":
...
def evaluate_run(run_dir: Path, model: dict, args: argparse.Namespace) -> list[dict]:
    ...
    for alpha in [float(x) for x in str(args.alphas).split(",") if x.strip()]:
        for beta in [float(x) for x in str(args.betas).split(",") if x.strip()]:
            scored = []
            for row in rows:
                s = repair_and_score(row["feature"], model, alpha, beta, str(args.pred_mode))
            ...
            if not source.any() or not proxy.any() or not known.any() or not unknown.any():
                continue
            score_raw = {
                "max_sim": np.asarray([r["max_sim"] for r in scored], dtype=np.float64),
                "sim_margin": np.asarray([r["sim_margin"] for r in scored], dtype=np.float64),
                "neg_min_mah": np.asarray([r["neg_min_mah"] for r in scored], dtype=np.float64),
                "mah_margin": np.asarray([r["mah_margin"] for r in scored], dtype=np.float64),
            }
            scores = {k: rank_scale(v, source | proxy) for k, v in score_raw.items()}
            scores["classwise_repair_score"] = 0.35 * scores["max_sim"] + 0.20 * scores["sim_margin"] + 0.35 * scores["neg_min_mah"] + 0.10 * scores["mah_margin"]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--pred_mode", default="raw_class", choices=["raw_class", "oracle_class"])
    p.add_argument("--var_eps", type=float, default=1.0e-4)
    p.add_argument("--run_tag", default="V15_classwise_leo_repair")
    return p.parse_args()
...
def main() -> int:
    ...
    summary = {
        "phase": "phase1_classwise_leo_repair_reject_v15",
