...
def main():
    ...
    lines+=['## 完整性与原始数据','',
        f"- 共{len(rows)}模型、{sum(v['predictions'] for v in d['verification'].values()):,}条预测。各测试批次内部全部四场景预测关闭后才评分；合并报告不表示跨批次同时封存。",
        ...
        f"- 各批次预测及评分累计耗时{d['complete']['total_seconds']/60:.2f}分钟；不含被用户要求替换的单卡partial run，不是隔离吞吐测量。",
