...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_seennew_contrast_probe_20260707}"
...
QKNN_K="${QKNN_K:-8}"
INCLUDE_EVENT_RESULTS="${INCLUDE_EVENT_RESULTS:-0}"
DRY_RUN="${DRY_RUN:-0}"
...
declare -a PROFILES=(
  "CONTRAST_W025_M00"
  "CONTRAST_W050_M02"
  "CENTER_CONTRAST_W025_M00"
)

mkdir -p "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}"
echo "[STAGE2C-SEENNEW-CONTRAST] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-SEENNEW-CONTRAST] profiles=${PROFILES[*]} k=${K_SHOTS[*]}"
echo "[STAGE2C-SEENNEW-CONTRAST] diagnostic_only=true unknown_query_eval_only=true"
...
profile_args() {
  ...
  case "${profile}" in
    CONTRAST_W025_M00)
      PROFILE_ARGS+=(--seen_new_old_contrast_weight 0.25 --seen_new_old_contrast_margin 0.00)
      ;;
    CONTRAST_W050_M02)
      PROFILE_ARGS+=(--seen_new_old_contrast_weight 0.50 --seen_new_old_contrast_margin 0.02)
      ;;
    CENTER_CONTRAST_W025_M00)
      PROFILE_ARGS+=(
        ...
        --seen_new_old_contrast_margin 0.00
      )
      ;;
    *)
...
run_one() {
  ...
  echo "[STAGE2C-SEENNEW-CONTRAST-RUN] variant=${variant} profile=${profile} k=${k} feature=${feature_npz}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
  EXTRA_ARGS=()
...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-SEENNEW-CONTRAST-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_seennew_contrast_probe_summary.json" <<'PY'
import csv
import json
import sys
...
for path in sorted(case_dir.glob("*/*/stage2c_qknn_*_k*.json")):
    ...
    for row in payload.get("summary_rows", []):
        ...
        item["feature_adapter_policy"] = meta.get("feature_adapter_policy", "")
        ...
        item["collapse_score"] = (
            ...
            + 0.5 * float(item.get("old_acc", 0.0))
            + 0.5 * float(item.get("min_old_class_acc", 0.0))
            - float(item.get("unknown_FAR", 1.0))
        )
        rows.append(item)
rows.sort(
    key=lambda r: (
        float(r.get("collapse_score", -999.0)),
...
out = {"rows": rows, "best_collapse_score": rows[:10], "best_feasible": feasible[:10]}
...
print(json.dumps({"best_collapse_score": rows[:5], "best_feasible": feasible[:5], "out": str(out_path), "csv": str(csv_path)}, ensure_ascii=False, indent=2))
PY

echo "[STAGE2C-SEENNEW-CONTRAST-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
