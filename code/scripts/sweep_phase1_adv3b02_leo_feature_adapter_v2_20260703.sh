...
MATRIX_LOG_ROOT="${MATRIX_LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_leo_feature_adapter_v2_matrix_20260703}"
...
declare -a ADAPTERS=(
  "LEOADAPT2_IDENTITY identity 64 1.0 0.00 1 0.0001 1.0 0.0 0.0 0.0"
  "LEOADAPT2_MEANSHIFT mean_shift 64 1.0 0.00 1 0.0001 1.0 0.0 0.0 0.0"
  "LEOADAPT2_NORMSHIFT norm_mean_shift 64 0.7 0.00 1 0.0001 1.0 0.0 0.0 0.0"
  "LEOADAPT2_LINR_ID linear_residual 64 0.35 0.00 120 0.0007 0.4 1.5 1.0 0.08"
)

declare -a HEAD_POLICIES=(
  "ADAPT2_LIN_SRC9999 linear 64 450 0.020 0.9999 source_accept"
  "ADAPT2_LIN_MIN05 linear 64 450 0.020 1.0000 min_source_proxy"
  "ADAPT2_MLP64_SRC9999 mlp 64 320 0.005 0.9999 source_accept"
  "ADAPT2_MLP64_MIN05 mlp 64 320 0.005 1.0000 min_source_proxy"
)

declare -a PROTO_POLICIES=(
  "ADAPT2_PROTO_COS_SRC9999 cosine 0.0 0.0 0.0 source_accept 0.9999 0.05"
  "ADAPT2_PROTO_COS_MIN05 cosine 0.0 0.0 0.0 min_source_proxy 1.0000 0.05"
  "ADAPT2_PROTO_MAH_MIN05 diag_mahalanobis 0.0 0.0 0.0 min_source_proxy 1.0000 0.05"
)

echo "[PHASE1-LEO-FEATURE-ADAPTER-V2-SWEEP] start=$(date -Is) seed=${SEED} gpu=${GPU} shard=${CELL_SHARD_INDEX}/${CELL_SHARD_COUNT}"
for cell_idx in "${!CELLS[@]}"; do
  ...
  MV_DIR="${RUNS_ROOT}/ADV3B02_CORE90_SOFT_E200_PHASE1_MULTIVIEW"
  CLEAN_NPZ="${MV_DIR}/clean.npz"
  TRAIN_SAT_CLEAR="${MV_DIR}/sat_clear.npz"
  TRAIN_SAT_LOW="${MV_DIR}/sat_low.npz"
  TRAIN_SAT_RAIN="${MV_DIR}/sat_rain.npz"
  SAT_NPZ="${RUNS_ROOT}/ADV3B02_CORE90_SOFT_E200_PHASE1_SATUNKNOWN_SINGLEVIEW/features_satunknown_singleview.npz"
  if [[ ! -f "${CLEAN_NPZ}" || ! -f "${TRAIN_SAT_CLEAR}" || ! -f "${TRAIN_SAT_LOW}" || ! -f "${TRAIN_SAT_RAIN}" || ! -f "${SAT_NPZ}" ]]; then
    echo "[PHASE1-LEO-FEATURE-ADAPTER-V2-SKIP] run_id=${RUN_ID} missing_inputs=1"
    continue
  fi
  echo "[PHASE1-LEO-FEATURE-ADAPTER-V2-CELL] run_id=${RUN_ID} unknown=${UNKNOWN_TX_IDS}"
...
"${PYTHON}" - <<'PY' "${ROOT}/runs" "${MATRIX_LOG_ROOT}/leo_feature_adapter_v2_summary.csv"
import csv
import json
import sys
...
out = Path(sys.argv[2])
rows = []
for adapter_metrics in sorted(runs_root.glob("phase1_adv3b02_multiview_keepold_*_20260702/LEOADAPT2_*/adapter_metrics.json")):
    adapt = json.loads(adapter_metrics.read_text(encoding="utf-8"))
    val = adapt.get("val_alignment", {})
    for metrics_path in sorted(adapter_metrics.parent.glob("ADAPT2_*/metrics.json")):
...
PY
echo "[PHASE1-LEO-FEATURE-ADAPTER-V2-SWEEP-DONE] end=$(date -Is) summary=${MATRIX_LOG_ROOT}/leo_feature_adapter_v2_summary.csv"
