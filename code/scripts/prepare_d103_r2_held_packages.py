#!/usr/bin/env python3
"""Prepare truth-separated source-held packages and the frozen TX probe receipt."""
...
def _load_fit(
    ...
    *,
    ...
    if (
        ...
        raise ValueError(f"incomplete or drifted fit: {fit_id}")
    with np.load(teacher_path, allow_pickle=False) as archive:
        if set(archive.files) != TEACHER_KEYS:
            raise ValueError(f"teacher member closure drift: {fit_id}")
...
def main() -> int:
    ...
    if output.exists() or output.is_symlink():
        raise FileExistsError(f"immutable held package root exists: {output}")
    manifest = _read_json(manifest_path)
    if (
        manifest.get("role") != "source_val_scorer_only"
        ...
        raise ValueError("source-val scorer manifest drift")
    with np.load(archive_path, allow_pickle=False) as archive:
        if set(archive.files) != SCORER_KEYS:
            raise ValueError("source-val scorer member closure drift")
    ...
    provisional_tx_sha = canonical_sha256(
        {"schema": "d103_r2_tx_probe_provisional", "persisted": False}
    )
    probe_rows = []
    for receiver in receivers:
        ...
        probe_rows.append(
            run_fixed_tx_probe(
                encode_domain_rows(
                    bundle, np.asarray(arrays["z_dom"][local], dtype=np.float32)
                ),
    ...
    tx_receipt = {
        "schema": "cvs.d103_r2.rxid_crossreceiver.tx_probe.v1",
        "fold_count": 7,
        "folds": probe_rows,
    ...
    for receiver in receivers:
        for k_shot in (1, 5, 10):
            ...
            truth_rows.append(
                {
                    "package_id": identity,
                    "query_physical_ids": arrays["physical_ids"][query].astype(str).tolist(),
                    "query_truth_labels": arrays["labels"][query].astype(str).tolist(),
                }
            )
    package_manifest = {
        "schema": "cvs.d103_r2.rxid_crossreceiver.held_packages.v1",
