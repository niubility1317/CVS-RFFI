...
RUN_ID="${RUN_ID:-stage2_spaceborne_next64bm_retry1_20260618_233820}"
...
declare -a CAND_ID=(S2N80R1_GPU4_G_STAGE2A_K5_HARM_LOWPROTO_G S2N80R1_GPU4_H_STAGE2A_K20_HARM_FRONTIER_H S2N80R1_GPU7_G_STAGE2A_K10_HARM_NEG_STRICT_G)
declare -a CAND_GPU=(4 4 7)
declare -a CAND_KIND=(sfe sfe sfe)
declare -a CAND_SLOT=(GPU4/G GPU4/H GPU7/G)
declare -a CAND_DESC=(retry_source_open_set_lowproto retry_source_open_set_frontier retry_source_open_set_neg)
declare -a CAND_LANE=(phase2 phase2 phase2)
declare -a META_ABLATION=(not_applicable not_applicable not_applicable)
declare -a META_SEED=(0 0 0)
declare -a META_MAX_SAMPLES=(16 16 16)
declare -a SFE_CHANNEL=(satellite satellite satellite)
declare -a SFE_PROFILE=(source_open_set source_open_set source_open_set)
declare -a SFE_SHOTS=(0 0 0)
declare -a SFE_SEED=(47500 47507 47514)
declare -a SFE_SAT_SEED=(48500 48507 48514)
declare -a SFE_MAX_SAMPLES=(260 260 260)
declare -a SFE_SOURCE_PROTO=(40 60 40)
declare -a SFE_QUERY=(50 50 50)
declare -a SFE_SCENARIOS=(clear_leo,low_elev_leo,rain_leo,storm_mp,mixed_orbit clear_leo,low_elev_leo,rain_leo,storm_mp,mixed_orbit clear_leo,low_elev_leo,rain_leo,storm_mp,mixed_orbit)
declare -a FTRC_EVAL_PROFILE=(- - -)
declare -a FTRC_EVAL_SUPPORT=(0 0 0)
declare -a FTRC_EVAL_KAPPA=(3.0 3.0 3.0)
declare -a FTRC_EVAL_SEED=(0 0 0)
declare -a FTRC_EVAL_SAT_SEED=(0 0 0)
declare -a FTRC_EVAL_MAX_SAMPLES=(260 260 260)
declare -a FTRC_EVAL_SOURCE_PROTO=(0 0 0)
declare -a FTRC_EVAL_QUERY=(0 0 0)
declare -a FTRC_EVAL_SCENARIOS=(- - -)
...
if [[ "${PHASE1_LANE_ACTIVE}" == "1" ]]; then
  for i in "${!CAND_ID[@]}"; do
    if [[ "${CAND_LANE[$i]:-}" == "phase1" ]]; then
      ...
      event_row "${CAND_ID[$i]}" "DEFERRED_RETRY_CAPACITY" "gpu=${CAND_GPU[$i]}" "lane=phase1" "reason=phase1_lane_active_or_capacity_retry;exact_retry=RUN_ID=${RUN_ID}_phase1_training PHASE1_LANE_ACTIVE=0 PHASE2_LANE_ACTIVE=1 bash ${ROOT}/code/scripts/launch_stage2_optimizer_20260618_233820_next64bm_retry1.sh"
    fi
  done
fi
...
if [[ "${PHASE2_LANE_ACTIVE}" == "1" ]]; then
  for i in "${!CAND_ID[@]}"; do
    if [[ "${CAND_LANE[$i]:-}" == "phase2" ]]; then
      ...
      event_row "${CAND_ID[$i]}" "DEFERRED_RETRY_CAPACITY" "gpu=${CAND_GPU[$i]}" "lane=phase2" "reason=phase2_lane_active_or_capacity_retry;exact_retry=RUN_ID=${RUN_ID}_phase2_retry PHASE1_LANE_ACTIVE=0 PHASE2_LANE_ACTIVE=0 bash ${ROOT}/code/scripts/launch_stage2_optimizer_20260618_233820_next64bm_retry1.sh"
    fi
  done
fi
