#!/usr/bin/env python3
"""Run B0-B3 G0-anchored residual methods on the complete paired 125 grid."""
...
from cvsrffi.stage2_m24_row_executor import run_m25_anchored_row_from_base_cache
...
MATRIX_SCHEMA = "cvs.erbt_idr.m25.anchored_residual_prediction_matrix.v1"
EVIDENCE_ARMS = (D1, B1, B2, B3)
...
def _validate_receipt(task: dict[str, Any], receipt: dict[str, Any]) -> None:
    ...
    identity_fields = ("arm", "receiver", "k_shot", "new_class_count")
    drift = {
        field: {"task": task.get(field), "receipt": receipt.get(field)}
        for field in identity_fields
        if receipt.get(field) != task.get(field)
    }
    if drift:
        raise ValueError(f"task/receipt identity drift: {drift}")
    if task["arm"] == D1:
        parity = receipt.get("d1_historical_parity") or {}
        if (
            parity.get("prediction_disagreements") != 0
            or parity.get("before_prediction_disagreements") != 0
        ):
...
def _run_one(task: dict[str, Any]) -> dict[str, Any]:
    ...
    receipt = run_m25_anchored_row_from_base_cache(
        arm=str(task["arm"]),
