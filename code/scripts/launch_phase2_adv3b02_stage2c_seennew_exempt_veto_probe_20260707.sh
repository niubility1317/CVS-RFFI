...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_seennew_exempt_veto_probe_20260707}"
...
declare -a PROFILES=(
  "EXEMPT_STRICT_E080_L080_S085_C070_X3P070R070 0.80 0.80 0.85 0.70 2 3 0.70 0.70"
  "EXEMPT_MID_E085_L085_S085_C070_X3P070R070 0.85 0.85 0.85 0.70 2 3 0.70 0.70"
  "EXEMPT_OLDPROT_E080_L080_S085_C070_X4P080R080 0.80 0.80 0.85 0.70 2 4 0.80 0.80"
  "EXEMPT_OPENSEEN_E080_L080_S085_C070_X2P050R050 0.80 0.80 0.85 0.70 2 2 0.50 0.50"
  "EXEMPT_STRICTVETO_E075_L075_S085_C065_X3P070R070 0.75 0.75 0.85 0.65 2 3 0.70 0.70"
)

mkdir -p "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}"
echo "[STAGE2C-SEENNEW-EXEMPT-VETO] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-SEENNEW-EXEMPT-VETO] variants=${VARIANTS[*]} k=${K_SHOTS[*]} profiles=${#PROFILES[@]}"
echo "[STAGE2C-SEENNEW-EXEMPT-VETO] diagnostic_only=true unknown_query_eval_only=true"
...
run_one() {
  ...
  local min_sources="$7"
  local exempt_support="$8"
  local exempt_pvalue="$9"
  local exempt_reliability="${10}"
  local k="${11}"
  ...
  mkdir -p "${out_dir}"
  echo "[STAGE2C-SEENNEW-EXEMPT-VETO-RUN] variant=${variant} profile=${profile} k=${k} veto=${event_veto}/${label_veto}/${shell_veto}/${component_veto} min_sources=${min_sources} exemption=${exempt_support}/${exempt_pvalue}/${exempt_reliability}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
...
for variant in "${VARIANTS[@]}"; do
  for row in "${PROFILES[@]}"; do
    read -r profile event_veto label_veto shell_veto component_veto min_sources exempt_support exempt_pvalue exempt_reliability <<<"${row}"
    for k in "${K_SHOTS[@]}"; do
      run_one "${variant}" "${profile}" "${event_veto}" "${label_veto}" "${shell_veto}" "${component_veto}" "${min_sources}" "${exempt_support}" "${exempt_pvalue}" "${exempt_reliability}" "${k}"
    done
  done
done

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-SEENNEW-EXEMPT-VETO-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_seennew_exempt_veto_probe_summary.json" <<'PY'
import csv
import json
import sys
...
for path in sorted(case_dir.glob("*/*/stage2c_qknn_*_k*.json")):
    ...
    for row in payload.get("summary_rows", []):
        ...
        item["strict_far_feasible"] = float(item.get("unknown_FAR", 1.0)) <= 0.05
        item["utility"] = (
...
echo "[STAGE2C-SEENNEW-EXEMPT-VETO-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
