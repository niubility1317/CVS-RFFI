...
CUDA_VISIBLE_DEVICES="${GPU}" "${PYTHON}" -u code/scripts/train_apply_phase1_iq_preadapter_20260703.py \
  ...
  --runs_root "${RUN_ROOT}" \
  --out_subdir "ADV3B02_CORE90_SOFT_E200_PHASE1_HARDPAIR_${PROFILE}" \
  --out_name "features_hardpair_${PROFILE}.npz" \
  --clean_out_name "features_clean_hardpair_${PROFILE}.npz" \
  --cells "MANYNEW10_HARDPAIR_${PROFILE}:${TARGET_RX}:${NEW_TX}" \
...
FEATURE_NPZ="${RUN_ROOT}/MANYNEW10_HARDPAIR_${PROFILE}/ADV3B02_CORE90_SOFT_E200_PHASE1_HARDPAIR_${PROFILE}/features_hardpair_${PROFILE}.npz"
EVAL_DIR="${RUN_ROOT}/${PROFILE}/qknn_eval"
mkdir -p "${EVAL_DIR}"

"${PYTHON}" -u code/scripts/phase2_support_metric_qknn_probe.py \
  --feature_npz "${FEATURE_NPZ}" \
  --output_json "${EVAL_DIR}/k10_coreproto_hardpair_${PROFILE}.json" \
  --output_csv "${EVAL_DIR}/k10_coreproto_hardpair_${PROFILE}.csv" \
  --old_tx_ids "${OLD_TX}" \
  --new_tx_ids "${NEW_TX}" \
...
"${PYTHON}" -u code/scripts/phase2_support_metric_qknn_probe.py \
  --feature_npz "${FEATURE_NPZ}" \
  --output_json "${EVAL_DIR}/k5_sourceguard_hardpair_${PROFILE}.json" \
  --output_csv "${EVAL_DIR}/k5_sourceguard_hardpair_${PROFILE}.csv" \
  --old_tx_ids "${OLD_TX}" \
  --new_tx_ids "${NEW_TX}" \
