...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_normsep_protocol_20260707}"
...
declare -a VARIANTS=(
  "STAGE2C_NORM_SEP id_norm_late_feature 45 0.00008 1.00 2.00 22.00 4.50 7.50 0.003 0.20 0.10 0.05 0.16 0.12 0.12 0.14 0.07 0.12 0.05"
  "STAGE2C_HEAD_SEP id_feature_head 45 0.00010 1.15 2.20 20.00 4.00 7.00 0.004 0.22 0.10 0.05 0.16 0.10 0.12 0.14 0.07 0.12 0.05"
)

if (( ${#GPUS[@]} < ${#VARIANTS[@]} )); then
  echo "[STAGE2C-NORMSEP-GPU-COUNT] need ${#VARIANTS[@]} GPUs but got ${GPUS_CSV}" >&2
  exit 2
fi

CASE_ID="PHASE2_STAGE2C_RX${TARGET_RX//-/_}"
CELLS_ARG="${CASE_ID}:${TARGET_RX}:${TARGET_NEW_TX_IDS}:${TARGET_UNKNOWN_TX_IDS}"

mkdir -p "${LOG_ROOT}" "${RUNS_ROOT}"
echo "[STAGE2C-NORMSEP] run_id=${RUN_ID} dry_run=${DRY_RUN} target_rx=${TARGET_RX} gpus=${GPUS_CSV}"
echo "[STAGE2C-NORMSEP] target_new=${TARGET_NEW_TX_IDS}"
echo "[STAGE2C-NORMSEP] target_unknown=${TARGET_UNKNOWN_TX_IDS}"
echo "[STAGE2C-NORMSEP] proxy_pool_excludes_target_new_and_target_unknown=true"
echo "[STAGE2C-NORMSEP] success=K5/K10 old_acc>=0.80, improved seen_new floor, unknown_FAR<=0.05"

run_variant() {
  local idx="$1"
  local row="${VARIANTS[$idx]}"
  local gpu="${GPUS[$idx]}"
  read -r NAME MODE EPOCHS LR MSE_W COS_W CLEAN_W FEAT_MARGIN_W CLEAN_MARGIN_W MARGIN_TOL PROTO_CE_W PROXY_SEP_W PROXY_MAX_COS SUPCON_W DISTILL_W PROXY_PROTO_W PAIR_W PAIR_MARGIN OLD_PAIR_W OLD_MARGIN <<<"${row}"
  local log_path="${LOG_ROOT}/${NAME}_train_export.out"
  echo "[STAGE2C-NORMSEP-VARIANT] name=${NAME} mode=${MODE} gpu=${gpu} start=$(date -Is)"
  if [[ "${DRY_RUN}" == "1" ]]; then
    echo "[STAGE2C-NORMSEP-DRY-RUN] ${NAME} -> ${log_path}"
    return 0
  fi
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" \
    ...
    --runs_root "${RUNS_ROOT}" \
    --out_subdir "${NAME}" \
    --out_name "features_stage2c_leo_repaired.npz" \
    --clean_out_name "features_stage2c_clean_repaired.npz" \
    --identity_subdir "STAGE2C_IDENTITY" \
    ...
    --seed "${SEED}" \
    > "${log_path}" 2>&1
  echo "[STAGE2C-NORMSEP-VARIANT-DONE] name=${NAME} gpu=${gpu} end=$(date -Is)"
}
...
if [[ "${failed}" != "0" ]]; then
  echo "[STAGE2C-NORMSEP-TRAIN-FAILED] one or more variants failed" >&2
  exit 3
fi

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-NORMSEP-DRY-RUN-DONE]"
  exit 0
fi

run_stage2c_diag() {
  local name="$1"
  local feature_npz="${RUNS_ROOT}/${CASE_ID}/${name}/features_stage2c_leo_repaired.npz"
  ...
  for k in 5 10; do
    "${PYTHON}" -u "${ROOT}/code/scripts/phase2_frozen_manytx_unknown_diagnostic.py" \
      ...
      --output_json "${out_dir}/stage2c_qknn_k${k}.json" \
      --output_summary_csv "${out_dir}/stage2c_qknn_k${k}.summary.csv" \
      --k_shot "${k}" \
      --query_per_class 70 \
      --qknn_k 8 \
      ...
      > "${LOG_ROOT}/${name}_stage2c_qknn_k${k}.out" 2>&1
  done
}

for row in "${VARIANTS[@]}"; do
  read -r NAME _rest <<<"${row}"
  run_stage2c_diag "${NAME}"
done

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_normsep_protocol_summary.json" <<'PY'
import csv
import json
import sys
...
rows = []
for path in sorted(case_dir.glob("*/stage2c_qknn_k*.json")):
    payload = json.loads(path.read_text(encoding="utf-8"))
    for row in payload.get("summary_rows", []):
        item = dict(row)
        item["variant"] = path.parent.name
...
rows.sort(
    key=lambda r: (
        ...
        float(r.get("unknown_reject_rate", 0.0)),
    ),
    reverse=True,
)
...
print(json.dumps({"best": rows[:3], "out": str(out_path), "csv": str(csv_path)}, ensure_ascii=False, indent=2))
PY

echo "[STAGE2C-NORMSEP-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
