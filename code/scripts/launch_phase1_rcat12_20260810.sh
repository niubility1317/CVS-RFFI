...
# Invoke explicitly as: bash scripts/launch_phase1_rcat12_20260810.sh [--dry-run]
# Frozen six-fold P1-RCAT C/G continuation matrix. C is the exact GeoSat-C
# continuation; G adds only the frozen RCAT auxiliary. All other candidate
# routes are explicitly disabled and no method selection is performed here.
RUN_ID="${RUN_ID:-phase1_rcat12_20260810_v1}"
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in
    C) arm_args=(--phase1_rcat_enabled false --lambda_rcat 0) ;;
    G) arm_args=(--phase1_rcat_enabled true --lambda_rcat 0.02) ;;
