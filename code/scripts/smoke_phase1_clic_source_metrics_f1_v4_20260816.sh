...
# Structural-only v4 smoke: use the original formal inputs, write only beneath
...
# unchanged.
RUN_ID="phase1_clic_source_metrics_20260816_v4"
SMOKE_ROOT_NAME=".smoke_phase1_clic_source_metrics_20260816_v4_F1"
...
claim_exact_root() {
  local path="$1"
  if ! mkdir -- "${path}"; then
    echo "refusing to overwrite source metrics v4 smoke run/log root" >&2
    exit 3
  fi
}
...
open_exclusive_fd() {
  ...
  if ! { set -o noclobber; exec {OPEN_FD}>"${path}"; }; then
    set +o noclobber
    echo "refusing to overwrite source metrics v4 smoke log evidence" >&2
    exit 3
  fi
  set +o noclobber
}
