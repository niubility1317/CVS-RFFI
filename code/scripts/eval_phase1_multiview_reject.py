#!/usr/bin/env python
"""Evaluate Phase1-only multi-view consistency rejection.

Rows are grouped by sample metadata across repeated clean/satellite exports.
The rejection head is trained only on source old vs source proxy-unknown groups.
Target old and target unknown groups remain evaluation-only.
"""
...
def _group_payload(payload: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for key, idx in sorted(groups.items()):
        ...
        z_mean = F.normalize(z.mean(dim=0, keepdim=True), dim=1).squeeze(0)
        cos = (z_norm @ z_mean).clamp(-1.0, 1.0)
        energy = -torch.logsumexp(lo, dim=1)
        ...
        stats = torch.cat(
            [
                ...
                mean_logits,
                lo.std(dim=0, unbiased=False),
                mean_probs,
                probs.std(dim=0, unbiased=False),
                torch.tensor(
                    [
                        ...
                        float(energy.std(unbiased=False).item()),
...
def _loss_balance(y: torch.Tensor) -> torch.Tensor:
    ...
    return torch.where(y > 0.5, torch.full_like(y.float(), float(pos_w)), torch.full_like(y.float(), float(neg_w)))
...
def _train_scores(
    ...
    *,
    ...
    if head == "linear":
        model: nn.Module = nn.Linear(x_train.size(1), 1)
    elif head == "mlp":
        model = nn.Sequential(nn.Linear(x_train.size(1), int(hidden_dim)), nn.ReLU(), nn.Linear(int(hidden_dim), 1))
    else:
...
def _threshold(source_scores: np.ndarray, proxy_scores: np.ndarray, *, policy: str, source_q: float, proxy_q: float):
    source_t = float(np.quantile(source_scores, float(source_q)))
    proxy_t = float(np.quantile(proxy_scores, float(proxy_q)))
...
def evaluate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    x_all, scale_info = _standardize(x_raw[train_mask], x_raw)
    ...
    scores, losses, head_info = _train_scores(
        x_all[train_mask],
        y_train,
        x_all,
    ...
    threshold, th_info = _threshold(
        scores[train_known_mask.numpy()],
        scores[proxy_mask.numpy()],
        ...
        rows.append(
            {
                ...
                "min_confidence": f"{float(g['min_confidence']):.8f}",
                "mean_cosine": f"{float(g['mean_cosine']):.8f}",
                "min_cosine": f"{float(g['min_cosine']):.8f}",
            }
        )
    ...
    metrics = {
        "phase": "phase1_only_multiview_consistency_reject",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--seed", type=int, default=4070211)
