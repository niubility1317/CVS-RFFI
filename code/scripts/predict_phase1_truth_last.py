...
from cvsrffi.truth_last import SCENARIOS as FCR_PREDICTION_SCENARIOS
...
def select_identity_logits(outputs, **kwargs):
    # This release evaluates the registered A1 identity path, without FCR routing.
    logits=outputs['tx_logits']
    if not torch.is_tensor(logits): raise TypeError('Missing A1 identity logits')
    return logits
