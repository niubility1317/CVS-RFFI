#!/usr/bin/env python3
"""Run the frozen ERBT-IDR M2.3 F0-F5 same-row suite."""
...
from cvsrffi.stage2_m23_row_executor import M23_ARMS, run_m23_row_from_caches


SUITE_SCHEMA = "cvs.erbt_idr.m23.prediction_suite.v1"


def _arm_seed_plan(seed: int, arms: tuple[str, ...]) -> dict[str, int]:
    """Freeze one method seed across every arm in a paired same-row suite."""

    return {str(arm): int(seed) for arm in arms}
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--arm", action="append", choices=M23_ARMS)
    return parser
...
def main() -> int:
    ...
    arms = tuple(args.arm) if args.arm else M23_ARMS
    if len(set(arms)) != len(arms):
        raise ValueError("duplicate M2.3 arm")
    ...
    for arm in arms:
        ...
        receipt = run_m23_row_from_caches(
            arm=arm,
            row_id=row_id,
            ...
            base_feature_cache_manifest_sha256=(
                args.base_feature_cache_manifest_sha256
            ),
