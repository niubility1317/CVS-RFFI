...
# Invoke explicitly as: bash scripts/launch_phase1_icmt12_20260810.sh [--dry-run]
# Frozen six-fold P1-ICMT C/G continuation matrix. C is the exact GeoSat-C
# continuation; G adds only the fixed L-only ICMT tail-tightening loss.
RUN_ID="${RUN_ID:-phase1_icmt12_20260810_v1}"
...
COMMON=(
  ...
  --phase1_cp_sfce_frozen_mode false --phase1_cp_sfce_enabled false --lambda_cp_sfce 0 --phase1_icmt_frozen_mode true --max_grad_norm 5.0 --checkpoint_selection final_only --best_metric clean_val_tx --phase1_source_val_selection_only true
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in
    C) arm_args=(--phase1_icmt_enabled false --lambda_icmt 0) ;;
    G) arm_args=(--phase1_icmt_enabled true --lambda_icmt 0.05) ;;
