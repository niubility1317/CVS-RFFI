...
# Fresh v4 identity for the frozen file-backed F1 one-shot technical smoke.
...
refuse_existing_roots() {
  if [[ -e "${SMOKE_RUN_ROOT}" || -e "${SMOKE_LOG_ROOT}" ]]; then
    echo "refusing to overwrite ADV target prediction v4 smoke run/log root" >&2
    exit 3
  fi
}
...
open_exclusive_fd() {
  ...
  if ! { set -o noclobber; exec {OPEN_FD}>"${path}"; }; then
    set +o noclobber
    echo "refusing to overwrite ADV target prediction v4 smoke log/PID evidence" >&2
    exit 3
  fi
  set +o noclobber
}
...
[[ -f "${TRAIN_CONFIG}" && -f "${RECEIPT}" ]] || {
  echo "ADV target prediction v4 smoke did not close" >&2
  exit 2
}
