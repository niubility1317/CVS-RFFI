...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_farconstrained_gate_sweep_20260707}"
...
declare -a GATE_ROWS=(
  "U074_MNEG02 0.74 0.76 -0.02"
  "U074_M000 0.74 0.76 0.00"
  "U078_MNEG02 0.78 0.80 -0.02"
  "U078_M000 0.78 0.80 0.00"
  "U082_MNEG02 0.82 0.84 -0.02"
  "U082_M000 0.82 0.84 0.00"
  "U086_MNEG02 0.86 0.88 -0.02"
  "U086_M000 0.86 0.88 0.00"
  "U090_MNEG02 0.90 0.92 -0.02"
  "U090_M000 0.90 0.92 0.00"
)

mkdir -p "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}"
echo "[STAGE2C-FAR-GATE] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-FAR-GATE] variants=${VARIANTS[*]} k=${K_SHOTS[*]} rows=${#GATE_ROWS[@]}"
echo "[STAGE2C-FAR-GATE] diagnostic_only=true unknown_query_eval_only=true"

run_one() {
  local variant="$1"
  local gate_name="$2"
  local unknown_risk="$3"
  local candidate_risk="$4"
  local accept_margin="$5"
  local k="$6"
  local feature_npz="${SOURCE_RUNS_ROOT}/${CASE_ID}/${variant}/features_stage2c_leo_repaired.npz"
  local out_dir="${RUNS_ROOT}/${CASE_ID}/${variant}/${gate_name}"
  local out_json="${out_dir}/stage2c_qknn_${gate_name}_k${k}.json"
  local out_csv="${out_dir}/stage2c_qknn_${gate_name}_k${k}.summary.csv"
  local out_log="${LOG_ROOT}/${variant}_${gate_name}_k${k}.out"
  mkdir -p "${out_dir}"
  echo "[STAGE2C-FAR-GATE-RUN] variant=${variant} gate=${gate_name} k=${k} unknown_risk=${unknown_risk} candidate_risk=${candidate_risk} accept_margin=${accept_margin}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
  "${PYTHON}" -u "${ROOT}/code/scripts/phase2_frozen_manytx_unknown_diagnostic.py" \
    ...
    --candidate_set_max_label_unknown_risk "${candidate_risk}" \
    --candidate_set_max_event_unknown_risk "${candidate_risk}" \
    --candidate_set_max_label_risk_component_agreement 1.0 \
    --candidate_set_unknown_reject_risk "${candidate_risk}" \
    > "${out_log}" 2>&1
}

for variant in "${VARIANTS[@]}"; do
  for row in "${GATE_ROWS[@]}"; do
    read -r gate_name unknown_risk candidate_risk accept_margin <<<"${row}"
    for k in "${K_SHOTS[@]}"; do
      run_one "${variant}" "${gate_name}" "${unknown_risk}" "${candidate_risk}" "${accept_margin}" "${k}"
    done
  done
done

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-FAR-GATE-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_farconstrained_gate_sweep_summary.json" <<'PY'
import csv
import json
import sys
...
for path in sorted(case_dir.glob("*/*/stage2c_qknn_*_k*.json")):
    ...
    for row in payload.get("summary_rows", []):
        ...
        item["gate_profile"] = path.parent.name
...
echo "[STAGE2C-FAR-GATE-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
