...
# Invoke explicitly as: bash scripts/launch_phase1_recte12_20260810.sh [--dry-run]
# Frozen six-fold P1-RECTE C/G continuation matrix. C is the exact GeoSat-C
# continuation; G adds only the fixed source-L receiver/class occupied-pair
# equilibrium term. All other candidate routes remain explicitly disabled.
RUN_ID="${RUN_ID:-phase1_recte12_20260810_v1}"
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in
    C) arm_args=(--phase1_recte_enabled false --lambda_recte 0) ;;
    G) arm_args=(--phase1_recte_enabled true --lambda_recte 0.02) ;;
