#!/usr/bin/env python3
"""静态规划并调度PairBiCAD-CV2 E200 source-only 24行矩阵。

候选、fold、seed、GPU分配和训练配置在模块加载时一次解析。worker只消费
已解析的row，不根据运行结果重写候选或配置；每张GPU最多运行两个直属worker。
本文件是修复版入口，旧日期入口保留用于历史重放。
"""
...
RUN_ID_DEFAULT = "phase1_pairbicad_cv2_fixed11_e200_seed392002_20260901_r1"
...
PLAN_SCHEMA = "pairbicad_cv2_screen24_e200_plan_v1"
WORKER_STATUS_SCHEMA = "pairbicad_cv2_screen24_e200_worker_status_v1"
...
def _static_candidate_data() -> dict[str, tuple[Mapping[str, Any], Mapping[str, Any]]]:
    """Resolve every candidate and method lock before any plan is built."""

    resolved: dict[str, tuple[Mapping[str, Any], Mapping[str, Any]]] = {}
    for candidate_id in CANDIDATE_IDS:
        config = candidate_config(candidate_id)
        configuration = _freeze(cv2_configuration_payload(config))
        lock = dict(method_lock_payload(config))
        lock["configuration"] = configuration
        resolved[candidate_id] = (configuration, _freeze(lock))
    return resolved
        ...
        for fold in FOLDS:
            ...
            rows.append(
                PlanRow(
                    ...
                    termination_mode=str(configuration["termination_mode"]),
...
def _validate_row_shape(row: PlanRow) -> None:
    if row.candidate_id not in CANDIDATE_IDS:
        raise RuntimeError(f"unknown CV2 candidate: {row.candidate_id}")
    if row.fold not in FOLDS or row.seed != SEED:
        raise RuntimeError("CV2 rows must use folds 1,8 and seed392002")
    if row.epochs != CV2_EPOCHS or row.termination_mode != CV2_TERMINATION_MODE:
        raise RuntimeError("CV2 rows must terminate after 200 epochs")
    if row.gpu_id not in GPU_IDS:
        raise RuntimeError("CV2 row GPU must be one of GPU0-7")
    if row.source_receivers not in {(3, 4, 6, 8), (1, 3, 4, 6)}:
        raise RuntimeError("CV2 rows must use the declared source receiver folds")
    if row.train_days != TRAIN_DAYS:
    ...
    if row.method_lock.get("dynamic_alias") is not False:
        raise RuntimeError("CV2 rows must disable dynamic aliases")
    if row.method_lock.get("frozen") is not True:
...
def _validate_static_matrix(rows: Sequence[PlanRow]) -> None:
    ...
    if {row.candidate_id for row in rows} != set(CANDIDATE_IDS):
    ...
    for row in rows:
        ...
        if row.method_lock.get("runtime_contracts", {}).get("termination") != {
            "mode": CV2_TERMINATION_MODE,
            "epochs": CV2_EPOCHS,
        }:
            raise RuntimeError("CV2 rows must carry the E200 termination contract")
...
def build_plan_payload(
    ...
    *,
    ...
    if len({row.row_id for row in selected}) != len(selected):
        raise ValueError("plan rows must have unique row IDs")
    for row in selected:
        _validate_row_shape(row)
    capacities = _normalise_gpu_capacities(gpu_capacities)
    assigned = Counter(row.gpu_id for row in selected)
    queued_rows = sum(
        max(0, assigned[gpu_id] - capacities[gpu_id]) for gpu_id in GPU_IDS
    )
    return {
        "schema": PLAN_SCHEMA,
        "run_id": str(run_id),
        ...
        "termination": {"mode": CV2_TERMINATION_MODE, "epochs": CV2_EPOCHS},
        "dynamic_alias": False,
...
def build_train_command(
    ...
    *,
    ...
    """Build the exact source-only command for one frozen E200 row."""
    ...
    if "--epochs" not in option_names:
        raise ValueError("E200 command must declare --epochs")
    return command
...
def launch_detached_worker(
    ...
    *,
    ...
    """Start one row worker in a new process group and return its handle."""
...
def dispatch_detached_workers(
    ...
    *,
    ...
    """Run within per-GPU capacity and queue remaining rows."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Publish and dispatch the fixed PairBiCAD-CV2 E200 source-only screen24 matrix."
    )
...
def _parse_gpu_capacities(raw: str) -> dict[int, int]:
    ...
    for item in str(raw).split(","):
        ...
        try:
            ...
            raise ValueError("gpu-capacities GPU and slots must be integers") from exc
        if gpu_id in parsed:
