#!/usr/bin/env python3
"""Sweep old-class score bias for class-max KNN Stage2-C heads."""
...
def _classmax_predict(
    ...
    scores = []
    for label in labels:
        support = support_by_class[label]
        class_score = np.max(query @ support.T, axis=1)
        if label in old_labels:
            class_score = class_score + float(old_bias)
        else:
            class_score = class_score + float(new_bias)
        scores.append(class_score)
    score_matrix = np.vstack(scores).T
    return np.asarray([labels[int(idx)] for idx in np.argmax(score_matrix, axis=1)], dtype=object)
...
def main() -> None:
    ...
    parser.add_argument("--old_bias_values", default="0,0.0025,0.005,0.0075,0.01,0.0125,0.015,0.0175,0.02,0.025,0.03,0.035,0.04,0.045,0.05")
    parser.add_argument("--new_bias", type=float, default=0.0)
    ...
    if not old_splits:
        raise RuntimeError("No eligible old splits")
    if not new_splits:
        raise RuntimeError("No eligible new splits")
    ...
    old_bias_values = _parse_float_csv(args.old_bias_values)

    rows: list[dict[str, Any]] = []
    for combo in combos:
        for old_bias in old_bias_values:
            rows.append(
                _evaluate(
                    combo,
                    old_bias,
    ...
    summary = {
        ...
        "old_bias_values": old_bias_values,
        "new_bias": float(args.new_bias),
