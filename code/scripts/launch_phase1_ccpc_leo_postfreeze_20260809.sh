...
# Final-only postfreeze diagnostics for the completed P1-CCPC-LEO C/G matrix.
# It never trains, fits, calibrates, sweeps, or selects a checkpoint.
POSTFREEZE_RUN_ID="${POSTFREEZE_RUN_ID:-phase1_ccpc_leo_postfreeze_20260809}"
...
TRAIN_RUN_ROOT="${TRAIN_RUN_ROOT:-${PROJECT_ROOT}/runs/phase1_ccpc_leo12_20260809}"
...
PAIR_EVAL_SCRIPT="${CODE_ROOT}/scripts/eval_phase1_ccpc_leo_pair.py"
DRY_RUN="${DRY_RUN:-0}"
...
EXPORT_SEED="7281105"
MAX_PER_TX="400" # train4=1600; each held/proxy singleton TX=400
EXPORT_BATCH="32"
...
# The frozen 4/1/1 source-known roles and the C/G GPU schedule are inherited
# verbatim from the successful CCPC continuation launcher.
FOLD_TRAIN_TX=(
...
# At most two candidate pipelines are active per card, and each pipeline is
# serial clean export -> LEO export -> source-calibrated proxy score.
...
run_pair() {
  ...
  local fold_index=$((fold - 1))
  local train_tx="${FOLD_TRAIN_TX[fold_index]}"
  local c_candidate="F${fold}C_CCPCLEO12"
  local g_candidate="F${fold}G_CCPCLEO12"
