#!/usr/bin/env python3
"""Prepare, run the truth-free K1 smoke, and execute Target125 shards."""
...
from cvsrffi.stage2_d92_e0_full_only_target125 import (  # noqa: E402
    ARM_ID,
    ARM_ORDER,
    CANDIDATE_ID,
    CANONICAL_SELECTION_SHA256,
    CONTEXT_SHA256,
    D92E0FullOnlyTarget125Error,
    SCENES,
    SHARD_COUNT,
    SMOKE_OUTER_KEY,
...
try:  # Reuse the established closure and command construction, not its matrix.
...
class Target125RunnerError(RuntimeError):
    """Raised when the immutable Target125 runner boundary would drift."""


D92E0FullOnlyTarget125RunnerError = Target125RunnerError
...
def _json_bytes(value: Mapping[str, Any]) -> bytes:
    return (
        json.dumps(dict(value), ensure_ascii=True, sort_keys=True, separators=(",", ":"), allow_nan=False)
        + "\n"
    ).encode("utf-8")
...
def _read_json_object(path: Path) -> dict[str, Any]:
    if not path.is_file() or path.is_symlink():
        raise Target125RunnerError(f"shared smoke artifact is missing: {path}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise Target125RunnerError(f"shared smoke artifact is invalid: {path}") from error
    if not isinstance(payload, dict):
        raise Target125RunnerError(f"shared smoke artifact must be an object: {path}")
    return payload
...
def _load_manifest(path: str | Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if source.is_symlink() or not source.is_file() or _sha256_file(source) != str(expected_sha256).lower():
        raise Target125RunnerError("matrix manifest must be an immutable regular file")
    try:
        payload = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise Target125RunnerError("matrix manifest JSON drift") from error
    if not isinstance(payload, dict):
        raise Target125RunnerError("matrix manifest must be an object")
    if payload.get("smoke_outer_key") != SMOKE_OUTER_KEY:
        raise Target125RunnerError("matrix manifest smoke_outer_key drift")
    try:
    ...
    except (KeyError, OSError, TypeError, ValueError, D92E0FullOnlyTarget125Error) as error:
        raise Target125RunnerError(f"Target125 manifest contract drift: {error}") from error
    return payload
    ...
    *,
    ...
    try:
        _write_json_new(
            ...
            {
                "schema": "cvs.phase2.d92_e0_full_only_target125.systemic_stop.v1",
...
def _record_shared_pre_prediction_failure(
    ...
    try:
        _write_json_new(
            record,
            {
                "schema": "cvs.phase2.d92_e0_full_only_target125.pre_prediction_failure.v1",
                "timestamp": _now(),
...
def _is_full_matrix(manifest: Mapping[str, Any]) -> bool:
    ...
    return int(manifest.get("job_count", -1)) == 125 and isinstance(jobs, list) and len(jobs) == 125
...
def _smoke_job(manifest: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if len(matches) != 1:
        raise Target125RunnerError("K1 liveness smoke row identity drift")
    return matches[0]
...
def _validate_shared_smoke(
    ...
    if not _is_full_matrix(manifest):
        raise Target125RunnerError("Target125 full matrix identity drift")
    ...
    identity_ok = (
        receipt.get("schema") == "cvs.phase2.d92_e0_full_only_target125.smoke_receipt.v1"
        and receipt.get("status") == "D92_E0_FULL_ONLY_TARGET125_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS"
    ...
    if not identity_ok:
        raise Target125RunnerError("shared smoke receipt identity/protocol drift")
    ...
    if receipt.get("command") != expected_command:
        raise Target125RunnerError("shared smoke command identity drift")
    try:
        if Path(str(receipt.get("prediction_root"))).resolve() != expected_prediction_root:
            raise Target125RunnerError("shared smoke prediction root drift")
    except (OSError, TypeError, ValueError) as error:
        raise Target125RunnerError("shared smoke prediction root drift") from error
    closure = receipt.get("prediction_closure")
    if not isinstance(closure, Mapping):
        raise Target125RunnerError("shared smoke prediction closure missing")
    for field, path in expected_files.items():
        if not path.is_file() or path.is_symlink():
            raise Target125RunnerError(f"shared smoke prediction closure missing: {path}")
        digest = _sha256_file(path)
        if receipt.get(field) != digest or closure.get(field) != digest:
            raise Target125RunnerError("shared smoke prediction hash drift")
    if receipt.get("fit_audit_protocol_closed") is not True or _prediction_closure_status(expected_prediction_root)[0] != "closed":
        raise Target125RunnerError("shared smoke fit audit protocol closure drift")


def prepare(args: argparse.Namespace) -> dict[str, Any]:
    output = Path(args.output_root)
    if output.exists():
        raise Target125RunnerError("matrix output already exists")
    ...
    return {
        "status": "TARGET125_MATRIX_PREPARED",
...
def truth_free_smoke(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if Path(args.output_root).resolve() != output:
        raise Target125RunnerError("full-matrix smoke output must be manifest output_root/smoke")
    if output.exists():
        raise Target125RunnerError("smoke output already exists")
    output.mkdir(parents=True)
    ...
    if completed.returncode != 0:
        ...
        raise Target125RunnerError("truth-free smoke prediction closure failed")
    ...
    if closure_status == "protocol_p0":
        ...
        raise Target125RunnerError("truth-free smoke query protocol closure failed")
    ...
    receipt = {
        "schema": "cvs.phase2.d92_e0_full_only_target125.smoke_receipt.v1",
        "status": "D92_E0_FULL_ONLY_TARGET125_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS",
...
def run_shard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if int(args.shard_count) != SHARD_COUNT or int(args.shard_index) not in range(SHARD_COUNT):
        raise Target125RunnerError("shard identity drift")
    selected = [job for job in manifest["jobs"] if int(job["planned_shard_index"]) == int(args.shard_index)]
    if not selected:
        raise Target125RunnerError("selected shard has no jobs")
    ...
    if events_path.exists() or summary_path.exists():
        raise Target125RunnerError("shard evidence already exists")
    ...
    for job in selected:
        ...
        with prediction_stdout.open("x", encoding="utf-8", newline="") as stdout, prediction_stderr.open("x", encoding="utf-8", newline="") as stderr:
        ...
        with score_stdout.open("x", encoding="utf-8", newline="") as stdout, score_stderr.open("x", encoding="utf-8", newline="") as stderr:
        ...
        receipt = {
            "schema": "cvs.phase2.d92_e0_full_only_target125.job_receipt.v1",
    ...
    summary = {
        "schema": "cvs.phase2.d92_e0_full_only_target125.shard_summary.v1",
        "status": status,
...
def main() -> int:
    ...
    return 0 if result["status"] in {"TARGET125_MATRIX_PREPARED", "D92_E0_FULL_ONLY_TARGET125_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS", "PASS"} else 1
