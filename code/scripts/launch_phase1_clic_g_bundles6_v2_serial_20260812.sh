...
RUN_ID="phase1_clic_g_bundles_20260812_v2_serial"
...
emit_command() {
  local fold="$1"; shift
  printf '[DRY-RUN] stage=CLIC_G_BUNDLE_SERIAL fold=%s arm=G' "${fold}"
  printf ' %q' "$@"
  printf '\n'
}
...
[[ ! -e "${RUN_ROOT}" && ! -e "${LOG_ROOT}" ]] || { echo "refusing to overwrite serial G bundle run/log root" >&2; exit 3; }
check_inputs
mkdir -p "${RUN_ROOT}" "${LOG_ROOT}"
printf 'pid|fold|stage|log_path|exit_code\n' >"${LOG_ROOT}/pids_g_bundles6_serial.tsv"
...
for fold in 1 2 3 4 5 6; do
  ...
  exit_code="$?"
  set -e
  printf '%s|%s|CLIC_G_BUNDLE_SERIAL|%s|%s\n' \
