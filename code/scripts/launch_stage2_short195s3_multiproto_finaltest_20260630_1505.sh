...
RUN_ID="${RUN_ID:-stage2_short195s3_multiproto_finaltest_20260630_1505}"
...
echo "[STAGE2-SHORT195S3] run_id=${RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2-SHORT195S3] teacher=${TEACHER_CKPT}"
echo "[STAGE2-SHORT195S3] policy=adapter_train_then_final_test_only no_alpha_eval_sweep"
...
run_case() {
  ...
  echo "[STAGE2-SHORT195S3-CANDIDATE] id=${name} gpu=${gpu} protocol=Stage2-B k_old=10 multiproto=${multiproto}"
  local eval_common=(
    --protocol ftrc
  ...
  local no_reject_flags=(
    --unknown_threshold -1000000000
    --gate_mode none
  )
  local no_reject_multiproto_flags=()
  if [[ "${multiproto}" == "1" ]]; then
    no_reject_multiproto_flags=(
      --oa_mse_multiproto_score
      --multiproto_topk 5
      --multiproto_temperature 0.12
      --multiproto_score_weight 0.65
  ...
  local eval_no_reject_cmd=(
    "'${PYTHON}' -u '${ROOT}/code/eval_spaceborne_fewshot.py'"
    "--output_json '${out_dir}/metrics_no_reject.json'"
    "--manifest_json '${out_dir}/manifest_no_reject.json'"
    "--score_table_csv '${out_dir}/score_table_no_reject.csv'"
  )
  printf -v eval_common_s " %q" "${eval_common[@]}"
  printf -v reject_flags_s " %q" "${reject_flags[@]}"
  printf -v no_reject_flags_s " %q" "${no_reject_flags[@]}"
  local multiproto_flags_s=""
  local no_reject_multiproto_flags_s=""
  if (( ${#multiproto_flags[@]} > 0 )); then
    printf -v multiproto_flags_s " %q" "${multiproto_flags[@]}"
  fi
  if (( ${#no_reject_multiproto_flags[@]} > 0 )); then
    printf -v no_reject_multiproto_flags_s " %q" "${no_reject_multiproto_flags[@]}"
  fi

  local chained_no_reject=""
  if [[ "${RUN_NO_REJECT}" == "1" ]]; then
    chained_no_reject="; ${eval_no_reject_cmd[*]} ${eval_common_s} ${no_reject_flags_s} ${no_reject_multiproto_flags_s}"
  fi
...
'${PYTHON}' -u '${ROOT}/code/export_spaceborne_features.py' \
  ...
  --device cuda:0 \
  --seed '${seed}'; \
${eval_reject_cmd[*]} ${eval_common_s} ${reject_flags_s} ${multiproto_flags_s}${chained_no_reject}"
  )
  printf "[STAGE2-SHORT195S3-CMD] "
  printf "%q " "${cmd[@]}"
  printf "\n"
  if [[ "${DRY_RUN}" != "1" ]]; then
    "${cmd[@]}" > "${log_path}" 2>&1 &
    echo "[STAGE2-SHORT195S3-LAUNCHED] id=${name} pid=$! gpu=${gpu} log=${log_path}"
  fi
}

run_case "SHORT195S3_STAGE2B_FINALTEST_BASELINE_K10" "0" "362001" "0"
run_case "SHORT195S3_STAGE2B_FINALTEST_MULTIPROTO_K10" "1" "362001" "1"
