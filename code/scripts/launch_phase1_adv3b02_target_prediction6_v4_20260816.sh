...
# Fresh v4 identity for the unchanged six-fold blind-prediction implementation.
...
RUN_ID="phase1_adv3b02_target_prediction_20260816_v4"
SMOKE_ID=".smoke_phase1_adv3b02_target_prediction_20260816_v4_F1"
...
SMOKE_ENTRY="${CODE_ROOT}/smoke_phase1_adv3b02_target_prediction_f1_v4.py"
...
if [[ "${DRY_RUN}" == "0" ]]; then
  [[ -x "${PYTHON}" && -f "${SMOKE_ENTRY}" ]] || {
    echo "formal v4 requires the versioned F1 target-prediction smoke entry" >&2
    exit 2
  }
  [[ -f "${TRAIN_CONFIG}" && -f "${RECEIPT}" ]] || {
    echo "formal v4 requires a complete F1 target-prediction smoke receipt: ${RECEIPT}" >&2
    exit 2
  }
...
TRANSFORMED_LAUNCHER="$(mktemp "${TMPDIR:-/tmp}/adv-target-prediction-v4.XXXXXX.sh")"
...
grep -Fq "${RUN_ID}" "${TRANSFORMED_LAUNCHER}" || {
  echo "failed to render ADV target prediction v4 run identity" >&2
  exit 2
}
if grep -Fq "${V1_RUN_ID}" "${TRANSFORMED_LAUNCHER}"; then
  echo "stopped v1 ADV target prediction identity remains in v4 launcher" >&2
  exit 2
fi
