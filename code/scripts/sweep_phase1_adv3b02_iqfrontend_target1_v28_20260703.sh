...
LOG_ROOT="${LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_iqfrontend_target1_v28_20260703}"
...
if [[ ! -f "${SOURCE_CLEAN_NPZ}" ]]; then
  echo "[V28-MISSING] ${SOURCE_CLEAN_NPZ}" >&2
  exit 2
fi
...
declare -a VARIANTS=(
  "LEOIQ28_RAW_FEAT raw 32 0.18 50 0.0008 1.00 2.00 8.00 2.00 2.00 0.010 0.030"
  "LEOIQ28_RAW_CONSERVE raw 32 0.08 45 0.0007 0.80 1.60 12.00 1.50 3.00 0.005 0.080"
  "LEOIQ28_CANON_FEAT canonical 32 0.16 50 0.0008 1.00 2.00 10.00 2.00 2.00 0.010 0.040"
  "LEOIQ28_CANON_CONSERVE canonical 32 0.08 45 0.0007 0.80 1.60 14.00 1.50 3.00 0.005 0.100"
)
...
run_variant() {
  ...
  local gpu="${GPUS[$idx]}"
  read -r NAME INPUT_REPAIR HIDDEN ALPHA EPOCHS LR MSE_W COS_W CLEAN_W FEAT_MARGIN_W CLEAN_MARGIN_W MARGIN_TOL RES_W <<<"${row}"
  echo "[V28-VARIANT] name=${NAME} gpu=${gpu} input_repair=${INPUT_REPAIR} start=$(date -Is)"
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" \
    ...
    --identity_subdir "LEOIQ28_IDENTITY" \
  ...
  echo "[V28-VARIANT-DONE] name=${NAME} gpu=${gpu} end=$(date -Is)"
}

echo "[PHASE1-IQFRONTEND-TARGET1-V28] start=$(date -Is) gpus=${GPUS_CSV} seed=${SEED}"
pids=()
...
if [[ "${failed}" != "0" ]]; then
  echo "[V28-FAILED] one or more variants failed" >&2
  exit 3
fi

VARIANT_NAMES="$(printf '%s\n' "${VARIANTS[@]}" | awk '{print $1}' | paste -sd, -)"
SUMMARY_CSV="${LOG_ROOT}/target1_strong_v28_summary.csv"
METRICS_JSON="${LOG_ROOT}/target1_strong_v28_metrics.json"
env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
  ...
  --clean_relpath "LEOIQ28_IDENTITY/features_clean_repaired.npz" \
  --identity_relpath "LEOIQ28_IDENTITY/features_leo_repaired.npz" \
  --feature_relpath "{variant}/features_leo_repaired.npz" \
  --clean_repaired_relpath "{variant}/features_clean_repaired.npz" \
  --variants "${VARIANT_NAMES}" \
  > "${LOG_ROOT}/target1_strong_v28_eval.out" 2>&1

"${PYTHON}" - "${SUMMARY_CSV}" "${LOG_ROOT}/target1_strong_v28_best.json" <<'PY'
import csv
import json
import math
import sys
...
echo "[PHASE1-IQFRONTEND-TARGET1-V28-DONE] end=$(date -Is) summary=${SUMMARY_CSV}"
