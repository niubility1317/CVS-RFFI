#!/usr/bin/env python3
"""Truth-last scorer for an M2.8 B0/B3/C1/C2 paired matrix."""
...
from scripts.run_m28_local_flip_risk_matrix import EVIDENCE_ARMS, matrix_spec
from scripts.score_m24_safe_residual_suite import _legacy_scorer_receipts


SCORED_MATRIX_SCHEMA = "cvs.erbt_idr.m28.local_flip_risk_scored_matrix.v1"
...
def _validate_matrix(matrix: dict[str, Any]) -> None:
    try:
        ...
        raise ValueError("M2.8 prediction matrix is incomplete") from error
    ...
    for entry in entries:
        ...
        if (
            ...
            raise ValueError("M2.8 prediction matrix identity drift")
        row_ids.add(row_id)
    ...
    if (
        ...
        raise ValueError("M2.8 prediction matrix is incomplete")
...
def main() -> int:
    ...
    for key, group in groups.items():
        if set(group) != set(EVIDENCE_ARMS):
            raise ValueError(f"M2.8 paired group incomplete: {key}")
        for arm in EVIDENCE_ARMS[1:]:
            ...
            write_row_record_exclusive(
                forgetting_path,
                {
                    "schema": "cvs.erbt_idr.m28.standardized_forgetting.v1",
                    "reference_arm": D1,
                    "candidate_arm": arm,
