...
LOG_ROOT="${LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_hardtx_fullfeature_target1_v31_20260703}"
...
if [[ ! -f "${SOURCE_CLEAN_NPZ}" ]]; then
  echo "[V31-MISSING] ${SOURCE_CLEAN_NPZ}" >&2
  exit 2
fi
...
declare -a VARIANTS=(
  "LEOFEAT31_FULL_HARD_BAL id_full_feature 45 0.000035 0.80 1.60 22.00 4.00 7.00 0.003 0.30 0.20 0.15 0.30 1.4,2.6,2.2,2.6,0.8,0.8"
  "LEOFEAT31_FULL_HARD_FLOOR id_full_feature 50 0.000030 0.70 1.40 24.00 5.50 8.00 0.002 0.45 0.12 0.18 0.20 1.6,3.0,2.4,3.0,0.7,0.7"
  "LEOFEAT31_FULL_HARD_USEP id_full_feature 45 0.000030 0.80 1.60 24.00 4.00 8.00 0.003 0.25 0.35 0.12 0.30 1.4,2.6,2.2,2.6,0.8,0.8"
  "LEOFEAT31_FULL_HARD_SAFE id_full_feature 40 0.000020 0.60 1.20 30.00 3.00 10.00 0.002 0.20 0.15 0.18 0.50 1.3,2.4,2.0,2.4,0.8,0.8"
  "LEOFEAT31_LATE_HARD_BAL id_late_feature 70 0.00010 0.90 1.80 18.00 4.50 6.50 0.003 0.35 0.20 0.15 0.10 1.5,2.8,2.3,2.8,0.8,0.8"
  "LEOFEAT31_LATE_HARD_USEP id_late_feature 70 0.00009 0.90 1.80 20.00 4.00 7.00 0.003 0.30 0.40 0.12 0.10 1.5,2.8,2.3,2.8,0.8,0.8"
  "LEOFEAT31_NORM_HARD_BAL id_norm_late_feature 65 0.00008 0.80 1.60 20.00 4.00 7.50 0.003 0.30 0.20 0.15 0.30 1.5,2.8,2.3,2.8,0.8,0.8"
  "LEOFEAT31_NORM_HARD_SAFE id_norm_late_feature 60 0.00006 0.70 1.40 26.00 3.00 9.00 0.002 0.22 0.18 0.18 0.50 1.4,2.6,2.1,2.6,0.8,0.8"
)

if (( ${#GPUS[@]} < ${#VARIANTS[@]} )); then
  echo "[V31-GPU-COUNT] need ${#VARIANTS[@]} GPUs but got ${#GPUS[@]} from ${GPUS_CSV}" >&2
  exit 2
fi
...
run_variant() {
  ...
  local gpu="${GPUS[$idx]}"
  read -r NAME MODE EPOCHS LR MSE_W COS_W CLEAN_W FEAT_MARGIN_W CLEAN_MARGIN_W MARGIN_TOL PROTO_CE_W PROXY_SEP_W PROXY_MAX_COS DISTILL_W CLASS_W <<<"${row}"
  echo "[V31-VARIANT] name=${NAME} mode=${MODE} gpu=${gpu} start=$(date -Is)"
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" \
    ...
    --identity_subdir "LEOFEAT31_IDENTITY" \
  ...
  echo "[V31-VARIANT-DONE] name=${NAME} mode=${MODE} gpu=${gpu} end=$(date -Is)"
}

echo "[PHASE1-HARDTX-FULLFEATURE-TARGET1-V31] start=$(date -Is) gpus=${GPUS_CSV} seed=${SEED}"
pids=()
...
if [[ "${failed}" != "0" ]]; then
  echo "[V31-FAILED] one or more variants failed" >&2
  exit 3
fi

VARIANT_NAMES="$(printf '%s\n' "${VARIANTS[@]}" | awk '{print $1}' | paste -sd, -)"
SUMMARY_CSV="${LOG_ROOT}/target1_strong_v31_summary.csv"
METRICS_JSON="${LOG_ROOT}/target1_strong_v31_metrics.json"
env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
  ...
  --clean_relpath "LEOFEAT31_IDENTITY/features_clean_repaired.npz" \
  --identity_relpath "LEOFEAT31_IDENTITY/features_leo_repaired.npz" \
  --feature_relpath "{variant}/features_leo_repaired.npz" \
  --clean_repaired_relpath "{variant}/features_clean_repaired.npz" \
  --variants "${VARIANT_NAMES}" \
  > "${LOG_ROOT}/target1_strong_v31_eval.out" 2>&1

"${PYTHON}" - "${SUMMARY_CSV}" "${LOG_ROOT}/target1_strong_v31_best.json" <<'PY'
import csv
import json
import math
import sys
...
echo "[PHASE1-HARDTX-FULLFEATURE-TARGET1-V31-DONE] end=$(date -Is) summary=${SUMMARY_CSV}"
