...
RUN_ID="phase1_clic_g_bundles_20260812_v1"
...
emit_command() {
  local fold="$1"; shift
  printf '[DRY-RUN] stage=CLIC_G_BUNDLE fold=%s arm=G' "${fold}"
  printf ' %q' "$@"
  printf '\n'
}
...
[[ ! -e "${RUN_ROOT}" && ! -e "${LOG_ROOT}" ]] || { echo "refusing to overwrite G bundle run/log root" >&2; exit 3; }
check_inputs
...
printf 'pid|fold|stage|log_path\n' >"${LOG_ROOT}/pids_g_bundles6.tsv"
for index in "${!pids[@]}"; do
  printf '%s|%s|CLIC_G_BUNDLE|%s\n' \
