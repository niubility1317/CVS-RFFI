#!/usr/bin/env python3
"""Create the full D51 performance ledger from complete matched logs."""
...
SPEC = importlib.util.spec_from_file_location("d51_summary_helper", HELPER_PATH)
    ...
    for phase in ("before", "final"):
        ...
        result[phase] = {
            "resultant": base.stats(
                value for audit in audits for value in audit["d51_resultant_norm_by_class"]
            ),
            "gamma": base.stats(
                value for audit in audits for value in audit["d51_gamma_by_class"]
            ),
            "residual_rms": base.stats(
                audit["d51_residual_support_logit_rms"] for audit in audits
            ),
            "residual_direction_l2": base.stats(
                sum(value * value for value in vector) ** 0.5
                for audit in audits
                for vector in audit["d51_residual_direction_fp64"]
            ),
            "correction_l2": base.stats(
                sum(value * value for value in vector) ** 0.5
                for audit in audits
                for vector in audit["d51_coefficient_correction_fp64"]
            ),
            "row_norm_error": base.stats(
                audit["d51_support_row_norm_max_abs_error"] for audit in audits
            ),
        }
    result["by_scene"] = {}
...
def resources(rows: list[dict[str, Any]]) -> dict[str, Any]:
    keys = (
        ...
        "d51_extra_adaptation_mac_equivalents",
        "d51_coordinate_median_comparison_upper_bound",
    ...
    result["invariants"] = {
        ...
        for key in (
            ...
            "d51_resource_reuses_d45_exact_inventory",
        )
    }
    return result
...
def main() -> None:
    ...
    output = {
        ...
        "all_candidates": base.candidate_summary(logs["D51"]),
        "D51_INT8": {
            "aggregate": base.aggregate(target["D51"]),
            "by_scene": {
                scene: base.aggregate(group)
                for scene, group in base.scene_groups(target["D51"]).items()
            },
            "classes": base.class_summary(target["D51"]),
            "outer_rows": base.detailed_rows(target["D51"]),
            "mechanism": mechanism(target["D51"]),
            "training": base.training_summary(target["D51"]),
            "quantization": helper.quantization_summary(target["D51"]),
            "resources": resources(target["D51"]),
        },
        "D51_FP32_MATCHED": {
        ...
        "D51_vs_D45": base.matched_delta(target["D51"], target["D45"]),
        "D51_vs_D46": base.matched_delta(target["D51"], target["D46"]),
    }
