#!/usr/bin/env python3
"""Run D113 BCAT M_DA on the pinned 588-row tap without opening truth."""
...
from cvsrffi.stage2_d113_bcat_qknn import (  # noqa: E402
    bcat_inverse,
    fit_d113_state,
    score_d113_da_logits,
)
from cvsrffi.stage2_d113_g0_source_bundle import (  # noqa: E402
    build_d113_g0_source_bundle,
)
...
base = d111.base
SCHEMA = "cvs.phase1.d113.bcat_g0.one_shot.v1"
STATUS = "REAL_ARCHIVE_G0_EXECUTED_NON_FORMAL_FUNCTIONAL_EVIDENCE"
CANDIDATE_ID = "D113_BCAT_QKNN_M_DA"


class D113G0Error(ValueError):
    """Raised when the one-shot path drifts from the frozen G0 semantics."""
...
def _fold_audit(
    ...
    *,
    ...
    if not np.array_equal(baseline_logits, direct_baseline):
        raise D113G0Error("D113 G0 baseline qKNN path drift")
    state = fit_d113_state(bundle, bank)
    candidate_features = bcat_inverse(query, state.b).astype(np.float32)
    candidate_logits = score_d113_da_logits(state, bank, query)
    ...
    feature = base._array_changed_metric(
        metric="bcat_canonical_feature_binary32",
        query_root_sha256=query_root,
        baseline=np.asarray(baseline_features, dtype=np.float32).view(np.uint32),
        candidate=candidate_features.view(np.uint32),
    )
    ...
    payload = {
        ...
        "feature_changed_count": int(feature["changed_count"]),
        ...
        "candidate_score_root_sha256": base.g0._array_root(
...
def _aggregate(
    ...
    if len(audits) != base.g0.EXPECTED_FOLDS:
        raise D113G0Error("D113 G0 fold count drift")
    result: dict[str, Any] = {
        "K": active_k,
    ...
    for name in ("feature", "score", "margin", "argmax"):
        result[f"{name}_changed_count"] = sum(
            int(item[f"{name}_changed_count"]) for item in audits
        )
    result["positive_rho_count"] = sum(
        int(item["positive_rho_count"]) for item in audits
    )
    result["b_nonzero_fold_count"] = sum(float(item["b_norm"]) > 0.0 for item in audits)
    result["max_b_norm"] = max(float(item["b_norm"]) for item in audits)
    result["max_delta_norm"] = max(float(item["delta_norm"]) for item in audits)
    result["max_rho"] = max(float(item["max_rho"]) for item in audits)
    result["max_v_shift"] = max(float(item["max_v_shift"]) for item in audits)
    ...
    result["functional_nonzero"] = bool(
        result["feature_changed_count"] > 0
...
def _decision(per_k: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    zero_argmax = [int(item["K"]) for item in per_k if int(item["argmax_changed_count"]) == 0]
    if zero_argmax:
        status = "REJECT_REVISION_NO_FUNCTION"
    else:
        status = "G0_ALL_K_ARGMAX_NONZERO_PROCEED_G1"
    return {
        "zero_argmax_k_values": zero_argmax,
        "functional_status": status,
        "g1_entry_allowed": not zero_argmax,
    }
...
def run_one_shot(
    *,
    ...
    if type(run_id) is not str or not run_id or len(run_id.encode("utf-8")) > 160:
        raise D113G0Error("run ID must be a short non-empty string")
    ...
    bundle = build_d113_g0_source_bundle(
        archive_path,
    ...
    if len(query_order) != base.g0.EXPECTED_ROWS or len(set(query_order)) != base.g0.EXPECTED_ROWS:
        raise D113G0Error("D113 G0 common query closure drift")
