...
LOG_ROOT="${LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_modeladapter_target1_v29_20260703}"
...
if [[ ! -f "${SOURCE_CLEAN_NPZ}" ]]; then
  echo "[V29-MISSING] ${SOURCE_CLEAN_NPZ}" >&2
  exit 2
fi
...
declare -a VARIANTS=(
  "LEOMODEL29_HEAD_FEAT id_feature_head 70 0.00025 1.00 2.00 10.00 2.00 3.00 0.005 0.00"
  "LEOMODEL29_HEAD_DISTILL id_feature_head 70 0.00020 1.00 2.00 12.00 2.00 3.50 0.005 0.50"
  "LEOMODEL29_LATE_FEAT id_late_feature 60 0.00015 0.90 1.80 12.00 1.50 4.00 0.005 0.00"
  "LEOMODEL29_NORM_LATE_CONSERVE id_norm_late_feature 50 0.00010 0.70 1.40 16.00 1.00 5.00 0.003 0.30"
)
...
run_variant() {
  ...
  local gpu="${GPUS[$idx]}"
  read -r NAME MODE EPOCHS LR MSE_W COS_W CLEAN_W FEAT_MARGIN_W CLEAN_MARGIN_W MARGIN_TOL DISTILL_W <<<"${row}"
  echo "[V29-VARIANT] name=${NAME} mode=${MODE} gpu=${gpu} start=$(date -Is)"
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" \
    ...
    --identity_subdir "LEOMODEL29_IDENTITY" \
  ...
  echo "[V29-VARIANT-DONE] name=${NAME} mode=${MODE} gpu=${gpu} end=$(date -Is)"
}

echo "[PHASE1-MODELADAPTER-TARGET1-V29] start=$(date -Is) gpus=${GPUS_CSV} seed=${SEED}"
pids=()
...
if [[ "${failed}" != "0" ]]; then
  echo "[V29-FAILED] one or more variants failed" >&2
  exit 3
fi

VARIANT_NAMES="$(printf '%s\n' "${VARIANTS[@]}" | awk '{print $1}' | paste -sd, -)"
SUMMARY_CSV="${LOG_ROOT}/target1_strong_v29_summary.csv"
METRICS_JSON="${LOG_ROOT}/target1_strong_v29_metrics.json"
env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
  ...
  --clean_relpath "LEOMODEL29_IDENTITY/features_clean_repaired.npz" \
  --identity_relpath "LEOMODEL29_IDENTITY/features_leo_repaired.npz" \
  --feature_relpath "{variant}/features_leo_repaired.npz" \
  --clean_repaired_relpath "{variant}/features_clean_repaired.npz" \
  --variants "${VARIANT_NAMES}" \
  > "${LOG_ROOT}/target1_strong_v29_eval.out" 2>&1

"${PYTHON}" - "${SUMMARY_CSV}" "${LOG_ROOT}/target1_strong_v29_best.json" <<'PY'
import csv
import json
import math
import sys
...
echo "[PHASE1-MODELADAPTER-TARGET1-V29-DONE] end=$(date -Is) summary=${SUMMARY_CSV}"
