"""Bounded user-requested cancellation of B3a/B3b, preserving all artifacts."""
import argparse
import datetime
import json
import os
...
def main():
    ...
    expected={3537463:'B3a',3547432:'B3b'}
    ...
    for pid,row in expected.items():
        old=parents[pid]
        assert old['ppid']==3537313
        assert old['argv'][old['argv'].index('--run_name')+1]==row
        assert any(v.endswith('/phase1_ecrs_v2_bridge_s392005_e200_20260908_r2/'+row) for v in old['argv'])
    for old in targets:
        now=current(old['pid'])
