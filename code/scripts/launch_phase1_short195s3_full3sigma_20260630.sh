...
RUN_ID="${RUN_ID:-phase1_short195s3_full3sigma_20260630}"
...
wait_for_gpu_slot() {
  ...
  while true; do
    ...
    echo "[PHASE1-FULL3S-WAIT] gpu=${gpu} active=${active} max=${STAGE2_MAX_ACTIVE_PER_GPU}"
    sleep 60
  done
}
...
launch_candidate() {
  ...
  local fuse_max_components="${13}"
  local fuse_merge_angle="${14}"
  local fuse_tail_abs="${15}"

  echo "[PHASE1-FULL3S-CANDIDATE] id=${cid} base=PHASE1_GPU0_JOINTSAFE36_SOFTPSEUDO_190X10_SHORT195_S3 protocol=Safe-SSDG-CVS-R01 target_visibility=source_only_ground_training_no_target_receiver"
  CMD=(env PYTHONPATH="${ROOT}/code:${ROOT}:${PYTHONPATH:-}" CUDA_VISIBLE_DEVICES="${gpu}" "${PYTHON}" -u "${ROOT}/code/SSDG/train_ssdg.py"
    ...
    --device cuda:0
    --seed "${seed}")
  printf "[PHASE1-FULL3S-CMD] "; printf "%q " "${CMD[@]}"; printf "\n"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return
  fi
  ...
  echo "[PHASE1-FULL3S-LAUNCHED] id=${cid} pid=$! gpu=${gpu} log=${LOG_ROOT}/${cid}.out"
}
...
echo "[PHASE1-FULL3S] run_id=${RUN_ID} dry_run=${DRY_RUN} base=PHASE1_GPU0_JOINTSAFE36_SOFTPSEUDO_190X10_SHORT195_S3 candidates=14 mode=all_joint_full3sigma two_per_gpu_no_gpu0"
launch_candidate "FULL3S_C0_CONSERVE_E200" 1 361003 200 190 10 0.003 0.0020 0.15 0.000 0.20 0.002 4 6 30
launch_candidate "FULL3S_C1_SATSAFE_E220" 2 361013 220 205 15 0.003 0.0015 0.10 0.000 0.15 0.003 4 6 30
launch_candidate "FULL3S_C2_PROTO_MID_E200" 3 361023 200 190 10 0.005 0.0020 0.25 0.010 0.25 0.003 4 6 30
launch_candidate "FULL3S_C3_OW_WARM_E220" 4 361033 220 205 15 0.004 0.0030 0.20 0.010 0.20 0.003 4 6 30
launch_candidate "FULL3S_C4_DOMAIN_E200" 5 361043 200 190 10 0.004 0.0020 0.35 0.020 0.20 0.004 4 6 30
launch_candidate "FULL3S_C5_C3REF_E220" 6 361053 220 205 15 0.004 0.0025 0.25 0.005 0.20 0.003 4 6 30
launch_candidate "FULL3S_C6_STRESS_NEG_E180" 7 361063 180 170 10 0.010 0.0100 0.50 0.040 0.50 0.008 4 6 30
launch_candidate "FULL3S_C7_CONSERVE_FUSE3_E200" 1 361103 200 190 10 0.003 0.0018 0.12 0.000 0.15 0.002 3 8 28
launch_candidate "FULL3S_C8_SATSAFE_FUSE3_E220" 2 361113 220 205 15 0.003 0.0012 0.08 0.000 0.12 0.003 3 8 28
launch_candidate "FULL3S_C9_PROTO_FUSE2_E200" 3 361123 200 190 10 0.004 0.0020 0.20 0.005 0.20 0.003 2 8 28
launch_candidate "FULL3S_C10_OWTAIL_E220" 4 361133 220 205 15 0.004 0.0025 0.18 0.005 0.30 0.003 3 8 28
launch_candidate "FULL3S_C11_DOMAIN_LITE_E200" 5 361143 200 190 10 0.004 0.0018 0.25 0.010 0.18 0.004 3 8 28
launch_candidate "FULL3S_C12_C5REF_FUSE3_E220" 6 361153 220 205 15 0.004 0.0022 0.20 0.005 0.18 0.003 3 8 28
launch_candidate "FULL3S_C13_STRESS2_E180" 7 361163 180 170 10 0.008 0.0080 0.40 0.030 0.40 0.006 3 8 28
echo "[PHASE1-FULL3S-SUBMIT-COMPLETE] run_id=${RUN_ID}"
