#!/usr/bin/env python3
"""Prepare, smoke-test, and execute the frozen QIC Hard9+K1 matrix."""
...
from cvsrffi.stage2_d92_qic_hard9_k1 import (  # noqa: E402
    ARM_ID,
...
_SCORE_BINDING_SCHEMA = "cvs.phase2.d92_qic_hard9_k1.score_binding.v1"
_ACTIVE_PROCESS_SCHEMA = "cvs.phase2.d92_qic_hard9_k1.active_process.v1"
_ACTIVE_PROCESS_ACTION_SCHEMA = "cvs.phase2.d92_qic_hard9_k1.stop_action.v1"
...
class D92QICHard9K1RunnerError(RuntimeError):
    """Raised when a direct QIC Hard9+K1 runner boundary drifts."""
...
def _read_json_object(path: str | Path, *, label: str) -> dict[str, Any]:
    ...
    if not source.is_file() or source.is_symlink():
        raise D92QICHard9K1RunnerError(f"{label} is missing: {source}")
    try:
        payload = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise D92QICHard9K1RunnerError(f"{label} is invalid: {source}") from error
    if not isinstance(payload, dict):
        raise D92QICHard9K1RunnerError(f"{label} must be an object: {source}")
    return payload


def _finite(value: Any, label: str, *, lower: float | None = None) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise D92QICHard9K1RunnerError(f"{label} is not finite")
    result = float(value)
    if not math.isfinite(result) or (lower is not None and result < lower):
        raise D92QICHard9K1RunnerError(f"{label} is below bound")
    return result
...
def _integer(value: Any, label: str, *, lower: int = 0) -> int:
    ...
    if result != float(int(result)):
        raise D92QICHard9K1RunnerError(f"{label} is not an integer")
    return int(result)
...
def _resource_value(row: Mapping[str, Any], field: str) -> float:
    ...
    if not isinstance(resource, Mapping):
        raise D92QICHard9K1RunnerError("fit audit resource receipt missing")
...
def _query_access_is_zero(row: Mapping[str, Any]) -> bool:
    for field in QUERY_ZERO_FIELDS:
        for candidate in (field, f"d92_e0d_qic_{field}"):
...
def _require_qic_lifecycle(row: Mapping[str, Any], *, k_shot: int) -> None:
    active = int(k_shot) > 2
    prefix = "d92_e0d_qic_"
    expected_postprocess = (
        "d42_quantization_intercept_closure" if active else None
    )
    if row.get("after_state_postprocess_mode") != expected_postprocess:
        raise D92QICHard9K1RunnerError("fit audit postprocess drift")
    inventory = row.get("after_actual_component_inventory")
    if not isinstance(inventory, Mapping):
        raise D92QICHard9K1RunnerError("fit audit component inventory missing")
    ...
    if active:
        if (total, actual, mode) != (
            ...
            raise D92QICHard9K1RunnerError("fit audit QIC K>2 inventory drift")
        if (
        ...
        if (
            row.get(prefix + "modified_state_field_names") != ["intercept_fp16"]
            or row.get(prefix + "intercept_byte_exact") is not False
            or any(row.get(prefix + name) is not True for name in _QIC_BYTE_EXACT)
            or _integer(row.get(prefix + "coefficient_decode_count"), "QIC decode count")
            != 1
            or any(
                _integer(row.get(prefix + name), f"QIC {name}") != 0
            ...
            or _integer(
                row.get(prefix + "persistent_state_bytes_delta"),
                "QIC state delta",
            )
            != 0
            or _integer(row.get(prefix + "query_macs_delta"), "QIC query MAC delta")
            != 0
        ):
            raise D92QICHard9K1RunnerError("fit audit QIC state/resource lifecycle drift")
        e0_state = row.get(prefix + "e0_state_sha256")
        final_state = row.get(prefix + "final_state_sha256")
        if (
            not _is_sha256(e0_state)
            or not _is_sha256(final_state)
            or e0_state == final_state
            or _integer(
                row.get(prefix + "intercept_fp16_bit_change_count"),
                "QIC intercept bit change",
            )
            <= 0
            or _integer(
                row.get(prefix + "candidate_intercept_fp16_bit_change_count"),
            ...
            raise D92QICHard9K1RunnerError("fit audit QIC intercept transition drift")
        e0_residual = _finite(row.get(prefix + "e0_residual_l1"), "QIC E0 residual", lower=0.0)
        candidate_residual = _finite(
            row.get(prefix + "candidate_residual_l1"),
            "QIC candidate residual",
            lower=0.0,
        )
        reduction = _finite(
            row.get(prefix + "residual_reduction_l1"),
            "QIC residual reduction",
            lower=0.0,
        )
        if (
            not candidate_residual < e0_residual
            or reduction <= 0.0
            or reduction != e0_residual - candidate_residual
        ):
            raise D92QICHard9K1RunnerError("fit audit QIC residual drift")
        return
    if (total, actual, mode) != (
        FIT_GATE["k1_total"],
        FIT_GATE["k1_actual"],
        ...
        raise D92QICHard9K1RunnerError("fit audit K1 alias inventory drift")
    if (
        ...
        != 0
        or any(
            row.get(prefix + name) is not None
            for name in (
                "e0_residual_l1",
        ...
        raise D92QICHard9K1RunnerError("fit audit K1 alias lifecycle drift")
...
def _validate_fit_audit(
    ...
    *,
    ...
    """Close QIC support/resource gates without opening query truth.
    ...
    source = Path(path)
    if not source.is_file() or source.is_symlink():
        raise D92QICHard9K1RunnerError(f"fit audit is missing: {source}")
    try:
        rows = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise D92QICHard9K1RunnerError(f"fit audit is invalid: {source}") from error
    if (
        not isinstance(rows, list)
        or len(rows) != len(SCENES)
        or any(not isinstance(row, Mapping) for row in rows)
    ):
        raise D92QICHard9K1RunnerError("fit audit scene closure drift")
    by_scene = {str(row.get("scenario")): row for row in rows}
    if len(by_scene) != len(SCENES) or set(by_scene) != set(SCENES):
        raise D92QICHard9K1RunnerError("fit audit scene identity drift")
    if set(reference_resources) != set(SCENES):
        raise D92QICHard9K1RunnerError("reference resource scene identity drift")
    ...
    for scene in SCENES:
        ...
        if row.get("arm_id") != ARM_ID or row.get("candidate_id") != CANDIDATE_ID:
            raise D92QICHard9K1RunnerError("fit audit arm/candidate identity drift")
        if not _query_access_is_zero(row):
            raise D92QICHard9K1RunnerError("fit audit query access is not zero")
        _require_qic_lifecycle(row, k_shot=k_shot)
        ...
        if candidate_macs != class_count * 288 or candidate_macs != reference_macs:
            raise D92QICHard9K1RunnerError("fit audit query MAC drift")
        if candidate_state != reference_state:
            raise D92QICHard9K1RunnerError("fit audit state drift")
...
def _verify_manifest_artifacts(manifest: Mapping[str, Any]) -> None:
    ...
    for job in manifest.get("jobs", []):
        if not isinstance(job, Mapping):
            raise D92QICHard9K1RunnerError("manifest job is invalid")
        ...
        if (
            ...
            raise D92QICHard9K1RunnerError("truth sidecar metadata drift")
        packages = job.get("packages")
        if not isinstance(packages, Mapping):
            raise D92QICHard9K1RunnerError("package manifest is missing")
        for name, item in packages.items():
            if not isinstance(item, Mapping):
                raise D92QICHard9K1RunnerError(f"package entry is invalid: {name}")
            ...
            if (
                ...
                raise D92QICHard9K1RunnerError(f"package seal SHA drift: {name}")
...
def _verify_runtime_source_files(
    ...
    *,
    ...
    if not isinstance(runtime_source, Mapping) or set(runtime_source) != {
        ...
        raise D92QICHard9K1RunnerError("runtime source lock schema drift")
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("runtime source lock identity drift")
    ...
    if git_runner is not None:
        ...
        except D92QICHard9K1RunnerError as error:
            raise D92QICHard9K1RunnerError(
    ...
    for relative_path, record in files.items():
        if (
            ...
            raise D92QICHard9K1RunnerError("runtime source record drift")
...
def _verify_runtime_source_lock(
    ...
    *,
    ...
    if not isinstance(lock, Mapping):
        raise D92QICHard9K1RunnerError("runtime source method-lock drift")
...
def _verify_truth_sidecar_snapshot(
    ...
    *,
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("truth sidecar snapshot/hash drift")
    return expected
...
def _prediction_closure_hashes(
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("prediction closure hash path drift")
    evidence_paths = {
    ...
    if tuple(evidence_paths) != _PREDICTION_CLOSURE_SHA_FIELDS:
        raise D92QICHard9K1RunnerError("prediction closure hash schema drift")
...
def _validate_score_artifact(
    ...
    *,
    ...
    if not score_path.is_file() or score_path.is_symlink():
        raise D92QICHard9K1RunnerError("score artifact is missing")
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("score artifact input/identity drift")
    if score_binding_path is not None:
        ...
        if (
            ...
            raise D92QICHard9K1RunnerError("score binding identity drift")
        if any(
            str(binding.get(field, "")).lower() != expected
            for field, expected in closure_hashes.items()
        ):
            raise D92QICHard9K1RunnerError("score binding closure hash drift")
    return {
        **identity,
...
def _write_job_receipt(
    ...
    *,
    ...
    if root.is_symlink() or not root.is_dir():
        raise D92QICHard9K1RunnerError("job receipt root drift")
    if set(closure_hashes) != set(_PREDICTION_CLOSURE_SHA_FIELDS) or any(
        ...
        raise D92QICHard9K1RunnerError("job receipt closure hash drift")
    normalized_hashes = {
    ...
    if any(
        ...
        raise D92QICHard9K1RunnerError("job receipt closure overwrite risk")
    ...
    if not isinstance(evidence, Mapping) or any(
        ...
        raise D92QICHard9K1RunnerError("job receipt score evidence drift")
...
def _load_manifest(path: str | Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("matrix manifest must be immutable regular file")
...
def _load_verified_e0_resource_records(
    ...
    try:
        ...
        raise D92QICHard9K1RunnerError("E0 resource job identity drift") from error
    resource = job.get("e0_resource")
    if not isinstance(resource, Mapping) or set(resource) != {"fit_audit", "scenes"}:
        raise D92QICHard9K1RunnerError("E0 resource manifest record drift")
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("E0 resource sealed record drift")
    if not isinstance(declared_scenes, Mapping) or set(declared_scenes) != set(
        SCENES
    ):
        raise D92QICHard9K1RunnerError("E0 resource scene identity drift")
    if not outer_key.endswith(f"__k_{k_shot}__new_{new_class_count}"):
        raise D92QICHard9K1RunnerError("E0 resource job identity drift")
    ...
    for scene in SCENES:
        ...
        if not isinstance(values, Mapping) or set(values) != {
            ...
            raise D92QICHard9K1RunnerError("E0 resource scene field drift")
        actual_scene = {
        ...
        if actual_scene["registration_wall_time_ns"] <= 0:
            raise D92QICHard9K1RunnerError("E0 resource wall is zero")
        if actual_scene["query_macs"] <= 0 or actual_scene["state_bytes"] <= 0:
            raise D92QICHard9K1RunnerError("E0 resource query/state drift")
        if actual_scene["query_macs"] != (6 + new_class_count) * 288:
            raise D92QICHard9K1RunnerError("E0 resource query MAC identity drift")
...
def _bind_e0_resource_observations(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(jobs, list):
        raise D92QICHard9K1RunnerError("E0 resource manifest jobs drift")
    declared: dict[str, str] = {}
    for job in jobs:
        if not isinstance(job, dict):
            raise D92QICHard9K1RunnerError("E0 resource manifest job drift")
...
def _stop_score_dispatch_lock(
    ...
    *,
    ...
    if not owner_id or purpose not in {
        ...
        raise D92QICHard9K1RunnerError("stop/score coordination identity drift")
...
def _write_active_process_receipt(
    ...
    *,
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("active-process receipt identity drift")
...
def _read_active_process_receipt(path: Path, *, output_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("active-process receipt contract drift")
    return receipt
...
def _send_posix_signal(pid: int, signal_name: str) -> None:
    if os.name != "posix":
        raise D92QICHard9K1RunnerError("coordinator stop is unsupported on this host")
    selected = {"SIGTERM": signal.SIGTERM, "SIGKILL": signal.SIGKILL}.get(signal_name)
    if selected is None:
        raise D92QICHard9K1RunnerError("coordinator signal identity drift")
...
def _acquire_stop_coordinator(output_root: Path, coordinator_id: str) -> None:
    ...
    try:
        ...
        if (
            ...
            raise D92QICHard9K1RunnerError("stop coordinator ownership drift")
...
def stop_verified_active_processes(
    ...
    *,
    ...
    if not coordinator_id or not _shared_stop_path(root).is_file():
        raise D92QICHard9K1RunnerError("systemic stop/coordinator identity drift")
    with _stop_score_dispatch_lock(
        root,
        coordinator_id=str(coordinator_id),
        purpose="stop_action",
...
def _smoke_job(manifest: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if len(matches) != 1:
        raise D92QICHard9K1RunnerError("K>2 QIC smoke row identity drift")
    return matches[0]
...
def _validate_shared_smoke(
    ...
    *,
    ...
    if int(manifest.get("job_count", -1)) != 10 or len(manifest.get("jobs", [])) != 10:
        raise D92QICHard9K1RunnerError("Hard9+K1 matrix identity drift")
    ...
    identity = (
        receipt.get("schema") == "cvs.phase2.d92_qic_hard9_k1.smoke_receipt.v1"
        and receipt.get("status")
        == "D92_QIC_HARD9_K1_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS"
    ...
    if not identity:
        raise D92QICHard9K1RunnerError("smoke receipt identity/protocol drift")
    ...
    if receipt.get("command") != expected_command:
        raise D92QICHard9K1RunnerError("smoke command identity drift")
    if _prediction_closure_status(prediction_root) != ("closed", "closed"):
        raise D92QICHard9K1RunnerError("smoke prediction closure drift")
    ...
    if (
        ...
        raise D92QICHard9K1RunnerError("smoke resource gate receipt drift")
...
def smoke(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if output_root.exists() or output_root.is_symlink():
        raise D92QICHard9K1RunnerError("smoke output identity/immutability drift")
    output_root.mkdir(parents=True)
    ...
    receipt: dict[str, Any] = {
        "schema": "cvs.phase2.d92_qic_hard9_k1.smoke_receipt.v1",
        "status": "D92_QIC_HARD9_K1_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS",
...
def run_shard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if int(args.shard_count) != SHARD_COUNT or int(args.shard_index) not in range(
        ...
        raise D92QICHard9K1RunnerError("shard identity drift")
    ...
    if not selected or events_path.exists() or summary_path.exists():
        raise D92QICHard9K1RunnerError("shard selection/evidence identity drift")
    ...
    for job in selected:
        ...
        if prediction_child is None:  # pragma: no cover - guarded by prediction_started
            raise D92QICHard9K1RunnerError("prediction start value drift")
        ...
        if score_value is None:  # pragma: no cover - guarded by score_started
            raise D92QICHard9K1RunnerError("score start value drift")
...
def main() -> int:
    ...
    except (D92QICHard9K1RunnerError, D92QICHard9K1Error, ValueError) as error:
        print(f"D92 QIC Hard9+K1 failed: {error}", file=sys.stderr)
        return 2
    print(json.dumps(value, ensure_ascii=True, sort_keys=True))
    return 0 if value["status"] in {
        "QIC_HARD9_K1_MATRIX_PREPARED",
        "D92_QIC_HARD9_K1_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS",
        "PASS",
