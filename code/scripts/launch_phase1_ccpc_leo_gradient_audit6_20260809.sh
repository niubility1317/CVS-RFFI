...
# One immutable technical-health entry: six G arms only, no C/G performance
# comparison and no postfreeze evaluator.  It tests whether the CCPC branch
# has a finite *unscaled* feature gradient under the same frozen continuation
# ingredients that were used by CCPC-LEO12.
RUN_ID="${RUN_ID:-phase1_ccpc_leo_gradient_audit6_20260809}"
...
# Fixed 4/1/1 source-only roles inherited from the GeoSat-C matrix.  The
# validation/proxy IDs are declarations that train_ssdg excludes from fitting.
FOLD_TRAIN_TX=(
...
# This is deliberately the original G loss and AMP path, not a method change:
# lambda/T, data, sampler, checkpoint origin and seeds remain fixed.  No
# proxy/held fitting, threshold, reject head, performance selection or
# postfreeze scoring command is reachable here.
COMMON=(
  --dataset wisig
  --wisig_pkl "${WISIG_PKL}"
  --from_scratch false
  ...
  --source_val_heavy_eval_start_epoch 15
  --source_val_heavy_eval_interval 15
  --source_val_heavy_eval_final_window 0
  --test_eval_policy interval_final
  --test_eval_start_epoch 15
  --test_eval_interval 15
  --batch_size 128
  --eval_batch_size 256
  --num_workers 4
  --prefetch_factor 2
...
declare -a pids folds gpus outs logs train_txs known_val_txs proxy_txs ckpts
...
launch_fold() {
  ...
  local candidate="F${fold}G_CCPCLEO_AUDIT6"
...
# GPU0--5 each own exactly one fold; no C arm is launched.
launch_fold 1 0
launch_fold 2 1
launch_fold 3 2
launch_fold 4 3
...
printf 'pid|fold|arm|physical_gpu|output_dir|log_path|train_tx|known_validation_tx|proxy_tx|baseline_ckpt|epochs|final_only|gradient_audit_only\n' >"${LOG_ROOT}/pids.tsv"
for index in "${!pids[@]}"; do
  printf '%s|%s|G|%s|%s|%s|%s|%s|%s|%s|15|true|true\n' \
    "${pids[index]}" "${folds[index]}" "${gpus[index]}" \
    "${outs[index]}" "${logs[index]}" "${train_txs[index]}" \
    "${known_val_txs[index]}" "${proxy_txs[index]}" "${ckpts[index]}" >>"${LOG_ROOT}/pids.tsv"
done

printf 'pid|fold|arm|physical_gpu|output_dir|log_path|train_tx|known_validation_tx|proxy_tx|baseline_ckpt|epochs|final_only|gradient_audit_only|exit_code\n' >"${LOG_ROOT}/completion.tsv"
status=0
for index in "${!pids[@]}"; do
  rc=0
  ...
  printf '%s|%s|G|%s|%s|%s|%s|%s|%s|%s|15|true|true|%s\n' \
    "${pids[index]}" "${folds[index]}" "${gpus[index]}" \
    "${outs[index]}" "${logs[index]}" "${train_txs[index]}" \
