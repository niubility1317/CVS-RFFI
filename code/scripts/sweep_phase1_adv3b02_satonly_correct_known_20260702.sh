...
declare -a HEADS=(
  "SATCORR_LIN linear 64 500 0.020"
  "SATCORR_MLP64 mlp 64 350 0.005"
)
declare -a POLICIES=(
  "SRC9999 0.9999"
  "SRC99995 0.99995"
...
echo "[PHASE1-SATONLY-CORRECT-KNOWN-SWEEP] start=$(date -Is)"
for cell in "${CELLS[@]}"; do
  ...
  mkdir -p "${LOG_ROOT}"
  echo "[PHASE1-SATONLY-CORRECT-KNOWN-CELL] run_id=${RUN_ID} unknown=${UNKNOWN_TX_IDS}"
  for head_row in "${HEADS[@]}"; do
    ...
    for policy_row in "${POLICIES[@]}"; do
      ...
      env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
        ...
        > "${LOG_ROOT}/${NAME}_satonly_correct_sweep1.out" 2>&1
    done
  done
done

"${PYTHON}" - <<'PY' "${ROOT}/runs" "${MATRIX_LOG_ROOT}/satonly_correct_known_sweep_summary.csv"
import csv
import json
import sys
...
out = Path(sys.argv[2])
rows = []
for metrics_path in sorted(runs_root.glob("phase1_adv3b02_multiview_keepold_*_20260702/SATCORR_*_SRC*/metrics.json")):
    ...
    rows.append({
        ...
        "source_train_base_count": data.get("source_train_base_count"),
        "source_train_closed_correct_count": data.get("source_train_closed_correct_count"),
        "train_known_count": data.get("train_known_count"),
...
PY
echo "[PHASE1-SATONLY-CORRECT-KNOWN-SWEEP-DONE] end=$(date -Is) summary=${MATRIX_LOG_ROOT}/satonly_correct_known_sweep_summary.csv"
