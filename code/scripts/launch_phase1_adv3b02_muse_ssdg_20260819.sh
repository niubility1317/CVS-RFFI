...
RC4_CACHE_ANCHOR_LOGITS="${RC4_CACHE_ANCHOR_LOGITS:-false}"
...
RC4_ENABLE_PARTIAL_SET="${RC4_ENABLE_PARTIAL_SET:-true}"
RC4_ENABLE_PARTIAL_CONDITIONAL="${RC4_ENABLE_PARTIAL_CONDITIONAL:-true}"
RC4_ENABLE_NEGATIVE="${RC4_ENABLE_NEGATIVE:-true}"
RC4_DECOUPLE_PARTIAL_NEGATIVE_APS="${RC4_DECOUPLE_PARTIAL_NEGATIVE_APS:-false}"
RC4_PARTIAL_THRESHOLD_SCOPE="${RC4_PARTIAL_THRESHOLD_SCOPE:-predicted_class}"
RC4_HARD_EFFECTIVE_BUDGET="${RC4_HARD_EFFECTIVE_BUDGET:-0}"
...
RC4_CLASS_RX_EFFECTIVE_BUDGET="${RC4_CLASS_RX_EFFECTIVE_BUDGET:-0}"
...
RC4_LAMBDA_ZDOM="${RC4_LAMBDA_ZDOM:--1}"
RC4_LAMBDA_DISCRIMINATOR="${RC4_LAMBDA_DISCRIMINATOR:--1}"
RC4_LAMBDA_CONFUSION="${RC4_LAMBDA_CONFUSION:--1}"
RC4_LAMBDA_PARTIAL_SET="${RC4_LAMBDA_PARTIAL_SET:--1}"
RC4_LAMBDA_PARTIAL_CONDITIONAL="${RC4_LAMBDA_PARTIAL_CONDITIONAL:--1}"
RC4_LAMBDA_FEATURE_ANCHOR="${RC4_LAMBDA_FEATURE_ANCHOR:-0}"
RC4_IDENTITY_TAIL_HARD_FINAL="${RC4_IDENTITY_TAIL_HARD_FINAL:-0.60}"
RC4_IDENTITY_TAIL_PARTIAL_SET_FINAL="${RC4_IDENTITY_TAIL_PARTIAL_SET_FINAL:-0.20}"
RC4_IDENTITY_TAIL_PARTIAL_CONDITIONAL_FINAL="${RC4_IDENTITY_TAIL_PARTIAL_CONDITIONAL_FINAL:-0}"
RC4_GRADIENT_TELEMETRY_EPOCHS="${RC4_GRADIENT_TELEMETRY_EPOCHS:-1,41,91,161,181,200}"
RC4_RECOVERY_CHECKPOINT_INTERVAL="${RC4_RECOVERY_CHECKPOINT_INTERVAL:-0}"
IDENTITY_DOMAIN_OBJECTIVE_MODE="${IDENTITY_DOMAIN_OBJECTIVE_MODE:-grl_ce}"
IDENTITY_DOMAIN_DISCRIMINATOR_SCALE="${IDENTITY_DOMAIN_DISCRIMINATOR_SCALE:-0.50}"
...
build_train_command() {
  ...
  if [[ "${FASTTRUST_RC4}" == "true" ]]; then
    ...
    TRAIN_CMD+=(
      ...
      --rc4_cache_anchor_logits "${RC4_CACHE_ANCHOR_LOGITS}"
      --rc4_use_correctness_calibration "${RC4_USE_CALIBRATION}"
      --rc4_enable_hard "${RC4_ENABLE_HARD}"
      --rc4_enable_partial "${RC4_ENABLE_PARTIAL}"
      --rc4_enable_partial_set "${RC4_ENABLE_PARTIAL_SET}"
      --rc4_enable_partial_conditional "${RC4_ENABLE_PARTIAL_CONDITIONAL}"
      --rc4_enable_negative "${RC4_ENABLE_NEGATIVE}"
      --rc4_decouple_partial_negative_aps "${RC4_DECOUPLE_PARTIAL_NEGATIVE_APS}"
      --rc4_partial_threshold_scope "${RC4_PARTIAL_THRESHOLD_SCOPE}"
      --rc4_hard_effective_budget "${RC4_HARD_EFFECTIVE_BUDGET}"
      ...
      --rc4_class_receiver_effective_budget "${RC4_CLASS_RX_EFFECTIVE_BUDGET}"
      ...
      --rc4_tail_transition_start_epoch "${RC4_TAIL_TRANSITION_START}"
      --rc4_tail_transition_epochs "${RC4_TAIL_TRANSITION_EPOCHS}"
      --rc4_tail_transition_floor "${RC4_TAIL_TRANSITION_FLOOR}"
      --rc4_lambda_hard 0.60
      --rc4_lambda_partial 0.40
      --rc4_lambda_partial_set "${RC4_LAMBDA_PARTIAL_SET}"
      --rc4_lambda_partial_conditional "${RC4_LAMBDA_PARTIAL_CONDITIONAL}"
      --rc4_lambda_negative 0.20
      --rc4_lambda_domain "${RC4_LAMBDA_DOMAIN}"
      --rc4_lambda_zdom "${RC4_LAMBDA_ZDOM}"
      --rc4_lambda_discriminator "${RC4_LAMBDA_DISCRIMINATOR}"
      --rc4_lambda_confusion "${RC4_LAMBDA_CONFUSION}"
      --rc4_lambda_feature_anchor "${RC4_LAMBDA_FEATURE_ANCHOR}"
      ...
      --rc4_identity_tail_hard_final "${RC4_IDENTITY_TAIL_HARD_FINAL}"
      --rc4_identity_tail_partial_set_final "${RC4_IDENTITY_TAIL_PARTIAL_SET_FINAL}"
      --rc4_identity_tail_partial_conditional_final "${RC4_IDENTITY_TAIL_PARTIAL_CONDITIONAL_FINAL}"
      --rc4_gradient_telemetry_epochs "${RC4_GRADIENT_TELEMETRY_EPOCHS}"
      --rc4_recovery_checkpoint_interval "${RC4_RECOVERY_CHECKPOINT_INTERVAL}"
      --identity_domain_objective_mode "${IDENTITY_DOMAIN_OBJECTIVE_MODE}"
      --identity_domain_discriminator_scale "${IDENTITY_DOMAIN_DISCRIMINATOR_SCALE}"
      --identity_domain_confusion_scale "${IDENTITY_DOMAIN_CONFUSION_SCALE}"
      --sat_anchor_hard_max_fraction 0.25
