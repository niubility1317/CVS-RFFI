...
RUN_ID="${RUN_ID:-optimizer_20260607_033254_fed19_fishr_next8}"
...
run_cmd() {
  ...
  echo "[FED19-FISHR-CANDIDATE] lane=federated_vmb candidate=${candidate_id} run=${run_name} gpu=${gpu} dry_run=${DRY_RUN}"
  printf '[FED19-FISHR-CMD]'
  print_cmd "${cmd[@]}"
...
SAT_BASELINE_VIEW=(
  ...
  --fl_baseline_view_ce_weight 0.34
)

VMB_STYLE_FISHR_BASE=(
  --train_mode fedcvs_vmb
  --fl_local_objective receiver_agnostic_bex02
  --fl_local_epochs 1
  --fl_vmb_ra_stylebank_profile
  --fl_vmb_stage auto
  ...
  --fl_style_replay_start_round 60
  --fl_style_phys_start_round 60
...
cd "${ROOT}"
echo "[FED19-FISHR] run_id=${RUN_ID} dry_run=${DRY_RUN} max_train_per_gpu=${MAX_TRAIN_PER_GPU}"

launch_fed FED19F_R01 FED19F_R01_fedprox_nostyle_nofishr_control_r010 0 \
  --train_mode fedprox --fl_local_objective receiver_agnostic_bex02 --fedprox_mu 0.008 --fl_local_epochs 1 \
  --lambda_fishr 0.000 --fishr_min_domains 2 --lambda_rx_adv 0.12 --grl_lambda 0.24 \
  "${SAT_BASELINE_VIEW[@]}"

launch_fed FED19F_R02 FED19F_R02_vmb_style_nofishr_control_r010 1 \
  "${VMB_STYLE_FISHR_BASE[@]}" \
  --lambda_fishr 0.000 --lambda_rx_adv 0.12 --grl_lambda 0.24

launch_fed FED19F_R03 FED19F_R03_vmb_style_fishr_low_r010 2 \
  "${VMB_STYLE_FISHR_BASE[@]}" \
  --lambda_fishr 0.003 --lambda_rx_adv 0.12 --grl_lambda 0.24

launch_fed FED19F_R04 FED19F_R04_vmb_style_fishr_mid_primary_r010 3 \
  "${VMB_STYLE_FISHR_BASE[@]}" \
  --lambda_fishr 0.008 --lambda_rx_adv 0.12 --grl_lambda 0.24

launch_fed FED19F_A05 FED19F_A05_vmb_style_fishr_high_stress_r010 4 \
  "${VMB_STYLE_FISHR_BASE[@]}" \
  --lambda_fishr 0.020 --lambda_rx_adv 0.12 --grl_lambda 0.24

launch_fed FED19F_A06 FED19F_A06_vmb_style_fishr_grl_r010 5 \
  "${VMB_STYLE_FISHR_BASE[@]}" \
  --lambda_fishr 0.008 --lambda_rx_adv 0.26 --grl_lambda 0.46 \
  --use_fed_cgrl --fed_cgrl_base_lambda 0.20 --fed_cgrl_min_lambda 0.03 --fed_cgrl_max_lambda 0.60 \
  ...
  --fed_cgrl_conflict_source vmb --fed_cgrl_ema 0.55 --fl_conflict_agg cosine_clip

launch_fed FED19F_A07 FED19F_A07_vmb_style_fishr_proto_r010 6 \
  "${VMB_STYLE_FISHR_BASE[@]}" \
  --lambda_fishr 0.008 --lambda_rx_adv 0.12 --grl_lambda 0.24 \
  --use_fed_proto_stats --lambda_fed_proto 0.010 --fed_proto_momentum 0.95 \
  --lambda_vmb_tx_proto 0.030 --lambda_vmb_rx_proto 0.018

launch_fed FED19F_A08 FED19F_A08_vmb_style_fishr_coral_sat_r010 7 \
  "${VMB_STYLE_FISHR_BASE[@]}" \
  --lambda_fishr 0.008 --lambda_rx_adv 0.12 --grl_lambda 0.24 \
  --fl_baseline_view_ce_weight 0.42 --sat_cons_start_epoch 50 \
  --use_fed_coral --lambda_fl_coral_zid_global 0.00012 --fl_coral_stage all --fl_coral_start_round 130 \
  --fl_coral_feature z_id --fl_coral_cov_mode diag --fl_coral_min_count 2 --fl_coral_collect_views all
