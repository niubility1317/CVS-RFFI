...
GPU="${GPU:-0}"
RUN_ID="${RUN_ID:-phase2_qknn_fftlogmag_v53_20260706}"
REPORT_ID="phase2_qknn_hardpair_n20_20260706"
ARTIFACT_DIR="${ROOT}/automation_reports/CV-SincNet/${REPORT_ID}/artifacts/v53_fftlogmag_20260706"
...
for spec in \
  "k5:5:421038:0.4000000000000000:0.1000000000000000:0.2200000000000000" \
  "k10:10:421057:0.3982997966719710:0.1045338755414108:0.2186398373375768"
do
  IFS=: read -r tag k seed proto_mix transform_strength aux_weight <<< "${spec}"
  run_cmd "${PY_CMD[@]}" \
    ...
    --transform_strengths "${transform_strength}" \
    --topm_grid 4 \
