...
RUN_ID="${RUN_ID:-stage2_spaceborne_next64cr_20260622_001259}"
...
if [[ "${PHASE1_LANE_ACTIVE}" == "1" ]]; then
  for i in "${!CAND_ID[@]}"; do
    if [[ "${CAND_LANE[$i]:-}" == "phase1" ]]; then
      ...
      event_row "${CAND_ID[$i]}" "DEFERRED_RETRY_CAPACITY" "gpu=${CAND_GPU[$i]}" "lane=phase1" "reason=phase1_lane_active_or_capacity_retry;exact_retry=RUN_ID=${RUN_ID}_phase1_retry PHASE1_LANE_ACTIVE=0 PHASE2_LANE_ACTIVE=1 bash ${ROOT}/code/scripts/launch_stage2_optimizer_20260622_001259_next64cr.sh"
    fi
  done
fi
...
if [[ "${PHASE2_LANE_ACTIVE}" == "1" ]]; then
  for i in "${!CAND_ID[@]}"; do
    if [[ "${CAND_LANE[$i]:-}" == "phase2" ]]; then
      ...
      event_row "${CAND_ID[$i]}" "DEFERRED_RETRY_CAPACITY" "gpu=${CAND_GPU[$i]}" "lane=phase2" "reason=phase2_lane_active_or_capacity_retry;exact_retry=RUN_ID=${RUN_ID}_phase2_retry PHASE1_LANE_ACTIVE=1 PHASE2_LANE_ACTIVE=0 bash ${ROOT}/code/scripts/launch_stage2_optimizer_20260622_001259_next64cr.sh"
    fi
  done
fi
