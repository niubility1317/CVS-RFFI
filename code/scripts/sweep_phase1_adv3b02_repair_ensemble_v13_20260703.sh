...
VARIANT="${VARIANT:-v13a}"
LOG_ROOT="${LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_repair_ensemble_${VARIANT}_20260703}"
...
case "${VARIANT}" in
  v13a)
    ...
    TAG="V13a_all_repairers"
    ;;
  v13b)
    ADAPTERS="${ADAPTERS:-LEOADAPT3_LINR_COS,LEOADAPT3_MLP_ID}"
    TAG="V13b_trained_repairers"
    ;;
  *)
    echo "[V13-UNKNOWN-VARIANT] ${VARIANT}" >&2
    exit 2
    ;;
esac

SUMMARY_CSV="${LOG_ROOT}/repair_ensemble_${VARIANT}_summary.csv"
METRICS_JSON="${LOG_ROOT}/repair_ensemble_${VARIANT}_metrics.json"
BEST_JSON="${LOG_ROOT}/repair_ensemble_${VARIANT}_best.json"

echo "[PHASE1-REPAIR-ENSEMBLE-V13] start=$(date -Is) variant=${VARIANT} adapters=${ADAPTERS}"
env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
  ...
  --adapters "${ADAPTERS}" \
  --run_tag "${TAG}" \
  > "${LOG_ROOT}/repair_ensemble_${VARIANT}.out" 2>&1
...
echo "[PHASE1-REPAIR-ENSEMBLE-V13-DONE] end=$(date -Is) summary=${SUMMARY_CSV} best=${BEST_JSON}"
