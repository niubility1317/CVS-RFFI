#!/usr/bin/env python3
"""Prepare, smoke-test and execute the frozen ParetoDistill Hard11 screen."""
...
from cvsrffi.stage2_d92_pareto_distill_hard11 import (  # noqa: E402
    ARM_ID,
    CANDIDATE_ID,
...
class D92ParetoDistillHard11RunnerError(RuntimeError):
    """Raised when immutable ParetoDistill runner closure would drift."""


D92ParetoDistillHard11ErrorAlias = D92ParetoDistillHard11RunnerError
D92ParetoDistillHard11Error = D92ParetoDistillHard11RunnerError
...
def _read_json_object(path: Path) -> dict[str, Any]:
    if not path.is_file() or path.is_symlink():
        raise D92ParetoDistillHard11RunnerError(f"smoke artifact is missing: {path}")
    try:
        payload = json.loads(path.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise D92ParetoDistillHard11RunnerError(f"smoke artifact is invalid: {path}") from error
    if not isinstance(payload, dict):
        raise D92ParetoDistillHard11RunnerError(f"smoke artifact must be an object: {path}")
    return payload
...
def _validate_fit_audit(path: str | Path, *, k_shot: int) -> None:
    ...
    if not source.is_file() or source.is_symlink():
        raise D92ParetoDistillHard11RunnerError(f"fit audit is missing: {source}")
    try:
        rows = json.loads(source.read_text(encoding="utf-8-sig"))
    except (OSError, ValueError) as error:
        raise D92ParetoDistillHard11RunnerError(f"fit audit is invalid: {source}") from error
    if not isinstance(rows, list) or len(rows) != len(SCENES) or any(not isinstance(row, Mapping) for row in rows):
        raise D92ParetoDistillHard11RunnerError("fit audit scene closure drift")
    if {str(row.get("scenario")) for row in rows} != set(SCENES):
        raise D92ParetoDistillHard11RunnerError("fit audit scene identity drift")
    active = int(k_shot) > 2
    expected_total, expected_actual, expected_mode = ((4, 2, "pareto_distill") if active else (3, 3, "d92_full_alias"))
    prefix = "d92_e0d_pareto_distill_"
    count_fields = {
        "covariance_estimation_count": 1,
        "robust_center_transform_count": 1,
        "full_solve_count": 1,
        "block_solve_count": 1,
        "loo_fit_count": 0,
        "fisher_fit_count": 0,
    ...
    for row in rows:
        if row.get("arm_id") != ARM_ID or row.get("candidate_id") != CANDIDATE_ID:
            raise D92ParetoDistillHard11RunnerError("fit audit arm/candidate identity drift")
        if any(row.get(field) is not False for field in QUERY_ZERO_FIELDS):
            raise D92ParetoDistillHard11RunnerError("fit audit query access is not zero")
        ...
        if active:
            if active_value is not True or fallback_value is not False or reason is not None:
                raise D92ParetoDistillHard11RunnerError("fit audit ParetoDistill active/fallback drift")
            for field, expected in count_fields.items():
                if row.get(prefix + field) != expected:
                    raise D92ParetoDistillHard11RunnerError(f"fit audit {field} drift")
            if row.get(prefix + "deployed_support_constraints_pass") is not True:
                raise D92ParetoDistillHard11RunnerError("fit audit deployed support constraints drift")
            if row.get(prefix + "deployed_full_head_byte_exact") is not False:
                raise D92ParetoDistillHard11RunnerError("fit audit deployed full-head byte-exact drift")
            delta = row.get(prefix + "persistent_state_bytes_delta")
            if isinstance(delta, bool) or not isinstance(delta, (int, float)) or delta != 0:
                raise D92ParetoDistillHard11RunnerError("fit audit persistent state delta drift")
            for aliases in (("support_optimization_macs_upper_bound", "support_macs", "support_macs_upper_bound"), ("support_transient_bytes_upper_bound", "support_transient_bytes", "support_optimization_transient_bytes_upper_bound")):
                present = [row.get(prefix + field) for field in aliases if prefix + field in row]
                value = present[0] if present else None
                if value is None or isinstance(value, bool) or not isinstance(value, (int, float)) or not math.isfinite(float(value)) or float(value) < 0:
                    raise D92ParetoDistillHard11RunnerError("fit audit support resource drift")
        elif active_value is not False or fallback_value is not False or reason != "K1_K2_EXACT_D92_FULL_ALIAS":
            raise D92ParetoDistillHard11RunnerError("fit audit ParetoDistill K1 alias drift")
...
def _verify_manifest_artifacts(manifest: Mapping[str, Any]) -> None:
    ...
    for job in manifest.get("jobs", []):
        ...
        if not truth.is_file() or truth.is_symlink():
            raise D92ParetoDistillHard11RunnerError(f"truth sidecar is missing: {truth}")
        expected_truth = str(job.get("truth_sidecar_sha256", "")).lower()
        if len(expected_truth) != 64 or any(ch not in "0123456789abcdef" for ch in expected_truth) or _sha256_file(truth) != expected_truth:
            raise D92ParetoDistillHard11RunnerError(f"truth sidecar SHA drift: {truth}")
        packages = job.get("packages")
        if not isinstance(packages, Mapping):
            raise D92ParetoDistillHard11RunnerError("package manifest is missing")
        for name, item in packages.items():
            if not isinstance(item, Mapping):
                raise D92ParetoDistillHard11RunnerError(f"package entry is invalid: {name}")
            ...
            if not package_root.is_dir() or package_root.is_symlink() or not seal.is_file() or seal.is_symlink():
                raise D92ParetoDistillHard11RunnerError(f"package artifact is missing: {name}")
            ...
            if len(expected) != 64 or any(ch not in "0123456789abcdef" for ch in expected) or _sha256_file(seal) != expected:
                raise D92ParetoDistillHard11RunnerError(f"package seal SHA drift: {name}")
...
def _load_manifest(path: str | Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if source.is_symlink() or not source.is_file() or _sha256_file(source) != str(expected_sha256).lower():
        raise D92ParetoDistillHard11RunnerError("matrix manifest must be an immutable regular file")
    try:
    ...
    except (OSError, ValueError, KeyError, TypeError, D92ParetoDistillHard11Error) as error:
        raise D92ParetoDistillHard11RunnerError(f"Hard11 manifest contract drift: {error}") from error
    return payload
...
def _publish_shared_stop(output_root: Path, *, reason: str, fingerprint: str, distinct_outer_count: int) -> None:
    try:
        _write_json_new(_shared_stop_path(output_root), {"schema": "cvs.phase2.d92_pareto_distill_hard11.systemic_stop.v1", "status": "STOPPED_EARLY_SYSTEMIC_TECHNICAL_FAILURE", "timestamp": _now(), "reason": str(reason), "fingerprint": str(fingerprint), "distinct_outer_count": int(distinct_outer_count), "performance_result_allowed": False, "fresh_run_retry_authorized": False})
...
def _record_pre_prediction_failure(output_root: Path, job: Mapping[str, Any], fingerprint: str) -> bool:
    ...
    try:
        _write_json_new(record, {"schema": "cvs.phase2.d92_pareto_distill_hard11.pre_prediction_failure.v1", "timestamp": _now(), "fingerprint": normalized, "outer_key": job["outer_key"], "job_id": job["job_id"], "arm_id": job["arm_id"]})
    except FileExistsError:
        pass
...
def _smoke_job(manifest: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if len(matches) != 1:
        raise D92ParetoDistillHard11RunnerError("K>2 active-method smoke row identity drift")
    return matches[0]
...
def _liveness_job(manifest: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if len(matches) != 1:
        raise D92ParetoDistillHard11RunnerError("K1 exact-alias liveness row identity drift")
    return matches[0]
...
def _validate_shared_smoke(manifest: Mapping[str, Any], *, manifest_sha256: str, device: str) -> None:
    if int(manifest.get("job_count", -1)) != 11 or len(manifest.get("jobs", [])) != 11:
        raise D92ParetoDistillHard11RunnerError("Hard11 matrix identity drift")
    ...
    if receipt.get("command") != expected_command:
        raise D92ParetoDistillHard11RunnerError("shared smoke command identity drift")
    ...
    if any(receipt.get(field) != value for field, value in hashes.items()) or receipt.get("prediction_closure") != hashes or _prediction_closure_status(prediction_root)[0] != "closed":
        raise D92ParetoDistillHard11RunnerError("shared smoke prediction closure drift")


def prepare(args: argparse.Namespace) -> dict[str, Any]:
    output = Path(args.output_root)
    if output.exists():
        raise D92ParetoDistillHard11RunnerError("matrix output already exists")
    ...
    return {"status": "PARETO_DISTILL_HARD11_MATRIX_PREPARED", "matrix_manifest": str(output / "matrix_manifest.json"), "matrix_manifest_sha256": digest, "job_count": 11, "scene_arm_count": 33}
...
def truth_free_smoke(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if Path(args.output_root).resolve() != output or output.exists():
        raise D92ParetoDistillHard11RunnerError("smoke output identity/immutability drift")
    output.mkdir(parents=True)
    ...
    if completed.returncode != 0:
        ...
        raise D92ParetoDistillHard11RunnerError("truth-free smoke prediction failed")
    ...
    if status != "closed":
        ...
        raise D92ParetoDistillHard11RunnerError("truth-free smoke prediction closure failed")
    ...
    receipt = {"schema": "cvs.phase2.d92_pareto_distill_hard11.smoke_receipt.v1", "status": "D92_PARETO_DISTILL_HARD11_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS", "matrix_manifest_sha256": str(args.matrix_manifest_sha256).lower(), "selection_sha256": manifest["selection_sha256"], "smoke_outer_key": SMOKE_OUTER_KEY, "outer_index": job["outer_index"], "outer_key": job["outer_key"], "job_id": job["job_id"], "outer_role": job["outer_role"], "arm_id": ARM_ID, "candidate": CANDIDATE_ID, "k_shot": int(job["k_shot"]), "truth_sidecar_sha256": job["truth_sidecar_sha256"], "prediction_root": str(prediction_root), "command": command, **hashes, "prediction_closure": hashes, "fit_audit_protocol_closed": True, "query_truth_joined_only_after_immutable_predictions": True, "prediction_and_scorer_processes_isolated": True, **{field: False for field in QUERY_ZERO_FIELDS}, "truth_open": False}
...
def run_shard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if int(args.shard_count) != SHARD_COUNT or int(args.shard_index) not in range(SHARD_COUNT):
        raise D92ParetoDistillHard11RunnerError("shard identity drift")
    ...
    if not selected or events_path.exists() or summary_path.exists():
        raise D92ParetoDistillHard11RunnerError("shard selection/evidence identity drift")
    ...
    for job in selected:
        ...
        except D92ParetoDistillHard11RunnerError as error:
        ...
        receipt = {"schema": "cvs.phase2.d92_pareto_distill_hard11.job_receipt.v1", "status": "PREDICTIONS_AND_POST_PREDICTION_SCORE_COMPLETE", "job_id": job["job_id"], "outer_key": job["outer_key"], "outer_role": job["outer_role"], "k_shot": job["k_shot"], "arm_id": ARM_ID, "candidate": CANDIDATE_ID, "role": "primary", "matrix_manifest_sha256": str(args.matrix_manifest_sha256).lower(), "method_lock_sha256": manifest["method_lock_sha256"], "selection_sha256": manifest["selection_sha256"], "prediction_command": command, "score_command": score_command, "before_prediction_sha256": _sha256_file(paths["before_prediction"]), "after_prediction_sha256": _sha256_file(paths["after_prediction"]), "score_sha256": _sha256_file(score_path), "truth_sidecar_sha256": job["truth_sidecar_sha256"], "truth_sidecar_exposed_to_predictor": False, "query_truth_joined_only_after_immutable_predictions": True, "query_truth_fed_back_to_predictor": False, "prediction_and_scorer_processes_isolated": True, "fresh_run_retry_authorized": False}
...
def main() -> int:
    ...
    return 0 if value["status"] in {"PARETO_DISTILL_HARD11_MATRIX_PREPARED", "D92_PARETO_DISTILL_HARD11_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS", "PASS"} else 1
