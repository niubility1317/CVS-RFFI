...
# Invoke explicitly as: bash scripts/launch_phase1_icmt_postfreeze_20260810.sh [--dry-run]
# One-shot final-only closure for the completed P1-ICMT v1 C/G matrix.
# Each candidate runs exactly one ICMT clean L/V/proxy export, one generic
...
POSTFREEZE_RUN_ID="${POSTFREEZE_RUN_ID:-phase1_icmt_postfreeze_20260810_v2}"
...
TRAIN_RUN_ROOT="${TRAIN_RUN_ROOT:-${PROJECT_ROOT}/runs/phase1_icmt12_20260810_v1}"
POSTFREEZE_ROOT="${POSTFREEZE_ROOT:-${PROJECT_ROOT}/runs/${POSTFREEZE_RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/${POSTFREEZE_RUN_ID}}"
ICMT_CLEAN_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_icmt_features.py"
ICMT_LEO_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_icmt_leo_features.py"
LOGITS_REJECT_SCRIPT="${CODE_ROOT}/scripts/eval_phase1_logits_open_set_reject.py"
PAIR_EVAL_SCRIPT="${CODE_ROOT}/evaluate_phase1_icmt_postfreeze_pair.py"
DRY_RUN="${DRY_RUN:-0}"
...
MAX_PER_TX="400" # LEO local4=1600; proxy singleton=400.
EXPORT_BATCH="32"
...
[[ "$(basename "${TRAIN_RUN_ROOT}")" == "phase1_icmt12_20260810_v1" ]] || {
  echo "TRAIN_RUN_ROOT leaf must be phase1_icmt12_20260810_v1" >&2
  exit 3
}
...
for required in "${ICMT_CLEAN_EXPORT_SCRIPT}" "${ICMT_LEO_EXPORT_SCRIPT}" "${LOGITS_REJECT_SCRIPT}" "${PAIR_EVAL_SCRIPT}"; do
...
run_candidate() {
  ...
  local candidate="F${fold}${arm}_ICMT12"
  ...
  local -a clean_command=(
    "${PYTHON}" -u "${ICMT_CLEAN_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}"
    --wisig_pkl "${WISIG_PKL}"
    --out_npz "${clean_npz}"
    ...
    --proxy_days "${SOURCE_DAYS}"
    --proxy_rxs "${SOURCE_RXS}"
    --max_proxy_samples_per_tx "${MAX_PER_TX}"
    --batch_size "${EXPORT_BATCH}"
    --device cuda:0
  )
  local -a leo_command=(
    "${PYTHON}" -u "${ICMT_LEO_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}"
    --wisig_pkl "${WISIG_PKL}"
    --out_npz "${leo_npz}"
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command ICMT_CLEAN_EXPORT "${gpu}" "${clean_command[@]}"
...
# Preserve the frozen ICMT continuation GPU mapping.  Candidate-internal stages are
# sequential; the twelve independent candidate pipelines run concurrently.
...
run_pair() {
  ...
  local fold_index=$((fold - 1))
  local train_tx="${FOLD_TRAIN_TX[fold_index]}"
  local c_candidate="F${fold}C_ICMT12"
  local g_candidate="F${fold}G_ICMT12"
...
# F6 seals the aggregate only after the immutable F1--F5 pair JSONs exist.
run_pair 1
run_pair 2
run_pair 3
run_pair 4
