...
MATRIX_LOG_ROOT="${MATRIX_LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_sattta_rxlight_matrix_20260702}"
...
declare -a CELLS=(
  "phase1_adv3b02_sattta_rx20_1_u10_20260702 20-1 10-1,10-10"
  "phase1_adv3b02_sattta_rx20_1_u1_20260702 20-1 1-16,4-10"
  "phase1_adv3b02_sattta_rx3_19_u10_20260702 3-19 10-1,10-10"
  "phase1_adv3b02_sattta_rx3_19_u1_20260702 3-19 1-16,4-10"
  "phase1_adv3b02_sattta_rx7_14_u10_20260702 7-14 10-1,10-10"
  "phase1_adv3b02_sattta_rx7_14_u1_20260702 7-14 1-16,4-10"
  "phase1_adv3b02_sattta_rx7_7_u10_20260702 7-7 10-1,10-10"
  "phase1_adv3b02_sattta_rx7_7_u1_20260702 7-7 1-16,4-10"
  "phase1_adv3b02_sattta_rx8_8_u10_20260702 8-8 10-1,10-10"
  "phase1_adv3b02_sattta_rx8_8_u1_20260702 8-8 1-16,4-10"
)

declare -a POLICIES=(
  "SATTA_LIN_SRC9999 linear 64 500 0.020 0.9999"
  "SATTA_LIN_SRC1000 linear 64 500 0.020 1.0000"
  "SATTA_MLP64_SRC9999 mlp 64 350 0.005 0.9999"
  "SATTA_MLP64_SRC1000 mlp 64 350 0.005 1.0000"
)

echo "[PHASE1-SAT-TTA-RXLIGHT-SWEEP] start=$(date -Is) gpu=${GPU}"
for cell in "${CELLS[@]}"; do
  read -r RUN_ID TARGET_RECEIVER_IDS UNKNOWN_TX_IDS <<<"${cell}"
  RUNS_ROOT="${ROOT}/runs/${RUN_ID}"
  LOG_ROOT="${ROOT}/logs/${RUN_ID}"
  FEATURE_DIR="${RUNS_ROOT}/ADV3B02_CORE90_SOFT_E200_PHASE1_SAT_TTA_RXLIGHT5"
  FEATURE_NPZ="${FEATURE_DIR}/features_sattta_rxlight5.npz"
  mkdir -p "${RUNS_ROOT}" "${LOG_ROOT}" "${FEATURE_DIR}"
  echo "[PHASE1-SAT-TTA-RXLIGHT-CELL] run_id=${RUN_ID} target_rx=${TARGET_RECEIVER_IDS} unknown=${UNKNOWN_TX_IDS}"
  ...
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${GPU}" \
    ...
    --device "cuda:0" \
    --seed 4070221 \
    > "${LOG_ROOT}/export_sattta_rxlight5.out" 2>&1
  ...
  for policy_row in "${POLICIES[@]}"; do
    ...
    env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
      ...
      > "${LOG_ROOT}/${NAME}_sattta_rxlight5.out" 2>&1
  done
done

"${PYTHON}" - <<'PY' "${ROOT}/runs" "${MATRIX_LOG_ROOT}/sattta_rxlight_sweep_summary.csv"
import csv
import json
import sys
...
out = Path(sys.argv[2])
rows = []
for metrics_path in sorted(runs_root.glob("phase1_adv3b02_sattta_*_20260702/SATTA_*_SRC*/metrics.json")):
...
PY
echo "[PHASE1-SAT-TTA-RXLIGHT-SWEEP-DONE] end=$(date -Is) summary=${MATRIX_LOG_ROOT}/sattta_rxlight_sweep_summary.csv"
