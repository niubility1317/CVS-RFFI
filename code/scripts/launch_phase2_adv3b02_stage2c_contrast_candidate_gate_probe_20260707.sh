...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_contrast_candidate_gate_probe_20260707}"
...
declare -a PROFILES=(
  "CAND_U050_W025 0.50 0.25 0.00"
  "CAND_U060_W025 0.60 0.25 0.00"
  "CAND_U070_W025 0.70 0.25 0.00"
  "CAND_U070_W050M02 0.70 0.50 0.02"
)

mkdir -p "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}"
echo "[STAGE2C-CONTRAST-CAND-GATE] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-CONTRAST-CAND-GATE] variants=${VARIANTS[*]} k=${K_SHOTS[*]} profiles=${#PROFILES[@]}"
echo "[STAGE2C-CONTRAST-CAND-GATE] diagnostic_only=true unknown_query_eval_only=true"
...
run_one() {
  ...
  mkdir -p "${out_dir}"
  echo "[STAGE2C-CONTRAST-CAND-GATE-RUN] variant=${variant} profile=${profile} k=${k} gate=${gate} weight=${contrast_weight} margin=${contrast_margin}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
...
for variant in "${VARIANTS[@]}"; do
  for row in "${PROFILES[@]}"; do
    read -r profile gate contrast_weight contrast_margin <<<"${row}"
    for k in "${K_SHOTS[@]}"; do
      run_one "${variant}" "${profile}" "${gate}" "${contrast_weight}" "${contrast_margin}" "${k}"
    done
  done
done

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-CONTRAST-CAND-GATE-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_contrast_candidate_gate_probe_summary.json" <<'PY'
import csv
import json
import sys
...
for path in sorted(case_dir.glob("*/*/stage2c_qknn_*_k*.json")):
    ...
    for row in payload.get("summary_rows", []):
        ...
        item["utility"] = (
            ...
            - 3.0 * float(item.get("unknown_FAR", 1.0))
        )
        rows.append(item)
rows.sort(
    key=lambda r: (
...
echo "[STAGE2C-CONTRAST-CAND-GATE-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
