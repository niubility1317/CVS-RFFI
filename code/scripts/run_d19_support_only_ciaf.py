#!/usr/bin/env python3
"""Run D19 CIAF development selection on sealed K10 enrollment support only.
...
from cvsrffi.stage2_ciaf import (  # noqa: E402
    CiafConfig,
...
from cvsrffi.stage2_diag_cosine_exploration import forward_zid160  # noqa: E402
...
def _load_component(
    ...
    *,
    ...
    if (
        binding_value.get("schema") != "cvs.phase2.d19_adv3b02_class_binding.v1"
...
def preregistered_candidates() -> dict[str, CiafConfig]:
    """Return the small unified candidate set fixed before support opening."""

    return {
        IDENTITY_CANDIDATE: CiafConfig(
            support_alpha_min=1.0,
...
def _extract_scene_zid(
    ...
    for iq in np.asarray(rows["iq"], dtype=np.float32):
        feature = forward_zid160(
            model, np.asarray(iq, dtype=np.float32)[None, ...], device=device, batch_size=1
        )
        if feature.shape != (1, 160):
...
def _evaluate_fold(
    ...
    *,
    ...
    before_predictions = [predict_one(before, row)[0] for row in z_id[held & old]]
    after_old_predictions = [predict_one(after, row)[0] for row in z_id[held & old]]
    after_new_predictions = [predict_one(after, row)[0] for row in z_id[held & new]]
    ...
    return {
        ...
        "old_score_columns_bitwise_unchanged": bool(
            np.array_equal(before.prototypes, after.prototypes[: len(old_classes)])
            and np.array_equal(before.score_bias, after.score_bias[: len(old_classes)])
        ),
    }
...
def _select_candidate(
    ...
    for candidate_id, rows in folds_by_candidate.items():
        ...
        strict = (
            aggregate["mean_H_old_new"]
            > zero_aggregate["mean_H_old_new"] + tolerance
            or aggregate["worst_joint_floor"]
            > zero_aggregate["worst_joint_floor"] + tolerance
            or aggregate["max_forgetting"]
            < zero_aggregate["max_forgetting"] - tolerance
        )
        ...
        decision = {
            **aggregate,
            ...
            "eligible_positive_route": bool(atomic and strict),
            "fallback": False,
        }
        decisions.append(decision)
        if atomic and strict:
            eligible.append(
                (
                    candidate_id,
...
def _deployment_state_audit(
    ...
    *,
    ...
    old = np.isin(labels, np.asarray(old_classes))
    new = np.isin(labels, np.asarray(new_classes))
    before = fit_old_ciaf(component, z_id[old], labels[old], config=config)
    after = register_new_classes(
        before, z_id[new], labels[new], registered_classes=new_classes
    )
    resource = after.resource_audit()
    identity_qknn_macs = len(after.classes) * 10 * 160
    resource.update(
        {
            "deployment_k_shot": 10,
            "registered_class_count": len(after.classes),
            ...
            "estimated_score_mac_reduction_vs_identity_single_qknn": float(
                1.0 - resource["estimated_macs_per_query"] / identity_qknn_macs
            ),
...
def run(
    *,
    ...
    if mode != MODE:
        raise D19RunnerError("D19 historical component runner is development-only")
    if output.exists():
    ...
    for scenario in FORMAL_LEO_WEAK_SCENARIOS:
        ...
        features, extraction = _extract_scene_zid(model, device, after_rows)
    ...
    for candidate_id, config in candidates.items():
        for scenario in FORMAL_LEO_WEAK_SCENARIOS:
            for fold_index, held_ranks in enumerate(HELD_RANKS):
                ...
                row.update(
                    {
                        "schema": "cvs.phase2.d19_ciaf.support_fold.v1",
                        "scenario": scenario,
    ...
    support_audit = {
        "schema": "cvs.phase2.d19_ciaf.support_audit.v1",
        ...
        "derived_support_views_per_physical_sample": 0,
    ...
    selection = {
        "schema": "cvs.phase2.d19_ciaf.selection.v1",
        "selected_candidate_id": selected_id,
        "selected_positive_route": selected_id != IDENTITY_CANDIDATE,
        "fallback_to_identity": selected_id == IDENTITY_CANDIDATE,
        "selection_rule": (
            "all_15_folds_noninferior_on_before_old_floor_after_old_floor_"
            "after_new_floor_H_and_forgetting_plus_strict_aggregate_improvement"
        ),
    ...
    resource_sha256 = _write_json(
        ...
        {
            "schema": "cvs.phase2.d19_ciaf.resource_matrix.v1",
            "selected_candidate_id": selected_id,
            "by_scenario": deployment_resources,
        },
    )
    receipt = {
        "schema": "cvs.phase2.d19_ciaf.receipt.v1",
