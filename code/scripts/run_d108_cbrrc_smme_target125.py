#!/usr/bin/env python3
"""Prepare, smoke, shard, merge, or validate D108-CB-RRC-SMME Target125."""
...
from cvsrffi.stage2_d108_target125_runner import (  # noqa: E402
    predict_d108_target125,
    prepare_d108_target125_run,
    smoke_d108_target125_prepared_state,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    prepare = commands.add_parser("prepare", help="pin D92 packages, ground, and authority")
    ...
    smoke = commands.add_parser("smoke", help="exercise one row/scene and all four arms")
    ...
    shard = commands.add_parser("predict-shard", help="seal one immutable modulo-8 shard")
    _add_prepared_inputs(shard)
    shard.add_argument("--output-dir", type=Path, required=True)
    shard.add_argument("--shard-index", type=int, required=True, choices=range(8))
    shard.add_argument("--device", required=True)
    shard.add_argument("--feature-batch-size", type=int, default=64)

    merge = commands.add_parser("merge", help="merge exactly eight completed shards")
    merge.add_argument("--shard-manifest", type=Path, action="append", required=True)
    merge.add_argument("--output-dir", type=Path, required=True)

    validate = commands.add_parser("validate", help="validate a sealed 3,000-surface manifest")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "prepare":
        result = prepare_d108_target125_run(
        ...
        result = smoke_d108_target125_prepared_state(
        ...
        result = validate_d108_target125_prediction_manifest(
