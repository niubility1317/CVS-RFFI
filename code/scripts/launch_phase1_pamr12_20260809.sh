...
# Frozen six-fold P1-PAMR C/G continuation matrix: C is the exact GeoSat-C
# continuation control and G differs only by lambda_pamr=0.05.
RUN_ID="${RUN_ID:-phase1_pamr12_20260809}"
...
for arg in "$@"; do case "${arg}" in --dry-run) DRY_RUN=1 ;; *) echo "unknown argument: ${arg}" >&2; exit 2 ;; esac; done
...
COMMON=(
  ...
  --phase1_pamr_frozen_mode true --max_grad_norm 5.0 --checkpoint_selection final_only --best_metric source_val_sat_hmean --phase1_source_val_selection_only true
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in C) arm_args=(--phase1_pamr_enabled false --phase1_pamr_audit_only false --lambda_pamr 0) ;; G) arm_args=(--phase1_pamr_enabled true --phase1_pamr_audit_only false --lambda_pamr 0.05) ;; *) echo "unsupported arm: ${arm}" >&2; return 2;; esac
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then printf '[DRY-RUN] CUDA_VISIBLE_DEVICES=%q PYTHONPATH=%q' "${gpu}" "${CODE_ROOT}"; printf ' %q' "${command[@]}"; printf '\n'; return 0; fi
  mkdir -p "${output_dir}"; CUDA_VISIBLE_DEVICES="${gpu}" PYTHONPATH="${CODE_ROOT}" "${command[@]}" >"${log_path}" 2>&1 &
...
for index in "${!pids[@]}"; do printf '%s|%s|%s|%s|%s|%s|40|true\n' "${pids[index]}" "${folds[index]}" "${arms[index]}" "${gpus[index]}" "${outs[index]}" "${logs[index]}" >>"${LOG_ROOT}/pids.tsv"; done
status=0
