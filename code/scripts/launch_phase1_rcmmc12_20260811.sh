...
# Invoke explicitly as: bash scripts/launch_phase1_rcmmc12_20260811.sh [--dry-run]
# Frozen six-fold P1-RCMMC C/G continuation matrix. C is the exact GeoSat-C
# continuation control; G adds only the frozen source-L RCMMC auxiliary.
# This launcher is mechanical; scientific RCMMC constants are validated by the core.
RUN_ID="${RUN_ID:-phase1_rcmmc12_20260811_v1}"
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in
    C) arm_args=(--phase1_rcmmc_enabled false --lambda_rcmmc 0) ;;
    G) arm_args=(--phase1_rcmmc_enabled true --lambda_rcmmc 0.02) ;;
