#!/usr/bin/env python3
"""Prepare, smoke, and run the frozen D92-BE Hard12 matrix."""
...
from cvsrffi.stage2_d92_be_hard12 import (  # noqa: E402
...
PREDICTION_ENTRY = CODE_ROOT / "scripts" / "run_d92_be_prediction.py"
...
class D92BEHard12RunnerError(RuntimeError):
    """Raised when the immutable Hard12 runner contract drifts."""
...
def _load_manifest(path: str | Path, expected_sha256: str) -> dict[str, Any]:
    ...
    if source.is_symlink() or not source.is_file():
        raise D92BEHard12RunnerError("matrix manifest must be a regular file")
    if _sha256_file(source) != str(expected_sha256).lower():
        raise D92BEHard12RunnerError("matrix manifest SHA drift")
    payload = json.loads(source.read_text(encoding="utf-8-sig"))
    if (
        payload.get("schema") != "cvs.phase2.d92_be_hard12.matrix.v1"
        ...
        raise D92BEHard12RunnerError("matrix manifest contract drift")
    return payload
    ...
    for state in ("before", "after"):
        for phase in ("enrollment", "apply"):
            ...
            if item.get("expected_seal_sha256") is None:
                raise D92BEHard12RunnerError("package seal SHA was not materialized")
            command.extend(
                [
...
def _child_env(cpu_threads: int) -> dict[str, str]:
    ...
    if threads <= 0:
        raise D92BEHard12RunnerError("CPU thread count must be positive")
...
def prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if output.exists():
        raise D92BEHard12RunnerError("matrix output already exists")
...
def smoke(args: argparse.Namespace) -> dict[str, Any]:
    ...
    matches = [
        ...
        if job["outer_key"] == "rx_3_19__seed_713104__k_1__new_20"
        and job["arm_id"] == "FULL"
    ]
    if len(matches) != 1:
        raise D92BEHard12RunnerError("smoke row identity drift")
    output = Path(args.output_root)
    if output.exists():
        raise D92BEHard12RunnerError("smoke output already exists")
    output.mkdir(parents=True)
    ...
    if completed.returncode != 0:
        raise D92BEHard12RunnerError("truth-free smoke prediction failed")
    for state in ("before", "after"):
        if not (prediction_root / state / "COMMIT.json").is_file():
            raise D92BEHard12RunnerError("smoke prediction commit missing")
    receipt = {
        "status": "D92_BE_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS",
...
def run_shard(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if int(args.shard_count) != 8 or int(args.shard_index) not in range(8):
        raise D92BEHard12RunnerError("shard identity drift")
    selected = [
        job
        for job in manifest["jobs"]
    ...
    if not selected:
        raise D92BEHard12RunnerError("selected shard has no jobs")
    ...
    if events_path.exists() or summary_path.exists():
        raise D92BEHard12RunnerError("shard evidence already exists")
    ...
    for job in selected:
        ...
        _append_event(
            events_path,
            {"timestamp": _now(), "event": "JOB_PREDICTION_START", "job_id": job["job_id"], "command": prediction_command},
        )
        ...
        for state in ("before", "after"):
            if not (job_root / "diag" / state / "COMMIT.json").is_file():
                raise D92BEHard12RunnerError("prediction child returned without commit")
    ...
    summary = {
        "schema": "cvs.phase2.d92_be_hard12.shard_summary.v1",
        "status": status,
...
def main() -> int:
    ...
    return 0 if result["status"] in {
        "HARD12_MATRIX_PREPARED",
        "D92_BE_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS",
        "PASS",
    } else 1
