...
MATRIX_LOG_ROOT="${MATRIX_LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_satphysmv11_matrix_20260702}"
...
declare -a CELLS=(
  "phase1_adv3b02_satphysmv11_rx20_1_u10_20260702 20-1 10-1,10-10"
  "phase1_adv3b02_satphysmv11_rx20_1_u1_20260702 20-1 1-16,4-10"
  "phase1_adv3b02_satphysmv11_rx3_19_u10_20260702 3-19 10-1,10-10"
  "phase1_adv3b02_satphysmv11_rx3_19_u1_20260702 3-19 1-16,4-10"
  "phase1_adv3b02_satphysmv11_rx7_14_u10_20260702 7-14 10-1,10-10"
  "phase1_adv3b02_satphysmv11_rx7_14_u1_20260702 7-14 1-16,4-10"
  "phase1_adv3b02_satphysmv11_rx7_7_u10_20260702 7-7 10-1,10-10"
  "phase1_adv3b02_satphysmv11_rx7_7_u1_20260702 7-7 1-16,4-10"
  "phase1_adv3b02_satphysmv11_rx8_8_u10_20260702 8-8 10-1,10-10"
  "phase1_adv3b02_satphysmv11_rx8_8_u1_20260702 8-8 1-16,4-10"
)

declare -a POLICIES=(
  "SATPHY11_LIN_SRC9999 linear bce 64 500 0.020 0.0001 source_accept 0.9999 0.05 1.0 1.0 1.0"
  "SATPHY11_LIN_SRC999 linear bce 64 500 0.020 0.0001 source_accept 0.9990 0.05 1.0 1.0 1.0"
  "SATPHY11_LIN_PROXY05 linear bce 64 500 0.020 0.0001 proxy_far 1.0000 0.05 1.0 1.0 1.0"
  "SATPHY11_LIN_MIN05 linear bce 64 500 0.020 0.0001 min_source_proxy 1.0000 0.05 1.0 1.0 1.0"
  "SATPHY11_MLP64_SRC9999 mlp bce 64 350 0.005 0.0001 source_accept 0.9999 0.05 1.0 1.0 1.0"
  "SATPHY11_MLP64_PROXY05 mlp bce 64 350 0.005 0.0001 proxy_far 1.0000 0.05 1.0 1.0 1.0"
  "SATPHY11_MLP64_MIN05 mlp bce 64 350 0.005 0.0001 min_source_proxy 1.0000 0.05 1.0 1.0 1.0"
  "SATPHY11_MLP64_MARGIN_SRC5 mlp margin 64 350 0.003 0.0001 source_accept 0.9999 0.05 5.0 1.0 1.0"
  "SATPHY11_MLP64_MARGIN_PROXY05 mlp margin 64 350 0.003 0.0001 proxy_far 1.0000 0.05 5.0 1.0 1.0"
  "SATPHY11_MLP64_MARGIN_MIN05 mlp margin 64 350 0.003 0.0001 min_source_proxy 1.0000 0.05 5.0 1.0 1.0"
)

echo "[PHASE1-SATPHYSMV11-SWEEP] start=$(date -Is) gpu=${GPU}"
for cell in "${CELLS[@]}"; do
  read -r RUN_ID TARGET_RECEIVER_IDS UNKNOWN_TX_IDS <<<"${cell}"
  RUNS_ROOT="${ROOT}/runs/${RUN_ID}"
  LOG_ROOT="${ROOT}/logs/${RUN_ID}"
  FEATURE_DIR="${RUNS_ROOT}/ADV3B02_CORE90_SOFT_E200_PHASE1_SATPHYSMV11"
  FEATURE_NPZ="${FEATURE_DIR}/features_satphysmv11.npz"
  mkdir -p "${RUNS_ROOT}" "${LOG_ROOT}" "${FEATURE_DIR}"
  echo "[PHASE1-SATPHYSMV11-CELL] run_id=${RUN_ID} target_rx=${TARGET_RECEIVER_IDS} unknown=${UNKNOWN_TX_IDS}"
  ...
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${GPU}" \
    ...
    --satellite_tta_policy sat_rx_phys11 \
    ...
    --device "cuda:0" \
    --seed 4070231 \
    > "${LOG_ROOT}/export_satphysmv11.out" 2>&1

  for policy_row in "${POLICIES[@]}"; do
    read -r NAME HEAD_TYPE LOSS_MODE HIDDEN_DIM EPOCHS LR L2 TH_POLICY SRC_Q PROXY_Q SRC_W PROXY_W MARGIN <<<"${policy_row}"
    ...
    env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
      ...
      > "${LOG_ROOT}/${NAME}_satphysmv11.out" 2>&1
  done
done

"${PYTHON}" - <<'PY' "${ROOT}/runs" "${MATRIX_LOG_ROOT}/satphysmv11_sweep_summary.csv"
import csv
import json
import sys
...
out = Path(sys.argv[2])
rows = []
for metrics_path in sorted(runs_root.glob("phase1_adv3b02_satphysmv11_*_20260702/SATPHY11_*/metrics.json")):
...
PY
echo "[PHASE1-SATPHYSMV11-SWEEP-DONE] end=$(date -Is) summary=${MATRIX_LOG_ROOT}/satphysmv11_sweep_summary.csv"
