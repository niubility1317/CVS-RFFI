...
RUN_ID="${RUN_ID:-phase2_adv3b02_manynew10_pairprotected_20260704}"
...
if (( ${#GPUS[@]} < ${#VARIANTS[@]} )); then
  echo "[MANYNEW10-PAIR-GPU-COUNT] need ${#VARIANTS[@]} GPUs but got ${GPUS_CSV}" >&2
  exit 2
fi
...
echo "[MANYNEW10-PAIRPROTECTED] run_id=${RUN_ID} dry_run=${DRY_RUN} target_rx=${TARGET_RX} gpus=${GPUS_CSV}"
echo "[MANYNEW10-PAIRPROTECTED] target_new=${TARGET_NEW_TX_IDS}"
echo "[MANYNEW10-PAIRPROTECTED] proxy_pool_excludes_target_new=true"
echo "[MANYNEW10-PAIRPROTECTED] success=old_acc>=0.80 and every seen-new class>=0.75"
...
run_variant() {
  ...
  echo "[MANYNEW10-PAIR-VARIANT] name=${NAME} mode=${MODE} gpu=${gpu} start=$(date -Is)"
  if [[ "${DRY_RUN}" == "1" ]]; then
    echo "[MANYNEW10-PAIR-DRY-RUN] ${NAME} -> ${log_path}"
    return 0
  fi
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" \
    ...
    --seed "${SEED}" \
    > "${log_path}" 2>&1
  echo "[MANYNEW10-PAIR-VARIANT-DONE] name=${NAME} gpu=${gpu} end=$(date -Is)"
}
...
if [[ "${failed}" != "0" ]]; then
  echo "[MANYNEW10-PAIR-TRAIN-FAILED] one or more variants failed" >&2
  exit 3
fi

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[MANYNEW10-PAIR-DRY-RUN-DONE]"
  exit 0
fi
...
for row in "${VARIANTS[@]}"; do
  ...
  "${PYTHON}" -u "${ROOT}/code/scripts/phase2_newtx_pair_sweep.py" \
    ...
    --seed 422947 \
    > "${LOG_ROOT}/${NAME}_manynew10_k20_eval.out" 2>&1

  "${PYTHON}" -u "${ROOT}/code/scripts/phase2_confusion_aware_qknn_probe.py" \
    --feature_npz "${FEATURE_NPZ}" \
    --output_json "${RUNS_ROOT}/${CASE_ID}/${NAME}/manynew10_k50_topm_eval.json" \
    --output_csv "${RUNS_ROOT}/${CASE_ID}/${NAME}/manynew10_k50_topm_eval.csv" \
    ...
    --scenario_aware \
    --skip_support_loo \
    > "${LOG_ROOT}/${NAME}_manynew10_k50_topm_eval.out" 2>&1
done

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/manynew10_pairprotected_summary.json" <<'PY'
import json
import sys
...
for path in sorted(case_dir.glob("*/manynew10_k20_eval.json")):
    ...
    if best:
        ...
        rows.append(row)
for path in sorted(case_dir.glob("*/manynew10_k50_topm_eval.json")):
    payload = json.loads(path.read_text(encoding="utf-8"))
    best = payload.get("best_by_query", [])
    if best:
        row = dict(best[0])
        row["variant"] = path.parent.name
        row["eval"] = "k50_topm_density_query_audit"
        row["json_path"] = str(path)
        rows.append(row)
rows.sort(
    key=lambda r: (
        r.get("passes_joint_target", r.get("query_passes_joint_target", False)),
        r.get("min_seen_new_class_acc", r.get("query_min_seen_new_class_acc", 0.0)),
        r.get("seen_new_acc", r.get("query_seen_new_acc", 0.0)),
        r.get("old_acc", r.get("query_old_acc", 0.0)),
    ),
    reverse=True,
)
...
echo "[MANYNEW10-PAIRPROTECTED-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
