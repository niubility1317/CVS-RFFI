...
RUN_ID="${RUN_ID:-phase2_adv3b02_manynew10_repr_repair_20260704}"
...
# Proxy pool deliberately excludes the ten evaluation seen-new TX IDs.
...
declare -a VARIANTS=(
  "MANYNEW10_HEAD_SEP id_feature_head 35 0.00016 0.90 1.80 14.00 3.00 4.50 0.004 0.22 0.20 0.15 0.20"
  "MANYNEW10_NORM_SEP id_norm_late_feature 35 0.00010 0.75 1.50 16.00 2.50 5.00 0.003 0.20 0.18 0.16 0.30"
)

if (( ${#GPUS[@]} < ${#VARIANTS[@]} )); then
  echo "[MANYNEW10-GPU-COUNT] need ${#VARIANTS[@]} GPUs but got ${GPUS_CSV}" >&2
  exit 2
fi
...
echo "[MANYNEW10-REPR-REPAIR] run_id=${RUN_ID} dry_run=${DRY_RUN} target_rx=${TARGET_RX} gpus=${GPUS_CSV}"
echo "[MANYNEW10-REPR-REPAIR] target_new=${TARGET_NEW_TX_IDS}"
echo "[MANYNEW10-REPR-REPAIR] proxy_pool_excludes_target_new=true"
echo "[MANYNEW10-REPR-REPAIR] success=old_acc>=0.80 and min_seen_new_class_acc>=0.75"
...
run_variant() {
  ...
  echo "[MANYNEW10-VARIANT] name=${NAME} mode=${MODE} gpu=${gpu} start=$(date -Is)"
  if [[ "${DRY_RUN}" == "1" ]]; then
    echo "[MANYNEW10-DRY-RUN] ${NAME} -> ${log_path}"
    return 0
  fi
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" \
    ...
    --seed "${SEED}" \
    > "${log_path}" 2>&1
  echo "[MANYNEW10-VARIANT-DONE] name=${NAME} gpu=${gpu} end=$(date -Is)"
}
...
if [[ "${failed}" != "0" ]]; then
  echo "[MANYNEW10-TRAIN-FAILED] one or more variants failed" >&2
  exit 3
fi

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[MANYNEW10-DRY-RUN-DONE]"
  exit 0
fi
...
echo "[MANYNEW10-REPR-REPAIR-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
