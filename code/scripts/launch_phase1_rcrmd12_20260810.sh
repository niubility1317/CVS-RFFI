...
# Invoke explicitly as: bash scripts/launch_phase1_rcrmd12_20260810.sh [--dry-run]
# Frozen six-fold P1-RCRMD C/G continuation matrix.  C is the exact GeoSat-C
# continuation; G adds only the fixed source-L RX×local4 relative-margin-drop
# term.  All other candidate routes remain explicitly disabled.
RUN_ID="${RUN_ID:-phase1_rcrmd12_20260810_v1}"
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in
    C) arm_args=(--phase1_rcrmd_enabled false --lambda_rcrmd 0) ;;
    G) arm_args=(--phase1_rcrmd_enabled true --lambda_rcrmd 0.02) ;;
