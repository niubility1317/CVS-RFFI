...
# Invoke explicitly as: bash scripts/launch_phase1_cagm_postfreeze_20260810.sh [--dry-run]
# The CAGM postfreeze contract has exactly 42 operations: 12 clean L/V/proxy
# exports, 12 source-only three-LEO exports plus CAGM bindings, 12 frozen
# logits proxy JSON/CSV bindings, then six CPU-serial C/G pair closures.
POSTFREEZE_RUN_ID="${POSTFREEZE_RUN_ID:-phase1_cagm_postfreeze_20260810_v2}"
...
TRAIN_RUN_ROOT="${TRAIN_RUN_ROOT:-${PROJECT_ROOT}/runs/phase1_cagm12_20260810_v2}"
POSTFREEZE_ROOT="${POSTFREEZE_ROOT:-${PROJECT_ROOT}/runs/${POSTFREEZE_RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/${POSTFREEZE_RUN_ID}}"
CAGM_CLEAN_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_cagm_features.py"
CAGM_LEO_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_cagm_leo_features.py"
LOGITS_REJECT_SCRIPT="${CODE_ROOT}/scripts/eval_phase1_logits_open_set_reject.py"
PAIR_EVAL_SCRIPT="${CODE_ROOT}/evaluate_phase1_cagm_postfreeze_pair.py"
DRY_RUN="${DRY_RUN:-0}"
...
[[ "${POSTFREEZE_RUN_ID}" == "phase1_cagm_postfreeze_20260810_v2" ]] || {
  echo "POSTFREEZE_RUN_ID is frozen to phase1_cagm_postfreeze_20260810_v2" >&2
  exit 3
}
[[ "$(basename "${TRAIN_RUN_ROOT}")" == "phase1_cagm12_20260810_v2" ]] || {
  echo "TRAIN_RUN_ROOT leaf must be phase1_cagm12_20260810_v2" >&2
  exit 3
}
[[ "$(basename "${POSTFREEZE_ROOT}")" == "phase1_cagm_postfreeze_20260810_v2" ]] || {
  echo "POSTFREEZE_ROOT leaf must bind the frozen CAGM matrix" >&2
  exit 3
}
...
for required in "${CAGM_CLEAN_EXPORT_SCRIPT}" "${CAGM_LEO_EXPORT_SCRIPT}" "${LOGITS_REJECT_SCRIPT}" "${PAIR_EVAL_SCRIPT}"; do
  [[ -f "${required}" ]] || { echo "missing CAGM postfreeze script: ${required}" >&2; exit 2; }
done
...
run_candidate() {
  ...
  local candidate="F${fold}${arm}_CAGM12"
  ...
  # CAGM-only and the pair evaluator rejects any ICMT identity.
  ...
  local -a clean_command=(
    "${PYTHON}" -u "${CAGM_CLEAN_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}"
    --wisig_pkl "${WISIG_PKL}"
    --out_npz "${clean_npz}"
  ...
  local -a leo_command=(
    "${PYTHON}" -u "${CAGM_LEO_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}"
    --wisig_pkl "${WISIG_PKL}"
    --out_npz "${leo_npz}"
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command CAGM_CLEAN_EXPORT "${gpu}" "${clean_command[@]}"
    print_command CAGM_LEO_EXPORT_AND_BIND "${gpu}" "${leo_command[@]}"
    print_command FROZEN_LOGITS_PROXY_BINDING "${gpu}" "${proxy_command[@]}"
    return 0
  fi
  [[ -f "${checkpoint}" ]] || { echo "missing CAGM final-only checkpoint: ${checkpoint}" >&2; return 2; }
...
# is serial; independent CAGM candidates are concurrent only at this level.
...
run_pair() {
  ...
  local fold_index=$((fold - 1))
  local train_tx="${FOLD_TRAIN_TX[fold_index]}"
  local c_candidate="F${fold}C_CAGM12"
  local g_candidate="F${fold}G_CAGM12"
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command CAGM_PAIR_SCORE CPU "${pair_command[@]}"
    return 0
  fi
