...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_center_veto_probe_20260707}"
...
declare -a PROFILES=("CENTER_VETO_E95_ANY_M1" "CENTER_VETO_E92L92S92_M2")

mkdir -p "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}"
echo "[STAGE2C-CENTER-VETO] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-CENTER-VETO] profiles=${PROFILES[*]} k=${K_SHOTS[*]}"
echo "[STAGE2C-CENTER-VETO] diagnostic_only=true unknown_query_eval_only=true"
...
run_one() {
  ...
  echo "[STAGE2C-CENTER-VETO-RUN] variant=${variant} profile=${profile} k=${k} feature=${feature_npz}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
  EXTRA_ARGS=()
...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-CENTER-VETO-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_center_veto_probe_summary.json" <<'PY'
import csv
import json
import sys
...
echo "[STAGE2C-CENTER-VETO-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
