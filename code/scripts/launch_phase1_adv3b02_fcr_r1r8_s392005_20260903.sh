...
RUN_ID="${RUN_ID:-phase1_adv3b02_fcr_r1r8_s392005_equalized_20260903_v1}"
...
COMMON_ARGS="--seed 392005 --wisig_equalized 1 --wisig_train_days ${SOURCE_DAYS} --wisig_train_rxs ${SOURCE_RXS} --wisig_test_days ${TARGET_DAYS} --wisig_test_rxs ${TARGET_RXS} --wisig_allow_day_overlap_if_receiver_disjoint true --labeled_ratio 0.07 --unlabeled_ratio 0.63 --source_val_ratio 0.30 --phase1_source_role_protocol legacy_l_u_v --checkpoint_selection final_only --test_eval_policy never --phase1_external_final_eval true"
...
ROOT="${ROOT}" PYTHON="${PYTHON}" WISIG_PKL="${WISIG_PKL}" RUN_ID="${RUN_ID}_ADV3B02" \
  RUNS_ROOT="${OUTPUT_ROOT}/ADV3B02" LOG_ROOT="${LOG_ROOT}/ADV3B02" \
  EXTRA_ARGS="${COMMON_ARGS}" STAGE2_MAX_ACTIVE_PER_GPU=1 \
  "${ROOT}/code/scripts/launch_phase1_adv3_mechanism32_queue_20260701.sh" \
  --only="${BASE_CANDIDATE}"
