#!/usr/bin/env python3
"""Run the B0/B3/V1/V2 ERBT-IDR M2.7 paired screen or full125 grid."""
...
from cvsrffi.stage2_m24_row_executor import run_m27_spectral_veto_row_from_base_cache
from cvsrffi.stage2_m24_safe_residual import D1
from cvsrffi.stage2_m25_anchored_residual import B3
from cvsrffi.stage2_m27_spectral_veto import V1, V2
...
MATRIX_SCHEMA = "cvs.erbt_idr.m27.spectral_veto_prediction_matrix.v1"
EVIDENCE_ARMS = (D1, B3, V1, V2)
...
def _phase_root(
    root: Path, receiver: str, method_seed: int, k_shot: int, new_count: int
) -> Path:
    result = (
        root
    ...
    if not (result / "phase32.manifest.json").is_file() or not (
        result / "phase32.npz"
    ).is_file():
        raise FileNotFoundError(
            f"missing Phase32 cache for rx={receiver}, seed={method_seed}, K={k_shot}, new={new_count}"
        )
    return result
...
def _validate_receipt(task: dict[str, Any], receipt: dict[str, Any]) -> None:
    ...
    if task["arm"] in (V1, V2):
        if any(
            audit.get("query_application", {}).get("row_source_allowlist")
            != ["B0", "B3"]
            for audit in receipt.get("scenario_audit", {}).values()
        ):
            raise ValueError("M2.7 row-source allowlist drift")


def _run_one(task: dict[str, Any]) -> dict[str, Any]:
    manifest_path = Path(task["manifest_path"])
    payload_path = Path(task["payload_path"])
    phase_manifest_path = Path(task["phase_manifest_path"])
    phase_payload_path = Path(task["phase_payload_path"])
    output_root = Path(task["output_root"])
    manifest = json.loads(manifest_path.read_text(encoding="utf-8-sig"))
    phase_manifest = json.loads(
        phase_manifest_path.read_text(encoding="utf-8-sig")
    )
    receipt = run_m27_spectral_veto_row_from_base_cache(
        arm=str(task["arm"]),
        ...
        phase_side_cache_payload=(phase_payload_path if task["arm"] == V2 else None),
        phase_side_cache_manifest=(phase_manifest_path if task["arm"] == V2 else None),
        phase_side_cache_payload_sha256=(
            str(phase_manifest["payload_sha256"]) if task["arm"] == V2 else None
        ),
        phase_side_cache_manifest_sha256=(
            _canonical_manifest_sha(phase_manifest_path) if task["arm"] == V2 else None
        ),
    ...
    return {
        ...
        "phase_cache_root": str(phase_manifest_path.parent),
        "receipt_path": str(
...
def main() -> int:
    ...
    phase_root = Path(args.phase_root).absolute()
    ...
    for receiver in spec["receivers"]:
        for method_seed in spec["seeds"]:
            for k_shot, new_count in spec["conditions"]:
                ...
                phase_cache = _phase_root(
                    phase_root, receiver, method_seed, k_shot, new_count
                )
                    ...
                    tasks.append(
                        {
                            ...
                            "phase_manifest_path": str(
                                phase_cache / "phase32.manifest.json"
                            ),
                            "phase_payload_path": str(phase_cache / "phase32.npz"),
                            "output_root": str(output_root),
                            "device": args.device,
                        }
                    )
    if len(tasks) != spec["expected_method_rows"]:
        raise ValueError("M2.7 task matrix size drift")
