...
echo "[OSFIX16] run_id=${RUN_ID} dry_run=${DRY_RUN} candidates=${#CANDIDATES[@]} max_active_per_gpu=${MAX_ACTIVE_PER_GPU} two_per_gpu=1 teacher=ADV3B02_CORE90_SOFT_E200 base=EPOC_CONCAT_SAT_DIRECT_METRIC_UOPT phase1_dataset=ManySig_only source_only=1 dg_primary=1 leo_primary=1 direct_open_set_metric_loss=1 unlabeled_quarantine_accept=1 concat_sat_mode=full_2b_core_domain concat_sat_ce_only=0 stage2_success_claim=0 deployment_success_claim=0 only=${ONLY_CANDIDATES:-ALL}"
