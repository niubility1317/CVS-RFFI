...
LOG_ROOT="${LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_classwise_repair_v15_20260703}"
...
for required in "${CLEAN_NPZ}" "${TRAIN_SAT_CLEAR}" "${TRAIN_SAT_LOW}" "${TRAIN_SAT_RAIN}"; do
  if [[ ! -f "${required}" ]]; then
    echo "[V15-MISSING] ${required}" >&2
    exit 2
  fi
done

SUMMARY_CSV="${LOG_ROOT}/classwise_repair_v15_summary.csv"
METRICS_JSON="${LOG_ROOT}/classwise_repair_v15_metrics.json"
BEST_JSON="${LOG_ROOT}/classwise_repair_v15_best.json"

echo "[PHASE1-CLASSWISE-REPAIR-V15] start=$(date -Is)"
env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
  "${PYTHON}" -u "${ROOT}/code/scripts/eval_phase1_classwise_leo_repair_reject_20260703.py" \
  --runs_root "${RUNS_ROOT}" \
  --clean_npz "${CLEAN_NPZ}" \
  ...
  > "${LOG_ROOT}/classwise_repair_v15.out" 2>&1
...
echo "[PHASE1-CLASSWISE-REPAIR-V15-DONE] end=$(date -Is) summary=${SUMMARY_CSV} best=${BEST_JSON}"
