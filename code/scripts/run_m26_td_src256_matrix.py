#!/usr/bin/env python3
"""Run the B0/T1-T5 ERBT-IDR M2.6 paired screen or complete 125 grid."""
...
from cvsrffi.stage2_m24_row_executor import run_m26_td_src256_row_from_base_cache
...
MATRIX_SCHEMA = "cvs.erbt_idr.m26.td_src256_prediction_matrix.v1"
EVIDENCE_ARMS = (D1, *M26_ARMS)
...
def _validate_receipt(task: dict[str, Any], receipt: dict[str, Any]) -> None:
    ...
    if task["arm"] == D1:
        ...
        if parity.get("prediction_disagreements") != 0 or parity.get("before_prediction_disagreements") != 0:
...
def _run_one(task: dict[str, Any]) -> dict[str, Any]:
    ...
    receipt = run_m26_td_src256_row_from_base_cache(
        arm=str(task["arm"]),
        ...
        source_anchor_path=task["source_anchor"],
        expected_checkpoint_sha256=str(task["checkpoint_sha256"]),
...
def main() -> int:
    ...
    anchor = load_m26_spectral_anchor(
        args.source_anchor, expected_checkpoint_sha256=args.checkpoint_sha256
    )
            ...
            for k_shot, new_count in spec["conditions"]:
                ...
                for arm in EVIDENCE_ARMS:
                    ...
                    tasks.append({
                        ...
                        "source_anchor": str(Path(args.source_anchor).absolute()),
                        "checkpoint_sha256": str(args.checkpoint_sha256).lower(),
                        "output_root": str(output_root),
                        "device": args.device,
                    })
    if len(tasks) != spec["expected_method_rows"]:
        raise ValueError("M2.6 task matrix size drift")
    ...
    matrix = {
        ...
        "conditions": [{"k_shot": k, "new_class_count": n} for k, n in spec["conditions"]],
        "arms": list(EVIDENCE_ARMS),
        "feature_roots": [str(root) for root in feature_roots],
        "source_anchor": {
            "path": str(Path(args.source_anchor).absolute()),
