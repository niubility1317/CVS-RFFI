...
# Final-only postfreeze closure for the completed P1-CB-SFCE C/G matrix.
...
POSTFREEZE_RUN_ID="${POSTFREEZE_RUN_ID:-phase1_cb_sfce_postfreeze_20260809_v1}"
...
TRAIN_RUN_ROOT="${TRAIN_RUN_ROOT:-${PROJECT_ROOT}/runs/phase1_cb_sfce12_20260809_v1}"
...
PAIR_EVAL_SCRIPT="${CODE_ROOT}/scripts/eval_phase1_cb_sfce_pair.py"
DRY_RUN="${DRY_RUN:-0}"
...
# Frozen source train/known-validation/proxy roles and the training GPU map.
FOLD_TRAIN_TX=(
...
run_candidate() {
  ...
  # Fixed source-calibrated proxy score is diagnostic-only.  It is not read by
  # any export or pair computation except its pre-registered non-compensating
  # AUROC/FAR guardrail.
...
# Same fixed mapping as the 12-task frozen continuation launcher; at most two
# candidate pipelines are active on each physical GPU.
...
run_pair() {
  ...
  local fold_index=$((fold - 1))
  local train_tx="${FOLD_TRAIN_TX[fold_index]}"
  local c_candidate="F${fold}C_CB_SFCE12"
  local g_candidate="F${fold}G_CB_SFCE12"
...
# Pair evaluation is intentionally CPU-serial; F6 emits the six-fold 18-cell
# aggregate only after F1--F5 immutable per-fold JSONs exist.
run_pair 1
run_pair 2
run_pair 3
run_pair 4
