...
# Frozen six-fold P1-GD-ProtoNLL C/G continuation matrix.  C is the exact
# GeoSat-C continuation control; G adds only the frozen L-only GD focal term.
RUN_ID="${RUN_ID:-phase1_gd_proto_nll12_20260809_v1}"
...
COMMON=(
  ...
  --phase1_cb_sfce_frozen_mode false --phase1_cb_sfce_enabled false --lambda_cb_sfce 0 --phase1_cp_sfce_frozen_mode false --phase1_cp_sfce_enabled false --lambda_cp_sfce 0
  --phase1_gd_proto_nll_frozen_mode true --gd_proto_nll_gamma 1 --max_grad_norm 5.0 --checkpoint_selection final_only --best_metric source_val_sat_hmean --phase1_source_val_selection_only true
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in
    C) arm_args=(--phase1_gd_proto_nll_enabled false --lambda_gd_proto_nll 0) ;;
    G) arm_args=(--phase1_gd_proto_nll_enabled true --lambda_gd_proto_nll 0.10) ;;
