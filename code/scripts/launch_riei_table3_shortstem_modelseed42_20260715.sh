...
# RIEI Table III confirmation for pre-registered model seed 42 while keeping
# the stable data partition fixed to split seed 1337.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT="${ROOT:-$(cd "${SCRIPT_DIR}/../.." && pwd)}"
PYTHON_BIN="${PYTHON_BIN:-/home/szu2070436088/.conda/envs/CVS-RFFI/bin/python}"
WISIG_PKL="${WISIG_PKL:-${ROOT}/Dataset_WigSig/ManySig.pkl}"
RUN_ID="${RUN_ID:-paper_repro_riei_table3_shortstem_modelseed42_split1337_20260715_100000}"
...
echo "[RIEI-TABLE3-SHORTSTEM-SEED42] run_id=${RUN_ID} model_seed=${SEED} split_seed=${SPLIT_SEED} dry_run=${DRY_RUN} rows=${#ROWS[@]}"
echo "[RIEI-TABLE3-SHORTSTEM-SEED42] fed_variant=${FED_VARIANT} split=stable_group_seed_shared_train_test_holdout metric=paper_last10 reduction=mean"
...
for i in "${!ROWS[@]}"; do
  ...
  gpu="${GPU_IDS[$((i % 8))]}"
  job_id="riei_shortstem_table3_row${row}_${combo}_modelseed${SEED}_split${SPLIT_SEED}"
  ...
  cmd=(env "METHODS=riei_fd" "WISIG_PROTOCOL=riei_original" "GPU_IDS=${gpu}"
    ...
    "PYTHON_BIN=${PYTHON_BIN}" "WISIG_PKL=${WISIG_PKL}" "BASELINE_EPOCHS=200" "SEED=${SEED}" "WISIG_SPLIT_SEED=${SPLIT_SEED}" "SAT_EVAL=0"
  ...
  echo "[JOB] row=${row} variant=${FED_VARIANT} model_seed=${SEED} split_seed=${SPLIT_SEED} id=${job_id} gpu=${gpu} train=${train_rxs} test=${test_rxs} paper=${paper_mean}+/-${paper_sd}"
  echo "[CMD] $(format_cmd "${cmd[@]}")"
  [[ "${DRY_RUN}" != 1 ]] || continue
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "${row}" "${FED_VARIANT}" "${SEED}" "${SPLIT_SEED}" "${job_id}" "${gpu}" "${train_rxs}" "${test_rxs}" "${paper_mean}" "${paper_sd}" "${run_dir}" "$(format_cmd "${cmd[@]}")" >> "${RUN_ROOT}/scheduler_manifest.tsv"
...
echo "[RIEI-TABLE3-SHORTSTEM-SEED42] submitted dry_run=${DRY_RUN}"
