#!/usr/bin/env python3
"""Run one truth-free D92-BE prediction job from sealed predictor packages."""
...
from cvsrffi.stage2_d92_be_query_evaluation import (  # noqa: E402
    run_d92_be_query_evaluation,
)
from cvsrffi.stage2_d92_be_slim import D92_BE_ARMS  # noqa: E402


class D92BEPredictionEntryError(RuntimeError):
    """Raised when a prediction job would violate its immutable boundary."""
...
def _readonly_file(path: Path) -> None:
    if (
        ...
        raise D92BEPredictionEntryError(f"prediction output is not read-only: {path}")


def run(args: argparse.Namespace) -> dict[str, Any]:
    output = Path(args.output_root)
    if output.exists():
        raise D92BEPredictionEntryError(
            f"prediction output already exists; refusing overwrite: {output}"
        )
    result = run_d92_be_query_evaluation(
        arm_id=args.arm,
    ...
    return {
        "schema": "cvs.phase2.d92_be.truth_free_prediction_entry.v1",
        "status": "D92_BE_TRUTH_FREE_PREDICTIONS_COMPLETE",
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--arm", choices=tuple(D92_BE_ARMS), required=True)
