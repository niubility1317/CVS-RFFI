...
# Third source-only postfreeze wave.  Build one immutable received-IQ cache
# per fold, then reuse those exact bytes for the corresponding C and G forward.
RUN_ID="${RUN_ID:-phase1_clic_source_leo_20260812_v3}"
...
[[ "${RUN_ID}" == "phase1_clic_source_leo_20260812_v3" ]] || { echo "source LEO run ID drift" >&2; exit 2; }
