...
# One-shot final-only closure for the completed P1-GD-ProtoNLL v3 C/G matrix.
...
TRAIN_RUN_ROOT="${TRAIN_RUN_ROOT:-${PROJECT_ROOT}/runs/phase1_gd_proto_nll12_20260809_v3}"
POSTFREEZE_ROOT="${POSTFREEZE_ROOT:-${PROJECT_ROOT}/runs/${POSTFREEZE_RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${PROJECT_ROOT}/logs/${POSTFREEZE_RUN_ID}}"
GD_CLEAN_EXPORT_SCRIPT="${CODE_ROOT}/export_phase1_gd_proto_nll_features.py"
...
PAIR_EVAL_SCRIPT="${CODE_ROOT}/cvsrffi/eval_phase1_gd_proto_nll_pair.py"
DRY_RUN="${DRY_RUN:-0}"
...
[[ "$(basename "${TRAIN_RUN_ROOT}")" == "phase1_gd_proto_nll12_20260809_v3" ]] || {
  echo "TRAIN_RUN_ROOT leaf must be phase1_gd_proto_nll12_20260809_v3" >&2
  exit 3
}
...
for required in "${GD_CLEAN_EXPORT_SCRIPT}" "${FEATURE_EXPORT_SCRIPT}" "${LOGITS_REJECT_SCRIPT}" "${PAIR_EVAL_SCRIPT}"; do
...
run_candidate() {
  ...
  local candidate="F${fold}${arm}_GD_PROTO_NLL12"
  ...
  local clean_npz="${candidate_dir}/gd_clean_l_v_proxy_final_only.npz"
  ...
  local -a clean_command=(
    "${PYTHON}" -u "${GD_CLEAN_EXPORT_SCRIPT}"
    --ckpt "${checkpoint}"
    --wisig_pkl "${WISIG_PKL}"
    --out_npz "${clean_npz}"
  ...
  if [[ "${DRY_RUN}" == "1" ]]; then
    print_command GD_CLEAN_EXPORT "${gpu}" "${clean_command[@]}"
...
# Preserve the v3 continuation GPU mapping.  Candidate-internal stages are
...
run_pair() {
  ...
  local fold_index=$((fold - 1))
  local train_tx="${FOLD_TRAIN_TX[fold_index]}"
  local c_candidate="F${fold}C_GD_PROTO_NLL12"
  local g_candidate="F${fold}G_GD_PROTO_NLL12"
  ...
  local -a pair_command=(
    ...
    --c-clean-npz "${POSTFREEZE_ROOT}/${c_candidate}/gd_clean_l_v_proxy_final_only.npz"
    --g-clean-npz "${POSTFREEZE_ROOT}/${g_candidate}/gd_clean_l_v_proxy_final_only.npz"
