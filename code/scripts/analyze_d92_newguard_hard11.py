#!/usr/bin/env python3
"""Analyze one immutable D92 NewGuard Hard11 artifact set."""
...
from cvsrffi.stage2_d92_newguard_hard11_analysis import analyze_d92_newguard_hard11  # noqa: E402
...
def _markdown(result: Mapping[str, Any]) -> str:
    ...
    lines = ["# D92 E0_FULL_BIDIRECTIONAL_NEWGUARD_MAXMIN Hard11分析", "", f"- 状态：`{result['status']}`", f"- 判定：`{result['verdict']}`", "- 范围：`DEVELOPMENT_ONLY_HARD_SCREEN`；K1仅作liveness，不进入性能均值。", "", "## 八项严格Pareto", "", "|指标|候选均值|相对E0_FULL_ONLY|", "|---|---:|---:|"]
