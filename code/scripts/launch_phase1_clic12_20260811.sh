...
if [[ "${DRY_RUN}" == "0" ]]; then
  ...
  for ckpt in "${FOLD_BASELINE_CKPT[@]}"; do
    [[ -f "${ckpt}" ]] || { echo "missing baseline checkpoint: ${ckpt}" >&2; exit 2; }
  done
  mkdir -p "${RUN_ROOT}" "${LOG_ROOT}"
fi

COMMON=(
  --dataset wisig --wisig_pkl "${WISIG_PKL}" --from_scratch false --freeze_backbone false
  --model_variant lite_d --id_feature_key feat_joint --branch_ablation no_dac
  --split_mode tx_rx_day_1_6_3 --labeled_ratio 0.07 --unlabeled_ratio 0.63 --source_val_ratio 0.30
  --epochs 40 --label_epochs 40 --pseudo_epochs 0 --seed 7281164 --sat_view_seed 7281164
  --lr 0.0002 --weight_decay 0.0001 --label_smoothing 0.01 --batch_size 128 --eval_batch_size 256
  --num_workers 4 --prefetch_factor 2 --device cuda:0 --amp true
  --phase1_clic_frozen_mode true
  --lambda_u 0 --lambda_ent 0 --lambda_u_domain 0 --lambda_u_adv 0 --lambda_u_sat_cons 0
  --lambda_u_direct_metric_accept 0 --lambda_u_quarantine_accept 0 --lambda_domain 0 --lambda_adv 0
  --lambda_orth 0 --lambda_cons 0 --lambda_group_ce 0 --lambda_fishr 0
  ...
  --lambda_zid_compact 0 --lambda_source_episode 0 --lambda_direct_metric_accept 0
  ...
  --phase1_pamr_frozen_mode false --phase1_pamr_enabled false --lambda_pamr 0
  --phase1_cb_sfce_frozen_mode false --phase1_cb_sfce_enabled false --lambda_cb_sfce 0
  --phase1_gd_proto_nll_frozen_mode false --phase1_gd_proto_nll_enabled false --lambda_gd_proto_nll 0
  --phase1_icmt_frozen_mode false --phase1_icmt_enabled false --lambda_icmt 0
  --phase1_cagm_frozen_mode false --phase1_cagm_enabled false --lambda_cagm 0
  --phase1_rcrmd_frozen_mode false --phase1_rcrmd_enabled false --lambda_rcrmd 0
  --phase1_rcat_frozen_mode false --phase1_rcat_enabled false --lambda_rcat 0
  --phase1_rcmmc_frozen_mode false --phase1_rcmmc_enabled false --lambda_rcmmc 0
  --phase1_hscf_frozen_mode false --phase1_hscf_enabled false --lambda_hscf 0
  --phase1_hnccd_frozen_mode false --phase1_hnccd_enabled false --lambda_hnccd 0
  --phase1_recte_frozen_mode false --phase1_recte_enabled false --lambda_recte 0
  --phase1_cp_sfce_frozen_mode false --phase1_cp_sfce_enabled false --lambda_cp_sfce 0
  --max_grad_norm 5.0 --checkpoint_selection final_only --best_metric clean_val_tx
  --phase1_source_val_selection_only true --eval_sat_channel false --source_val_heavy_eval_start_epoch 40
  --source_val_heavy_eval_interval 40 --source_val_heavy_eval_final_window 0
  --test_eval_policy interval_final --test_eval_start_epoch 40 --test_eval_interval 40
)
...
launch_arm() {
  ...
  local operator
  case "${arm}" in
    C) operator="raw_phase_control" ;;
    G) operator="complex_local_invariant_curvature" ;;
    *) echo "unsupported arm: ${arm}" >&2; return 2 ;;
  esac
  local -a command=("${PYTHON}" -u "${TRAIN_SCRIPT}" "${COMMON[@]}"
    --run_id "${RUN_ID}" --candidate_id "${candidate}" --run_name "${candidate}"
