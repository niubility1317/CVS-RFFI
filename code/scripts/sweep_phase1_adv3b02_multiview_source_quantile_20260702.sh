...
echo "[PHASE1-MULTIVIEW-SOURCE-Q-SWEEP] start=$(date -Is)"
for cell in "${CELLS[@]}"; do
  ...
  echo "[PHASE1-MULTIVIEW-SOURCE-Q-CELL] run_id=${RUN_ID} unknown=${UNKNOWN_TX_IDS}"
  for policy_row in "${POLICIES[@]}"; do
    read -r POLICY_NAME SRC_Q <<<"${policy_row}"
    NAME="MV_LIN_${POLICY_NAME}"
    ...
    env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
      ...
      > "${LOG_ROOT}/${NAME}_sourceq_sweep1.out" 2>&1
  done
done

"${PYTHON}" - <<'PY' "${ROOT}/runs" "${MATRIX_LOG_ROOT}/multiview_sourceq_sweep_summary.csv"
import csv
import json
import sys
...
out = Path(sys.argv[2])
rows = []
for metrics_path in sorted(runs_root.glob("phase1_adv3b02_multiview_keepold_*_20260702/MV_LIN_SRC*/metrics.json")):
    if metrics_path.parent.name not in {"MV_LIN_SRC999", "MV_LIN_SRC9995", "MV_LIN_SRC9999", "MV_LIN_SRC1000"}:
        continue
...
PY
echo "[PHASE1-MULTIVIEW-SOURCE-Q-SWEEP-DONE] end=$(date -Is) summary=${MATRIX_LOG_ROOT}/multiview_sourceq_sweep_summary.csv"
