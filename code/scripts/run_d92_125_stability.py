#!/usr/bin/env python3
"""Run the locked D92 125-job stability screen in eight GPU shards."""
...
def _configure_child_cpu_threads(cpu_threads: int) -> dict[str, str]:
    ...
    if threads <= 0:
        raise base.StabilityLauncherError("D92 cpu thread count must be positive")
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        ...
        type=int,
        default=2,
        help="CPU threads per D92 row for sklearn/numpy covariance work",
    )
    return result
...
def main() -> int:
    ...
    if len(_GROUND_MANIFEST_SHA256) != 64:
        raise base.StabilityLauncherError("D92 ground manifest SHA drift")
    ...
    base._job_command = _d92_job_command
