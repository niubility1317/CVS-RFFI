...
MATRIX_LOG_ROOT="${MATRIX_LOG_ROOT:-${ROOT}/logs/phase1_adv3b02_satrepair9_matrix_20260702}"
...
declare -a CELLS=(
  "phase1_adv3b02_satrepair9_rx20_1_u10_20260702 20-1 10-1,10-10"
  "phase1_adv3b02_satrepair9_rx20_1_u1_20260702 20-1 1-16,4-10"
  "phase1_adv3b02_satrepair9_rx3_19_u10_20260702 3-19 10-1,10-10"
  "phase1_adv3b02_satrepair9_rx3_19_u1_20260702 3-19 1-16,4-10"
  "phase1_adv3b02_satrepair9_rx7_14_u10_20260702 7-14 10-1,10-10"
  "phase1_adv3b02_satrepair9_rx7_14_u1_20260702 7-14 1-16,4-10"
  "phase1_adv3b02_satrepair9_rx7_7_u10_20260702 7-7 10-1,10-10"
  "phase1_adv3b02_satrepair9_rx7_7_u1_20260702 7-7 1-16,4-10"
  "phase1_adv3b02_satrepair9_rx8_8_u10_20260702 8-8 10-1,10-10"
  "phase1_adv3b02_satrepair9_rx8_8_u1_20260702 8-8 1-16,4-10"
)

declare -a POLICIES=(
  "SATREPAIR9_LIN_SRC9999 linear bce 64 500 0.020 0.0001 1.0 source_accept 0.9999 0.05 all"
  "SATREPAIR9_LIN_SRC999 linear bce 64 500 0.020 0.0001 1.0 source_accept 0.9990 0.05 all"
  "SATREPAIR9_LIN_PROXY05 linear bce 64 500 0.020 0.0001 1.0 proxy_far 1.0000 0.05 all"
  "SATREPAIR9_LIN_MIN05 linear bce 64 500 0.020 0.0001 1.0 min_source_proxy 1.0000 0.05 all"
  "SATREPAIR9_MLP64_SRC9999 mlp bce 64 350 0.005 0.0001 1.0 source_accept 0.9999 0.05 all"
  "SATREPAIR9_MLP64_PROXY05 mlp bce 64 350 0.005 0.0001 1.0 proxy_far 1.0000 0.05 all"
  "SATREPAIR9_MLP64_MIN05 mlp bce 64 350 0.005 0.0001 1.0 min_source_proxy 1.0000 0.05 all"
  "SATREPAIR9_MLP_M20_SRC9999 mlp margin 64 350 0.003 0.0001 20.0 source_accept 0.9999 0.05 all"
  "SATREPAIR9_MLP_M50_SRC9999 mlp margin 64 350 0.003 0.0001 50.0 source_accept 0.9999 0.05 all"
  "SATREPAIR9_MLP_M20_PROXY05 mlp margin 64 350 0.003 0.0001 20.0 proxy_far 1.0000 0.05 all"
  "SATREPAIR9_MLP_M50_PROXY05 mlp margin 64 350 0.003 0.0001 50.0 proxy_far 1.0000 0.05 all"
  "SATREPAIR9_MLP_M50_PROXY02 mlp margin 64 350 0.003 0.0001 50.0 proxy_far 1.0000 0.02 all"
  "SATREPAIR9_MLP_M50_COR_SRC9999 mlp margin 64 350 0.003 0.0001 50.0 source_accept 0.9999 0.05 correct"
  "SATREPAIR9_MLP_M50_COR_PROXY05 mlp margin 64 350 0.003 0.0001 50.0 proxy_far 1.0000 0.05 correct"
)

echo "[PHASE1-SATREPAIR9-SWEEP] start=$(date -Is) gpu=${GPU}"
for cell in "${CELLS[@]}"; do
  read -r RUN_ID TARGET_RECEIVER_IDS UNKNOWN_TX_IDS <<<"${cell}"
  RUNS_ROOT="${ROOT}/runs/${RUN_ID}"
  LOG_ROOT="${ROOT}/logs/${RUN_ID}"
  FEATURE_DIR="${RUNS_ROOT}/ADV3B02_CORE90_SOFT_E200_PHASE1_SATREPAIR9"
  FEATURE_NPZ="${FEATURE_DIR}/features_satrepair9.npz"
  mkdir -p "${RUNS_ROOT}" "${LOG_ROOT}" "${FEATURE_DIR}"
  echo "[PHASE1-SATREPAIR9-CELL] run_id=${RUN_ID} target_rx=${TARGET_RECEIVER_IDS} unknown=${UNKNOWN_TX_IDS}"
  ...
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${GPU}" \
    ...
    --satellite_tta_policy sat_rx_repair9 \
    ...
    --device "cuda:0" \
    --seed 4070249 \
    > "${LOG_ROOT}/export_satrepair9.out" 2>&1
  ...
  for policy_row in "${POLICIES[@]}"; do
    ...
    env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" \
      ...
      "${extra_flags[@]}" \
      > "${LOG_ROOT}/${NAME}_satrepair9.out" 2>&1
  done
done

"${PYTHON}" - <<'PY' "${ROOT}/runs" "${MATRIX_LOG_ROOT}/satrepair9_sweep_summary.csv"
import csv
import json
import sys
...
out = Path(sys.argv[2])
rows = []
for metrics_path in sorted(runs_root.glob("phase1_adv3b02_satrepair9_*_20260702/SATREPAIR9_*/metrics.json")):
...
PY
echo "[PHASE1-SATREPAIR9-SWEEP-DONE] end=$(date -Is) summary=${MATRIX_LOG_ROOT}/satrepair9_sweep_summary.csv"
