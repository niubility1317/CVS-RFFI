...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_contrast_shell_veto_probe_20260707}"
...
declare -a CONTRASTS=(
  "RELIEF_U095_W050M02_D008_R035 0.95 0.50 0.02 0.08 0.35 0.50 0.50"
  "RELIEF_U095_W050M02_D008_R050 0.95 0.50 0.02 0.08 0.50 0.50 0.50"
  "RELIEF_U095_W050M02_D005_R050 0.95 0.50 0.02 0.05 0.50 0.50 0.50"
)
declare -a SHELLS=(
  "SHELL_L065_R085 0.65 0.85"
  "SHELL_L075_R090 0.75 0.90"
  "SHELL_L085_R095 0.85 0.95"
)

mkdir -p "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}"
echo "[STAGE2C-CONTRAST-SHELL-VETO] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-CONTRAST-SHELL-VETO] variants=${VARIANTS[*]} k=${K_SHOTS[*]} contrast_profiles=${#CONTRASTS[@]} shell_profiles=${#SHELLS[@]}"
echo "[STAGE2C-CONTRAST-SHELL-VETO] diagnostic_only=true unknown_query_eval_only=true support_floor=false"
...
run_one() {
  ...
  local shell_label_risk="${10}"
  local shell_reject_risk="${11}"
  local k="${12}"
  ...
  mkdir -p "${out_dir}"
  echo "[STAGE2C-CONTRAST-SHELL-VETO-RUN] variant=${variant} profile=${profile} k=${k} gate=${gate} delta=${delta_min} scales=${label_scale}/${event_scale}/${component_scale} shell=${shell_label_risk}/${shell_reject_risk}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
  "${PYTHON}" -u "${ROOT}/code/scripts/phase2_frozen_manytx_unknown_diagnostic.py" \
    ...
    --candidate_set_max_label_shell_risk "${shell_label_risk}" \
    --candidate_set_max_label_risk_component_agreement 1.0 \
    --candidate_set_unknown_reject_risk "${gate}" \
    --candidate_set_shell_reject_risk "${shell_reject_risk}" \
...
for variant in "${VARIANTS[@]}"; do
  for contrast_row in "${CONTRASTS[@]}"; do
    read -r contrast_profile gate contrast_weight contrast_margin delta_min label_scale event_scale component_scale <<<"${contrast_row}"
    for shell_row in "${SHELLS[@]}"; do
      read -r shell_profile shell_label_risk shell_reject_risk <<<"${shell_row}"
      profile="${contrast_profile}_${shell_profile}"
      for k in "${K_SHOTS[@]}"; do
        run_one "${variant}" "${profile}" "${gate}" "${contrast_weight}" "${contrast_margin}" "${delta_min}" "${label_scale}" "${event_scale}" "${component_scale}" "${shell_label_risk}" "${shell_reject_risk}" "${k}"
      done
    done
  done
done
...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-CONTRAST-SHELL-VETO-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_contrast_shell_veto_probe_summary.json" <<'PY'
import csv
import json
import sys
...
for path in sorted(case_dir.glob("*/*/stage2c_qknn_*_k*.json")):
    ...
    for row in payload.get("summary_rows", []):
        ...
        item["candidate_set_max_label_shell_risk"] = payload.get("candidate_set_max_label_shell_risk", 1.0)
        item["candidate_set_shell_reject_risk"] = payload.get("candidate_set_shell_reject_risk", 1.0e12)
...
echo "[STAGE2C-CONTRAST-SHELL-VETO-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
