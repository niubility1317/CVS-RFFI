#!/usr/bin/env python3
"""Build or score the independent D108-CB-RRC-SMME Target125 truth plane."""
...
from cvsrffi.stage2_d108_truth_scorer import (  # noqa: E402
    build_d108_target125_truth_catalog,
    score_d108_target125,
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    build = commands.add_parser(
        "build-truth", help="verify sealed predictions, then join 125 D92 truth sidecars"
    )
    _add_prediction_input(build)
    _add_prepared_inputs(build)
    build.add_argument("--truth-catalog", type=Path, required=True)
    score = commands.add_parser("score", help="open a D108 truth catalog and score the full matrix")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "build-truth":
        result = build_d108_target125_truth_catalog(
