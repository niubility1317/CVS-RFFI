...
# DG-aware CEN31 -> low-latency student distillation matrix.
# Teacher sees the clean WiSig view; students also learn on satellite/channel views
# so the teacher boundary is transferred under deployment-relevant perturbations.
...
RUN_ID="${RUN_ID:-cen31_student_distill_kd2_dg_20260603}"
...
COMMON_KD_ARGS=(
  ...
  --sat_view_prob 1.0
  --lambda_sat_view_ce 1.00
  --lambda_sat_view_kd 0.45
  --lambda_sat_view_feature_kd 0.08
  --lambda_sat_view_relation_kd 0.02
  --eval_sat_channel
...
launch() {
  ...
  echo "[CEN31-STUDENT-KD2] candidate=${candidate_id} run=${run_name} gpu=${gpu} dry_run=${DRY_RUN}"
  printf '[CEN31-STUDENT-KD2-CMD]'
  print_cmd "${cmd[@]}"
...
launch KD2_F_DG_SATKD \
  CEN31KD2_lite_f_dg_satkd_r010 \
  0 \
...
launch KD2_F_DG_SOFTKD \
  CEN31KD2_lite_f_dg_softkd_r010 \
  1 \
  --arch_family cvsincnet \
  --model_variant lite_f \
  --branch_ablation no_dac,no_stats \
  ...
  --domain_enhancer_strength 0.20 \
  --lambda_kd 0.35 \
  --lambda_feature_kd 0.15 \
  --lambda_relation_kd 0.03 \
  --lambda_sat_view_kd 0.30 \
  --lambda_sat_view_feature_kd 0.06 \
  --lambda_sat_view_relation_kd 0.015
sleep 2

launch KD2_G_SOFT_DG \
  CEN31KD2_lite_g_soft_dg_r010 \
  2 \
  --arch_family cvsincnet \
  ...
  --lambda_relation_kd 0.03 \
  --lambda_sat_view_ce 0.80 \
  --lambda_sat_view_kd 0.25 \
  --lambda_sat_view_feature_kd 0.05 \
  --lambda_sat_view_relation_kd 0.015
sleep 2

launch KD2_SINCCVCNN_DG \
  CEN31KD2_sinc_cvcnn_dg_r010 \
  3 \
  ...
  --lambda_sat_view_ce 0.90 \
  --lambda_sat_view_kd 0.35 \
  --lambda_sat_view_feature_kd 0.06 \
  --lambda_sat_view_relation_kd 0.015
