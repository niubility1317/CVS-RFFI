...
RUN_ID="${RUN_ID:-phase2_adv3b02_manynew10_conflict_protected_20260705}"
...
declare -a VARIANTS=(
  "MANYNEW10_CONFLICT_HEAD id_feature_head 60 0.00008 1.20 2.40 20.00 5.00 7.00 0.004 0.22 0.10 0.05 0.18 0.12 0.12 0.14 0.07 0.12 0.05"
  "MANYNEW10_CONFLICT_NORM id_norm_late_feature 60 0.00006 1.00 2.00 22.00 4.50 7.50 0.003 0.20 0.10 0.05 0.16 0.16 0.12 0.14 0.07 0.12 0.05"
)

if (( ${#GPUS[@]} < ${#VARIANTS[@]} )); then
  echo "[MANYNEW10-CONFLICT-GPU-COUNT] need ${#VARIANTS[@]} GPUs but got ${GPUS_CSV}" >&2
  exit 2
fi
...
echo "[MANYNEW10-CONFLICT-PROTECTED] run_id=${RUN_ID} dry_run=${DRY_RUN} target_rx=${TARGET_RX} gpus=${GPUS_CSV}"
echo "[MANYNEW10-CONFLICT-PROTECTED] target_new=${TARGET_NEW_TX_IDS}"
echo "[MANYNEW10-CONFLICT-PROTECTED] proxy_pool_excludes_target_new=true"
echo "[MANYNEW10-CONFLICT-PROTECTED] strict_success=K5 and K10 old_acc>=0.80 and every seen-new class>=0.75"
...
run_variant() {
  ...
  echo "[MANYNEW10-CONFLICT-VARIANT] name=${NAME} mode=${MODE} gpu=${gpu} start=$(date -Is)"
  if [[ "${DRY_RUN}" == "1" ]]; then
    echo "[MANYNEW10-CONFLICT-DRY-RUN] ${NAME} -> ${log_path}"
    return 0
  fi
  env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" \
    ...
    --seed "${SEED}" \
    > "${log_path}" 2>&1
  echo "[MANYNEW10-CONFLICT-VARIANT-DONE] name=${NAME} gpu=${gpu} end=$(date -Is)"
}
...
if [[ "${failed}" != "0" ]]; then
  echo "[MANYNEW10-CONFLICT-TRAIN-FAILED] one or more variants failed" >&2
  exit 3
fi

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[MANYNEW10-CONFLICT-DRY-RUN-DONE]"
  exit 0
fi

run_strict_qknn_eval() {
  local name="$1"
  local feature_npz="${RUNS_ROOT}/${CASE_ID}/${name}/features_leo_repaired.npz"
  local out_dir="${RUNS_ROOT}/${CASE_ID}/${name}"
  "${PYTHON}" -u "${ROOT}/code/scripts/phase2_confusion_aware_qknn_probe.py" \
    --feature_npz "${feature_npz}" \
    --output_json "${out_dir}/manynew10_strict_k10_qknn.json" \
    --output_csv "${out_dir}/manynew10_strict_k10_qknn.csv" \
    ...
    --balanced_assignment \
    --skip_support_loo \
    > "${LOG_ROOT}/${name}_strict_k10_qknn.out" 2>&1

  "${PYTHON}" -u "${ROOT}/code/scripts/phase2_confusion_aware_qknn_probe.py" \
    --feature_npz "${feature_npz}" \
    --output_json "${out_dir}/manynew10_strict_k5_qknn.json" \
    --output_csv "${out_dir}/manynew10_strict_k5_qknn.csv" \
    --new_tx_ids "${TARGET_NEW_TX_IDS}" \
    --old_tx_ids "${EVAL_OLD_TX_IDS}" \
    --old_role target_old \
    --new_role target_unknown \
    ...
    --balanced_assignment \
    --skip_support_loo \
    > "${LOG_ROOT}/${name}_strict_k5_qknn.out" 2>&1
}

for row in "${VARIANTS[@]}"; do
  read -r NAME _rest <<<"${row}"
  run_strict_qknn_eval "${NAME}"
done

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/manynew10_conflict_protected_summary.json" <<'PY'
import json
import sys
...
rows = []
for path in sorted(case_dir.glob("*/manynew10_strict_k*_qknn.json")):
    payload = json.loads(path.read_text(encoding="utf-8"))
    best = payload.get("best_by_query", [])
    if not best:
        continue
    row = dict(best[0])
    row["variant"] = path.parent.name
    row["eval"] = path.stem.replace("manynew10_strict_", "").replace("_qknn", "")
...
echo "[MANYNEW10-CONFLICT-PROTECTED-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
