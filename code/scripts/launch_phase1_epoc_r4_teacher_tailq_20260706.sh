...
RUN_ID="${RUN_ID:-phase1_epoc_r4_teacher_tailq_20260706}"
...
wait_for_gpu_slot() {
  ...
  while true; do
    ...
    echo "[EPOC-R4-WAIT] gpu=${gpu} active=${active} max=${MAX_ACTIVE_PER_GPU}"
    sleep 60
  done
}
...
launch_candidate() {
  ...
  echo "[EPOC-R4-CANDIDATE] id=${cid} route=source_only_teacher_locked_tail_quarantine base=ADV3B02_CORE90_SOFT_E200 target_visibility=source_only_ground_training_no_target_receiver target_receiver_samples_in_training=0 real_unknown_classes_in_training=0 source_proxy_unknown_only=1 manytx_in_training=0"
  CMD=(env "PYTHONPATH=${ROOT}/code:${ROOT}:${PYTHONPATH:-}" "CUDA_VISIBLE_DEVICES=${gpu}" "${PYTHON}" -u "${ROOT}/code/SSDG/train_ssdg.py"
    ...
    --proxy_unknown_vaccept_weight 0.08
    --proxy_unknown_core_accept_weight 0.06
    --proxy_unknown_component_gate_weight 0.05
    --proxy_unknown_tail_quarantine_weight 0.14
    --proxy_unknown_source_safe_weight 0.08
    --proxy_unknown_bridge_accept_weight 0.08
    --proxy_unknown_shell_outward_accept_weight 0.12
    --proxy_unknown_low_density_accept_weight 0.08
    --proxy_unknown_energy_margin_quantile_weight 0.18
    --proxy_unknown_radius_budget_weight 0.10
    --proxy_unknown_radius_inter_ratio_weight 0.18
    ...
    --proxy_unknown_shell_outward_accept_target 0.08
    ...
    --proxy_unknown_radius_budget_deg 6
    --proxy_unknown_radius_max_budget_deg 10
    ...
    --lambda_energy_in 0.0015
    --lambda_energy_out 0.0035
    --energy_in_margin -8.0
    --energy_out_margin -0.5
    ...
    --device cuda:0
    --seed "${seed}")
  printf "[EPOC-R4-CMD] "; printf "%q " "${CMD[@]}"; printf "\n"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return
  fi
  ...
  "${CMD[@]}" > "${log_path}" 2>&1 &
  echo "[EPOC-R4-LAUNCHED] id=${cid} pid=$! gpu=${gpu} log=${log_path}"
...
echo "[EPOC-R4] run_id=${RUN_ID} dry_run=${DRY_RUN} candidates=2 teacher=ADV3B02_CORE90_SOFT_E200 route=source_only_teacher_locked_tail_quarantine real_unknown_classes_in_training=0 target_receiver_samples_in_training=0 manytx_in_training=0 only=${ONLY_CANDIDATES:-ALL}"

maybe_launch_candidate "EPOC_R4_TEACHER_LOCK_TAILQ" 4 706401 200 135 1.10 0.42 0.160 0.0035 0.0015 0.0020 0.0045 20 32 1.25 0.30 0.22 14 78 0.14 16 0.00009 0.20 0.08
maybe_launch_candidate "EPOC_R4_SOURCE_OUTWARD_SHELL" 5 706411 200 135 0.95 0.48 0.130 0.0045 0.0020 0.0024 0.0050 22 64 1.35 0.40 0.30 16 82 0.12 18 0.00010 0.16 0.06

echo "[EPOC-R4-SUBMIT-COMPLETE] run_id=${RUN_ID}"
