...
# Fresh v2 run after v1's detached `python -m` module-resolution failure.  The
# C entry is the immutable release file itself; all scientific inputs are the
# exact same training v5 / clean v4 / source-LEO v4 / PAIR v3 artifacts.
RUN_ID="phase1_clic_predictor_artifacts_20260812_v2"
...
[[ -f "${C_ENTRY}" ]] || { echo "missing C predictor entry" >&2; exit 2; }
