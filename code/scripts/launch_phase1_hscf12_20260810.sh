...
# Invoke explicitly as: bash scripts/launch_phase1_hscf12_20260810.sh [--dry-run]
# Frozen six-fold P1-HSCF C/G continuation matrix. C is the exact GeoSat-C
# continuation control; G adds only the frozen source-L HSCF auxiliary.
#
# This launcher is intentionally mechanical: all scientific HSCF constants
# (B=128, drop_last=True, K/local4=4, denominator=512 and lambda=.02) are
# validated by the core. The launcher only binds the existing common training
# route and the two explicit C/G arms.
RUN_ID="${RUN_ID:-phase1_hscf12_20260810_v1}"
...
COMMON=(
  ...
  --phase1_cb_sfce_frozen_mode false --phase1_cb_sfce_enabled false --lambda_cb_sfce 0 --phase1_gd_proto_nll_frozen_mode false --phase1_gd_proto_nll_enabled false --lambda_gd_proto_nll 0
  ...
  --max_grad_norm 5.0 --checkpoint_selection final_only --best_metric clean_val_tx --phase1_source_val_selection_only true
  --eval_sat_channel false --source_val_heavy_eval_start_epoch 40 --source_val_heavy_eval_interval 40 --source_val_heavy_eval_final_window 0
...
launch_arm() {
  ...
  local -a arm_args
  case "${arm}" in
    C) arm_args=(--phase1_hscf_enabled false --lambda_hscf 0) ;;
    G) arm_args=(--phase1_hscf_enabled true --lambda_hscf 0.02) ;;
