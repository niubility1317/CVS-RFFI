...
RUN_ID="${RUN_ID:-phase2_adv3b02_stage2c_seennew_rescue_sweep_20260707}"
...
declare -a PROFILES=(
  "SCORER_SEEN_RESCUE"
  "SCORER_CENTER_SEEN"
  "SCORER_CONFORMAL_SEEN"
  "SCG_SEEN_RESCUE"
)

mkdir -p "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}"
echo "[STAGE2C-SEENNEW-RESCUE] run_id=${RUN_ID} source_run_id=${SOURCE_RUN_ID} dry_run=${DRY_RUN}"
echo "[STAGE2C-SEENNEW-RESCUE] profiles=${PROFILES[*]} k=${K_SHOTS[*]}"
echo "[STAGE2C-SEENNEW-RESCUE] diagnostic_only=true unknown_query_eval_only=true"
...
profile_args() {
  ...
  case "${profile}" in
    SCORER_SEEN_RESCUE)
      PROFILE_ARGS+=(--fusion_policy scorer_cvs)
      ;;
    SCORER_CENTER_SEEN)
      PROFILE_ARGS+=(--fusion_policy scorer_cvs --feature_adapter_policy support_center --feature_adapter_strength 0.50)
      ;;
    SCORER_CONFORMAL_SEEN)
      PROFILE_ARGS+=(
      ...
      ;;
    SCG_SEEN_RESCUE)
      PROFILE_ARGS+=(--fusion_policy old_protected_unknown_confirm_cvs)
      ;;
    *)
...
run_one() {
  ...
  echo "[STAGE2C-SEENNEW-RESCUE-RUN] variant=${variant} profile=${profile} k=${k} feature=${feature_npz}"
  if [[ "${DRY_RUN}" == "1" ]]; then
    return 0
  fi
...
if [[ "${DRY_RUN}" == "1" ]]; then
  echo "[STAGE2C-SEENNEW-RESCUE-DRY-RUN-DONE]"
  exit 0
fi

"${PYTHON}" - "${RUNS_ROOT}/${CASE_ID}" "${LOG_ROOT}/stage2c_seennew_rescue_sweep_summary.json" <<'PY'
import csv
import json
import sys
...
out = {"rows": rows, "best_seen_new": rows[:10], "best_feasible": feasible[:10]}
...
print(json.dumps({"best_seen_new": rows[:5], "best_feasible": feasible[:5], "out": str(out_path), "csv": str(csv_path)}, ensure_ascii=False, indent=2))
PY

echo "[STAGE2C-SEENNEW-RESCUE-DONE] run_id=${RUN_ID} runs=${RUNS_ROOT} logs=${LOG_ROOT}"
