...
POSTFREEZE_MATRIX_ID="${POSTFREEZE_MATRIX_ID:-phase1_clic_postfreeze_20260812_v2}"
...
CLEAN_ROOT="${CLEAN_ROOT:-${PROJECT_ROOT}/runs/${CLEAN_RUN_ID}}"
LEO_ROOT="${LEO_ROOT:-${PROJECT_ROOT}/runs/${LEO_RUN_ID}}"
...
[[ "${POSTFREEZE_MATRIX_ID}" == "phase1_clic_postfreeze_20260812_v2" ]] || { echo "postfreeze matrix ID drift" >&2; exit 2; }
...
[[ "${CLEAN_ROOT}" == "${PROJECT_ROOT}/runs/${CLEAN_RUN_ID}" ]] || { echo "clean root drift" >&2; exit 2; }
[[ "${LEO_ROOT}" == "${PROJECT_ROOT}/runs/${LEO_RUN_ID}" ]] || { echo "source LEO root drift" >&2; exit 2; }
[[ "${RUN_ROOT}" == "${PROJECT_ROOT}/runs/${RUN_ID}" ]] || { echo "PAIR run root drift" >&2; exit 2; }
[[ "${LOG_ROOT}" == "${PROJECT_ROOT}/logs/${RUN_ID}" ]] || { echo "PAIR log root drift" >&2; exit 2; }
...
candidate_path() {
  local root="$1" fold="$2" arm="$3" name="$4"
  printf '%s/F%s%s_CLIC12/%s' "${root}" "${fold}" "${arm}" "${name}"
}
