...
class DRIFTBasicBlock1D(nn.Module):
    expansion = 1
...
class DRIFTResNet18_1DEncoder(nn.Module):
    """DRIFT paper ResNet18-1D feature extractor for IQ tensors `[B, C, L]`."""
    ...
    def __init__(
        ...
        for stage_i, (out_ch, n_blocks) in enumerate(zip(channels, blocks_per_stage)):
            ...
            blocks = [DRIFTBasicBlock1D(in_ch, out_ch, stride=stride)]
            in_ch = out_ch
            for _ in range(1, int(n_blocks)):
                blocks.append(DRIFTBasicBlock1D(in_ch, out_ch, stride=1))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() != 3:
            raise ValueError(f"DRIFTResNet18_1DEncoder expects [B,C,L], got {tuple(x.shape)}")
        x = self.stem(x)
        x = self.stages(x)
...
class DRIFTThreeLayerClassifier(nn.Module):
    """Three fully connected layers used by DRIFT classifier/discriminator heads."""
...
class DRIFTTwoLayerClassifier(nn.Module):
    """Two fully connected layers used by the DRIFT paper domain discriminator."""
