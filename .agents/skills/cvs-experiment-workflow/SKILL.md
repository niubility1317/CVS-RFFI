...
所有CVS和外部对比实验先使用[experiment-management](../experiment-management/SKILL.md)：启动前按统一模板登记名称、数据/seed、矩阵与存储；查找/恢复先搜索实验总索引；启动后、状态变化和评分后更新原run记录。它落实既有预登记，不新增审批或扩展实验权限。
