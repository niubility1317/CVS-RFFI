...
source_root="$project/runs/d92_e0ocf_source_snapshot_20260811_v2"
code_root="$source_root/code"
runtime_archive="$source_root/d92_e0ocf_runtime_closure_c55f4241.tar.gz"
context="$project/runs/d131_d92_lite160_qtie_target125_20260804_r3/prepared/target125_context.json"
method_lock="$source_root/configs/stage2_d92_e0ocf_5arm_hard12v3_v1.json"
output="$project/runs/d92_e0ocf_5arm_hard12v3_20260811_v2"
logs="$project/logs/d92_e0ocf_5arm_hard12v3_20260811_v2"
...
test "$(sha256sum "$runtime_archive" | awk '{print $1}')" = \
  "c55f42412c3f8750457bd78bd56134cb7d3c51f9e66417d6e621fbec5b87c988"
require_file "$method_lock"
test "$(sha256sum "$method_lock" | awk '{print $1}')" = \
  "f16b21e03969b7a1710594a76aac03fe40d1f6b1b4fd361b15e4e45153f0a3cc"
...
names = (
    ...
    "cvsrffi.stage2_d92_e0ocf_hard12",
    "cvsrffi.stage2_d92_e0ocf_analysis",
...
env PYTHONPATH="$code_root:$project" "$python" -u scripts/run_d92_e0ocf_hard12v3.py prepare \
...
env CUDA_VISIBLE_DEVICES=0 PYTHONPATH="$code_root:$project" "$python" -u scripts/run_d92_e0ocf_hard12v3.py smoke \
...
for shard in 0 1 2 3 4 5 6 7; do
  nohup env \
    ...
    "$python" -u scripts/run_d92_e0ocf_hard12v3.py run-shard \
      --matrix-manifest "$manifest" \
      --matrix-manifest-sha256 "$manifest_sha" \
      --shard-index "$shard" \
      --shard-count 8 \
