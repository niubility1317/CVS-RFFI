...
repair6 Git提交为`428e81ef`。15:45 CST direct preflight通过，live inventory为`active_training_processes=[]`、`gpu_compute=[]`，8张RTX 3090均约10MiB。覆盖前快照为`/home/szu2070436088/2510044040/CV-SincNet/code/snapshots/qknn_ground_effective8_r16_e12_leoonly_20260715_v14_repair6_before_sync_20260715_1546`，精确保存3个旧脚本、旧report、旧protocol plan、runner state/log、source validation目录和旧v1锁。同步映射为本地Git承载面的3个修改脚本→N607同相对路径、Git report→N607同相对路径、`local_artifacts/qknn_ground_effective8_v14_protocol_plan_repair6_20260715/`压缩包→run目录`protocol_plan_repair6_20260715.tar.gz`并解压到原`protocol_plan/`。本地/远端SHA一致：candidate lock builder=`afa7eda4...b0b565`、plan builder=`7db83596...f5aeac`、validator=`0ad25b8f...cf58e2`、report=`11216d8a...c755bd`、archive=`0cd0bcb7...3aaf31`、解压plan manifest=`8585ec5f...11091`；远端3个脚本`py_compile`通过。

SCP首文件返回时曾短暂观察到1个`ssh.exe`和1条N607:22连接，因此按规则立即停止后续同步；该进程在诊断读取前自行退出，复核为`ssh=0`、connection=0，随后其余SCP均逐次等待并确认断开。没有据此重传或重启远端任务。远端runner对repair6 plan验证为2/25/300/900/1/1，300个benchmark均引用`candidate_lock_v2.json`。恢复前state逐步比较结果为：步骤0、1、2均`complete`且命令相同，步骤3、4均`complete`但命令改变；因此runner只会重新运行`source_validation_v2`与v2 lock，不会重建cache或重训adapter。同步完成时`source_validation_v2/`和`candidate_lock_v2.json`均不存在，target matrix仍未启动。
