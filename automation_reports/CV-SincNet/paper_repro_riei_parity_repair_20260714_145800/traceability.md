...
| RIEI-P12 | 论文未公开SGD是否包含momentum | 当前固定momentum0 | 在row3/4/11/12上受控测试常用momentum0.9；其余协议固定 | pending | 预注册门槛：四行MAE降低且至少3/4行绝对误差下降 |
...
- 当前反向审计：`verified=9`、`verified-rejected=2`、`pending=1`、`deferred=0`、`blocked=0`。short stem把完整12行MAE降至3.14pp但命中仅7/12，正式结论仍为`NOT_REPRODUCED`。最高风险项转为未公开优化器momentum；若momentum诊断失败，再隔离固定partition下的模型seed敏感性。
