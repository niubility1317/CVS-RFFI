...
- v5 smoke PID`2347970`完整PASS，收据SHA=`680c1f70...fb65`；授权计划SHA=`0939d1ef...3676`，按8 GPU各一分片正式启动，GPU1/GPU7与残余v4诊断合计仍不超过每卡两个进程。

## V5最终结果

- 75/75个package、900/900个cell、2700个场景行、8/8分片PASS、0错误；summary audit SHA=`852e39c...71e80`，`protocol_guard_failures=[]`。
- 按全部新类规模聚合，CSIL在K=`1/5/10/20`的old/new/H为`57.65/15.93/21.14%`、`69.47/15.96/22.16%`、`71.23/16.24/22.30%`、`71.84/17.11/23.32%`。
- MoPC-HR对应为`39.16/28.69/31.61%`、`42.86/37.05/38.33%`、`43.03/38.56/39.28%`、`45.63/40.35/41.63%`；三方法最佳组是MoPC-HR、K=20、5新类，old/new/H=`52.61/55.35/52.79%`。
- Orthogonal Incremental对应H为`11.21/5.36/3.93/3.31%`，旧类在K=20时仅`1.97%`，判定为灾难性遗忘。
- 完整36组方法×K×新类表、2700行同row指标及资源表在`remote_artifacts/v5_summary/`；matched Stage2-C MRIOR-SDA仍待完成，当前不得作优于域适应基线的声明。
- 收尾inventory为空闲；共享bundle构建器远端SHA已更新为`f1a31974...43e1e`并保留旧版快照，远端语法检查通过。
