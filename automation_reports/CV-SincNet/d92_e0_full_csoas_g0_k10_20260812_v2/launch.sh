...
run_id=d92_e0_full_csoas_g0_k10_20260812_v2
scientific_commit=b8ebd4f4522fcc3e9e6b7dd18d722c329021f181
source_root="$project/runs/d92_csoas_g0_source_b8ebd4f4_20260812_v2"
...
"$python" - "$output/after/fit_audit.json" "$output/after/prediction_artifact.npz" "$logs/g0_validation.json" <<'PY'
...
with np.load(sys.argv[2], allow_pickle=False) as artifact:
    if set(artifact.files) != {"predicted_class_handles", "query_tokens", "scenarios"}:
        raise SystemExit("CSOAS prediction artifact schema drift")
    arrays = {name: [str(value) for value in artifact[name].tolist()] for name in artifact.files}
if any(len(values) != 660 for values in arrays.values()):
    raise SystemExit("CSOAS prediction row closure failed")
def sequence_sha256(values):
    payload = json.dumps(values, ensure_ascii=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()
query_sha = sequence_sha256(arrays["query_tokens"])
scenario_sha = sequence_sha256(arrays["scenarios"])
prediction_sha = sequence_sha256(arrays["predicted_class_handles"])
e0_query_sha = "12c37dacd13d14afce9df59d34ee134bb2b7e965eac2447e4362604ee8249d40"
e0_scenario_sha = "79dc3dcb37d5324ccb45128fadefcab180d9460de8d6d9009cbec368302d50d3"
e0_prediction_sha = "d539001ece0319b967023ea05dea7764264a731daa44bf7882a45660ba183cc0"
if query_sha != e0_query_sha or scenario_sha != e0_scenario_sha:
    raise SystemExit("CSOAS paired E0 query identity drift")
if prediction_sha == e0_prediction_sha:
    raise SystemExit("CSOAS deployed predictions are byte-identical to paired E0")
summary, walls = [], []
for row in rows:
    ...
    summary.append({
        ...
        "paired_e0_prediction_nonidentity_pass": True,
    })
p90 = max(walls)
if p90 > 150_000_000:
    raise SystemExit(f"CSOAS G0 wall gate failed: {p90}")
receipt = {
    "schema": "cvs.phase2.d92_csoas.truth_free_g0_validation.v2",
    ...
    "paired_e0_query_tokens_sha256": e0_query_sha,
    "paired_e0_scenarios_sha256": e0_scenario_sha,
    "paired_e0_predicted_class_handles_sha256": e0_prediction_sha,
    "candidate_predicted_class_handles_sha256": prediction_sha,
    "paired_e0_prediction_nonidentity_pass": True,
    "rows": summary,
}
pathlib.Path(sys.argv[3]).write_text(json.dumps(receipt, sort_keys=True, indent=2) + "\n", encoding="utf-8")
