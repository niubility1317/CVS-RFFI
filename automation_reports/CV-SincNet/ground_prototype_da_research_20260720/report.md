...
##### DSSC techfix4系统性INT8止损与ADV3B02/r2-affine冻结

techfix4 run=`dssc_zdom_jg_qknn_r4_bcrr_full125_r1f_techfix4_20c1cd0a_20260723_181353`唯一launcher PID=`875331`。首波不同row seed713104、713105、713106均在任何prediction前以同一`qKNN INT8 teacher gate failed`指纹退出；当时row receipt/prediction/score=`0/0/0`。runner按健康门核对归属后只向本run parent和8个活动子进程发送TERM，run-owned process最终为0，GPU0–7释放，本地SSH/TCP22残留为0；`launcher.exit=ABSENT`，不得虚构exit。终态=`STOPPED_EARLY_SYSTEMIC_TECHNICAL_FAILURE / NO_PERFORMANCE_RESULT`，报告提交=`a3528d4a`。
