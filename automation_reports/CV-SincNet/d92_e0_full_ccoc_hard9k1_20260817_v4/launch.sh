...
run_id=d92_e0_full_ccoc_hard9k1_20260817_v4
fresh_run_retry=false
source_root="$project/runs/d92_ccoc_hard9_k1_source_fbe4e03a_20260817_v4"
runtime_archive="$project/runs/d92_ccoc_hard9_k1_source_fbe4e03a_20260817_v4.tar.gz"
driver="$project/runs/d92_ccoc_hard9_k1_driver_${run_id}.sh"
output_root="$project/runs/d92_ccoc_hard9_k1_20260817_v4"
logs_root="$project/logs/${run_id}"
code_root="$source_root/code"
config="$source_root/configs/stage2_d92_ccoc_hard9_k1_v4.json"
runner="$code_root/scripts/run_d92_ccoc_hard9_k1.py"
source_manifest="$code_root/D92_CCOC_HARD9_K1_SOURCE_MANIFEST.sha256"
archive_sha256=2c8d02122b8092e4f6ebbeac280fcc155fd7e3c678627277f819493231cef807
archive_size_bytes=308983
...
env PYTHONPATH="$code_root:$project" "$python" -m py_compile \
  "$code_root/cvsrffi/stage2_d92_ccoc_hard9_k1.py" \
...
assert value.get("status") == "CCOC_HARD9_K1_MATRIX_PREPARED"
assert value.get("runtime_source_verification_mode") == "sha256_only"
observed = value.get("e0_resource_fit_audit_observed_sha256")
assert isinstance(observed, dict) and len(observed) == 10
assert all(isinstance(item, str) and len(item) == 64 for item in observed.values())
...
assert value.get("status") == "D92_CCOC_HARD9_K1_REAL_CHECKPOINT_TRUTH_FREE_SMOKE_PASS"
