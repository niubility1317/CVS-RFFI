...
SOURCE_ROOT="${RUN_ROOT}/source_165ca031"
...
die() {
  printf 'SVRN_QKNN_BCRR_HELD_R2_ERROR=%s\n' "$1" >&2
  exit "$2"
}
...
write_exit() {
  ...
  if ! (set -o noclobber; printf '%s\n' "${status}" > "${EXIT_FILE}"); then
    printf 'SVRN_QKNN_BCRR_HELD_R2_ERROR=exit_receipt_write_failed\n' >&2
    status=74
  fi
  exit "${status}"
}
...
require_sha256 "eb6231732df8952651dca5903c5d2b8dc27b9a10d4bc6d96e6a15d3d12cad236" "${HELD_FILE}"
require_sha256 "c1f990dcf8a7e9c62c0debe21d34603e3f284ea9598a68f77123b90d4e741e81" "${TEST_FILE}"
...
"${PYTHON}" -c 'import json,sys; d=json.load(open(sys.argv[1], encoding="utf-8")); m=d["metrics"]; assert d["candidate_revision"]=="SVRN-qKNN-BCRR/r3-held"; assert len(m)==72; assert len({r["row_id"] for r in m})==18; assert {r["arm"] for r in m}=={"M0","M_DA","M_OTHER","M_JOINT"}; assert d["decision"]["positive_I_syn_slice_count"] in range(19); print("score_metric_rows=72"); print("prediction_slices=18"); print("verdict="+d["decision"]["verdict"])' "${SCORE}"

sha256sum -- "${PACKET}" "${TRUTH}" "${QUERY}" "${PREDICTION}" "${SCORE}" > "${OUTPUT_ROOT}/sha256sums.txt"
printf '%s\n' "SVRN_QKNN_BCRR_HELD_R2_ARTIFACTS_COMPLETE" > "${OUTPUT_ROOT}/complete.marker"
printf 'SVRN_QKNN_BCRR_HELD_R2_COMPLETE run_id=%s truth_sha256=%s prediction_commit=%s\n' "${RUN_ID}" "${TRUTH_SHA256}" "${PREDICTION_COMMIT}"
