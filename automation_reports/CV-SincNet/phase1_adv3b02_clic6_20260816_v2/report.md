...
### 16.10 v2 N607 release与唯一F1 smoke技术停止（2026-08-16）

- 冻结commit=`f8a641610e7364aafc3b485467fc9e3bafd48ee8`；直连普通N607 preflight=`PASS`（`dell-DSS8440`，project/Python可见，GPU0—7均`1MiB/0%`）。本地archive=`/tmp/phase1_adv3b02_target_prediction_20260816_v2_f8a64161.tar.gz`，bytes=`35842852`，SHA256=`fc606211e85306f42313bd998af7f44b216d848e2cf204d6a1b64a14a0a63618`；SCP恰1，远端bytes/SHA一致；stage静态通过后原子rename至`/home/szu2070436088/2510044040/releases/phase1_adv3b02_target_prediction_20260816_v2_f8a64161`。
- release关键SHA闭合：smoke Python=`6272ffcf968ccd2ab1ce18e1086602f72d46ea207834b99195dca93d64a5b7be`，smoke shell=`d29bfe6d01977f40bb43742fcb97f6fbfe72b31ddcbc7dda39b8e0257a56dff1`，formal v2 shell=`5df7ce63dc508965d23f80a0e3a6df06ee67af50664e3c5ab499293402b3134b`。STATIC=`PASS`：smoke py_compile、两脚本`bash -n`、smoke dry-run=`1`行、formal dry-run=`12`行、6组ADV checkpoint/completion、12组C/G checkpoint/terminal/clean-v4、IQ-only package及fresh roots均闭合。
- 唯一SMOKE命令：`cd /home/szu2070436088/2510044040/CV-SincNet && nohup bash /home/szu2070436088/2510044040/releases/phase1_adv3b02_target_prediction_20260816_v2_f8a64161/code/scripts/smoke_phase1_adv3b02_target_prediction_f1_v2_20260816.sh > /home/szu2070436088/2510044040/CV-SincNet/.smoke_phase1_adv3b02_target_prediction_20260816_v2_F1_outer.out 2>&1 &`；`SMOKE_INVOCATION=1`、`RETRY=NO`、CWD为project root、GPU目标=`0`。F1 PID证据=`949727`；outer=`0B`；F1 log=`2211B`，路径为`logs/.smoke_phase1_adv3b02_target_prediction_20260816_v2_F1/F1_ADV3B02_CLIC.out`；run/log leaf仅创建，train-config/technical receipt未落地，进程已退出。
- 唯一技术异常：`ADV3B02TargetProtocolError: ADV checkpoint WiSig SHA256 is invalid`，发生于production sealer的checkpoint completion SHA校验；不是性能停止。最终GPU0—7均`1MiB/0%`，本地SSH/SCP/TCP22清零。状态封存为`SMOKE_STOPPED_TECHNICAL_FAILURE / FORMAL_INVOCATION=0 / NO_PERFORMANCE_RESULT / RETRY=NO`；不formal、不retry、不读target/truth/known/reference/metrics/performance。

### 16.11 v3核心预注册（2026-08-16）

- v2永久技术停止；首因为checkpoint args与completion顶层`wisig_pkl_sha256`为空，而嵌套自封`source_split_receipt.wisig_pkl_sha256`有效。
- 最小修复：以checkpoint嵌套自封receipt为WiSig权威；两个顶层字段仅在非空时要求相等，实际WiSig字节、completion嵌套receipt与clean-v4绑定保持强制。
- v3 run ID=`phase1_adv3b02_target_prediction_20260816_v3`；smoke ID=`.smoke_phase1_adv3b02_target_prediction_20260816_v3_F1`，全新run/log/receipt根，不复用v2。
- smoke命令：`cd /home/szu2070436088/2510044040/CV-SincNet && nohup bash <release>/code/scripts/smoke_phase1_adv3b02_target_prediction_f1_v3_20260816.sh > /home/szu2070436088/2510044040/CV-SincNet/.smoke_phase1_adv3b02_target_prediction_20260816_v3_F1_outer.out 2>&1 &`。
- formal命令：`cd /home/szu2070436088/2510044040/CV-SincNet && nohup bash <release>/code/scripts/launch_phase1_adv3b02_target_prediction6_v3_20260816.sh > /home/szu2070436088/2510044040/CV-SincNet/phase1_adv3b02_target_prediction_20260816_v3_outer.out 2>&1 &`。
- 调用上限：`SMOKE_INVOCATION=1 / FORMAL_INVOCATION=1 / RETRY=NO`；预期工件为6份`train_data_config.json`与6份`target_prediction.json`。任一输入绑定、strict重建、forward、不可覆盖或六折closure失败均按technical stop封存，smoke失败不得进入formal，不产生性能结论。

### 16.12 v3 N607核心执行状态（2026-08-16）
