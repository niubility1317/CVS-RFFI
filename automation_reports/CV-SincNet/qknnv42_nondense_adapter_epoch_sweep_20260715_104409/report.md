...
|任务规模|每个epoch为5个target receiver×5个seed×`K={1,2,5,10,20}`，共125任务；六档合计750任务|
|比较对象|历史60epoch全量非Oracle结果保留`dense_transductive`，不满足当前逐样本协议；本实验用于给出合法的非dense epoch曲线|
|特征与头|严格ADV3B02+`id_norm_late_feature` adapter+5-view TTA+FFT96；qKNN头使用support-only `support_diag_whiten_fisher`，逐样本argmax|
...
结果：30项测试通过；Python编译通过；shell语法通过；六档dry-run均各生成125行矩阵，合计750行。
...
1. 六档均完成125/125，合计750/750；失败、跳过均为0。
2. 每档5个feature manifest均满足严格ADV3B02加载，missing/unexpected/shape mismatch为0/0/0，并记录对应adapter epoch、5-view和FFT96。
3. 750个split manifest全部满足support/query无重叠；K嵌套和固定query审计通过。
4. 2250个场景全部满足`role_oracle_used=false`、`equal_class_quota_used=false`、`query_query_graph_used=false`、`query_batch_state_required=false`。
5. 完整扫描adapter训练日志和750个任务日志，无Traceback、OOM、Killed、NaN/Inf；保存完整adapter loss trace。
...
启动前本地设计、配置、fail-closed验证和750行dry-run已完成。N607预检、同步、正式启动、完整日志解析与最终逐K主表待执行。
