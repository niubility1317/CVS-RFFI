...
16行全部达到`ARTIFACTS_COMPLETE`后才进入分析。按`H(clean,min(LEO三场景))`、LEO mean、LEO floor、clean、seed升序冻结最佳种子。冻结后只对该种子做一次纯Phase1零适应目标接收机确认；按用户补充要求，目标测试覆盖全部4个日期并逐接收机、逐日期报告clean和三个LEO场景，目标结果不得反馈选种、调参、重训或选择性重跑。

## Phase1源域完成与目标测试预登记

- 16行状态：全部`ARTIFACTS_COMPLETE`；每行均为E200、非空`final_ssdg.pth`、strict checkpoint reconstruction、clean及3个LEO场景全量`V_select`评估，missing/unexpected/shape mismatch均为0、无fallback。
- 排序规则：`H(clean,min(三LEO))`、LEO mean、LEO floor、clean、seed升序。
- 冻结seed：`713104`，冻结后不得由目标结果反馈选种、调参、重训或选择性重跑。

|seed|clean|clear|low|rain|LEO mean|LEO floor|H|
|---:|---:|---:|---:|---:|---:|---:|---:|
|713104|98.4000|90.4222|87.6889|87.6889|88.6000|87.6889|92.7362|
|713115|98.4074|88.7407|86.4370|86.0963|87.0914|86.0963|91.8411|
|713114|98.3037|88.0963|85.1778|85.4000|86.2247|85.1778|91.2712|
|713106|98.3704|87.9259|85.0667|85.7111|86.2346|85.0667|91.2361|
|713103|98.3481|88.0000|85.0667|85.1481|86.0716|85.0667|91.2265|
|713107|98.0593|88.5185|85.0963|84.8370|86.1506|84.8370|90.9702|
|713110|97.8222|88.2815|84.7704|85.5481|86.2000|84.7704|90.8298|
|713108|98.0444|87.1111|83.6815|83.8148|84.8691|83.6815|90.2954|
|713102|98.2741|86.7111|84.6444|83.2593|84.8716|83.2593|90.1457|
|713109|98.2667|86.8370|82.4222|83.3852|84.2148|82.4222|89.6498|
|713105|98.3407|85.2593|82.6444|82.2815|83.3951|82.2815|89.5972|
|713116|97.8222|86.1630|82.5481|82.8889|83.8667|82.5481|89.5385|
|713101|97.6000|86.6963|83.6963|82.5111|84.3012|82.5111|89.4235|
|713112|97.5926|85.2889|82.2963|83.3037|83.6296|82.2963|89.2941|
|713111|97.7778|85.6593|82.2519|81.9259|83.2790|81.9259|89.1527|
|713113|98.0000|84.3778|82.0370|81.0000|82.4716|81.0000|88.6927|

目标测试冻结：

- 类型：纯Phase1、零适应、只读推理；checkpoint和模型参数不更新。
- checkpoint：seed`713104`的E200 `final_ssdg.pth`。
- 数据：ManySig全部4个日期，内部索引`0,1,2,3`，分别为`2021_03_01,2021_03_08,2021_03_15,2021_03_23`。
- 目标接收机：索引`0,2,5,7,9,10,11`，标签`1-1,14-7,2-1,20-1,7-14,7-7,8-8`。
- 结果粒度：`7接收机×4日期=28`个独立cell；每个cell报告clean、leo_clear_weak、leo_low_elev_weak、leo_rain_weak，并另给逐接收机和总体汇总。
- 目标Run ID：`phase1_adv3b03_seed713104_targetrx_alldays_zeroadapt_20260830_r1`。
- 输出根：`/home/szu2070436088/2510044040/CV-SincNet/runs/phase1_adv3b03_seed713104_targetrx_alldays_zeroadapt_20260830_r1`，不可覆盖。
- GPU：0；当前8张GPU均空闲。
- 技术停止：错误checkpoint/日期/接收机、source-target receiver重叠、严格重建失败、输出碰撞、缺少28个cell或任一必要场景artifact。低性能不得停止或重跑。

## 全日期目标接收机零适应测试结果

- 目标Run ID：`phase1_adv3b03_seed713104_targetrx_alldays_zeroadapt_20260830_r1`。
- 最终状态：`ARTIFACTS_COMPLETE / ANALYZED`。
- checkpoint：冻结seed`713104`、epoch200。
- 覆盖：7个目标接收机×4个日期=28个cell；每cell 6,000条，共168,000条clean样本，每个LEO场景同为168,000条。
- 严格性：`strict_requested=true`、`checkpoint_load_strict=true`、`fallback_used=false`，missing/unexpected/shape mismatch均为0；`state_updates=false`。
- release4评估文件本地/远端SHA256均为`3fe1228e5d1152a58d559f24d46167bb335d5762d6fa36419e83e8a6b407ca95`，远端编译通过。
- 真实smoke r1在复合分组累计处发现`int(tuple)`错误，未写指标；原artifact保留。定点TDD修复提交为`961d75f3127377bc7c4c76a9ccc6893207fe537b`，19项聚焦测试通过；新release4与smoke r2严格闭合后才执行正式测试。

### 总体结果

|clean|leo_clear_weak|leo_low_elev_weak|leo_rain_weak|LEO mean|LEO floor|
|---:|---:|---:|---:|---:|---:|
|77.6054|63.9512|62.0440|62.1351|62.7101|62.0440|

### 逐接收机汇总（全部4天合并）

|接收机|clean|clear|low|rain|LEO mean|LEO floor|
|---|---:|---:|---:|---:|---:|---:|
|1-1|82.3125|74.6333|71.5875|71.5625|72.5944|71.5625|
|14-7|70.3333|50.9875|51.7042|52.1625|51.6181|50.9875|
|2-1|84.9542|75.0000|73.0000|72.6125|73.5375|72.6125|
|20-1|70.0083|41.8625|41.0250|41.1917|41.3597|41.0250|
|7-14|79.9583|74.5458|71.1208|71.5458|72.4042|71.1208|
|7-7|74.3750|65.9208|63.6500|63.0500|64.2069|63.0500|
|8-8|81.2958|64.7083|62.2208|62.8208|63.2500|62.2208|

### 逐日期汇总（全部7个接收机合并）

|内部day|日期|clean|clear|low|rain|
|---:|---|---:|---:|---:|---:|
|0|2021_03_01|78.8119|61.7524|60.2095|60.3333|
|1|2021_03_08|78.0905|65.4667|63.8310|63.8786|
|2|2021_03_15|70.9381|63.0286|61.0071|61.2405|
|3|2021_03_23|82.5810|65.5571|63.1286|63.0881|

### 全部28个receiver×day cell

|接收机|day|日期|clean|clear|low|rain|样本数/场景|
|---|---:|---|---:|---:|---:|---:|---:|
