...
die() {
  printf 'SVRN_QKNN_BCRR_K5_SCOREFIX2_ERROR=%s\n' "$1" >&2
  exit "$2"
}
...
write_exit() {
  ...
  if ! (set -o noclobber; printf '%s\n' "${status}" > "${EXIT_FILE}"); then
    printf 'SVRN_QKNN_BCRR_K5_SCOREFIX2_ERROR=exit_receipt_write_failed\n' >&2
    status=74
  fi
  exit "${status}"
}
...
require_sha256 "9745068bc5961ebe90f6305c672cacc8ce338d745579e1c97d4ea503cb3d06d8" "${TRUTH}"
...
printf '%s\n' "SVRN_QKNN_BCRR_K5_SCOREFIX2_ARTIFACTS_COMPLETE" > "${OUTPUT_ROOT}/complete.marker"
printf 'SVRN_QKNN_BCRR_K5_SCOREFIX2_COMPLETE run_id=%s truth_sha256=%s prediction_commit=%s\n' "${RUN_ID}" "${TRUTH_SHA256}" "${PREDICTION_COMMIT}"
