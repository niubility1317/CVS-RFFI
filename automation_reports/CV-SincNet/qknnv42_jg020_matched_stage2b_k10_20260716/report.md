...
N607已经生成25/25个cache、predictor package及detached seal，严格配对硬审计为`25/25 PASS`。第一次smoke的三场景request schema问题已由`3cd7bfd`修复；第二次smoke的candidate lock member误读已由`cfa058a`修复并通过33项相关测试。两次失败均在训练前且没有性能结果，日志与当时runtime seal均已归档。已直接强制重建runtime seal，代码hash确认更新且25份evidence/preopen audit PASS；下一步只重跑同一row。
