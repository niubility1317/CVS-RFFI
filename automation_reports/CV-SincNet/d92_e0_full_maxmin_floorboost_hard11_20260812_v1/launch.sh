...
source_root="$project/runs/d92_e0_full_maxmin_floorboost_source_snapshot_20260812_v1"
code_root="$source_root/code"
runtime_archive="$source_root/d92_floorboost_runtime_closure_25047050.tar.gz"
context="$project/runs/d131_d92_lite160_qtie_target125_20260804_r3/prepared/target125_context.json"
method_lock="$source_root/configs/stage2_d92_full_maxmin_floorboost_hard11_v1.json"
output="$project/runs/d92_e0_full_maxmin_floorboost_hard11_20260812_v1"
logs="$project/logs/d92_e0_full_maxmin_floorboost_hard11_20260812_v1"
...
test "$(sha256sum "$runtime_archive" | awk '{print $1}')" = \
  "da99c0d36541c12b355f947c706092f569d04fbfb7e26b8e7c8f6773282041dc"
require_file "$method_lock"
test "$(sha256sum "$method_lock" | awk '{print $1}')" = \
  "8d5ca23cc2f0cdbe995dd7cb5ba382550d9ca172ec2d81e39abd029669bfd510"
...
require_file "$code_root/cvsrffi/stage2_d92_floorboost_hard11.py"
...
require_file "$code_root/scripts/run_d92_floorboost_hard11.py"
mkdir -p "$logs"
cd "$code_root"
...
names = (
    ...
    "cvsrffi.stage2_d92_floorboost_hard11",
    ...
    "scripts.run_d92_floorboost_hard11",
)
for name in names:
...
env PYTHONPATH="$code_root:$project" "$python" -u scripts/run_d92_floorboost_hard11.py prepare \
...
env CUDA_VISIBLE_DEVICES=0 PYTHONPATH="$code_root:$project" "$python" -u scripts/run_d92_floorboost_hard11.py smoke \
...
for shard in 0 1 2 3 4 5 6 7; do
  nohup env \
    ...
    "$python" -u scripts/run_d92_floorboost_hard11.py run-shard \
      --matrix-manifest "$manifest" \
      --matrix-manifest-sha256 "$manifest_sha" \
      --shard-index "$shard" \
      --shard-count 8 \
