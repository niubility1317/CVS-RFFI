...
### 2026-07-14 14:19+08:00只读监控与DRIFT v2协议审计

- 8张per-GPU queue仍存活；当前8个RIEI训练到epoch156–157，另外4个RIEI仍排队。8个DRIFT均已完成，当前run共已有16个`metrics.json`，训练硬错误日志数为0。
- 预检显示GPU0-7显存约4056–4154MiB、利用率71%–76%；实时compute仍为每GPU一个Phase1进程和一个本任务进程，没有新增DRIFT任务，也没有超过2/GPU。
- 本轮发现此前成功口径引用的是2025-10-10的DRIFT v1；当前[DRIFT v2](https://arxiv.org/html/2510.09405v2)已改为batch=256、每TX/RX随机抽取800/200、5个不同seed的最终epoch均值，并声明信道均衡是唯一信号级预处理。同一训练接收机组合的v2论文结果为73.54%，不是v1的75.62%。
...
### 2026-07-14 14:34+08:00 RIEI前8行完成对比

首批8个RIEI Table III行已完成200epoch、last10和final；剩余4行已由顺序队列启动到epoch12–13。因此以下是同receiver组合的8/12阶段结果，不是Table III最终12行结论。

|Table III行|train RX→test RX|fixopt last10|论文|差值|较repaired|final|阶段判定|
|---:|---|---:|---:|---:|---:|---:|---|
|1|`1-1,7-7`→`1-19`|69.04±5.18%|77.88±2.23%|-8.84pp|+2.63pp|67.79%|未进入±2SD|
|2|`1-1,8-8`→`1-19`|67.00±3.92%|79.43±1.66%|-12.43pp|-1.14pp|73.62%|未进入±2SD|
|3|`1-1,14-7`→`1-19`|52.03±3.53%|66.09±0.67%|-14.06pp|+0.40pp|52.12%|未进入±2SD|
|4|`7-7,8-8`→`1-19`|73.28±3.04%|70.51±3.53%|+2.77pp|-2.97pp|72.42%|进入±2SD|
|5|`7-7,14-7`→`1-19`|65.35±2.42%|77.35±1.53%|-12.00pp|+6.05pp|63.73%|未进入±2SD|
|6|`8-8,14-7`→`1-19`|58.19±0.96%|75.48±1.21%|-17.29pp|+0.33pp|58.19%|未进入±2SD|
|7|`1-1,1-19`→`14-7`|52.22±4.60%|71.91±2.08%|-19.69pp|+12.00pp|55.23%|未进入±2SD|
|8|`1-1,7-7`→`14-7`|45.60±3.58%|68.33±2.37%|-22.73pp|+1.66pp|44.08%|未进入±2SD|

前8行fixopt均值=60.34%，论文同8行均值=73.37%，均值差=-13.03pp，逐行MAE=13.73pp，±2SD命中1/8。相对repaired同8行均值57.97%提升2.37pp，但改善高度不均匀：第7行提升12.00pp、第5行提升6.05pp，而第2/4行反而下降。`lambda_feature_norm=1e-4`抑制了低范数退化的一部分表现，但没有普遍恢复跨receiver泛化，当前阶段仍为`NOT_REPRODUCED_PRELIMINARY_8_OF_12`。

8份完整训练日志均无硬错误。epoch200的CE为0.0019–0.3205、MI为0.0045–0.0501、IE为156.30–158.24、feature-norm loss为0.387–0.933；训练目标稳定但receiver组合间泛化差异仍大。最终Table III判断等待剩余4行完成后按12行统一计算。
...
## 2026-07-14 15:48完成结论

- 20/20个job和全部queue自然完成；完整读取48份日志、14098行文本、20份metrics及4000条epoch记录，硬错误为0且每份训练均完整覆盖epoch1–200。
- DRIFT最佳`D02_cap3500_mse020`：last5=`66.06±1.52%`，final=`65.52%`，较论文75.62%低9.56pp；七receiver为`62.77,64.15,83.50,52.97,52.13,86.62,60.30%`。
- RIEI期刊last5完整12行均值`56.07%`，论文`73.30%`，MAE=`17.54pp`，±2SD命中`1/12`；历史last10均值`56.29%`，较repaired提高`2.30pp`。
- 两方法均判定`NOT_REPRODUCED`。详细逐候选、逐receiver及RIEI 12行同row结果保存在根目录同名报告；下一步进入DRIFT v2和RIEI parity矩阵。
