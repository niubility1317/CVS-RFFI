...
v11 PID=`1114226`完成5epoch/20步，适配0.7397s，峰值CUDA178,206,720B，support前向样本等效7,800；无momentum/部署optimizer状态，完整84行日志错误扫描为空。两View/epoch与5-slot场景/shift轮换逐项匹配预注册，最终state文件65,078B、SHA256=`2b725085...ac817`。但trainer只在manifest记录state路径，未写state SHA字段，严格联合benchmark在读取query前会拒绝；因此该run标记为`METADATA_INCOMPLETE_NO_QUERY_READ`。原artifact不覆盖。已修复trainer自动写state SHA并新增回归，完全相同训练将写入新根`qknn_extreme_light_sourceinit_keyft5_rxshiftpair_20260715_v11r1`后再审计。
