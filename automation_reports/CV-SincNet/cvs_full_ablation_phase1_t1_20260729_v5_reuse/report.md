...
完整解析30条训练的6000个epoch、全部stdout/CSV/JSONL/held-out/resource/terminal/receipt。指标单位为%，`drop=strict UDU−sat strict floor`。

|arm|overall|strict UDU|receiver floor|sat strict floor|drop|训练时长h|峰值显存GiB|
|---|---:|---:|---:|---:|---:|---:|---:|
|P1-FULL|86.818±1.009|80.848±1.625|69.698±1.942|62.171±1.096|18.677±2.353|4.020±0.393|9.494±0.001|
|P1-SUP|85.752±0.601|80.058±1.109|67.948±4.722|53.016±1.982|27.042±2.096|1.689±0.003|9.614±0.000|
|P1-A0|86.520±0.631|80.536±1.075|69.602±3.265|62.330±1.078|18.206±1.680|3.216±0.057|9.381±0.000|
|P1-B0|85.879±1.136|79.391±1.490|69.783±3.019|60.740±1.407|18.650±2.350|4.067±0.131|9.494±0.001|
|P1-C0|86.626±1.054|80.678±1.945|69.038±2.027|61.794±0.836|18.884±2.215|3.393±0.068|9.494±0.001|
|P1-D0|86.637±1.734|81.155±2.738|71.545±6.046|53.272±2.696|27.883±1.705|2.882±0.392|9.493±0.000|

同seed相对FULL的关键配对差：

|arm|Δstrict|Δreceiver floor|Δsat strict floor|Δdrop|
|---|---:|---:|---:|---:|
|P1-A0|−0.312±1.189|−0.097±4.858|+0.159±0.830|−0.471±1.016|
|P1-B0|−1.457±1.982|+0.085±3.521|−1.431±0.638|−0.026±2.329|
|P1-C0|−0.170±0.784|−0.660±1.591|−0.377±0.853|+0.207±0.342|
|P1-D0|+0.307±3.307|+1.847±6.956|−8.899±2.435|+9.206±1.960|

模块判定：

- A：strict、floor和泄漏主终点均未证明FULL优于A0，整体作用未成立。
- B：strict和卫星floor有部分收益，但receiver floor、pseudo precision和coverage没有共同稳定变化，整体门槛未满足。
- C：floor和Q95的paired CI跨0，tail CVaR在C0反而更低，且min-class未导出，整体作用未成立。
- D：D0的sat strict floor在5/5个seed下降8.899个百分点，95%CI为[−11.083,−7.380]；drop在5/5个seed恶化9.206个百分点，95%CI为[8.083,10.951]。模块D的LEO鲁棒性作用稳定。

因此按设计9.3只触发D家族，A/B/C不进入内部参数扫描。完整per-receiver、场景、泄漏、角几何、训练稳定性和资源表见`cvs_full_ablation_completed_matrix_analysis_20260731/report.md`。

训练日志中数值型NaN/Inf为0，但全部44条Phase1训练合计有304个epoch触发`train_skipped_nonfinite_grad>0`；不能写成“nonfinite batch=0”。本run状态更新为`ANALYZED / T1_COMPLETE`。
