...
|当前状态|`ANALYZED / SCREENING_COMPLETE`|
...
`2026-07-31 08:20:39 CST`执行正式低频只读检查；本节只统计执行健康与artifact闭合，不读取性能值，不执行数据审计或任何任务修改。

|检查项|结果|
|---|---|
|main/worker|main PID=`1935906`存活；16个直接子进程，全部为predictor，scorer子进程0|
|physical状态|`COMPLETE=1180`、`FAILED=0`、运行中16、排队154，合计1350|
|复用闭合|641/641个`reuse_prediction`均为`COMPLETE`|
|fresh execute进度|539/709=`76.02%`，已跨过75%门槛；execute失败0|
|prediction闭合|539个`predictions.cvspred`、539个`row_execution_receipt.json`|
|score闭合|1183个score、1183个completion；比terminal status暂多3个，属于并发收口中先发布产物、后写入status的瞬时状态|
|GPU资源|GPU0-7各2个compute进程；瞬时利用率均0%，每卡691MiB，每进程约338MiB|
|异常控制|P0=0；非`COMPLETE`状态0；规范化异常指纹集合为空|
|连接清理|direct preflight通过；短SSH结束后本地`ssh.exe=0`、N607/bridge`ESTABLISHED TCP22=0`|
...
终态判定：v5补跑已自然完成，设计冻结的Stage2-C完整物理执行面与逻辑评分面均闭合，技术失败和协议异常均为0。

## same-row完整矩阵分析

数值均为75个逻辑行的均值；`Ao`为注册后old准确率，`An`为seen-new准确率，`H`为old/new调和均值，`F`为遗忘，`min-old/min-new`为类级下界。所有指标来自同一arm的完整行集。

|arm|机制/类别|注册前old|Ao|An|H|F|min-old|min-new|
|---|---|---:|---:|---:|---:|---:|---:|---:|
|FULL|完整方法|0.7494|0.5923|0.5332|0.5542|0.1571|0.2784|0.2193|
|BASE-COS|余弦基线|0.7494|0.5354|0.4281|0.4700|0.2140|0.2209|0.1242|
|BASE-EUCLID|欧氏基线|0.7494|0.5354|0.4297|0.4711|0.2140|0.2196|0.1249|
|BASE-QKNN|qKNN基线|0.7494|0.5306|0.4715|0.4929|0.2189|0.2231|0.1542|
|BASE-DIAG|对角度量基线|0.7494|0.4867|0.3669|0.4116|0.2627|0.1678|0.0878|
|BASE-POOLED|池化基线|0.7494|0.5826|0.5339|0.5496|0.1669|0.2687|0.2207|
|BASE-FULLBLOCK|全块基线|0.7494|0.5803|0.5315|0.5474|0.1691|0.2653|0.2171|
|BASE-ADAPTER|适配器基线|0.7494|0.5229|0.4038|0.4499|0.2265|0.2149|0.1018|
|A0|移除核心A模块|0.7494|0.3794|0.1923|0.2460|0.3700|0.0900|0.0224|
|B0|移除B模块|0.7494|0.5923|0.5331|0.5541|0.1571|0.2782|0.2189|
|C3|C变体|0.7494|0.5843|0.5353|0.5515|0.1651|0.2713|0.2189|
|D0|D变体0|0.7494|0.5930|0.5356|0.5557|0.1564|0.2813|0.2202|
|D1|D变体1|0.7494|0.5799|0.5296|0.5464|0.1696|0.2638|0.2120|
|D2|D变体2|0.7494|0.5835|0.5349|0.5509|0.1659|0.2698|0.2182|
|E0|E变体|0.7494|0.5879|0.5343|0.5527|0.1615|0.2733|0.2193|
|F0|未压缩头|0.7494|0.5923|0.5332|0.5541|0.1571|0.2782|0.2193|
|F1|压缩头1|0.7494|0.5923|0.5333|0.5541|0.1571|0.2784|0.2193|
|F2|压缩头2|0.7494|0.5922|0.5328|0.5540|0.1573|0.2760|0.2182|
|F3|压缩头3|0.7494|0.5923|0.5332|0.5542|0.1571|0.2784|0.2193|

## 配对效应与解释

- A模块是决定性贡献。FULL相对A0的`Ao/An/H`分别提高0.2129、0.3410和0.3081，95% CI分别为[0.1701,0.2531]、[0.2984,0.3774]和[0.2803,0.3261]；遗忘降低0.2129。
- B0与FULL近乎完全一致，当前筛选矩阵没有支持B模块的独立贡献。
- C3和E0表现为小幅old/H收益与轻微new侧代价；效应量远小于A，不能夸大为主导机制。
- D0的H均值略高于FULL，但D0/D1/D2替换了整个构建器并绕过Fisher/rollback路径，因此不能把差异归因于单一D因素；D组结论为混杂且不稳定。
- FULL相对最强的池化与全块基线，H仅提高0.00458[0.00218,0.00696]和0.00676[0.00487,0.00853]；相对qKNN、欧氏、余弦和适配器基线的H优势分别为0.06128、0.08308、0.08415和0.10428。
- 场景切片中，FULL的H为clear 0.5995、low 0.5332、rain 0.5297，退化场景仍是主要短板。
- K/Cn切片显示最弱条件`K=1,Cn=20`的H仅0.3148，而`K=10,Cn=5`达到0.7673；性能高度依赖支持样本量和新类规模。

## F组资源结果

|arm|H|头部字节|相对F0压缩|预测翻转率|一致率|头部计算时延|
|---|---:|---:|---:|---:|---:|---:|
|F0|0.55414|26,588|0%|不适用|不适用|0.0190ms|
|F1|0.55414|13,294|50.0%|0.000333|0.999667|0.0708ms|
|F2|0.55395|6,808|74.4%|0.015395|0.984605|0.0966ms|
|F3|0.55415|13,570|49.0%|0.000197|0.999803|0.1157ms|

F组只证明存储压缩与预测近似一致；当前没有整数推理内核、端到端延迟或加速证据，不能声称运行加速。

## 证据边界

该矩阵覆盖5个接收机、3个开发种子、1个类抽样和3个场景，属于完整的筛选矩阵，不是确认性复现实验。完整逐接收机、逐类、K/Cn和资源分解见`cvs_full_ablation_completed_matrix_analysis_20260731/report.md`。在用户明确恢复发布前，不启动新的确认矩阵。
