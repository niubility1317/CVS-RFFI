...
2026-07-24对LEO组900条CSIL结构化场景记录、300条同cell汇总、matched无LEO组对应记录及官方训练循环进行完整复核。当前本地回收面没有LEO shard逐batch loss日志，因此本节不声称重新解析逐batch loss曲线；优化步数由冻结实现的确定性切分与`drop_last`规则计算，并由注册前后指标逐cell一致性及无LEOmatched结果交叉验证。

CSIL官方仓库执行路径先保留`floor(0.6*N)-1`条新类训练记录，再以`batch=20`只执行完整批次，共3epoch。对应本矩阵：

| 新类数 | K | 新类support总数 | 增量训练行数 | 优化步数 | LEO组新类准确率为0的cell |
|---:|---:|---:|---:|---:|---:|
| 1 | 1/5/10/20 | 1/5/10/20 | 0/2/5/11 | 0/0/0/0 | 25/25、25/25、25/25、25/25 |
| 3 | 1/5/10/20 | 3/15/30/60 | 0/8/17/35 | 0/0/0/3 | 25/25、25/25、25/25、0/25 |
| 20 | 1/5/10/20 | 20/100/200/400 | 11/59/119/239 | 0/6/15/33 | 25/25、0/25、0/25、0/25 |

因此，小新类数量没有降低训练难度，反而使官方切分和丢尾批规则产生零优化步。`new1`所有K均未训练；`new3`只有K20发生3步更新；`new20-K1`也未训练。零步组合中旧类准确率逐cell保持不变，而新类仅依赖随机小权重和support均值初始化的低维新指纹，LEO与无LEO均为0，证明LEO不是这些组合失败的主因。

`new3-K20`虽然发生3步更新，LEO新类均值达到0.3404，但旧类准确率平均下降约0.836，H仍接近0；无LEO新类均值提高到0.4509，旧类仍接近0。该现象是极少步、新类-only监督和CSIL冻结旧通道/旧指纹共同形成的旧新类决策失衡。

结论：这些小K结果是官方仓库在超小训练池上的真实零步或极少步行为，不是训练异常，也不能解释为“充分训练后的CSIL仍完全失效”。若继续保持官方仓库语义，不应修改`drop_last`或缩小batch；应增加每类新样本量。`new1`至少需要35个support才产生每epoch第一个完整batch，`new3`至少需要每类12个support。若目标改为论文实验复现，则应单独采用论文的初始20类、每阶段新增20类、完整新类训练池、10epoch和batch64设置，不能与当前K-shot缩减诊断混称。
