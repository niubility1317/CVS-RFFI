...
一次性注册使用`batch_5`，逐session注册使用`singleton_forward`。两者共享相同的5个outer、3个scene、K10、seed713106、support/query和新类集合。只有S5与一次性注册拥有相同的5类注册集合，因而构成严格性能比较；S1–S4仅用于展示生命周期，不能当作相同类别规模的胜负。

|指标|一次性`batch_5`|逐session S5|S5−一次性|
|---|---:|---:|---:|
|已注册新类数|5|5|0|
|H|74.53%|74.53%|0.00pp|
|old BA|76.56%|76.56%|0.00pp|
|old floor|49.67%|49.67%|0.00pp|
|seen-new|73.33%|73.33%|0.00pp|
|平均遗忘|8.33pp|8.33pp|0.00pp|
|new→old|17.93%|17.93%|0.00pp|
|old→new|13.33%|13.33%|0.00pp|
|状态/预测/八指标严格相等|参考轨迹|15/15|全部相等|

|生命周期状态|注册方式|已注册新类数|H|old BA|old floor|seen-new|平均遗忘|相对一次性H|
|---|---|---:|---:|---:|---:|---:|---:|---:|
|一次性终态|`batch_5`|5|74.53%|76.56%|49.67%|73.33%|8.33pp|0.00pp|
|S1|逐session|1|71.93%|80.61%|54.67%|67.00%|4.28pp|−2.59pp|
|S2|逐session|2|75.22%|79.94%|53.00%|72.17%|4.94pp|+0.70pp|
|S3|逐session|3|74.23%|78.39%|51.67%|71.67%|6.50pp|−0.29pp|
|S4|逐session|4|73.63%|77.17%|49.33%|71.75%|7.72pp|−0.90pp|
|S5|逐session|5|74.53%|76.56%|49.67%|73.33%|8.33pp|0.00pp|

|资源项|一次性`batch_5`|逐session `S1+…+S5`|比较|
|---|---:|---:|---:|
|每个outer×scene的fit次数|1|5|5倍|
|累计注册wall均值|1986.46ms|9357.70ms|4.71倍|
|累计注册wall范围|最大3134.24ms|7660.36–12126.73ms|逐session显著更慢|
|单次fit增量working-set最大值|2.15MiB|3.89MiB|均低于4MiB|
|300ms资源门通过|0/15|0/75|两者均失败|

这组结果说明当前连续实现选择了“保留全部累计support并重算联合D92头”。它换来了与一次性注册完全相同的终态，却没有降低注册延迟；相反，五次累计重算将平均wall成本放大到4.71倍。逐session的研究价值主要是提供S1–S4可观测生命周期和真实到达顺序，而不是获得更好的最终精度或资源效率。
