...
D49选择新的单一机制轴：`support-LOO global normalized-cosine prototype fusion`。保持D45完整路径不变，增加同一固定288d received-IQ表征上的三块独立L2归一化cosine prototype affine head；target-old和target-new各自只由本类合法support生成prototype，使用完全相同公式。inner LOO同时产生D45 fused logits与cosine logits，各自按inner-train RMS进入同一score单位，再用全类class-balanced CE闭式确定一个global likelihood weight；完整support只合成一次并进入既有int8/FP16生命周期。它吸收已验证的normalized concatenation、cosine head和old/new独立prototype成功经验，但不使用类ID、角色、场景、receiver、query、clean/source、阈值、扫描或逐类截距。

D49预期可观察结果是：相对D45至少真实改变outer决策，同时不再出现D48的大规模正确→错误；优先提高rain O3与low-elev N0/最低new，且D45 aggregate before/after/new/H不得退化。若cosine全局权重退化为0/1、预测完全不变、或任何aggregate/scene/floor门失败，则记为新负结果，不扫描温度或权重。该候选仍只跑同一development 105行；通过全部门后才讨论正式化，不能直接进入125。
