...
RUN_ID="d92_e0_continuous_session_k10new5_20260817_v6"
...
ARCHIVE_PATH="${RUN_ROOT}/release/d92_e0_continuous_session_v6.tar.gz"
...
"${PYTHON}" "${RUNNER}" prepare --config "${CONFIG}" --output-root "${OUTPUT_ROOT}" >"${LOG_ROOT}/prepare.out" 2>&1
"${PYTHON}" "${RUNNER}" prepare-deltas --manifest "${MANIFEST_PATH}" >"${LOG_ROOT}/prepare-deltas.out" 2>&1
CUDA_VISIBLE_DEVICES=0 "${PYTHON}" "${RUNNER}" smoke --manifest "${MANIFEST_PATH}" --device cuda:0 >"${LOG_ROOT}/smoke_gpu0.out" 2>&1
...
for index in "${!JOB_IDS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="${index}" "${PYTHON}" "${RUNNER}" run-job --manifest "${MANIFEST_PATH}" --job-id "${job_id}" --device cuda:0 >"${LOG_ROOT}/${job_id}.out" 2>&1 &
  PIDS+=("$!")
done
...
set -e
"${PYTHON}" "${RUNNER}" status --manifest "${MANIFEST_PATH}" >"${LOG_ROOT}/status.out" 2>&1 || status_rc=$?
