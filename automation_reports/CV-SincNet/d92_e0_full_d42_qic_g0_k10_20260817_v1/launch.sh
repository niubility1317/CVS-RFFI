...
RUN_ID=d92_e0_full_d42_qic_g0_k10_20260817_v1
SOURCE_BASENAME=d92_qic_g0_source_82ce7476_20260817_v1
SOURCE_ROOT="$RUNS/$SOURCE_BASENAME"
SOURCE_ARCHIVE="$RUNS/$SOURCE_BASENAME.tar.gz"
RUN_ROOT="$RUNS/$RUN_ID"
LOG_ROOT="$REMOTE_ROOT/logs/$RUN_ID"
LAUNCH_BASENAME=d92_qic_g0_launch_82ce7476_20260817_v1
PYTHON=/home/szu2070436088/.conda/envs/CVS-RFFI/bin/python

test -f "$SOURCE_ARCHIVE"
test "$(sha256sum "$SOURCE_ARCHIVE" | awk '{print $1}')" = "05aab57ce47f4dfcce23aaef805959c4b6fc5278901dce09ba9e68a15af7a14b"
test "$(stat -c '%s' "$SOURCE_ARCHIVE")" = "218585"
test "$(tar -tzf "$SOURCE_ARCHIVE" | wc -l | tr -d ' ')" = "38"
...
"$PYTHON" -m py_compile \
  ...
  "$SOURCE_ROOT/code/cvsrffi/stage2_d92_d42_quantization_intercept_closure.py" \
  "$SOURCE_ROOT/code/cvsrffi/stage2_d92_e0d_slim.py" \
  "$SOURCE_ROOT/code/cvsrffi/stage2_d92_e0d_query_evaluation.py" \
  "$SOURCE_ROOT/code/scripts/run_d92_qic_g0.py" \
  >"$LOG_ROOT/import_closure.out" 2>"$LOG_ROOT/import_closure.err"
PYTHONPATH="$SOURCE_ROOT/code:$REMOTE_ROOT" "$PYTHON" -c 'import importlib, pathlib; root=pathlib.Path("'"$SOURCE_ROOT"'/code").resolve(); names=("cvsrffi.stage2_d92_d42_quantization_intercept_closure", "cvsrffi.stage2_d92_e0d_slim", "cvsrffi.stage2_d92_e0d_query_evaluation", "scripts.run_d92_qic_g0"); paths=[pathlib.Path(importlib.import_module(name).__file__).resolve() for name in names]; assert all(root in path.parents for path in paths), paths' \
  >"$LOG_ROOT/import_closure.out" 2>"$LOG_ROOT/import_closure.err"

env CUDA_VISIBLE_DEVICES=0 OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 \
  PYTHONPATH="$SOURCE_ROOT/code:$REMOTE_ROOT" \
  "$PYTHON" -u "$SOURCE_ROOT/code/scripts/run_d92_qic_g0.py" \
