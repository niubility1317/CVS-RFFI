...
source_root="$project/runs/d92_pareto_distill_source_snapshot_20260812_v1"
code_root="$source_root/code"
runtime_archive="$source_root/d92_pareto_distill_runtime_closure_ba866ff8.tar.gz"
context="$project/runs/d131_d92_lite160_qtie_target125_20260804_r3/prepared/target125_context.json"
method_lock="$source_root/configs/stage2_d92_full_block_pareto_distill_hard11_v1.json"
output="$project/runs/d92_e0_full_block_pareto_distill_hard11_20260812_v1"
logs="$project/logs/d92_e0_full_block_pareto_distill_hard11_20260812_v1"
...
test "$(sha256sum "$runtime_archive" | awk '{print $1}')" = \
  "8351fcf9241a73b2ee89865d2a12c6add4aba517dd168716b7e8a8fb88a3dab5"
require_file "$method_lock"
test "$(sha256sum "$method_lock" | awk '{print $1}')" = \
  "6c3e1a1b41e08ecf7444c30607cbfdf5d59bcea06f9a902eacac91186d8f62c7"
...
require_file "$code_root/cvsrffi/stage2_d92_full_block_pareto_distill.py"
require_file "$code_root/cvsrffi/stage2_d92_registration_balanced_covariance.py"
require_file "$code_root/cvsrffi/stage2_d92_e0d_slim.py"
require_file "$code_root/cvsrffi/stage2_d92_e0d_query_evaluation.py"
require_file "$code_root/cvsrffi/stage2_d92_pareto_distill_hard11.py"
...
require_file "$code_root/scripts/run_d92_pareto_distill_hard11.py"
mkdir -p "$logs"
cd "$code_root"
...
names = (
    ...
    "scripts.run_d92_pareto_distill_hard11",
)
for name in names:
...
env PYTHONPATH="$code_root:$project" "$python" -u scripts/run_d92_pareto_distill_hard11.py prepare \
...
env CUDA_VISIBLE_DEVICES=0 PYTHONPATH="$code_root:$project" "$python" -u scripts/run_d92_pareto_distill_hard11.py smoke \
...
for shard in 0 1 2 3 4 5 6 7; do
  nohup env \
    ...
    "$python" -u scripts/run_d92_pareto_distill_hard11.py run-shard \
      --matrix-manifest "$manifest" \
      --matrix-manifest-sha256 "$manifest_sha" \
      --shard-index "$shard" \
      --shard-count 8 \
