...
|META-08|APSTA复盘；设计规格§7～8|用独立query outer loss学习support梯度泛化，记录梯度余弦|`meta_objectives.py`、`meta_trainer.py`|partial|Task5聚焦测试：adapt/guard/floor/topology/zero-step目标、类别置换不变、别名冲突/shape负测和梯度回归通过；真实单/双`return_aux`探针分别15/30个inner adapter梯度通过，双模型`z_id_key=feat_joint`与`id_feat_joint`严格一致|outer纯目标已实现；meta-trainer、真实source meta-validation和梯度余弦诊断仍待Task6/7|
