...
|META-06|用户轻型快速要求；设计规格§5、§9|Phase2训练参数≤1%，固定3步，最多5步，硬上限40步|adapter参数白名单、`meta_checkpoint.py`、Phase2 runner|partial|Task4单测验证真实总参数计数、inner`down/up/gate`白名单、`log_step_size`冻结和>1%失败；Task6聚焦测试验证函数式固定步更新、原模型参数/buffer不变及非有限保护；Phase2固定步数审计待Task9|Task6实现source诊断步数`0～10`；不实现Phase2 wrapper、optimizer或Phase2更新|
|META-07|设计报告元学习路线；设计规格§6|真实FOMAML内外循环和模块级Meta-SGD|`meta_inner_loop.py`、`meta_trainer.py`|partial|Task6聚焦17项测试通过：真实`torch.autograd.grad`支持更新、梯度`detach`的一阶路径、同seed确定性、单/双真实forward和模块级step size outer梯度；meta-trainer/source meta-validation仍待Task7|一阶函数式内循环已实现；训练器、optimizer scope和有限差分扩展不在本任务|
|META-08|APSTA复盘；设计规格§7～8|用独立query outer loss学习support梯度泛化，记录梯度余弦|`meta_objectives.py`、`meta_trainer.py`|partial|Task5目标函数与Task6内循环回归通过；query outer loss对adapter初始化和对应`log_step_size`取得有限非零梯度；真实单/双`return_aux`签名均经函数式路径验证|outer纯目标与inner梯度路径已实现；meta-trainer、真实source meta-validation和梯度余弦诊断仍待Task7|
|META-09|项目Phase1协议；设计规格§6.1|复用ADV3B02 Core90 LEO_WEAK日程|Phase1 launcher/config|specified|配置单测和日志字段|严格复用|
|META-10|设计规格§8|checkpoint只由source meta-validation选择|`meta_trainer.py`、`meta_checkpoint.py`、selection summary|partial|Task4只持久化明确allowlist中的Phase1源域`source_split/criterion/seed`，保存和严格加载双侧拒绝嵌套改名、target/query/truth或cache夹带；实际source meta-validation选择器待Task7|不得把bundle保存视为已完成Phase1训练选择|
...
- Task4已完成legacy checkpoint迁移、固定schema bundle、嵌套metadata类型/形状双侧校验、`weights_only=True`严格重建、1%预算和冻结白名单；Task6聚焦17项及Task3/5邻近回归通过，已落地source侧一阶函数式inner loop、模块级Meta-SGD梯度路径和原模型状态保护。Task7/9涉及的meta-trainer、Phase2固定步数wrapper、真实checkpoint smoke、训练和性能证据仍为`pending`；当前不声明实验完成或性能结果。
