...
|META-09|项目Phase1协议；设计规格§6.1|复用ADV3B02 Core90 LEO_WEAK日程|Phase1 launcher/config|partial|Task8冻结配置锁定base checkpoint、source角色比例、adapter/episode参数和dry-run命令；真实训练日程与四场景评估留待Task14|不改变Core90基础checkpoint；本Task不启动训练|
...
|META-14|项目最小实验工作流；设计规格§12～14|RED→GREEN→邻近回归→真实smoke→一次审查→Git/N607发布→truth-last评分|tests、报告、release脚本|partial|Task8完成RED→GREEN、聚焦/邻近回归、py_compile、dry-run不可覆盖检查；真实smoke、审查、N607发布和truth-last评分留待后续|dry-run不创建训练进程或run root|
...
- Task7只实现机制和聚焦测试，不运行真实训练或改变Phase2权限；Task8已增加source-only入口、冻结配置和dry-run launcher，但Task9/14的Phase2固定步数wrapper、真实checkpoint无query smoke、完整clean+三LEO评估、训练和性能证据仍未完成，当前不声明实验完成或性能结果。
