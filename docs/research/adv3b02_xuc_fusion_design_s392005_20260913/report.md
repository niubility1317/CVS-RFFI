...
X/U均复用已计算的clean身份特征；U按块计算，X按整个合法clean batch计算。复用P4Q4K2同day块、4块/batch、uniform候选选择及V2角色轮换。U原实现还在L的拼接前向中冻结BN running-state、使用donor_only MixStyle；M01—M08、M12—M14统一保留该行为。不得一部分行有U时冻结BN、无U时自动恢复BN，从而让“U消融”暗含归一化变化。

这意味着M01是“网格＋角色mask＋L前向BN策略”的组合载体。其相对M00的变化不能只称为采样效应。本轮15组未单独分解这三项；若载体严重退化，先如实报告，不用其他行掩盖。
...
每个窗口最长10个epoch，并在E41、E80、E91、E131处切开，绝不跨越阶段、卫星CE启用或伪标签阶段边界。预先形成完整batch ticket：L/U物理索引、同day网格和角色、场景、实际applied mask、信道随机种子及与batch有关的随机输入。
...
CORE90完整参考参数见[core90_recipe_reference.json](core90_recipe_reference.json)，C2历史实际机制参数见[c2_legacy_reference.json](c2_legacy_reference.json)。二者是实现参照，不是直接可运行launcher配置。原配方中的历史`joint_safe`选模和目标周期评估不恢复；训练过程不构建/读取target loader，固定E200独立预测后再评分。
