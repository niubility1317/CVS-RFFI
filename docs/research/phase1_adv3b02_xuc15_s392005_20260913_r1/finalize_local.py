...
MIRROR=WT/'docs/research'/REPORT.name
text='''# XUC15发布与监控记录
...
'''
REPORT.mkdir(parents=True,exist_ok=True);MIRROR.mkdir(parents=True,exist_ok=True)
(REPORT/'report.md').write_text(text,encoding='utf-8',newline='\n')
for source,name in [(WT/'local_validation/xuc15_execution_v2/acceptance.json','execution_acceptance.json'),
                    (WT/'local_validation/xuc_acceptance.xml','xuc_acceptance.xml')]:
    if source.is_file():shutil.copy2(source,REPORT/name)
