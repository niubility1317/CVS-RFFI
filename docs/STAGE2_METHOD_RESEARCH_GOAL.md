...
本节的资源上限只约束Stage2主方法候选的部署与晋级，不约束CSIL、MoPC-HR等外部论文对比方法。对比方法按原论文完整参数、epoch、optimizer、可训练网络范围和内部状态运行，可超过以下参数、step和状态预算并作为正式matched baseline报告；仍须如实记录资源。对比方法的数据权限同样不受主方法`p2_min_v1`门禁约束，唯一强制条件是全部新类注册和新类评测样本必须叠加并记录LEO星地信道。对比权限不得反向用于主方法训练或协议声明。
