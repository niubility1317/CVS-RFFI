...
2.`IndexBuilder`：从一次已闭合扫描流式构建SQLite，以排他创建保证不覆盖既有目标；失败时保留部分数据库作为错误证据。
...
3.对新数据库路径执行排他创建，再流式建表、导入、建索引和核对行数；目标已存在时立即拒绝。
4.构建失败时不更新`latest.json`，不覆盖已有可用SQLite，也不删除部分数据库；部分数据库不能成为latest。
...
离线管理命令，从一个已闭合扫描的收据和外置CSV流式构建SQLite。它要求显式提供`--receipt`、`--external-root`和`--database`，只排他创建新的外置数据库，不更新原始清单、不连接N607，也不自动修改`latest.json`。日常Agent不调用此命令。
