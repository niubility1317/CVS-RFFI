# RTB-IDR-E0（D92 E0）方法原理、机制、输入输出、资源需求与论文复现方法对比报告

日期：2026-07-27

修订：v8，将方法身份切换为冻结消融配置`P2-E0`并重写Fisher、资源和实验口径
...
D92 E0是一种面向CVS Stage2-C的完整support-only少样本类增量判别方法。它严格对应全量消融冻结配置`P2-E0`，实现标识为`d92_d81_d46_without_fisher`。它以冻结Phase1部署bundle、固定LEO接收IQ和旧/新类标注support为输入，依次完成288维联合特征提取、类无关扰动谱建模、support类中心稳健化、旧/新任务自动收缩协方差估计、full/block双几何可靠性融合，以及量化仿射头编译与封存。D92 E0明确关闭Fisher residual、Fisher候选头、逐类Pareto替换门和原子联合检查。它不是重新训练Phase1主干的端到端网络，但完整覆盖Phase2从合法输入到全注册类预测artifact的状态构造与推理闭环。

本文将该完整配置命名为**RTB-IDR-E0**，英文全称为**Robust Task-Balanced Incremental Discriminant Registration, E0 Configuration**，中文名为**稳健任务均衡增量判别注册E0配置**。“Robust”对应扰动谱与Cauchy稳健中心，“Task-Balanced”对应旧/新任务固定等权协方差，“Incremental Discriminant Registration”对应新类注册、共享判别几何和统一全注册类仿射头；“E0”明确表示Fisher residual关闭。`D92 E0`是本文统一使用的方法标识，`P2-E0`是全量消融矩阵中的正式arm标识。
...
D92 E0同时处理三类困难：跨接收机和LEO弱信道造成的support中心扰动、高维小样本协方差不适定，以及新类数量增加造成的旧/新任务统计失衡。全量消融screen中，`P2-E0`覆盖75个identity和225个场景单位，注册后旧类准确率、已注册新类准确率、调和均值和最低旧类准确率的均值分别为58.79%、53.43%、55.27%和27.33%。与同排`P2-FULL`相比，关闭Fisher residual使调和均值下降0.14个百分点、旧类准确率下降0.44个百分点、新类准确率上升0.10个百分点、最低旧类准确率下降0.51个百分点。该screen说明Fisher分支的平均增益较小但并非严格为零；它不证明D92 E0优于完整Fisher版本，也不是fresh confirmation。D92 E0当前不能晋级。
...
资源上，RTB-IDR-E0仍属于“低频注册较重、长期推理很轻”的方法，但它不执行Fisher低秩变换和第二轮Fisher候选交叉拟合，因此注册计算低于原完整D92。K10基础full/block可靠性链的闭式分量拟合数由88次降为44次；最终26类头仍采用双层残差INT8系数、FP16块尺度/截距和FP32对角metric，其核心数组约16.11KiB，不保存FP32系数sidecar，分类部分为7,488MAC/query。现有证据尚未给出目标星载处理器上的E0独立WCET、峰值RAM、能耗、热和容错测量，因此不能把“关闭Fisher”直接等同于飞行级轻量化。

## 1.阅读约定：把D92 E0视为完整方法配置

本报告从这一节开始，不再按“D92 E0相对D81或D62增加了什么”组织方法。这里的D92 E0指实验中实际执行的完整Phase2分类方法：

> D92 E0是一种面向跨接收机少样本类增量RFFI的support-only稳健判别方法。它从固定LEO接收IQ提取身份、频谱和射频统计特征，从Phase1封存的域×类INT8聚合中心派生类无关扰动谱并据此稳健化每个注册类的support中心，分别估计旧类任务与新类任务的收缩协方差，以固定等权方式形成共享判别几何，再通过support内交叉拟合选择full/block几何，最后把融合结果量化编译成一个面对全部注册类的等先验仿射分类器。E0不构造Fisher residual候选。

D92 E0可以依赖冻结的Phase1编码器，就像线性探测、原型网络或LDA可以依赖预训练特征提取器一样。这不妨碍将D92 E0作为一套完整的Phase2方法讲解。方法的完整性由以下闭环定义：
...
历史开发编号只用于定位代码，不参与方法定义。D92 E0虽然源自对完整D92模块E的消融，但它拥有确定的输入、状态构造、输出、回退和推理闭环，因此在本文中按一种完整可执行方法讲解，而不是只讲“删除了什么”。下面先回答“D92 E0本身是什么、为什么成立、怎样运行”，最后才说明各模块在仓库中的实现文件。
...
flowchart LR
    ...
    H --> I["full与block3两种几何"]
    I --> J["support内留一可靠性融合"]
    J --> K["E0边界：Fisher residual关闭"]
    K --> M["量化编译与封存：双层INT8系数+FP16尺度/截距"]
    B --> N["单条query特征q"]
    M --> O["全部注册类分数"]
    N --> O
    O --> P["逐样本argmax预测"]
...
这六个活动模块只在support状态构造阶段运行。query到来后不会重新计算support协方差或LOO权重。E0没有Fisher残差和安全门，因此query端也不存在对应变换或候选分支。
...
当前D92 E0使用的`phase1_int8_domain_class_centroids_v1`不是协议中默认关闭的地面旧类压缩多原型组件。两者区别如下：

|比较项|D92 E0当前域×类中心组件|可选地面旧类压缩多原型|
|---|---|---|
|组织方式|每个地面域×每个旧类一条中心|每个旧类最多3个聚类原型|
|主要目的|估计跨接收机域扰动方向|表达一个类别内部可能存在的多模态结构|
|是否直接参与分类|D92 E0中不直接参与query分类|若未来启用，可按锁定聚合规则参与旧类打分|
|当前D92 E0是否启用|是|否|
|是否包含新类|否|否|
...
D92 E0任务均衡协方差只有在“已经注册新类且\(K>2\)”时启用。注册前只有6个旧类，或者\(K\in\{1,2\}\)时，当前实现精确回退到D81分类头，不构造D92 E0的旧/新\(0.5/0.5\)平衡协方差。因此K1结果不能被解释成“用一条向量成功估计了288维协方差”。
...
**本式符号说明：**\(c\)是注册类别索引；\(C\)是注册类总数；\(\mathbf w_c^{(0)}\in\mathbb R^{288}\)是类别\(c\)经过full/block可靠性融合后的基础权重；\(b_c^{(0)}\in\mathbb R\)是对应截距；上标\((0)\)表示未叠加Fisher residual的基础头，在D92 E0中它就是需要提交的浮点参考头。

D92 E0的第六个活动模块不再寻找新的判别方向，而是完成四件事：

1.明确执行E0恒等边界，不构造Fisher residual候选；
2.删除不影响\(\arg\max\)的类别公共项并固定类别顺序；
3.把浮点基础头编译为双层残差INT8系数、FP16尺度和FP16截距；
4.把量化头、旧类metric、类别表、配置身份和数值审计封存为不可变预测状态。

输入输出为：

|项目|形状或类型|作用|
|---|---:|---|
|基础融合权重\(\mathbf W^{(0)}\)|\(C\times288\)|量化前的全注册类判别系数|
|基础融合截距\(\mathbf b^{(0)}\)|\(C\)|量化前的全注册类截距|
...
### 10.1E0边界：Fisher residual严格关闭

完整D92的Fisher候选可以写成一个identity-primary线性变换。D92 E0把该变换固定为单位矩阵：
...
**本式符号说明：**\(\mathbf A_{\mathrm{E0}}\in\mathbb R^{p\times p}\)是E0配置下的特征变换；\(\mathbf I_p\)是\(p\)维单位矩阵；\(p=288\)是联合特征维数。单位矩阵满足\(\mathbf A_{\mathrm{E0}}\mathbf q=\mathbf q\)，所以E0既不增强也不削弱任何特征方向。
...
\mathbf W_{\mathrm{E0}}
=
\mathbf W^{(0)}\mathbf A_{\mathrm{E0}}^{\mathsf T}
=
\mathbf W^{(0)},
\qquad
\mathbf b_{\mathrm{E0}}
...
**本式符号说明：**\(\mathbf W^{(0)}\in\mathbb R^{C\times p}\)和\(\mathbf b^{(0)}\in\mathbb R^C\)是模块五输出；\(\mathbf A_{\mathrm{E0}}^{\mathsf T}=\mathbf I_p\)；\(\mathbf W_{\mathrm{E0}}\)和\(\mathbf b_{\mathrm{E0}}\)是E0量化前参考头。等式说明关闭Fisher不是“把未知结果设为零”，而是选择确定的恒等映射。

D92 E0不会计算类均值SVD方向\(\mathbf V\)、类间能量\(\beta_j\)、类内能量\(\nu_j\)、Fisher增益\(\gamma_j\)、残差候选可靠性\(\eta_{c,h}^{(R)}\)、逐类\(TP/FP\)替换门和原子联合检查。它也不会在query端保留隐藏的Fisher变换。冻结实现用`d92_d81_d46_without_fisher`标识这一调用链。

### 10.2为什么E0仍然是一种完整方法

模块五已经用当前support完成full/block主拟合、按shot秩留一、交叉熵估计、logit RMS对齐和类别级可靠性融合。对任意类别\(c\)，基础融合行为
...
**本式符号说明：**\(h\)遍历full和block3两个几何分支；\(\eta_{c,h}\)是类别\(c\)对分支\(h\)的support留一可靠性权重，且\(\sum_h\eta_{c,h}=1\)；\(r_h\)是分支\(h\)的类中心化logit RMS；\(\mathbf w_c^{(h)}\)和\(b_c^{(h)}\)是该分支的LDA权重与截距。D92 E0直接采用这条support-only融合结果，不需要Fisher候选才能形成分类器。

因此D92 E0仍具有完整的合法输入、状态构造、量化编译、统一推理、不可变输出和失败闭合。非有限、非正定、量化漂移或manifest绑定失败时，候选状态不会发布。
...
状态中不保存全部历史support IQ、query样本、query真值、query角色、FP32系数sidecar、Fisher方向、Fisher候选头或跨query重分配状态。
...
### 10.6E0相对Fisher版本的计算边界

E0删除类均值方向分解、方向能量与增益、第二套Fisher候选交叉拟合、逐类\(TP/FP\)替换检查和原子联合检查。它仍保留288维特征提取、稳健中心、类内Ledoit–Wolf协方差、旧/新任务均衡、full/block3两个LDA分支、support内LOO可靠性和量化编译。因此E0注册压力低于完整Fisher版本，但仍高于只求prototype均值的方法。

### 10.7对照机制：以下Fisher内容不属于D92 E0执行路径

以下小节保留完整D92的Fisher residual原理，只用于解释`P2-E0`关闭了什么以及为什么`P2-FULL-P2-E0`可作为Fisher模块的同排因果差。**这些公式、候选头、Pareto门和原子检查均不由D92 E0计算，也不计入D92 E0状态或资源。**
...
逐类接受仍不是最终结果，因为替换某个类别行会改变其他类别的argmax竞争。只有所有候选行同时替换后仍满足每类TP不降、FP不增，D92 E0才提交这组替换。

#### 10.7.1完整D92的Fisher计算不是梯度训练
...
残差候选不会直接沿用基础分支的融合权重。D92 E0在相同support留一划分上重新计算残差分支交叉熵
...
多个类别分别安全，不代表同时替换后仍安全。D92 E0把所有初始通过的行同时替换，再检查
...
10. 执行E0边界：固定A_E0=I_288，不构造Fisher残差候选。
11. 删除类别公共项，把基础融合头编译为双层INT8系数、FP16尺度/截距和FP32旧类metric。
12. 输出量化state、注册类顺序、配置身份和完整审计记录。
```

### 11.2 激活与回退条件

|条件|稳健中心|旧/新均衡协方差|E0 Fisher状态|最终行为|
|---|---|---|---|---|
|只有旧类，\(C=C_{\mathrm{o}}\)|按K决定|不激活，因为没有新类任务|关闭|形成`DA1_REG0`旧类状态|
|已注册新类且\(K>2\)|激活|激活|关闭|完整`DA1_REG1` D92 E0状态|
|已注册新类且\(K=1\)|恒等|回退|关闭|极少样本保守状态|
|已注册新类且\(K=2\)|恒等|回退|关闭|极少样本保守状态|
|任一协方差非有限或非正定|—|失败闭合|—|不发布预测状态|
|量化state非有限、越界或绑定失败|有效|有效|关闭|拒绝发布候选state|
...
所以，“整批一起计算”在这一层也是正确的。协方差汇总、LDA竞争、LOO可靠性和量化state都是全局耦合的；增加一个新类不只产生该类自身的一行，还会改变所有类别共享的判别几何。E0关闭Fisher并不会把方法变成局部追加分类器。
...
这条递推只节省“新任务协方差求平均”本身，不能让现有D92 E0变成局部追加算法。加入第\(m+1\)类后，平衡协方差、全部LDA行、LOO分支可靠性和量化state都会变化。当前冻结基础`P2-E0`实现也没有把单类到达定义为原位追加；它从当前完整support注册表重新构造状态。后续连续session扩展属于另一方法版本，不自动并入本报告。
...
query不会被类中心平移，因为类中心平移已经编译进由support拟合的判别头。query也不会参与重新估计\(\boldsymbol{\Sigma}_{\mathrm{o}}\)、\(\boldsymbol{\Sigma}_{\mathrm{n}}\)、融合权重或量化state。D92 E0没有Fisher安全门。
...
D92 E0不是只拟合一次协方差。full和block3两个分量都要做主拟合及support内LOO拟合，但E0不再构造Fisher候选的第二轮交叉拟合。对\(K>2\)，现有调用链中的闭式分量拟合总数为
...
**本式符号说明：**\(N_{\mathrm{fit}}\)是一次D92 E0注册中执行的闭式LDA分量拟合次数；\(K\)是每类shot数；常数4和\(4K\)分别来自基础融合链的主拟合组件与K个按shot秩留一组件。完整Fisher版本还有另一组\(4(K+1)\)候选审计拟合，E0不执行该组。该计数只适用于当前\(K>2\)路径。
...
K10的E0派生审计如下。表中LDA拟合与可靠性项由原完整调用链剔除Fisher候选的第二组同构拟合得到；Fisher专属稠密代数为0。它用于统一比较算法规模，不等于某一具体BLAS、CPU或FPGA实现的实测硬件MAC数。

|注册后类数\(C\)|闭式分量拟合数|旧类metric MAC|E0 LDA拟合MAC|E0可靠性MAC|Fisher专属MAC|合计MAC等价上界|
|---:|---:|---:|---:|---:|---:|---:|
|11|44|6.22M|1.364G|4.98M|0|约1.375G|
|16|44|6.22M|1.456G|9.25M|0|约1.471G|
|26|44|6.22M|1.641G|22.56M|0|约1.670G|

这张表给出的结论很明确：D92 E0关闭Fisher后，K10注册审计从原完整版本约11G量级降到约1.38–1.67GMAC等价量级，主要压力转为K折交叉拟合和288维收缩协方差/LDA稠密代数。它没有反向传播整个主干，也不需要数百个神经网络适配epoch，但仍不能归类为“几乎零计算的prototype更新”。表内值是从冻结调用链派生的保守审计，不是目标硬件实测。
...
本报告中的1.38–1.67GMAC是**D92 E0派生MAC等价上界**。它已排除完整D92中按\(8p^3\)计入的Fisher专属稠密代数和第二套候选交叉拟合，适合比较算法规模，但仍不是处理器性能计数器实测的乘加次数。若要判断是否能在星上实时执行，仍需报告：
...
|adapter qKNN|少量adapter梯度步+保存support|adapter前向+\(\mathcal{O}(CKd)\)|通常比完整D92 E0注册简单，但取决于adapter步数|
|Orthogonal Incremental|追加正交权重时较轻；若校准训练则上升|约\(\mathcal{O}(Cd)\)|简化版本注册轻于D92 E0，query同阶|
|D92 E0|K10约44次分量拟合，1.38–1.67GMAC派生等价上界|解码后约\(Cp\)，最大7,488MAC|注册仍重于prototype，但显著轻于含Fisher完整版本；query头很轻|
...
D92 E0由经过独立审计的D92基础组件和全量消融装配器共同实现。下面的历史编号只回答“组件从哪里复用”，不表示D61/D62等完整Fisher组件属于E0执行路径。

本节所指D92 E0正式arm由本报告所在Git仓库的`code/cvsrffi/stage2_ablation_factory.py`和`code/cvsrffi/stage2_ablation_executors.py`定义，冻结身份为`P2-E0/fisher_profile=off/d92_d81_d46_without_fisher`。表中的D81、D43、D46、D61和D62名称用于解释复用链；正式结果身份仍应以全量消融report、sealed plan、Git提交和文件哈希共同确认。
...
|identity-primary Fisher残差|D61相关实现；`P2-E0`不执行|
|逐类Pareto行替换与原子安全门|D62相关实现；`P2-E0`不执行|
|D92 E0冻结装配|`code/cvsrffi/stage2_ablation_executors.py`中的`P2-E0`分支|
|全query闭环|`code/cvsrffi/stage2_d92_query_evaluation.py`|

从科学方法角度看，D81稳健中心、D92任务均衡、D43结构化协方差、D46 LOO融合及量化编译共同构成D92 E0；D61/D62只构成被关闭的对照机制。从软件工程角度保留这些历史边界，便于复用测试、追踪哈希和审计每个不变量。
...
flowchart LR
    ...
    K2 --> K3["E0恒等边界：Fisher关闭"]
    K3 --> K4["双层INT8量化与state封存"]
    K4 --> L["逐query全注册类score与argmax"]
    L --> M["不可变prediction artifact"]
    M --> N["独立truth-side scorer"]
```

图中从冻结特征、稳健中心、任务均衡协方差、双几何融合到单一仿射状态和artifact发布，合起来才是本报告定义的完整D92 E0。ADV3B02负责提供冻结表征；其余模块属于D92 E0的Phase2状态构造和推理闭环。代码保留D81、D43、D62等历史文件名，只是实现复用关系，不改变这里的完整方法定义。
...
算法函数返回的FP32系数不等于最终允许长期保存FP32 sidecar。完整D92 E0流水线继续执行量化和状态封存；最终预测先封存，真值只在独立scorer侧连接。

## 18.稳健中心实现背景

### 18.1 稳健中心模块为何进入D92 E0
...
这套稳健中心是实验中D92 E0完整状态构造的一部分。代码沿用了早期模块文件名，但方法解释不把它视为外部基线或可忽略前置步骤。
...
再跨越FP32边界，使后续再次中心化近似幂等。原报告所述125-row漂移修复属于历史完整D92专项实现，不是`P2-E0`的性能来源。D92 E0的当前证据以全量消融冻结arm、75/75 logical artifact闭合和2026-07-31同排分析为准。
...
这是D92 E0生命周期中的注册前状态，不是另一个待与D92 E0拼接的方法。为了建立严格matched control，当前实现保证该状态的系数和截距与实验对照头逐值一致。
...
5.为全部旧类和新类计算统一LDA行。
6.经过full/block LOO可靠性融合、E0恒等边界和量化编译形成单一部署状态。
7.状态锁定后才打开query。
...
## 21.`P2-E0`全量screen设计与四状态口径

D92 E0的权威方法结果来自全量消融Stage2-C T1矩阵中的`P2-E0`，而不是原D92任务均衡专项矩阵。screen设计为：

|维度|取值|
|---|---|
|方法arm|`P2-E0`，`fisher_profile=off`|
...
下表每一行都保持同一arm的完整指标语境。均值和标准差来自225个场景单位；中位数列在斜杠后。

|arm|`DA1_REG1`旧类准确率|`DA1_REG1`新类准确率|H|F|最低旧类|最低新类|
|---|---:|---:|---:|---:|---:|---:|
...
`P2-E0`和`P2-FULL`的`DA1_REG0`旧类准确率均为0.7494±0.1330/0.7667，说明Fisher开关发生在新类注册状态构造内部，不改变同排注册前参考。

### 22.2关闭Fisher residual的配对效应

以下差值定义为`P2-E0-P2-FULL`，正值表示E0更高：

|指标|配对差|分层bootstrap 95%CI|解释|
|---|---:|---:|---|
|`DA1_REG1`旧类准确率|−0.0044|[−0.0068,−0.0022]|关闭Fisher后旧类平均下降0.44个百分点|
|`DA1_REG1`新类准确率|+0.0010|[+0.00006,+0.0020]|新类平均上升0.10个百分点|
|H|−0.0014|[−0.0025,−0.0004]|旧新调和均值下降0.14个百分点|
|F|+0.0044|[+0.0022,+0.0068]|遗忘增加0.44个百分点|
|最低旧类|−0.0051|[−0.0098,−0.0002]|旧类floor下降0.51个百分点|
|最低新类|约0|[−0.0036,+0.0033]|没有稳定差异|

该结果说明Fisher residual在本screen上的平均贡献较小，但H、旧类和旧类floor的配对区间均不跨0。正确表述是“D92 E0用更低注册复杂度换取小幅但稳定的旧类/H下降”，不能写成“Fisher完全无用”，也不能因为E0的新类平均提高0.10个百分点就宣布E0总体更优。

### 22.3逐receiver的H

|receiver|`P2-E0` H|`P2-FULL` H|E0−FULL|
|---|---:|---:|---:|
|`20-1`|0.560045|0.562576|−0.002531|
|`3-19`|0.412988|0.414054|−0.001066|
|`7-14`|0.597395|0.598420|−0.001025|
|`7-7`|0.612726|0.615255|−0.002529|
|`8-8`|0.580448|0.580459|−0.000011|

五个receiver上的H差值均不为正，方向与总体配对结果一致；`3-19`仍是最弱receiver。这里不能只报告几乎相等的`8-8`，也不能用单receiver替代完整矩阵判断。
...
- `FISHER_OFF_VERIFIED`：冻结实现确实使用`d92_d81_d46_without_fisher`；
...
## 23.Role-Oracle与后续E0扩展的证据边界

原技术报告曾引用另一条完整D92专项run的Role-Oracle诊断。该run不是`P2-E0`，所以其Oracle数字不能改名后转移给D92 E0。本报告只保留机制判断：Role-Oracle已知query属于old还是new，只能作为跨角色竞争上界，永久不可部署、不可晋级。

同理，2026年8月形成的`E0D`、`E0OCF`、`CCOC`、D42-QIC和连续session实验都在基础E0上增加了新的状态、校准、目标或生命周期机制。它们可以把`P2-E0`作为起点，但不是本报告定义的基础D92 E0。除非单独给出方法身份、同排输入和完整结果，不得把这些扩展的性能或资源写入D92 E0主表。

## 附录A：历史完整D92专项实验（非E0证据）

> **证据边界：**本节保留的是原报告对应的另一条完整D92专项矩阵，其arm不是`P2-E0`。本节全部性能和Role-Oracle数字不得转移给D92 E0，只用于保存历史追踪和解释旧/新任务均衡模块的先前证据。
...
每个slice的D81/历史完整D92结果是25个receiver×seed matched row均值，每个row内部覆盖三个LEO场景。为避免把阶段编号和注册前后状态混为一谈，本报告使用以下明确名称：
...
主表中的`S2A-old=75.21%`来自现有直接ADV3B02 Stage2-A矩阵，是实测值而非由`S2B-old`反推。该参考矩阵使用相同5个target receiver和5个seed规模，但seed为`713101–713105`；历史完整D92矩阵为`713102–713106`。历史完整D92 retry2的同矩阵汇总字段明确记录`direct_adv3b02_status=MISSING_NOT_RUN`，因此该列只能标作**非paired参考**，不得用于计算D81与历史完整D92的paired差值、置信区间或显著性。

### A.1历史完整D92与严格matched control的结果

本节表中的`D81`是实验记录沿用的matched control名称。它与历史完整D92共享冻结特征、稳健中心、双几何融合、安全门、数据切片、support/query和seed；唯一受控差异是注册后是否启用旧/新任务均衡协方差。因此，本节用于隔离任务均衡协方差的因果贡献，不用于定义D92 E0本身。
...
|K1/new20|历史完整D92|75.21%†|68.144%|44.033%|14.200%|27.150%|33.410%|24.111pp|
|K5/new20|D81|75.21%†|81.267%|61.400%|30.800%|59.293%|60.035%|19.867pp|
|K5/new20|历史完整D92|75.21%†|81.267%|63.711%|33.200%|58.883%|60.955%|17.556pp|
|K10/new5|D81|75.21%†|86.111%|76.322%|50.667%|73.613%|74.606%|9.789pp|
|K10/new5|历史完整D92|75.21%†|86.111%|76.189%|49.800%|74.133%|74.803%|9.922pp|
|K10/new10|D81|75.21%†|86.111%|71.533%|42.267%|66.693%|68.815%|14.578pp|
|K10/new10|历史完整D92|75.21%†|86.111%|72.533%|44.200%|66.353%|69.106%|13.578pp|
|K10/new20|D81|75.21%†|86.111%|68.711%|38.067%|68.803%|68.591%|17.400pp|
|K10/new20|历史完整D92|75.21%†|86.111%|71.333%|42.667%|68.150%|69.555%|14.778pp|

†`75.21%`是Stage2-A直接ADV3B02的描述性实测参考。它不属于历史完整D92 retry2的125行matched artifact，重复列出是因为Stage2-A不读取K-shot support，也不执行新类注册；不能把它解释为每个D81/历史完整D92 row都完成了Stage2-A配对测量。
...
- K10/new20的5个receiver旧类准确率均提高，但`3-19`在历史完整D92后仍只有`A-old=57.44%`、`Min-old=25.67%`、`New=49.10%`。
...
另有一个特许历史完整D92 Role-Oracle实验。它在同一次fresh run、同一support、同一状态和同一score matrix上，同时计算：
...
K10/new20中Oracle把旧类提高约11.98个百分点，而新类只提高约3.28个百分点，说明历史完整D92的大规模注册瓶颈主要是新类侵入旧类，而不只是角色内部旧类彼此混淆。K1/new20中旧类Oracle结果恰好回到注册前旧类准确率68.14%，表明24.11个百分点遗忘几乎都来自把新类加入统一候选空间后的跨角色竞争。合法方法必须在不知道query角色的情况下解决这个问题，不能把Oracle上限当作可部署方案。

## 24.与域适应论文复现方法的对比

### 24.1 为什么域适应方法不能直接与D92 E0的Stage2-C结果排名
...
下表中的域适应矩阵使用5个receiver、5个seed`713101–713105`；D92 E0列来自`P2-E0`screen的同K切片，覆盖5个receiver和3组冻结seed，但只有1个new-class draw。两个矩阵没有完成artifact哈希配对。因此只能看趋势，不能计算paired显著性或宣布严格胜负。
...
趋势上，D92 E0的注册前旧类状态在K5高于三个论文适配头，K10略低于MRIOR-SDA，K1明显低于直接ADV3B02及MRIOR-SDA。注册前尚无新类任务，所以这部分结果不能归因于任务均衡协方差或Fisher开关；它反映的是D92 E0进入注册前的旧类support状态。域适应论文结果证明的是不同Stage2-B适配管线的效果，不是D92 E0内部组件的严格消融。

## 25.与类增量论文复现方法的机制对比

### 25.1 CSIL

CSIL论文《Class-Incremental Learning for Wireless Device Identification in IoT》使用zero-bias cosine fingerprint层，通过通道扩展为新类增加表示容量，并用块状mask隔离新旧fingerprint；优化目标包含CE、知识蒸馏和EWC。它的核心思想是“扩展网络容量并限制旧知识更新”，而D92 E0不扩展encoder，而是用稳健support统计、任务均衡判别几何和交叉拟合安全选择构造增量状态。
...
### 26.1官方流程LEO完整矩阵与历史完整D92共同slice（非E0证据）

**证据边界：**本节为保留论文复现对照而列出历史完整D92的共同slice。D92 E0的`P2-E0`screen报告没有在本技术报告中展开相同逐slice表，因此不能把下列历史D92数值改名为D92 E0。D92 E0与论文方法的直接数值比较只能等待同seed、同draw、同manifest的独立导出。

CSIL和MoPC-HR官方仓库核心复现完成了5receiver×5seed×4K×4新类规模×2方法=800cell、2400个LEO场景row。其seed为`713101–713105`，历史完整D92为`713102–713106`；两者base训练、状态构造和方法权限也不同。因此下表是共同K/new切片上的描述性对照，不是严格paired结果。

|切片|方法|old-before|old-after|seen-new|H|forgetting|
|---|---|---:|---:|---:|---:|---:|
|K1/new20|历史完整D92|68.144%|44.033%|27.150%|33.410%|24.111pp|
|K1/new20|CSIL官方流程|42.833%|42.833%|0.000%|0.000%|0.000pp|
|K1/new20|MoPC-HR官方流程|45.322%|40.722%|1.363%|2.603%|4.600pp|
|K5/new20|历史完整D92|81.267%|63.711%|58.883%|60.955%|17.556pp|
|K5/new20|CSIL官方流程|42.833%|0.200%|5.557%|0.316%|42.633pp|
|K5/new20|MoPC-HR官方流程|45.322%|13.511%|17.433%|14.309%|31.811pp|
|K10/new5|历史完整D92|86.111%|76.189%|74.133%|74.803%|9.922pp|
|K10/new5|CSIL官方流程|42.833%|0.689%|20.413%|1.264%|42.144pp|
|K10/new5|MoPC-HR官方流程|45.322%|9.322%|49.547%|14.947%|36.000pp|
|K10/new10|历史完整D92|86.111%|72.533%|66.353%|69.106%|13.578pp|
|K10/new10|CSIL官方流程|42.833%|0.000%|10.460%|0.000%|42.833pp|
|K10/new10|MoPC-HR官方流程|45.322%|9.500%|32.900%|13.770%|35.822pp|
|K10/new20|历史完整D92|86.111%|71.333%|68.150%|69.555%|14.778pp|
|K10/new20|CSIL官方流程|42.833%|38.222%|1.660%|2.979%|4.611pp|
|K10/new20|MoPC-HR官方流程|45.322%|7.611%|25.187%|10.695%|37.711pp|

历史完整D92在这些共同slice上的H明显更高，但不能把差距全部归因于“历史完整D92算法优于论文算法”。CSIL/MoPC-HR流程在自己的base训练后得到的`old-before`仅约42.8%和45.3%，而历史完整D92在冻结ADV3B02表征和自身注册前状态构造下于K10达到86.1%；表征质量、模型生命周期、训练权限和CVS接口适配共同影响结果。

更可靠的结论是：

1.历史完整D92在当前ADV3B02主线中保持了更强的旧类target基础。
...
D92 E0没有new2切片，不能填入这张表做严格排名。74.803%的K10/new5 H属于历史完整D92，不属于`P2-E0`；它与qKNN E20的K10/new2 H=79.97%难度也不同。新类数从2增至5会改变候选空间、遗忘和跨角色混淆，不能用5.17个百分点差值宣布qKNN优于D92 E0或历史完整D92。
...
这些结果支持D92 E0对“旧新任务均衡”的关注，但也说明仅平衡协方差不足以解决全部跨角色冲突。

## 27.公平比较矩阵

|方法|原始任务|项目对比任务|source/base访问|target标签|支持新类注册|旧类保护机制|严格可与D92 E0 paired？|
|---|---|---|---|---|---|---|---|
|D92 E0|CVS Stage2-B/C|同原始任务|只读bundle，禁止source样本|旧类+新类K-shot|是|扰动谱稳健中心+任务均衡收缩协方差+双几何LOO融合；Fisher关闭|与`P2-FULL`同排；与论文方法当前否|
...
|---|---|---|---|---|---|
|D92 E0|20步旧类对角metric+44次K10闭式分量拟合+full/block LOO融合；无Fisher候选|中等；K10派生审计约1.38–1.67GMAC，仍有FP64稠密矩阵工作区|双层残差INT8系数+FP16尺度/截距+FP32 metric；C=26核心数组16.11KiB|解码后编译头\(Cp\)；C=26时7,488MAC|有条件适合：低频异步注册、长期高频推理|
...
1.协议边界清楚。D92 E0 fit只读取support和只读bundle，query完全不进入适配。
2.没有query角色Oracle。最终只有一个全注册类头。
3.没有按receiver、场景、seed、新类数或TX标识调参。
4.任务权重不随新类数量自然漂移。旧类只有6类、新类可达20类时，旧任务仍保留50%协方差权重。
5.状态和query计算轻。D92 E0把稳健中心和双几何选择编译进单一量化仿射状态，不引入query-query图或Fisher query变换。
6.全量screen覆盖75个identity和225个场景单位，并与`P2-FULL`保持同排输入，可直接隔离关闭Fisher的影响。
7.冻结arm身份为`P2-E0/fisher_profile=off`，不会把后续E0D、E0OCF或CCOC扩展混入基础方法。

## 29.D92 E0的局限

### 29.1 K1结构性无效

没有类内残差就不能可靠估计任务协方差。D92 E0在K1仍能输出保守分类状态，但稳健中心和旧/新均衡协方差等区别性模块不激活；Fisher residual在所有K下都由E0配置关闭，不能把K1回退误写成Fisher特有行为。

### 29.2关闭Fisher降低计算，但旧类与H小幅下降

在225个同排场景单位上，`P2-E0-P2-FULL`的旧类准确率、H和最低旧类准确率分别为−0.44、−0.14和−0.51个百分点，而新类准确率为+0.10个百分点。E0的优势主要是删除Fisher专属注册计算，不是平均性能提升。固定0.5/0.5平衡的仍只是协方差估计权，不保证score分布、prototype半径或logit标尺自动平衡。

### 29.3 没有显式ground→LEO域变换

D92 E0的扰动谱模块只用地面聚合知识做support可靠性加权和类中心平移。它没有学习显式的ground到target共享变换，也没有让地面旧类知识成为K1可用的弱先验。

### 29.4 共享协方差表达能力有限

全部类共享一份\(\boldsymbol{\Sigma}_{\mathrm{bal}}\)，无法表示各类半径、各类不确定度和局部非线性边界。旧类与新类在同一接收机上仍可能具有不同尺度和多模态结构。

### 29.5缺少E0专属Role-Oracle诊断

现有Role-Oracle数字来自历史完整D92专项run，不是`P2-E0`。因此D92 E0目前只能从无Oracle混淆和统一全类竞争机制判断跨角色校准仍是风险，不能引用历史7.20个百分点或11.98个百分点作为E0差距。

### 29.6全量screen绝对性能仍低

`P2-E0`跨225个场景单位的`DA1_REG1`旧类准确率、最低旧类准确率和新类准确率均值分别为58.79%、27.33%和53.43%。这些总体量不能替代预登记关键slice门，但已经足以说明screen没有形成可推广性能证据；完成artifact闭合不等于达到部署目标。

### 29.7尚无飞行级资源证据

现有实验在N607服务器和研究软件栈上完成，没有在目标星载CPU、DSP、FPGA或NPU上报告完整注册器的WCET、峰值RAM、单次注册能量、持续推理功耗、热稳态或辐射容错。D92 E0的轻量结论只适用于最终仿射头；完整系统仍需单独完成硬件剖析与飞行软件工程化。

## 30.如何正确使用D92 E0

D92 E0适合：

- 作为关闭Fisher residual后的低复杂度统计判别基线；
- 检验旧/新任务均衡协方差和full/block LOO融合在无Fisher候选时的独立行为；
...
- 声称解决了K1域适应；
- 把历史完整D92的Role-Oracle结果转移为E0性能；
- 只强调注册计算下降而不报告H、旧类和floor的配对下降；
...
下一步不应借D92 E0的query结果继续扫描\(\boldsymbol{\Sigma}_{\mathrm{o}}/\boldsymbol{\Sigma}_{\mathrm{n}}\)权重。E0冻结配置使用0.5/0.5任务权重；若用query性能寻找0.4/0.6或0.6/0.4会破坏方法锁，也难以解决K1和跨角色标尺。
...
D92 E0是一套从固定接收IQ到不可变prediction artifact的完整Phase2方法。它用冻结表征生成288维联合特征，以类无关扰动谱和Cauchy权重稳健化support中心，以Ledoit–Wolf收缩解决高维小样本协方差不适定，以旧/新任务固定等权抑制类别数量造成的统计偏置，再用full/block留一可靠性融合和双层INT8量化把状态编译为单一等先验仿射分类器。E0把Fisher变换固定为单位矩阵，不构造Fisher候选、Pareto替换门或原子联合检查。

全量screen中，`P2-E0`的旧类准确率、新类准确率和H均值分别为58.79%、53.43%和55.27%。相对同排`P2-FULL`，E0的H下降0.14个百分点、旧类准确率下降0.44个百分点、最低旧类下降0.51个百分点，新类准确率上升0.10个百分点。它证明的是“关闭Fisher可显著降低注册代数规模，但会产生小幅、稳定的旧类/H代价”，而不是“E0性能优于完整D92”。当前证据为screening，尚未fresh confirmation。

MRIOR-SDA和DADDA-SDA承担闭集接收机域适应，不承担新类注册；CSIL、MoPC-HR和Orthogonal Incremental承担类增量任务，却采用不同的base训练、增量更新和数据权限。现有论文复现结果只能用于机制和生命周期对照，历史完整D92的逐slice数字也不能改名为E0。当前最准确的定性是：

> D92 E0是一种协议合法、技术闭合、以关闭Fisher换取较低注册复杂度、但尚未达到推广门槛的support-only跨接收机少样本类增量方法。

资源结论同样不能简化成“轻量”二字：K10注册器包含约44次闭式分量拟合，派生审计约1.38–1.67GMAC等价上界；最终量化核心数组约7.46–16.11KiB，分类头为3,168–7,488MAC/query。与MRIOR、DADDA、CSIL和MoPC-HR的多轮梯度更新相比，D92 E0更接近星上低频后台注册；与ProtoNet、单qKNN相比，它仍牺牲注册简洁性换取固定大小、低query成本的参数化判别头。当前量化已经压缩常驻状态，但NumPy实现仍解码为FP32后计算，没有证明整数算子加速。现阶段最准确的部署判断是：**算法架构有条件适合星上，最终头适合星上，完整注册器需要工程化，当前研究实现不具备直接上星条件。**
...
以下D81、D61、D62、D92专项和Role-Oracle材料只用于解释模块来源、历史实现与对照机制，不是`P2-E0`主结果：
