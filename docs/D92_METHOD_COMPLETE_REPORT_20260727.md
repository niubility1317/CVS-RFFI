...
资源上，RTB-IDR属于“低频注册较重、长期推理很轻”的方法。K10完整状态构造按现有审计约为11.15–11.74GMAC等价上界；26类最终FP32仿射头仅29.35KiB，分类部分为7,488MAC/query。算法架构适合“偶发注册、大量推理”的星上工作模式，但当前Python、NumPy、PyTorch和scikit-learn研究实现没有目标星载处理器上的WCET、峰值RAM、能耗、热和容错证据，不能直接视为飞行软件。
...
### 13.6常驻状态：逻辑部署头与当前研究对象

所有复杂选择最终可编译为一套FP32仿射参数：

$$
\boldsymbol{W}\in\mathbb{R}^{C\times288},
\qquad
\boldsymbol{b}\in\mathbb{R}^{C}.
$$
...
|状态|类数\(C\)|FP32标量数|逻辑部署头大小|
|---|---:|---:|---:|
|注册前旧类头|6|1,734|6.77KiB|
|new5后|11|3,179|12.42KiB|
|new10后|16|4,624|18.06KiB|
|new20后|26|7,514|29.35KiB|

当前Python研究对象为了量化一致性检查和审计，还保留`log_diag_fp32`、两套int8分量系数、FP16尺度/截距以及FP32最终系数。忽略类注册JSON元数据时，其核心数组大小为
...
在\(C=26\)时为46,340B，即45.25KiB。星上产品实现应只保存经校验的最终头、类表、版本、校验和和回滚头；不应照搬Python审计对象的全部冗余数组。
...
第二列是单一FP32仿射头的实际代数下界；第三列保留现有研究流水线对特征变换和双分量路径的保守清单口径。完成产品级编译后应以生成的算子图重新计数，不能混用两种口径。

### 13.8端到端query不能只报7,488MAC
...
|最终仿射头|很低|6.77–29.35KiB逻辑FP32状态|不需要|每条query|
...
|---|---|---|---|---|---|
|D92|20步旧类对角metric+88次K10闭式分量拟合+LOO/Fisher安全门|中至高；K10为11.15–11.74GMAC等价上界，FP64稠密矩阵工作区|逻辑FP32头为\(4C(289)\)B；C=26时29.35KiB|编译头\(Cp\)；C=26时7,488MAC|有条件适合：低频异步注册、长期高频推理|
...
当\(C=11\)时，qKNN需要35,200B样本状态和17,600次点积MAC，而D92的11类编译头为12.42KiB和3,168MAC；当\(C=26\)时，qKNN约需81.25KiB和41,600MAC，D92为29.35KiB和7,488MAC。两者比值约为
...
因此qKNN注册明显更便宜，D92则在大量后续query上用一次较重的状态编译换取约5.56倍更低的分类头计算和不随K增长的常驻头。若使用288维qKNN或加入adapter，qKNN的状态与query开销还会增加。
...
6.评估FP32注册、FP16/INT8推理；任何降精度都必须重新验证125矩阵和数值闭合；
...
|部署部件|当前判断|原因|
|---|---|---|
|D92最终仿射头|适合|C=26时仅29.35KiB逻辑FP32状态、7,488MAC/query|
...
资源结论同样不能简化成“轻量”二字：K10注册器包含约88次分量拟合和11G量级审计上界，弱算力平台不适合高频重建；最终统一头却只有12.42–29.35KiB和3,168–7,488MAC/query。与MRIOR、DADDA、CSIL和MoPC-HR的多轮梯度更新相比，D92更接近星上低频后台注册；与ProtoNet、单qKNN相比，它牺牲注册简洁性换取固定大小、低query成本的参数化判别头。现阶段最准确的部署判断是：**算法架构有条件适合星上，最终头适合星上，完整注册器需要工程化，当前研究实现不具备直接上星条件。**
