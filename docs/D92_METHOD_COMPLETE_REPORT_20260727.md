...
12臂联合消融进一步给出该权衡的条件边界：完整几何下B0相对FULL为\(\Delta H=+0.606\)pp；但在D0几何、C3关闭时，B0使H变为−0.410pp，在D0几何、C3开启时变为−0.490pp。故B0的单因素结果不能被解释为跨几何路径恒定的模块效应；完整的二阶交互和端点结果见第12.7节。
...
联合矩阵表明C3同样不是独立于几何的常数效应：在完整几何下，B0/C3对H的二阶交互为−0.387pp；在D0下为−0.080pp，并伴随A-old的−0.556pp、F的+0.556pp交互。D2下四个B/C组合的离散评分指标完全相同，但有效配置哈希不同，所以这只能视为本切片未检出B/C条件差异，不能外推为内部状态或所有seed完全相同。
...
当B0和C3也同时变化时，D0/D2的解释更必须保持条件化：`B0+C3+D0`是全部兼容替换的端点，相对FULL的H为−0.677pp；相对D0档的B/C均完整基线低−0.715pp；若保持B0+C3状态不变、只将几何从FULL改为D0，则H下降−1.133pp。`B0+C3+D2`相对FULL的H为+0.291pp，却把A-old降低0.556pp并使F增加0.556pp。因此D0/D2不是可与B0、C3线性相加的“第三、第四个开关”，而是决定二者表现在哪种共享几何中的互斥路径。
...
|二：稳健中心|B0：普通support均值|单因素B0的(H)高0.606pp；联合矩阵中D0下转为负效应|效应依赖C3和几何，尚无全局胜出结论|
|三：自动收缩|S0：经验协方差加固定ridge|FULL相对S0的ΔH=+11.535pp|当前切片强烈支持自动收缩的稳定性价值|
|四：任务均衡协方差|C3：所有类别等权协方差|单因素C3的(H)高0.238pp；B0/C3交互随几何改变|小幅且条件化的权衡，不能只按H定胜负|
|五：双几何交叉拟合|D0：full-only；D2：固定半权|D0/D2相对FULL的方向随B0/C3状态改变|旧类保护与新类识别存在条件化权衡，待多seed确认|
...
### 12.7B0/C3/几何12臂联合消融：完整条件矩阵

此前第12.3节的单因素控制回答“其他模块保持FULL时，单项替换会怎样”。为避免把这些差值错当成可相加的模块贡献，本轮新增`d92_e0_256_joint_ablation_bc_geometry_20260826`。它固定相同接收机`3-19`、`K10/new5`、种子束、256维特征、S0、F3、Phase1bundle和三个LEO弱场景，仅交叉B0、C3与几何因素\(G\)。D0和D2是同一几何模块的互斥控制路径，因此完整合法矩阵是\(2\times2\times3=12\)臂而非16臂。

12个逻辑row对应12个独立物理执行，全部预测和独立truth-last评分均为`PASS`；没有失败row或alias row。每一row都带有`truth_opened_after_prediction_commit=true`和`scorer_output_must_not_feed_predictor=true`。下表是三个场景等权平均；B-old来自所有臂共有的注册前预测封存物，因而完全相同，变化只能在注册后的REG1状态出现。

|G|B|C|逻辑臂|B-old|A-old|New|H|F|相对FULL\(\Delta H\)|
|---|---|---|---|---:|---:|---:|---:|---:|---:|
|FULL|FULL|FULL|`P2-256-FULL`|73.056%|68.333%|62.333%|65.180%|4.722pp|+0.000pp|
|FULL|B0|FULL|`P2-256-B0`|73.056%|68.056%|63.667%|65.785%|5.000pp|+0.606pp|
|FULL|FULL|C3|`P2-256-C3`|73.056%|68.056%|63.000%|65.417%|5.000pp|+0.238pp|
|FULL|B0|C3|`P2-256-J-B0-C3`|73.056%|67.778%|63.667%|65.636%|5.278pp|+0.457pp|
|D0|FULL|FULL|`P2-256-D0`|73.056%|67.222%|63.333%|65.218%|5.833pp|+0.038pp|
|D0|B0|FULL|`P2-256-J-B0-D0`|73.056%|67.500%|62.333%|64.808%|5.556pp|-0.372pp|
|D0|FULL|C3|`P2-256-J-C3-D0`|73.056%|67.500%|62.667%|64.993%|5.556pp|-0.186pp|
|D0|B0|C3|`P2-256-J-B0-C3-D0`|73.056%|67.222%|62.000%|64.503%|5.833pp|-0.677pp|
|D2|FULL|FULL|`P2-256-D2`|73.056%|67.778%|63.333%|65.471%|5.278pp|+0.291pp|
|D2|B0|FULL|`P2-256-J-B0-D2`|73.056%|67.778%|63.333%|65.471%|5.278pp|+0.291pp|
|D2|FULL|C3|`P2-256-J-C3-D2`|73.056%|67.778%|63.333%|65.471%|5.278pp|+0.291pp|
|D2|B0|C3|`P2-256-J-B0-C3-D2`|73.056%|67.778%|63.333%|65.471%|5.278pp|+0.291pp|

对固定几何档\(g\)和任一指标\(m\)，B0/C3的二阶交互为
...
**本式符号说明：**\(m(B,C,g)\)是将B、C、G固定为括号内水平后得到的三个场景等权指标；\(B0\)、\(C3\)是控制替换而非“模块激活”；\(g\)取`FULL`、`D0`或`D2`；\(I_{BC}^{(m)}(g)\)单位为百分点。若交互为负，表示两个控制替换同时发生时的变化比把两个单独替换线性相加更差，不能直接把它命名为原模块间的物理冲突。
...
这给出三条谨慎结论。第一，FULL几何下B0与C3的H交互为−0.387pp，因此+0.606pp和+0.238pp不能相加成“预计+0.844pp”。第二，`B0+C3+D0`是用户所说“去掉全部兼容模块”的合法端点；它相对FULL的H为−0.677pp，相对D0档的B/C均完整基线低−0.715pp；若保持B0+C3状态不变、只将几何从FULL替换为D0，则H下降−1.133pp。第三，`B0+C3+D2`的H相对FULL高+0.291pp，但A-old低0.556pp、F高0.556pp；它不是可凭H单独宣布获胜的替代方案。D2下四臂的离散评分相同，但其有效配置哈希不同，只能说明本切片没有检测到B/C的行为差异。

所有12臂保持11类\(\times256=2816\)查询头MAC、5742B候选头状态和17216B部署状态。F3仅支持`storage_compression_only`：`integer_kernel_used=false`，且本轮没有F0/FP32对照臂。故本节的性能结论不混入量化因果结论；D0类个别row的量化翻转率也仅是F3保真回执，不改变truth-last评分边界。
...
- 当前结果仍缺少多seed、更多接收机/新类规模和fresh confirmation；本节12臂仅是一个接收机、一个种子束上的条件效应筛选。
...
本文的实验和资源表引用两组已完成的真实256维证据：(1)`analysis/d92_e0_256_module_ablation_hard11_20260826_v2.md`及对应的`automation_reports/CV-SincNet/d92_e0_256_module_ablation_hard11_20260826_v2/remote_results`，含7个独立物理执行和7个独立评分row；(2)`analysis/d92_e0_256_joint_ablation_bc_geometry_20260826.md`及对应的`automation_reports/CV-SincNet/d92_e0_256_joint_ablation_bc_geometry_20260826/remote_results`，含12个独立物理执行和12个独立评分row。两组每臂均在3个LEO弱场景独立评分。
