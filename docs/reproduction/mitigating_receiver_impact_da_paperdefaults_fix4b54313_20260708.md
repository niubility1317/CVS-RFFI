...
## 2026-07-08 Table II已实现方法全任务复现

运行：`mitigating_receiver_impact_da_table2_full_e10_implemented_20260708`。该运行覆盖配置中的5个Table II任务，但只包含当前代码已实现的`source_only`和`proposed`。DANN、MCD、SHOT未实现，不能作为本轮复现实验结果。

设置：full WiSig ManySig数据，`epochs=10`，`source_pretrain_epochs=10`，`batch_size=64`，`learning_rate=0.0006`，`tau=0.7`，`m=7`，`lambda=0.005`，`mu=0.5`，`class_prior_mode=uniform`，类别权重平滑/裁剪/均值归一化关闭。

| 任务 | 复现Source | 论文Source | 差值pp | 复现Proposed | 论文Proposed | 差值pp | 复现增益pp | 论文增益pp |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| `d01->d23` | 94.34 | 83.69 | +10.65 | 95.02 | 93.34 | +1.68 | +0.68 | +9.65 |
| `14-7->3-19` | 26.20 | 30.25 | -4.05 | 30.18 | 92.42 | -62.24 | +3.98 | +62.17 |
| `1-1->1-19` | 57.32 | 67.87 | -10.55 | 49.90 | 95.44 | -45.54 | -7.42 | +27.57 |
| `1-1->8-8` | 66.36 | 59.94 | +6.42 | 35.53 | 99.78 | -64.25 | -30.83 | +39.84 |
| `7-7->8-8` | 33.37 | 55.50 | -22.13 | 49.58 | 99.74 | -50.16 | +16.21 | +44.24 |
| **均值** | **55.52** | **59.45** | **-3.93** | **52.04** | **96.14** | **-44.10** | **-3.48** | **+36.69** |

诊断结论：

- source-only均值只低于论文3.93pp，且`d01->d23`超过论文10.65pp，说明基础数据任务和source训练不是整体失效点。
- proposed均值低于论文44.10pp，主要差距集中在4个cross-receiver任务。
- proposed在`1-1->1-19`和`1-1->8-8`出现负迁移，说明当前GAD/CPL训练动态仍不稳定。
- 多个任务训练中曾出现高于最终checkpoint的target_pred_acc，例如`1-1->8-8`训练中e1为70.87%但最终35.53%，`7-7->8-8`训练中e6为78.38%但最终49.58%。剩余差异更可能来自论文未公开的收敛/模型选择策略、BN路径或训练稳定化细节。
