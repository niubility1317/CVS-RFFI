...
|---|---|---|---|---|---|---|
|RSE-01|第四、十三节|固定E0结构，仅训练target head和`t3.norm(weight,bias)`|adapt、runner、配置、测试|verified|55项聚焦回归、独立P0/P1审查|禁止E1/E2扩展、类别bias、HardPair、Adapter、full t3、frequency/domain更新和EMA|
...
|RSE-08|第十一节|记录full forward、suffix forward/backward、head step、视图数和FBE输入计数|audit、runner、测试|implemented|receipt字段与validation cache计数测试|逐模块MAC/FBE在缺少MAC基线时标记`NOT_CAPTURED`|
|RSE-09|第九节|发布R0、R1、R2、R3最小矩阵；R4仅在R1或R2单独通过后运行|配置、run报告|implemented|矩阵解析与CLI回归；待真实artifact|R4不作为本轮前置gate|
|RSE-10|第九、十二节|使用新seed713103、旧6类K=10、最大独立Query120，truth-last同row评分|run报告、N607 artifact|pending|待prediction闭合后独立scorer|本轮不注册新类|
|RSE-11|第十节|满足时间、参数、delta、cache和内存资源合同|runner、benchmark、报告|pending|待GNU time与GPU采样|10次常驻P90不是首轮科学筛选gate，候选通过后再确认|
|RSE-12|第十二节|按域报告配对BA/floor/NLL；跨域统计升级留待多域矩阵|报告|deferred|单域结果只能支持方向性判断|receiver bootstrap、三场景和K退化曲线不扩大本轮最小矩阵|

当前计数：verified=7，implemented=2，pending=2，deferred=1，rejected=0，blocked=0。RSE结构已严格落地；0.05rad增强幅度属于报告未给出具体数值的`UNPUBLISHED_DEFAULT`。R4和跨域统计属于预先声明的条件阶段，不冒充本轮已验证结果。
