...
|PACE-08|第九节|OARC时间/参数/存储/内存/计算回执|runner、benchmark、测试|partial|既有10次resident benchmark可复用|能量/温度无传感器，标记`NOT_CAPTURED`|
|PACE-09|第十一节|E0–E3最小矩阵|config、run报告|verified|4行严格解析|E0 D0；E1 PACE-W；E2 PACE-All；E3 E2+bias|
|PACE-10|第十一节|新未暴露合法capsule truth-last验证|run报告、N607 artifact|ready|seed713102 package/sidecar只读存在性核对|独立Query120条，prediction完成前不连接truth|
...
当前计数：verified=8，ready=1，partial=1，pending=1，deferred=1，rejected=0，blocked=0。PACE-06的在线触发阈值尚无跨接收机地面冻结依据，因此本轮E1–E3按预登记矩阵强制执行，不从query调阈值；它不阻断验证矩阵。
