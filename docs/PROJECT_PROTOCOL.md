...
2026-07-17用户另行授权一次性`PRE_FORMAL_SUPPORT_ONLY_INT8_SCREEN`，用于避免在路线尚未证明有效前反复建设封存工程。它只可读取已预先压缩、hash固定且真实TX列映射固定的历史int8聚合组件和开发seed合法LEO_weak support，禁止query、正式指标声明、确认矩阵、全精度prototype及任何source/sample-level输入。正路线进入正式Stage2-B/C前仍须改用与checkpoint共同封存并重建method lock的bundle。
