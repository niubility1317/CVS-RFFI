...
### Experiment release readiness and health control

- Every N607 release must have one sole runner for its immutable run ID. The runner owns bounded preflight, exact-file sync, remote hash/compile checks, detached launch, PID/GPU/process verification, startup and first-wave health checks, short-connection monitoring, artifact retrieval, and the structured handoff. It must not change the method, tune from target/query results, restart a failed run, or intervene in unrelated workloads.
- Release readiness requires the actual Git-backed implementation, not a plan-only or artifact-validator-only shell. Before N607 handoff, complete targeted tests, protocol-negative tests, a real-checkpoint no-query smoke, an independent review with `P0=0` and `P1=0`, and a Git commit. A runnable release path must contain the real row executor, frozen full-matrix executor, immutable prediction publication, independent truth-side scorer, per-row logs/exit evidence, fail-closed artifact validation, and the current goal's required GPU scheduling. Author self-tests or `--help` success do not replace independent review.
- When the active goal requires a complete125 or another complete formal matrix, a technically healthy performance run must execute that entire frozen matrix. Do not select a receiver, seed, K, scene, row, or partial result because it looks favorable. Local or first-wave probes are technical-health evidence only and must not be read as performance or used to choose the method.
- Immediately after detached launch, verify the main PID, exact CWD/cmdline/run-root binding, GPU mapping, log growth, and launch artifacts. At the first completed or failed row, and again after the first worker wave, report launched/completed/succeeded/failed row counts, prediction/score counts, active worker PIDs, GPU utilization/memory, and normalized exception fingerprints. Continue later monitoring with short, bounded connections.
- Stop dispatching new rows and terminate the exact run-owned process tree when any P0 protocol/safety violation occurs, or when at least two distinct rows produce the same deterministic exception fingerprint before producing a prediction. A launcher-wide deterministic fault, output-overwrite risk, query leakage, wrong checkout/hash, or repeated zero-prediction failure is systemic; do not wait for the remaining matrix to fail naturally. Do not inspect accuracy, H, BA, floor, or other performance values to make this decision.
- Before stopping, resolve and record the exact run-owned main/child PIDs, CWDs and cmdlines. Send a bounded graceful termination first; escalate only for still-live PIDs already proven to belong to that run. Never use broad `pkill`, kill unrelated processes, delete outputs, or overwrite the run. After stopping, verify GPU release, local SSH cleanup, and preserve/retrieve partial logs, row exits, prediction/score counts, parity receipts and any archive/coverage artifacts that actually exist.
- A stopped run uses `STOPPED_EARLY_SYSTEMIC_TECHNICAL_FAILURE / NO_PERFORMANCE_RESULT`. Close its original report with the trigger, exception fingerprint, launched/completed/failed counts, PID/exit evidence, GPU/SSH cleanup, and actual artifact inventory. Fix locally, run independent review again, commit, create a new non-overwriting run ID/report, and repeat preflight. Never resume, overwrite, or relabel the failed run as a performance experiment.
