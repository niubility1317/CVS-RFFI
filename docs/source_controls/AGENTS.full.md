...
- The project release repository installs `scripts/post_commit_auto_push_hook.sh` through `scripts/install_auto_push_hook.sh`; automatic push is the default for every commit in this repository and its worktrees. A push failure leaves the commit intact and must be reported as `FAILED` or `UNKNOWN`; never force-push or stage unrelated dirty/untracked files.
