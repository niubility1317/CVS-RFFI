...
class CVSincNet(nn.Module):
    ...
    def __init__(
        ...
        self.f1 = DSConvBlock1d(4, int(freq_ch1), k=5, pool=2, drop=0.05) if self.use_freq_path else None
        ...
        self.freq_gate = FreqBandGate1d(4, k=5, alpha=freq_gate_alpha) if (self.use_freq_path and self.use_freq_band_gate) else nn.Identity()
        ...
        self.pa_stats_proj = (
            nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Dropout(drop * 0.25),
            ) if (self.use_stats_path and self.use_pa_path) else None
        )
    ...
    def forward(
        ...
        if need_freq:
            ...
            dac_delta = self.dac_subband_agg(feat_f) if (need_stats and self.dac_subband_agg is not None) else zero_emb
