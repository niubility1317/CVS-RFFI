...
def apply_model_variant_training_defaults(args):
    ...
    if variant == "lite_c":
        ...
        args.exp_desc = str(getattr(args, "exp_desc", "")) + " | Lite-C: probe/select/mono/cross_zero default off"
    return args
...
def selective_shift_margin_loss(main_shift: torch.Tensor, other_shift: torch.Tensor, margin: float = 0.05) -> torch.Tensor:
    main = torch.nan_to_num(main_shift.float(), nan=0.0, posinf=2.0, neginf=0.0).clamp(0.0, 2.0)
    other = torch.nan_to_num(other_shift.float(), nan=0.0, posinf=2.0, neginf=0.0).clamp(0.0, 2.0)
    return F.relu(float(margin) + other - main).mean()
...
def pairwise_monotonic_loss(strength: torch.Tensor, response: torch.Tensor, margin: float = 0.0, min_delta: float = 1e-4) -> torch.Tensor:
    s = torch.nan_to_num(strength.float().view(-1), nan=0.0, posinf=1.0, neginf=0.0).clamp(0.0, 1.0)
    r = torch.nan_to_num(response.float().view(-1), nan=0.0, posinf=2.0, neginf=0.0).clamp(0.0, 2.0)
    if s.numel() <= 1:
...
def compute_core_losses(
    ...
    if d is not None and valid is not None and bool(valid.any()):
        ...
        if domain_gates.get("probe", False):
            loss_probe = ce_dom(probe_dom_logits[valid].float(), d_valid)
            probe_acc = accuracy_from_logits(probe_dom_logits[valid], d_valid)
...
def compute_aux_losses(
    ...
    out = {
        ...
        "loss_pa_imp_inv": ref.new_tensor(0.0),
        "loss_pa_kl": ref.new_tensor(0.0),
        "loss_dac_reg": ref.new_tensor(0.0),
        "loss_pa_reg": ref.new_tensor(0.0),
        "loss_cross_zero": ref.new_tensor(0.0),
        "loss_dac_select": ref.new_tensor(0.0),
        "loss_pa_select": ref.new_tensor(0.0),
        "loss_dac_mono": ref.new_tensor(0.0),
        "loss_pa_mono": ref.new_tensor(0.0),
    ...
    if need_pa_aux and out_pa is not None:
        ...
        out["loss_pa_imp_inv"], out["cos_imp_pa"] = cosine_consistency_loss(pa_imp, clean_imp)
    ...
    if need_pa_aux and need_dac_aux and out_pa is not None and out_dac is not None:
        ...
        out["loss_pa_select"] = selective_shift_margin_loss(out["shift_pa_on_pa"], out["shift_pa_on_dac"], margin=float(args.select_margin))
        out["loss_dac_select"] = selective_shift_margin_loss(out["shift_dac_on_dac"], out["shift_dac_on_pa"], margin=float(args.select_margin))

    if need_pa_aux:
        out["loss_pa_mono"] = pairwise_monotonic_loss(s_pa, out["shift_pa_on_pa"], margin=float(args.mono_margin))
    if need_dac_aux:
        out["loss_dac_mono"] = pairwise_monotonic_loss(s_dac, out["shift_dac_on_dac"], margin=float(args.mono_margin))

    if need_pa_aux and need_dac_aux and out_pa is not None and out_dac is not None and cur_w["cross_zero"] > 0.0:
        ...
        out["loss_cross_zero"] = 0.5 * (smooth_strength_loss(dac_pred_pa, zeros) + smooth_strength_loss(pa_pred_dac, zeros))

    return out


def compute_selective_losses(aux_losses: Dict[str, Any]) -> Dict[str, torch.Tensor]:
    return {
        "loss_dac_select": aux_losses["loss_dac_select"],
        "loss_pa_select": aux_losses["loss_pa_select"],
        "loss_dac_mono": aux_losses["loss_dac_mono"],
        "loss_pa_mono": aux_losses["loss_pa_mono"],
        "loss_cross_zero": aux_losses["loss_cross_zero"],
    }
...
def safe_backward_step(model, optimizer, scaler, loss: torch.Tensor, args, use_amp: bool) -> Tuple[bool, Dict[str, float]]:
    ...
    domain_head_params = []
    for name in ("dom_head", "adv_head", "probe_head"):
...
def build_domain_label_map(dataset) -> Dict[int, int]:
    ...
    from unseen raw domains are mapped to -1 and ignored for dom/probe accuracy.
...
def print_domain_configuration(args, train_ds, split_info, domain_label_map: Dict[int, int]):
    ...
    print("[DOMAIN-LOSS] loss_probe: 仅训练 probe head 诊断 z_id 是否泄露该域；z_id.detach()，不直接反传改造 ID 特征。")
...
def remap_domain_tensor(d: Optional[torch.Tensor], domain_label_map: Dict[int, int], device) -> Optional[torch.Tensor]:
    ...
    ignored by domain/probe accuracy in evaluation and should not enter domain CE.
    """
    if d is None:
        return None
...
def main():
    ...
    parser.add_argument("--model_variant", type=str, default="base", choices=["base", "lite_a", "lite_b", "lite_c"],
                        help="Lightweight model variant. base keeps legacy behavior.")
    parser.add_argument(
        "--branch_ablation",
        type=str,
        default="none",
        help=(
            ...
            "Valid: none,no_time,no_dac,no_pa,no_freq,no_stats,no_aux_logits. "
    ...
    parser.add_argument("--mixstyle_mix", type=str, default="crossdomain", choices=["crossdomain", "random"])
    ...
    model = build_dual_model(args.num_classes, num_domains, model_size=args.model_size, dataset=args.dataset,
                             ...
                             mixstyle_mix=str(args.mixstyle_mix)).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                tx_logits = out_main["tx_logits"]
                z_id = out_main["z_id"]
                core = compute_core_losses(out_main, y, d, domain_stats, domain_gates, ce_tx, ce_dom)
                if not math.isnan(core["cons_cos"]):
                    cons_cos_vals.append(core["cons_cos"])
                m_domacc.update(core["dom_acc"])
                m_probeacc.update(core["probe_acc"])
                if anchor is None:
                    aux = {
                        "loss_cls_pa": z_id.new_tensor(0.0), "loss_cls_dac": z_id.new_tensor(0.0),
                        "loss_pa_joint_inv": z_id.new_tensor(0.0), "loss_pa_imp_inv": z_id.new_tensor(0.0),
                        "loss_pa_kl": z_id.new_tensor(0.0), "loss_dac_reg": z_id.new_tensor(0.0),
                        "loss_pa_reg": z_id.new_tensor(0.0), "loss_cross_zero": z_id.new_tensor(0.0),
                        "loss_dac_select": z_id.new_tensor(0.0), "loss_pa_select": z_id.new_tensor(0.0),
                        "loss_dac_mono": z_id.new_tensor(0.0), "loss_pa_mono": z_id.new_tensor(0.0),
                ...
                _ = compute_selective_losses(aux)
                ...
                loss_probe = core["loss_probe"]
                loss_cls_pa = aux["loss_cls_pa"]
                loss_cls_dac = aux["loss_cls_dac"]
                loss_pa_joint_inv = aux["loss_pa_joint_inv"]
                loss_pa_imp_inv = aux["loss_pa_imp_inv"]
                loss_pa_kl = aux["loss_pa_kl"]
                loss_dac_reg = aux["loss_dac_reg"]
                loss_pa_reg = aux["loss_pa_reg"]
                loss_cross_zero = aux["loss_cross_zero"]
                loss_dac_select = aux["loss_dac_select"]
                loss_pa_select = aux["loss_pa_select"]
                loss_dac_mono = aux["loss_dac_mono"]
                loss_pa_mono = aux["loss_pa_mono"]
                ...
                aux_terms = [
                    ...
                    cur_w["pa_imp_inv"] * sanitize_loss("pa_imp_inv", loss_pa_imp_inv, z_id, loss_warn_counts),
                    ...
                    cur_w["cross_zero"] * sanitize_loss("cross_zero", loss_cross_zero, z_id, loss_warn_counts),
                    cur_w["dac_select"] * sanitize_loss("dac_select", loss_dac_select, z_id, loss_warn_counts),
                    cur_w["pa_select"] * sanitize_loss("pa_select", loss_pa_select, z_id, loss_warn_counts),
                    cur_w["dac_mono"] * sanitize_loss("dac_mono", loss_dac_mono, z_id, loss_warn_counts),
                    cur_w["pa_mono"] * sanitize_loss("pa_mono", loss_pa_mono, z_id, loss_warn_counts),
                ]
                ...
                loss = (
                    ...
                    + cur_w["probe"] * sanitize_loss("probe", loss_probe, z_id, loss_warn_counts)
