...
try:
    from model_dual_rxadc_v3_satstable import build_dual_model as build_dual_model
except Exception:
    try:
        from model_dual_rxadc_v2 import build_dual_model as build_dual_model
    except Exception:
        from model_dual_cvsincnet import build_dual_model as build_dual_model
...
def _extract_nuisance_labels(extra, device: torch.device, target: str = "auto") -> Optional[torch.Tensor]:
    if extra is None or len(extra) == 0:
        return None
    d0 = extra[0]
    domain = None
    if torch.is_tensor(d0):
        domain = d0.to(device, non_blocking=True).view(-1)
    else:
        try:
            domain = torch.as_tensor(d0, device=device).view(-1)
        except Exception:
            domain = None

    meta = extra[1] if len(extra) > 1 and isinstance(extra[1], dict) else None
    def meta_get(key: str):
        if meta is None or key not in meta:
            return None
        v = meta[key]
        if torch.is_tensor(v):
            return v.to(device, non_blocking=True).view(-1)
        try:
            return torch.as_tensor(v, device=device).view(-1)
        except Exception:
            return None
    ...
    day = meta_get("day_i")
    if target == "rx":
        return rx if rx is not None else domain
    if target == "day":
        return day if day is not None else domain
    if target == "domain":
        return domain
    return rx if rx is not None else domain
...
def make_test_loader(args, batch_size: int, num_workers: int, device: torch.device) -> DataLoader:
    if str(args.dataset).lower() == "wisig":
        ...
        if WiFiRFFIDataset is None:
            raise RuntimeError("dataset.py / WiFiRFFIDataset import failed; oralce evaluation is unavailable in this environment.")
...
@torch.no_grad()
def eval_dual_aux(model: torch.nn.Module, loader: DataLoader, device: torch.device, nuisance_target: str = "auto") -> Dict[str, float]:
    if not hasattr(model, "rx_head"):
        return {}
    ...
    for batch in loader:
        xb, yb, extra = _unpack_xy_extra(batch)
        xb = xb.to(device, non_blocking=True)
        yb = yb.to(device, non_blocking=True)
        nlab = _extract_nuisance_labels(extra, device=device, target=nuisance_target)

        out = model(xb, y_tx=yb, grl_rx_on_tx=0.0, grl_tx_on_rx=0.0, return_aux=True)

        tx_logits = out.get("tx_logits", None)
        if torch.is_tensor(tx_logits):
            meters["tx_acc"].append((tx_logits.argmax(dim=1) == yb).float().mean().item() * 100.0)

        if nlab is not None:
            for key, meter_key in [
                ("rx_logits", "rx_acc"),
                ("adv_rx_logits", "adv_rx_on_tx_acc"),
                ("probe_rx_logits", "probe_rx_on_tx_acc"),
            ]:
                logits = out.get(key, None)
                if torch.is_tensor(logits):
                    meters[meter_key].append((logits.argmax(dim=1) == nlab).float().mean().item() * 100.0)

        for key, meter_key in [
            ("adv_tx_logits", "adv_tx_on_rx_acc"),
            ("probe_tx_logits", "probe_tx_on_rx_acc"),
        ]:
            logits = out.get(key, None)
            if torch.is_tensor(logits):
                meters[meter_key].append((logits.argmax(dim=1) == yb).float().mean().item() * 100.0)

    out_stats = {}
    for k, vals in meters.items():
        if len(vals) > 0:
            out_stats[k] = float(np.mean(vals))
    return out_stats
...
def main():
    ...
    num_classes = max(int(max(y_true.max(), y_pred.max()) + 1), int(getattr(model, "num_classes", int(args.num_classes))))
    ...
    aux_stats = eval_dual_aux(model, test_loader, device, nuisance_target=args.nuisance_target)
    if aux_stats:
        print("[DUAL-AUX]", aux_stats, flush=True)
        with open(os.path.join(args.out, "dual_aux_metrics.txt"), "w", encoding="utf-8") as f:
    ...
    try:
        for branch in ["time", "freq"]:
            ...
            nc = max(int(max(yt.max(), yp.max()) + 1), int(getattr(model, "num_classes", int(args.num_classes))))
