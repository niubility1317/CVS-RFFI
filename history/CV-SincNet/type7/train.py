...
def call_augmentor(
    ...
    """
    Generic wrapper around the external augmentor (DataAugmentation_v2.RFDataAugment).

    We add a *strength* knob that works even if the underlying augmentor doesn't expose
    its own intensity parameters.

    - strength=1.0: always apply augmentor (original behavior)
    - strength=0.0: identity (no augmentation)
    - 0<strength<1: either probabilistically apply the full augment (mix_mode='prob'),
      or linearly blend original/augmented (mix_mode='blend').

    Notes:
      - For RF I/Q signals, 'prob' is usually safer than 'blend' (blend can create
        superposition artifacts if the augmentor includes time-shifts).
      - dac_only=True keeps the view intended for DAC supervision; if strength<1,
        DAC view may become identity for some samples (dac_strength=0 for those).
    """
    if augmentor is None:
        if return_dac_strength:
            return x_iq, torch.zeros(x_iq.size(0), device=x_iq.device, dtype=x_iq.dtype)
        return x_iq
    ...
    def _apply_aug():
        # Underlying augmentor supports:
        #   - augmentor(x, return_dac_strength=bool)
        #   - optionally dac_only flag (if implemented)
        if return_dac_strength:
            try:
                return augmentor(x_iq, return_dac_strength=True, dac_only=dac_only)
            except TypeError:
                return augmentor(x_iq, return_dac_strength=True)
        else:
            try:
                return augmentor(x_iq, return_dac_strength=False, dac_only=dac_only)
            except TypeError:
                return augmentor(x_iq)

    if s >= 0.999:
        return _apply_aug()

    # Partial strength: compute augmented once then mix
    if return_dac_strength:
        x_aug, dac_strength = _apply_aug()
    else:
        x_aug = _apply_aug()
        dac_strength = None

    if mix_mode == "blend":
        x_out = x_iq + s * (x_aug - x_iq)
        if return_dac_strength:
            return x_out, dac_strength * s
        return x_out

    # Default: probabilistic application per-sample
    # mask shape [B,1,1] -> broadcast to [B,2,T]
    b = x_iq.size(0)
    mask = (torch.rand(b, device=x_iq.device) < s).view(b, 1, 1)
    x_out = torch.where(mask, x_aug, x_iq)

    if return_dac_strength:
        # if not augmented, dac_strength should be 0
        dac_strength = torch.where(mask.view(b), dac_strength, torch.zeros_like(dac_strength))
