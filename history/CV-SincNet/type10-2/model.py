...
class SincConv1d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        nyq = self.sample_rate / 2.0
        low = self.min_low_hz + F.softplus(self.low_hz_)
        band = self.min_band_hz + F.softplus(self.band_hz_)
...
class DilatedDSConvBlock1d(nn.Module):
    def __init__(self, cin: int, cout: int, k: int = 5, dilation: int = 1, pool: int = 1, drop: float = 0.08):
        super().__init__()
        pad = (k // 2) * dilation
        self.dw = nn.Conv1d(cin, cin, kernel_size=k, padding=pad, dilation=dilation, groups=cin, bias=False)
        self.pw = nn.Conv1d(cin, cout, kernel_size=1, bias=False)
        self.norm = nn.GroupNorm(_pick_gn_groups(cout), cout)
        self.act = nn.SiLU(inplace=True)
        self.pool = nn.AvgPool1d(pool) if (pool is not None and pool > 1) else nn.Identity()
        self.drop = nn.Dropout(drop) if (drop and drop > 0) else nn.Identity()
...
class PARegrowthAggregator(nn.Module):
    """
    PA-oriented spectral shortcut.
    Input : (B,2,K)
      ch0: symmetric log-power profile
      ch1: edge-emphasized symmetric profile
    Output: (B, emb_dim)
    """
    def __init__(self, emb_dim: int = 256, hidden: int = 32, k: int = 5):
        super().__init__()
        if k % 2 == 0:
            k += 1
        self.dw = nn.Conv1d(2, 2, kernel_size=k, padding=k // 2, groups=2, bias=False)
        self.pw = nn.Conv1d(2, hidden, kernel_size=1, bias=False)
        self.norm = nn.GroupNorm(_pick_gn_groups(hidden), hidden)
        self.act = nn.SiLU(inplace=True)
...
class MemoryPolynomialLift(nn.Module):
    ...
    """
    def __init__(self, memory_depth: int = 3, orders: Sequence[int] = (1, 3), clip: float = 2.0):
        super().__init__()
...
class PAProxyFeatures(nn.Module):
    """
    Lightweight PA-oriented proxies:
      1) envelope |x|
      2) envelope difference Δ|x|
      3) AM-PM proxy |x| * centered(Δphi)

    Input : (B,2,L)
    Output: (B,3,L)
    """
    def __init__(self, clip: float = 2.0):
        super().__init__()
        self.clip = float(clip)
        k = torch.tensor([[-1.0, 1.0]], dtype=torch.float32).view(1, 1, 2)
        self.register_buffer("k_diff", k)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        xr = torch.clamp(x[:, 0:1, :], -self.clip, self.clip)
        xi = torch.clamp(x[:, 1:2, :], -self.clip, self.clip)

        amp = torch.sqrt(torch.clamp(xr * xr + xi * xi, min=1e-8))
        damp = F.conv1d(amp, self.k_diff, padding=1)[..., : x.size(-1)]

        z = torch.complex(xr.squeeze(1), xi.squeeze(1))
        z_prev = torch.zeros_like(z)
        z_prev[:, 1:] = z[:, :-1]
        dphi = torch.angle(z * torch.conj(z_prev) + 1e-8).unsqueeze(1)
        dphi = dphi - dphi.mean(dim=-1, keepdim=True)
        ampm = amp * dphi

        return torch.cat([amp, damp, ampm], dim=1)


class EnvelopeGate1d(nn.Module):
    def __init__(self, out_ch: int, alpha: float = 0.35, k: int = 5):
        super().__init__()
        if k % 2 == 0:
            k += 1
        self.alpha = float(alpha)
...
class PhysicalAwareClassifier(nn.Module):
    """
    Classification head with cooperative DAC/PA fusion.

    Key idea:
      - keep explicit identity / DAC / PA embeddings
      - add DAC-PA cooperative feature
      - use soft branch weights so DAC/PA help each other instead of purely competing
    """
    def __init__(
        self,
        base_dim: int,
        ...
        self.dp_fuse = nn.Sequential(
            nn.Linear(2 * emb_dim, emb_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(drop * 0.30),
        )
        ...
        self.branch_score = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout(drop * 0.15),
            nn.Linear(emb_dim, 3),
        )

        self.coop_proj = nn.Sequential(
            nn.Linear(emb_dim, emb_dim),
            nn.ReLU(inplace=True),
            nn.Dropout(drop * 0.20),
        )
    ...
    def forward(
        ...
        if dac_delta is not None:
            feat_dac = feat_dac + 0.30 * dac_delta
        if pa_delta is not None:
            feat_pa = feat_pa + 0.35 * pa_delta

        dp_g = self.dp_gate(torch.cat([feat_dac, feat_pa], dim=1))
        feat_imp = self.dp_fuse(torch.cat([feat_dac * dp_g, feat_pa * dp_g], dim=1))

        id_g = self.id_gate(torch.cat([feat_dac, feat_pa], dim=1))
        feat_id = feat_id * (1.0 + self.gate_alpha * id_g)

        scores = self.branch_score(torch.cat([feat_id, feat_dac, feat_pa], dim=1))
        ...
        feat_coop = self.coop_proj(feat_coop)

        feat_joint = self.joint_proj(torch.cat([feat_id, feat_imp, feat_coop], dim=1))
...
class CVSincNet(nn.Module):
    ...
    Optimized version:
      1) Keep DAC branch unchanged.
      2) Replace PA branch with lighter PA-oriented trunk:
           - light memory polynomial lift (1,3) x depth 3
           - explicit AM-AM / AM-PM proxies
           - lightweight depthwise-separable dilated conv
           - PA-specific symmetric spectral regrowth shortcut
      3) Improve classifier so DAC and PA cooperate instead of pure parallel competition.

    Interface compatibility:
      - forward(x, y=None, return_aux=False)
      - build_model(...)
      - return_aux keeps legacy keys feat_cls / feat_imp / feat_dac / feat_con.
    """
    def __init__(
        self,
        num_classes: int = 16,
        ...
        margin_s: float = 30.0,
        margin_m: float = 0.35,
        pa_memory_depth: int = 3,
        pa_orders: Sequence[int] = (1, 3),
        pa_gate_alpha: float = 0.35,
    ):
        super().__init__()
        self.dataset = str(dataset)
        ...
        # PA branch: lighter, more PA-oriented
        self.pa_lift = MemoryPolynomialLift(memory_depth=pa_memory_depth, orders=pa_orders, clip=2.0)
        self.pa_proxy = PAProxyFeatures(clip=2.0)
        pa_in = 2 * pa_memory_depth * len(pa_orders) + 3
        self.pa_gate = EnvelopeGate1d(pa_in, alpha=pa_gate_alpha, k=5)
        self.pa_b1 = DilatedDSConvBlock1d(pa_in, 64, k=7, dilation=1, pool=2, drop=0.06)
        self.pa_b2 = DilatedDSConvBlock1d(64, 96, k=5, dilation=2, pool=2, drop=0.06)
        self.pa_b3 = DilatedDSConvBlock1d(96, 96, k=3, dilation=4, pool=1, drop=0.04)
        ...
        self.pa_regrowth_agg = PARegrowthAggregator(emb_dim=emb_dim, hidden=32, k=5)
    ...
    def _mirror_compressed_features(self, x: torch.Tensor):
        ...
        spec_kurtosis = torch.mean((tot_lin - mu) ** 4, dim=1, keepdim=True) / (var * var)
    ...
    def _pa_regrowth_features(self, feat_f: torch.Tensor) -> torch.Tensor:
        """
        feat_f: (B,4,K) = [logP_pos, logP_neg, logR, asym]
        return : (B,2,K)
        """
        sym = 0.5 * (feat_f[:, 0:1, :] + feat_f[:, 1:2, :])
        K = sym.size(-1)
        w = torch.linspace(-1.0, 1.0, K, device=sym.device, dtype=sym.dtype).abs().view(1, 1, K)
        edge = sym * w
        return torch.cat([sym, edge], dim=1)
    ...
    def forward(self, x: torch.Tensor, y: Optional[torch.Tensor] = None, return_aux: bool = False):
        ...
        # ----- PA branch -----
        pa_mp = self.pa_lift(x)
        pa_px = self.pa_proxy(x)
        pa_feat = torch.cat([pa_mp, pa_px], dim=1)
        ...
        pa_reg_feat = self._pa_regrowth_features(feat_f)
        pa_delta = self.pa_regrowth_agg(pa_reg_feat)
