...
# 多视图设置
N_VIEWS = 3                 # 2 or 3
TAU = 0.10                  # SupCon temperature
TAU_PROTO = 0.07            # Prototype temperature
...
# Warmup：先让 CE 起飞，再逐步加对比/原型/DAC
WARMUP_EPOCHS = 5
RAMP_EPOCHS = 20            # 线性从0->1
...
def main():
    ...
    # 2) Augmentors: weak vs strong
    # view0（保护源域精度）：建议直接用原始输入（不增强）
    # view1..：强增强（含 DAC）
    augmentor_strong = RFFIAugmentor(
        sampling_rate=5e6,
        p_phase_rotate=0.5,
        p_amp_scale=0.4,
        p_noise=0.5,
    ...
    # feat_dim 从模型里推断：用一次 dummy forward 得到 feat 维度
    with torch.no_grad():
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in train_loader:
            ...
            with torch.no_grad():
                for _ in range(N_VIEWS - 1):
                    xa, s = augmentor_strong(x, return_dac_strength=True)
                    views.append(xa)
                    dac_strengths.append(s.to(DEVICE))

            x_all = torch.cat(views, dim=0)              # (B*V,2,1024)
            y_all = y.repeat(N_VIEWS)                    # (B*V,)
            dac_t = torch.cat(dac_strengths, dim=0)      # (B*V,)

            # --------- Forward ----------
            logits_all, dac_pred_all, feat_all = model(x_all, return_aux=True)  # feat_all: (B*V,D)
            ...
            if lam_con > 0:
                D = feat_all.size(-1)
                feat_views = feat_all.view(N_VIEWS, B, D).permute(1, 0, 2).contiguous()  # (B,V,D)
        ...
        # save
        torch.save(model.state_dict(), os.path.join(WEIGHT_DIR, "last_model_run1.pth"))
        if test_acc > best_acc:
            best_acc = test_acc
            torch.save(model.state_dict(), os.path.join(WEIGHT_DIR, "best_model_run1.pth"))
