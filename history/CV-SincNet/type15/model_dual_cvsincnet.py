...
class SmallStatsHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with _autocast_disabled():
            ...
            q = xn[:, 1, :]
            amp = torch.sqrt((i * i + q * q).clamp_min(1e-8))
            corr_iq = (i * q).mean(dim=1) / (
                i.std(dim=1, unbiased=False).clamp_min(1e-6) * q.std(dim=1, unbiased=False).clamp_min(1e-6)
            )
            even = amp[:, 0::2]
            odd = amp[:, 1::2]
...
class DACOverlapExtractor(nn.Module):
    """
    Estimate the TX-DAC / RX-ADC overlap subspace, repel PA direction first,
    then apply a *bounded* subtraction so the cleaner cannot erase identity
    information excessively.
    """
    def __init__(self, tx_dim: int, rx_dim: int, drop: float = 0.1, max_clean_ratio: float = 0.20):
        super().__init__()
        self.tx_dim = int(tx_dim)
        self.rx_dim = int(rx_dim)
        self.max_clean_ratio = float(max_clean_ratio)
    ...
    def forward(self, z_txc: torch.Tensor, r_dac: torch.Tensor, feat_pa: torch.Tensor, beta: float = 0.16) -> Dict[str, torch.Tensor]:
        ...
        t_gate = F.normalize(z_txc_f.detach(), dim=1, eps=1e-4)
        r_gate = F.normalize(r_overlap, dim=1, eps=1e-4)
        ...
        delta = float(beta) * common
        delta_norm = delta.norm(dim=1, keepdim=True)
        z_norm = z_txc_f.norm(dim=1, keepdim=True).clamp_min(1e-6)
        clean_ratio = delta_norm / z_norm
        if self.max_clean_ratio > 0:
            scale = torch.clamp(self.max_clean_ratio / clean_ratio.clamp_min(1e-8), max=1.0)
            delta = delta * scale
            delta_norm = delta.norm(dim=1, keepdim=True)
            clean_ratio = delta_norm / z_norm

        clean = self.clean_norm(z_txc_f - delta)
        return {
            "r_up": r_up,
            "r_overlap": r_overlap,
            "pa_ref": pa_ref,
            ...
            "common_norm": common.norm(dim=1),
            "delta_norm": delta_norm.squeeze(1),
            "clean_ratio": clean_ratio.squeeze(1),
        }
...
class PADominantFusionHead(nn.Module):
    ...
    def forward(self, feat_pa: torch.Tensor, feat_id: torch.Tensor, z_dac_clean: torch.Tensor, eta_dac: Optional[float] = None) -> Dict[str, torch.Tensor]:
        ...
        return {"z_main": z_main, "z_cls": z_cls, "eta_dac": eta}
...
class DualRxDACOverlapPAAware(nn.Module):
    def __init__(
        ...
        drop: float = 0.1,
        overlap_max_clean_ratio: float = 0.20,
        **backbone_kwargs,
    ):
        super().__init__()
        ...
        self.overlap = DACOverlapExtractor(self.tx_dim, self.rx_dim, drop=drop, max_clean_ratio=overlap_max_clean_ratio)
    ...
    def forward(
        ...
        return {
            ...
            "common_norm": ov["common_norm"],
            "clean_ratio": ov["clean_ratio"],
            "delta_norm": ov["delta_norm"],
            "aux_id": aux_id,
        }
