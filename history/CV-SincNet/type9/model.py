...
#    - mirror-aware compressed features include asymmetry channel:
#      asym = |pos-neg|/(pos+neg)
#
# 6) NEW (Freq contribution hooks + lightweight freq enhancement):
#    - FreqBandGate1d: band-wise gate over K bands to emphasize informative spectral regions
#    - f_stats (hf_ratio / asym_hf_mean / flatness) -> small embedding delta (does NOT change fuse dim)
#    - aux heads: logits_t / logits_f for branch CE / consistency losses
#    - optional branch_drop_p: drop time embedding during training to force freq contribution
#
# ------------------------------------------------------------
...
# ----------------------- Freq band gate (lightweight) -----------------------
class FreqBandGate1d(nn.Module):
    """Band-wise gate over K bands for mirror-compressed spectral features.
    Input : (B, C=4, K)
    Output: (B, C=4, K)

    Gate is produced by a small Conv1d(C->1), then used to reweight bands:
        y = x * (1 + alpha * (2*sigmoid(g)-1))
    so the per-band scale is in [1-alpha, 1+alpha].
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.alpha <= 0:
            return x
        g = torch.sigmoid(self.conv(x))  # (B,1,K)
        scale = 1.0 + self.alpha * (2.0 * g - 1.0)
        return x * scale


# ----------------------- CosFace (AM-Softmax) head -----------------------
# ----------------------- Defect sub-band aggregation -----------------------

class SubbandGatedAggregator(nn.Module):
    """Split mirror-compressed spectral features into K sub-bands -> gate -> aggregate -> emb.

    Input : feat_f (B, C=4, K)
    Output: imp_delta (B, emb_dim)
    """
    ...
    def forward(self, feat_f: torch.Tensor) -> torch.Tensor:
        B, C, K = feat_f.shape
        x = feat_f.permute(0, 2, 1).contiguous()  # (B,K,C)
        g = feat_f.mean(dim=2)                    # (B,C)
        g2 = g.unsqueeze(1).expand(-1, K, -1)     # (B,K,C)
        logits = self.band_mlp(torch.cat([x, g2], dim=-1)).squeeze(-1)  # (B,K)
        w = torch.softmax(logits / max(1e-6, self.temperature), dim=1)  # (B,K)
        agg = torch.sum(x * w.unsqueeze(-1), dim=1)                     # (B,C)
        return self.proj(agg)                                           # (B,emb_dim)
...
class CosFaceHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor, labels: Optional[torch.Tensor] = None) -> torch.Tensor:
        """FP32-stable CosFace/AM-Softmax.

        Under AMP, float16 eps underflows, and normalize can produce 0/0 -> NaNs.
        We therefore compute normalization + cosine in float32 with a safe eps.
        """
        with torch.cuda.amp.autocast(enabled=False):
            x_f = x.float()
            w_f = self.weight.float()
            x_f = F.normalize(x_f, dim=1, eps=1e-4)
            w_f = F.normalize(w_f, dim=1, eps=1e-4)
            cos = F.linear(x_f, w_f)  # (B,C) float32

            if labels is None:
                return cos * float(self.s)

            labels = labels.view(-1).long()
            one_hot = torch.zeros_like(cos)
            one_hot.scatter_(1, labels.unsqueeze(1), 1.0)
            cos_m = cos - one_hot * float(self.m)
            return cos_m * float(self.s)
...
class DACAwareClassifier(nn.Module):
    """
    From base embedding -> feat_id & feat_imp -> joint embedding -> margin head.

    - feat_id: identity / classification embedding (should be more impairment-invariant)
    - feat_imp: impairment embedding (should be sensitive to DAC/PA defects)
    - joint embedding: concat([feat_id, feat_imp]) -> joint_proj -> CosFace head

    Also outputs:
      - dac_pred: sigmoid regression in [0,1] from feat_imp
      - pa_pred : sigmoid regression in [0,1] from feat_imp
    """
    def __init__(
        self,
        base_dim: int,
        ...
        # use impairment to gate identity (FiLM-like)
        if self.use_gate:
            self.imp_gate = nn.Sequential(
    ...
    def forward(
        ...
        imp_delta: Optional[torch.Tensor] = None,
    ):
        feat_id = self.id_proj(base)   # (B,emb_dim)
        feat_imp = self.imp_proj(base) # (B,emb_dim)
        if imp_delta is not None:
            feat_imp = feat_imp + imp_delta

        if self.use_gate and (self.imp_gate is not None):
            g = self.imp_gate(feat_imp)
            feat_id = feat_id * (1.0 + self.gate_alpha * g)

        feat_joint = self.joint_proj(torch.cat([feat_id, feat_imp], dim=1))  # (B,emb_dim)
        logits = self.head(feat_joint, labels=labels)

        dac_pred = torch.sigmoid(self.dac_head(feat_imp).squeeze(-1))  # (B,)
        pa_pred = torch.sigmoid(self.pa_head(feat_imp).squeeze(-1))    # (B,)

        if not return_emb:
            # keep backward compatibility: (logits, dac_pred)
            return logits, dac_pred

        return logits, dac_pred, pa_pred, feat_id, feat_imp, feat_joint
...
class CVSincNet(nn.Module):
    ...
    Forward usage:
      ...
      - out = model(x, y=labels, return_aux=True)  # dict with logits/dac_pred/feat_cls/feat_con/logits_t/logits_f/...
    ...
    def __init__(
        ...
        # Freq contribution hooks
        branch_drop_p: float = 0.0,        # drop time-emb with prob p during training
        use_freq_band_gate: bool = True,   # band-wise gate on mirror features
        freq_gate_alpha: float = 0.6,      # gate strength
        use_freq_stats: bool = True,       # add small stats->emb delta (keeps fuse dim)
        ...
        super().__init__()
        # ---- dataset / input-length adaptation ----
        # WiSig IdSig commonly uses 256 IQ samples, while ORALCE uses 1024.
        # The training / eval utilities expect these attributes to exist.
        self.dataset = str(dataset)
        self.input_len = int(input_len) if int(input_len) > 0 else 1024
        self.expected_len = self.input_len  # backward/forward compatible alias
        self.pad_crop_mode = str(pad_crop_mode)
        ...
        # ---- freq lightweight enhancement ----
        self.freq_gate = FreqBandGate1d(4, k=5, alpha=freq_gate_alpha) if self.use_freq_band_gate else nn.Identity()
        self.defect_subband_agg = SubbandGatedAggregator(in_ch=4, emb_dim=emb_dim, hidden=64, temperature=1.0)
        # stats: [hf_ratio, asym_hf_mean, flatness] -> emb delta (does NOT change fuse input dim)
        ...
        # ---- auxiliary heads to force branch contribution ----
    ...
    def _mirror_compressed_features(self, x: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor], torch.Tensor]:
        """
        ...
        Return:
          ...
          f_stats: (B,3) = [hf_ratio, asym_hf_mean, flatness]
        """
        B, _, L = x.shape
        eps = self.eps
        ...
        # lightweight stats (encourages learning mirror/high-freq structure)
        pos_lin = pool1d(pos)
        neg_lin = pool1d(neg)
        tot_lin = (pos_lin + neg_lin).clamp_min(eps)
        ...
        flatness = torch.exp(torch.mean(torch.log(tot_lin), dim=1, keepdim=True)) / (torch.mean(tot_lin, dim=1, keepdim=True) + eps)
        f_stats = torch.cat([hf_ratio, asym_hf_mean, flatness], dim=1)  # (B,3)
    ...
    def forward(
        ...
        return_aux:
          False -> logits
          True  -> dict with logits/dac_pred/feat_cls/feat_con/logits_t/logits_f/...
        """
        # input length adapt (e.g., ORALCE=1024, WiSig IdSig=256)
        ...
        # optionally drop time branch to force freq contribution
        ...
        # ----- freq branch -----
        feat_f, rho, f_stats = self._mirror_compressed_features(x)  # (B,4,K), (B,1)/None, (B,3)
        feat_f = self.freq_gate(feat_f)
        imp_delta = self.defect_subband_agg(feat_f)
        ...
        if self.use_freq_stats and (self.freq_stats_proj is not None):
            f_emb = f_emb + self.freq_stats_proj(f_stats)

        # auxiliary branch logits (for branch CE / consistency losses)
        ...
        logits, dac_pred, pa_pred, feat_id, feat_imp, feat_joint = self.cls_head(
            base, labels=y, return_emb=True, imp_delta=imp_delta
        )
        ...
        return {
            ...
            'feat_cls': feat_id,
            'feat_imp': feat_imp,
            'feat_dac': feat_imp,  # backward-compatible alias
            'feat_con': feat_con,
            'logits_t': logits_t,
            'logits_f': logits_f,
            't_emb': t_emb,
...
def build_model(
    ...
    """Factory used by train_fixed.py.

    - dataset: 'oralce' or 'wisig'
    - input_len: ORALCE typically 1024; WiSig IdSig typically 256
    - sample_rate_hz: affects SincConv filterbank spacing
    """
    ...
    # simple size presets (keep architecture compatible with checkpoints)
    if ms in ("S", "SMALL"):
        cfg = dict(sinc_out=32, sinc_kernel=63, time_bottleneck=96, emb_dim=192, freq_bands=48, drop=0.25)
    elif ms in ("L", "LARGE"):
        cfg = dict(sinc_out=64, sinc_kernel=79, time_bottleneck=128, emb_dim=256, freq_bands=96, drop=0.25)
    else:  # "M"
        cfg = dict(sinc_out=48, sinc_kernel=79, time_bottleneck=96, emb_dim=256, freq_bands=64, drop=0.25)
