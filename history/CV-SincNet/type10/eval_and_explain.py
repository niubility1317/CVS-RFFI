...
# ===================== 修改区（你只需要改这几项） =====================
WEIGHTS_PATH = "./best_model_v8DG.pth"   # ✅ 权重地址
...
# NEW: t-SNE 可视化参数（默认不开启，命令行 --tsne 才运行）
MAX_TSNE_SAMPLES = 3500         # t-SNE 复杂度高，默认 <= 4k
TSNE_PERPLEXITY = 30.0
TSNE_ITERS = 1200
TSNE_PCA_DIM = 50               # t-SNE 前先 PCA 到 50 维更稳更快
TSNE_FEAT = "feat_cls"          # 可选: base / feat_cls / feat_con
TSNE_SEED = 0
# =====================================================================
...
def eval_branch_only(model: torch.nn.Module, loader: DataLoader, device: torch.device, branch: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    for xb, yb in loader:
        ...
        if getattr(model, "use_circularity", False) and hasattr(model, "_mirror_compressed_features"):
            try:
                _feat_f, rho, _f_stats = model._mirror_compressed_features(xb)
            except Exception:
                rho = None
...
def _extract_features_for_vis(
    ...
    if feat_mode in ("feat_cls", "feat_con"):
        ...
        if isinstance(out, dict) and feat_mode in out and torch.is_tensor(out[feat_mode]):
            logits = _extract_logits(out)
            pred = logits.argmax(dim=1)
