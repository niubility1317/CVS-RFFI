...
def main():
    ...
    parser.add_argument("--save_path", type=str, default="best_model_v10.pth")
    ...
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=(device.type=="cuda"), drop_last=True, persistent_workers=(args.num_workers>0))
    test_loader = DataLoader(test_ds, batch_size=args.test_batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=(device.type=="cuda"), drop_last=False, persistent_workers=(args.num_workers>0))
    ...
    model = build_dual_model(args.num_classes, num_domains, model_size=args.model_size, dataset=args.dataset, input_len=input_len, sample_rate_hz=float(args.sample_rate_hz)).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        m_txacc, m_domacc, m_probeacc = AverageMeter(), AverageMeter(), AverageMeter()
        cons_cos_vals = []
        ...
        for batch in train_loader:
            ...
            with torch.cuda.amp.autocast(enabled=use_amp):
                ...
                if d is not None and num_domains > 1:
                    ...
                    m_domacc.update(accuracy_from_logits(dom_logits, d), x.size(0))
                    m_probeacc.update(accuracy_from_logits(probe_dom_logits, d), x.size(0))
                else:
                ...
                loss = loss_cls + float(args.lambda_dom)*loss_dom + float(args.lambda_adv)*loss_adv + float(args.lambda_orth)*loss_orth + float(args.lambda_cons)*loss_cons + float(args.lambda_probe)*loss_probe
        ...
        if epoch == 1 or epoch % max(1, args.eval_every) == 0 or epoch == args.epochs:
            eval_stats = evaluate(model, test_loader, device, num_domains_train=num_domains, max_batches=int(args.eval_max_batches))
            if eval_stats["tx_acc"] > best_tx:
                best_tx = eval_stats["tx_acc"]
                best_epoch = epoch
                torch.save({"model": model.state_dict(), "epoch": epoch, "best_tx_acc": best_tx, "args": vars(args), "split_info": split_info}, args.save_path)

        msg = (
            f"[E{epoch:03d}] lr={optimizer.param_groups[0]['lr']:.2e} loss={m_loss.avg:.4f} cls={m_cls.avg:.4f} dom={m_dom.avg:.4f} adv={m_adv.avg:.4f} "
            f"orth={m_orth.avg:.4f} cons={m_cons.avg:.4f} probe={m_probe.avg:.4f} | tx_acc={m_txacc.avg:.2f}% dom_acc={m_domacc.avg if m_domacc.count else float('nan'):.2f}% "
            f"probe_dom_acc={m_probeacc.avg if m_probeacc.count else float('nan'):.2f}% cons_cos={cons_cos_epoch:.4f} DBI={train_dbi:.4f} bias_flag={int(bias_flag)} | time={time.time()-t0:.1f}s"
        )
        if eval_stats is not None:
            msg += f" | val_tx={eval_stats['tx_acc']:.2f}% val_dom={eval_stats['dom_acc']:.2f}% val_probe={eval_stats['probe_dom_acc']:.2f}% best={best_tx:.2f}%@E{best_epoch:03d}"
