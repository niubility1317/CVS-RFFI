...
class RFFIAugmentor:
    ...
    New (backward compatible):
      __call__(..., dac_only=True) will apply only DAC (no other channel impairments).
      __call__(..., no_dac=True)  will apply channel impairments but skip DAC.
    """
    ...
    def __init__(
        ...
        amp_imbalance_max: float = 0.03,
        phase_imbalance_max_deg: float = 2.0,

        dac_strength_range: Tuple[float, float] = (0.1, 1.0),
        dac_bits_range: Tuple[int, int] = (7, 12),
        dac_poly_a3_max: float = 0.18,
        dac_poly_a5_max: float = 0.06,
        dac_slew_delta_range: Tuple[float, float] = (0.05, 0.50),
    ...
    def simulate_dac(self, x: torch.Tensor, strength: torch.Tensor) -> torch.Tensor:
        ...
        # normalize to ~[-1,1] using RMS-based "full-scale"
        rms = self._rms(x)
        full_scale = torch.clamp(3.0 * rms, min=1e-6)
        xn = torch.clamp(x / full_scale, -1.0, 1.0)

        # memoryless polynomial nonlinearity (odd-order) -> spectral regrowth
        ...
        # quantization bits: stronger -> fewer bits
        bmin, bmax = self.dac_bits_range
        bits_f = (bmax - (bmax - bmin) * s).squeeze(-1).squeeze(-1)
        ...
        # slew / limited delta per sample (simple first-order memory)
        dmin, dmax = self.dac_slew_delta_range
        max_delta = ((1.0 - s) * dmax + s * dmin).to(dtype=dtype)
    ...
    def apply_dac(self, x: torch.Tensor):
        B = x.size(0)
        device = x.device
        ...
        mask = self._should_apply(self.p_dac, B, device).squeeze(-1).squeeze(-1)
        if not mask.any():
            return x, dac_strength
    ...
    def __call__(
        ...
        *,
        ...
        """
        dac_only=True : only DAC (useful to isolate DAC effect as a separate view).
        no_dac=True   : disable DAC, apply other channel impairments only.
        """
        if no_dac:
            x_out = x
            dac_strength = torch.zeros(x.size(0), device=x.device, dtype=torch.float32)
        else:
            x_out, dac_strength = self.apply_dac(x)

        if not dac_only:
            x_out = self.apply_phase_rotate(x_out)
            x_out = self.apply_amp_scale(x_out)
            x_out = self.apply_cfo(x_out)
            x_out = self.apply_iq_imbalance(x_out)
            x_out = self.apply_noise(x_out)
