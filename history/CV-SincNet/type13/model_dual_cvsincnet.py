...
class ResidualFeatureAdapter(nn.Module):
    def __init__(self, dim: int, hidden: Optional[int] = None, drop: float = 0.1, init_scale: float = 1e-2):
        super().__init__()
        hidden = int(hidden) if hidden is not None else max(64, dim)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.norm_in(x.float())
        y = self.fc2(self.drop(self.act(self.fc1(y))))
        return self.norm_out(x.float() + self.res_scale * y)
...
class DualRxADCDisentangle(nn.Module):
    def __init__(
        ...
        **backbone_kwargs,
        ...
        self.tx_adapter = ResidualFeatureAdapter(self.tx_dim, hidden=max(128, self.tx_dim), drop=drop)
        self.rx_adapter = ResidualFeatureAdapter(self.rx_dim, hidden=max(128, self.rx_dim), drop=drop)
    ...
    def forward(
        ...
        z_tx_raw = self._pick(aux_id, ("feat_cls", "z_pa", "feat_con", "base"))
        z_joint = self._pick(aux_id, ("z_id", "feat_joint", "feat_con"))
        z_txc = self._pick(aux_id, ("z_txc", "feat_dac", "feat_imp"))

        z_rx_raw = self.rx_backbone(x)
        z_tx = self.tx_adapter(z_tx_raw)
        z_rx = self.rx_adapter(z_rx_raw)
