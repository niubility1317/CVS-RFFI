...
from model_dual_rxadc_v2 import build_dual_model
...
def make_sat_cfg(args) -> Optional[SatSimConfig]:
    if not bool(getattr(args, 'use_sat_view', False)):
        return None
    ...
    return SatSimConfig(
        ...
        loo_level=str(args.sat_loo_level),
...
def main():
    ...
    parser.add_argument("--save_path", type=str, default="best_model_dual_rxadc_v2.pth")
    parser.add_argument("--amp", dest="amp", action="store_true")
    parser.add_argument("--no_amp", dest="amp", action="store_false")
    parser.set_defaults(amp=True)
    parser.add_argument("--amp_warmup_epochs", type=int, default=5)
    ...
    parser.add_argument("--use_sat_view", dest="use_sat_view", action="store_true")
    parser.add_argument("--no_sat_view", dest="use_sat_view", action="store_false")
    parser.set_defaults(use_sat_view=True)
    parser.add_argument("--sat_start_epoch", type=int, default=6)
    parser.add_argument("--sat_tx_weight", type=float, default=0.30)
    parser.add_argument("--lambda_sat_cons_tx", type=float, default=0.10)
    ...
    parser.add_argument("--sat_weather", type=str, default="clear", choices=["clear", "rain", "storm"])
    parser.add_argument("--sat_loo_level", type=str, default="mid", choices=["low", "mid", "high"])
    ...
    if args.augment and (build_augmentor is not None):
        augmentor_main = maybe_build_augmentor(sample_rate_hz=args.sample_rate_hz)
        augmentor_proxy = maybe_build_augmentor(sample_rate_hz=args.sample_rate_hz)
        print("[AUG] enabled. main defects offloaded to proxy views; class_signature=False")
    else:
        print("[AUG] disabled.")

    sat_cfg = make_sat_cfg(args)
    use_sat_runtime = sat_cfg is not None
    if use_sat_runtime:
        print(
            f"[SAT] enabled. scenario={args.sat_scenario} weather={args.sat_weather} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        meters = {k: AverageMeter() for k in [
            ...
            "pa_proxy", "dac_proxy", "sat_tx", "sat_cons", "tx_acc", "rx_acc", "probe_rx_acc", "probe_tx_acc",
        ...
        for batch in train_loader:
            ...
            use_sat_batch = bool(use_sat_runtime) and (epoch >= int(args.sat_start_epoch))
            if use_sat_batch:
                x_sat, _, _ = apply_sat_gnd_channel_batch(x, sat_cfg, return_meta=False)
            else:
                x_sat = x
            ...
            x_sat = sanitize_batch_tensor(x_sat, abs_clip=clip_val)
        ...
        msg = (
            ...
            f"cons_cos={cons_cos_epoch:.4f} sat_cos={sat_cos_epoch:.4f} "
            ...
            f"stage=rx{scale['rx']:.2f}/arx{scale['adv_rx']:.2f}/atx{scale['adv_tx']:.2f}/sep{scale['sep']:.2f}/ctr{scale['center']:.2f}/con{scale['cons']:.2f}/px{scale['proxy']:.2f}/sc{scale['sat_cls']:.2f}/ss{scale['sat_cons']:.2f} "
