...
RUN_ROOT="${RUN_ROOT:-runs/paper_reproduction_explicit_sat_train_adapt_test_${STAMP}}"
LOG_ROOT="${LOG_ROOT:-logs/paper_reproduction_explicit_sat_train_adapt_test_${STAMP}}"
...
cat > "${RUN_ROOT}/launch_manifest.json" <<JSON
{
  ...
  "stage": "Stage2-C",
  "channel_line": "satellite_all_explicit_train_adapt_test",
  "source_train_satellite_augmentation": true,
...
for seed in ${SEEDS}; do
  ...
  launch_one "protonet_cda_explicit_sat_train_adapt_test" "paper_reproduction/configs/protonet_cda_cvs_stage2c_explicit_sat_train_adapt_test_n607.json" "${seed}" 0
  p1="${LAST_PID}"
  launch_one "feature_separation_explicit_sat_train_adapt_test" "paper_reproduction/configs/feature_separation_cvs_stage2c_explicit_sat_train_adapt_test_n607.json" "${seed}" 1
  p2="${LAST_PID}"
